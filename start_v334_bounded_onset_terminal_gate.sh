...
RUN="$J/v334_v333_bounded_onset_terminal_gate_seed1504_20260822"
...
taskset -c 0-7 "$PY" pipeline/scripts/test_v334_bounded_onset_terminal.py \
  "${COMMON[@]}" --output "$RUN/audit/contract_report.json" >>"$RUN/audit/contract.log" 2>&1
taskset -c 0-7 "$PY" pipeline/scripts/audit_v334_bounded_onset_terminal_gate.py \
...
taskset -c 0-7 "$PY" pipeline/scripts/audit_v334_public_recursive_stability.py \
...
printf 'V334_BOUNDED_ONSET_TERMINAL_GATE_PASSED\n'
