...
run="artifacts/strict_track2_joint_augmentation_20260810/v352_v350_public_holdout_seed1521_20260822"
...
v351="artifacts/strict_track2_joint_augmentation_20260810/v351_bridge_focused_train_seed1520_20260822"
...
if [[ ! -f "$run/release_registration.json" ]]; then /root/miniconda3/envs/go1/bin/python pipeline/scripts/prepare_v352_v350_public_holdout_recursive.py; fi
exec taskset -c 0-7 /root/miniconda3/envs/go1/bin/python pipeline/scripts/audit_v352_v350_public_holdout_recursive.py \
 ...
 --v335-baseline-report "$v335/audit/recursive_stability_report.json" --v335-causal-report "$v335/audit/causal_gate_report.json" \
 --v351-train-report "$v351/focused_train_gate_report.json" --preregistration "$run/release_registration.json" \
 --output "$run/holdout_recursive_gate_report.json" --device cuda --inference-batch-size 8 --reward-batch-size 32 --feature-workers 8
