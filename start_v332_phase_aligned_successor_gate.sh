...
RUN="$J/v332_v326_phase_aligned_successor_gate_seed1502_20260822"
...
taskset -c 0-7 "$PY" pipeline/scripts/test_v332_phase_aligned_successor.py \
  "${COMMON[@]}" --output "$RUN/audit/contract_report.json" >>"$RUN/audit/contract.log" 2>&1

taskset -c 0-7 "$PY" pipeline/scripts/audit_v332_phase_aligned_successor_gate.py \
...
taskset -c 0-7 "$PY" pipeline/scripts/audit_v332_public_recursive_stability.py \
...
printf 'V332_PHASE_ALIGNED_SUCCESSOR_GATE_PASSED\n'
