"""Runtime loader for the action-conditioned warp-and-residual Track 2 model."""
...
from .flow_residual_unet import ActionConditionedFlowResidualUNet
from .profile import ACTION_DIM, CONTEXT_FRAMES, PREDICTION_FRAMES


class Track2FlowResidualUNet:
    def __init__(self, checkpoint_dir: str | Path, device: str = "cuda") -> None:
        ...
        config = np.load(root / "track2_flow_residual_unet_config.npz", allow_pickle=False)
        if (int(config["context_frames"]), int(config["action_dim"]), int(config["prediction_frames"])) != (CONTEXT_FRAMES, ACTION_DIM, PREDICTION_FRAMES):
            raise RuntimeError("flow residual U-Net checkpoint does not match the official Track 2 profile")
        ...
        if state.get("format") != "track2-flow-residual-unet-v1":
            raise RuntimeError("unsupported flow residual U-Net checkpoint format")
        self.model = ActionConditionedFlowResidualUNet().to(self.device).eval()
    ...
    def predict(self, context_frames, history_actions, future_actions, seed, instruction):
        ...
        with torch.inference_mode():
            prediction = self.model(context, actions.float()).clamp(0.0, 1.0)
