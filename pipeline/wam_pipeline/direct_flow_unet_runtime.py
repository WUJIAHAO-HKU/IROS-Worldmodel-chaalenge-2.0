"""Runtime loader for the direct flow-supervised Track 2 model."""
...
class Track2DirectFlowUNet:
    def __init__(self, checkpoint_dir: str | Path, device: str = "cuda") -> None:
        ...
        config = np.load(root / "track2_direct_flow_unet_config.npz", allow_pickle=False)
        ...
        if actual != expected:
            raise RuntimeError("direct flow U-Net checkpoint does not match the official Track 2 profile")
        ...
        if state.get("format") != "track2-direct-flow-unet-v1":
            raise RuntimeError("unsupported direct flow U-Net checkpoint format")
        base_channels = int(config["base_channels"]) if "base_channels" in config.files else 64
        self.model = DirectActionFlowUNet(base_channels).to(self.device).eval()
