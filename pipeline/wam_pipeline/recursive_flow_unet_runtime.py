"""Runtime adapter for the recursive action-conditioned Track 2 flow model."""
...
from .recursive_flow_unet import RecursiveActionFlowUNet


class Track2RecursiveFlowUNet:
    """Generate all eight future RGB frames using only prior predicted frames."""
    ...
    def __init__(self, checkpoint_dir: str | Path, device: str = "cuda") -> None:
        ...
        config = np.load(root / "track2_recursive_flow_unet_config.npz", allow_pickle=False)
        ...
        if actual != expected:
            raise RuntimeError("recursive flow U-Net checkpoint does not match the official Track 2 profile")
        ...
        if state.get("format") != "track2-recursive-flow-unet-v1":
            raise RuntimeError("unsupported recursive flow U-Net checkpoint format")
        self.model = RecursiveActionFlowUNet(int(config["base_channels"])).to(self.device).eval()
    ...
    def predict(self, context_frames, history_actions, future_actions, seed, instruction):
        ...
        with torch.inference_mode(), torch.autocast(device_type=self.device.type, dtype=torch.bfloat16, enabled=self.device.type == "cuda"):
            for action in future.unbind(dim=1):
                frame = self.model(context, torch.cat((history, action[:, None]), dim=1)).clamp(0.0, 1.0)
