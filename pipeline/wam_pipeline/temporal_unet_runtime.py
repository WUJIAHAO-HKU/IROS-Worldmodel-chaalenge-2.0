"""Runtime loader for the time-resolved action-conditioned Track 2 model."""
...
from .temporal_unet import TemporalActionUNet


class Track2TemporalUNet:
    def __init__(self, checkpoint_dir: str | Path, device: str = "cuda") -> None:
        ...
        config = np.load(root / "track2_temporal_unet_config.npz", allow_pickle=False)
        if (int(config["context_frames"]), int(config["action_dim"]), int(config["prediction_frames"])) != (CONTEXT_FRAMES, ACTION_DIM, PREDICTION_FRAMES):
            raise RuntimeError("temporal U-Net checkpoint does not match the official Track 2 profile")
        ...
        if state.get("format") != "track2-temporal-unet-v1":
            raise RuntimeError("unsupported temporal U-Net checkpoint format")
        self.model = TemporalActionUNet().to(self.device).eval()
