"""V465 frozen-v169 right/post-close endpoint-only residual parent runtime."""
...
FORMAT="track2-v465-v169-endpoint-residual-parent-release-v1"
CHECKPOINT_FORMAT="strict-track2-v465-endpoint-residual-checkpoint-v1"
...
class EndpointResidualUNet128FiLM(nn.Module):
 ...
 def forward(self,baseline_endpoint,context_last,condition_features,arm_tokens):
  ...
  if context_last.shape!=(b,3,h,w) or condition_features.shape!=(b,ACTION_FEATURE_DIM) or arm_tokens.shape!=(b,2):raise ValueError("v465 tensor shape contract")
...
class Track2V465V169EndpointResidualParent:
 def __init__(self,release_dir,device="cuda"):
  root=Path(release_dir).resolve();manifest=json.loads((root/"v465_endpoint_residual_manifest.json").read_text());checkpoint=(root/manifest["checkpoint"]).resolve();v169_release=(root/manifest["v169_release"]).resolve();v169_library=(root/manifest["v169_library"]).resolve()
  if root not in checkpoint.parents or manifest.get("format")!=FORMAT or manifest.get("official_reward_runtime_used") is not False or sha(checkpoint)!=manifest["sha256"]["checkpoint"] or sha(Path(__file__))!=manifest["sha256"]["runtime_source"]:raise RuntimeError("bad v465 release")
  ...
  if state.get("format")!=CHECKPOINT_FORMAT or state.get("training_scope")!="all200" or state.get("precision")!="bf16" or state.get("feature_schema")!=expected_schema or state.get("runtime_sha256")!=manifest["sha256"]["runtime_source"] or state.get("closure_digest")!=manifest.get("closure_digest"):raise RuntimeError("bad v465 checkpoint")
  ...
  if self.mean.shape!=(14,) or self.std.shape!=(14,) or not np.isfinite(self.mean).all() or not np.isfinite(self.std).all() or np.any(self.std<=0):raise RuntimeError("bad v465 normalization")
 ...
 def predict_batch_with_baseline(self,context,history,future,seeds,instructions):
  ...
  if context.ndim!=5 or history.shape!=(n,4,14) or future.shape!=(n,8,14) or len(seeds)!=n or len(instructions)!=n or not np.isfinite(history).all() or not np.isfinite(future).all():raise ValueError("v465 request batch contract")
