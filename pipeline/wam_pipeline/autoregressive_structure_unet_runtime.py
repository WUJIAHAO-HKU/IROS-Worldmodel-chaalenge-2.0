...
class Track2AutoregressiveStructureUNet:
    ...
    def predict_with_structure_batch(
        self,
        context_frames,
        history_actions,
        future_actions,
        ...
        with torch.inference_mode():
            for action in future.unbind(dim=1):
                prediction, structure = self.model(
                    context,
                    torch.cat((history, action[:, None]), dim=1),
                ...
                context = torch.cat((context[:, 1:], recurrent[:, None]), dim=1)
                history = torch.cat((history[:, 1:], action[:, None]), dim=1)
        rgb = torch.stack(predictions, dim=1).mul(255).round().byte().permute(0, 1, 3, 4, 2).cpu().numpy().copy()
        masks = torch.stack(structures, dim=1).float().cpu().numpy().copy()
        return rgb, masks
