    ...
    def _v141(self, parent: np.ndarray, context: np.ndarray, history: np.ndarray, future: np.ndarray) -> np.ndarray:
        ...
        for candidate in shortlist:
            with np.load(self.library_paths[candidate], allow_pickle=False) as window:
                candidate_source = window["context_frames"][-1]
        ...
        if not (aligned and arm == 1 and intact_prefix and ratios[-1] >= 0.65):
            return output
        _, candidate, negative_ecc, warp = min(aligned)
        with np.load(self.library_paths[candidate], allow_pickle=False) as selected:
            selected_future = selected["target_frames"]
