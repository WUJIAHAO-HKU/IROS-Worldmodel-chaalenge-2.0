...
class Track2StructureGatedLocalFusion:
    ...
    def predict_batch(
        ...
        if history_actions.shape != (batch, CONTEXT_ACTIONS, ACTION_DIM) or future_actions.shape != (batch, PREDICTION_FRAMES, ACTION_DIM):
            raise ValueError("actions must be [B,4,14] history and [B,8,14] future")
        baseline = self.baseline.predict_batch(
            context_frames, history_actions, future_actions, seeds, instructions
        )
        structure, masks = self.structure_parent.predict_with_structure_batch(
            context_frames, history_actions, future_actions, seeds, instructions
        )
        torch = self.torch
        context = torch.from_numpy(np.ascontiguousarray(context_frames[:, -1])).permute(0, 3, 1, 2)[:, None].to(self.device).float().div(255.0)
        baseline_tensor = torch.from_numpy(np.ascontiguousarray(baseline)).permute(0, 1, 4, 2, 3).to(self.device).float().div(255.0)
        structure_tensor = torch.from_numpy(np.ascontiguousarray(structure)).permute(0, 1, 4, 2, 3).to(self.device).float().div(255.0)
        quantized_masks = np.rint(masks * 255.0).clip(0, 255).astype(np.uint8)
        mask_tensor = torch.from_numpy(quantized_masks).to(self.device).float().div(255.0)
        actions = torch.from_numpy(np.ascontiguousarray((future_actions - self.mean) / self.std)).to(self.device).float()
        with torch.inference_mode(), torch.autocast(device_type=self.device.type, dtype=torch.bfloat16, enabled=self.device.type == "cuda"):
            prediction, _, _, _ = self.model(
                context, baseline_tensor, structure_tensor, mask_tensor, actions
            )
        return prediction.clamp(0, 1).mul(255).round().to(torch.uint8).permute(0, 1, 3, 4, 2).cpu().numpy().copy()
