"""V467 frozen-v169 canonical-no-transport appearance + paired-dynamics parent."""
...
FORMAT="track2-v467-canonical-notransport-paired-delta-parent-release-v1"
CHECKPOINT_FORMAT="strict-track2-v467-canonical-notransport-paired-delta-checkpoint-v1"
...
class EndpointResidualUNet128FiLM(nn.Module):
 ...
 def forward(self,baseline_endpoint,context_last,condition_features,arm_tokens):
  ...
  if context_last.shape!=(b,3,h,w) or condition_features.shape!=(b,ACTION_FEATURE_DIM) or arm_tokens.shape!=(b,2):raise ValueError("v467 tensor shape contract")
...
class Track2V467CanonicalNoTransportPairedDeltaParent:
 def __init__(self,release_dir,device="cuda"):
  root=Path(release_dir).resolve();manifest=json.loads((root/"v467_canonical_notransport_paired_delta_manifest.json").read_text());appearance=(root/manifest["appearance_checkpoint"]).resolve();dynamics=(root/manifest["dynamics_checkpoint"]).resolve();v169_release=Path(manifest["canonical_v169_release"]).resolve();v169_library=Path(manifest["canonical_v169_library"]).resolve()
  if manifest.get("format")!=FORMAT or manifest.get("official_reward_runtime_used") is not False or root not in appearance.parents or root not in dynamics.parents or sha(Path(__file__))!=manifest["sha256"]["runtime_source"]:raise RuntimeError("bad v467 release")
  if sha(appearance)!=manifest["sha256"]["appearance_checkpoint"] or sha(dynamics)!=manifest["sha256"]["dynamics_checkpoint"]:raise RuntimeError("v467 checkpoint hash drift")
  ...
  if self.mean.shape!=(14,) or self.std.shape!=(14,) or not np.isfinite(self.mean).all() or not np.isfinite(self.std).all() or np.any(self.std<=0):raise RuntimeError("bad v467 normalization")
 ...
 def predict_batch_with_baseline(self,context,history,future,seeds,instructions):
  ...
  if context.ndim!=5 or history.shape!=(n,4,14) or future.shape!=(n,8,14) or seeds.shape!=(n,) or len(instructions)!=n or not np.isfinite(history).all() or not np.isfinite(future).all():raise ValueError("v467 request batch contract")
