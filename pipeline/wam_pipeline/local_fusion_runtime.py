...
class Track2LocalMotionTextureFusion:
    ...
    def predict_batch(
        ...
        first = self.autoregressive.predict_batch(
            context_frames, history_actions, future_actions, seeds, instructions
        )
        second = self.direct_flow.predict_batch(
            context_frames, history_actions, future_actions, seeds, instructions
        )
        torch = self.torch
        context = torch.from_numpy(np.ascontiguousarray(context_frames[:, -1])).permute(0, 3, 1, 2)[:, None].to(self.device).float().div(255.0)
        first_tensor = torch.from_numpy(np.ascontiguousarray(first)).permute(0, 1, 4, 2, 3).to(self.device).float().div(255.0)
        second_tensor = torch.from_numpy(np.ascontiguousarray(second)).permute(0, 1, 4, 2, 3).to(self.device).float().div(255.0)
        actions = torch.from_numpy(np.ascontiguousarray((future_actions - self.mean) / self.std)).to(self.device)
        with torch.inference_mode(), torch.autocast(device_type=self.device.type, dtype=torch.bfloat16, enabled=self.device.type == "cuda"):
            prediction, _, _ = self.model(
                context, first_tensor, second_tensor, actions.float()
            )
