#!/usr/bin/env python3
"""S0 audit for v442 train-only alignment, runtime purity, and RGB bounds."""
...
def main() -> int:
    ...
    checks = {
        "index_format": index.get("format") == "strict-track2-v442-close-trainonly-rgb-alignment-index-v1",
        "preregistration_format": prereg.get("format") == "strict-track2-v442-close-trainonly-preregistration-v1",
        ...
        "exact_15_samples": len(index.get("samples", [])) == 15,
        ...
        "close_gate_fixture": close.get("gate") is True and close.get("phase") == "close" and close.get("first_close_index") == 4,
        "postclose_repeat_disabled": repeated.get("gate") is False,
        "close_gate_structure_preregistered": prereg.get("construction", {}).get("gate") == "explicit right; right7D path > left7D path; history-last open; future first close then held; no mu",
        "s1_action_coverage_preregistered": prereg.get("s1_gate", {}).get("action_only_gate_active_exact") == 8 and prereg.get("s1_gate", {}).get("action_only_gate_chunk_pattern") == [1, 0, 0, 0] and prereg.get("s1_gate", {}).get("action_only_gate_repeat_active_exact") == 0,
    ...
    report = {
        "format": "strict-track2-v442-close-s0-static-contract-v1",
        "passed": passed,
        "checks": checks,
        "fixtures": {"close": close, "repeated_postclose": repeated, "left": left},
        "declared_s1_action_coverage": prereg.get("s1_gate", {}),
        "sha256": {
