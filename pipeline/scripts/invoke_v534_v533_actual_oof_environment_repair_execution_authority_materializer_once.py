#!/usr/bin/env python3
"""Exact-once Stage-A transport for the v534 environment-repaired actual-OOF execution authority."""
...
SELF_PATH = SCRIPTS / "invoke_v534_v533_actual_oof_environment_repair_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v534_v533_actual_oof_environment_repair_execution_authority_materialize_once.sh")
DEPLOYMENT_RECORD = SCRIPTS / "v534_v533_actual_oof_environment_repair_execution_authority_materialization_transport_deployment_record.json"
CONTRACT = SCRIPTS / "v534_v533_actual_oof_environment_repair_execution_authority_contract.json"
CONTRACT_SHA = "603b84c89582bc9e05bc6adbb9461b9a321f647ed6e24bd6b675ebe47afcc249"
CONTRACT_BYTES = 115984
MATERIALIZER = SCRIPTS / "materialize_v534_v533_actual_oof_environment_repair_execution_authority.py"
MATERIALIZER_SHA = "2ef7e0077d60fbcda2fa74c904f57975de63fff6831e01f3613832bcdcf8ff25"
MATERIALIZER_BYTES = 41992
EXECUTOR = SCRIPTS / "execute_v534_v533_actual_oof.py"
EXECUTOR_SHA = "1451682cc156574748a865cde4d99675599346c310f532dbbc1db4309ad29373"
EXECUTOR_BYTES = 65276
AUDITOR = SCRIPTS / "audit_v534_v533_actual_oof.py"
AUDITOR_SHA = "a721637813feea55c6aeeeb721a064bfaccb7a3fa1bc5002f27eb988ccf80a4d"
AUDITOR_BYTES = 12047
LAUNCHER = SCRIPTS / "launch_v534_v533_actual_oof.py"
LAUNCHER_SHA = "ce2d9f1110d738fad5b309f1d2595dfd44826a38f8ef34a129011c272ea54593"
LAUNCHER_BYTES = 16514
PREREGISTRATION = SCRIPTS / "v534_v533_actual_oof_execution_preregistration.json"
PREREGISTRATION_SHA = "0370169bdf7850dcf16e954e3833c37db26887be72d7794a8c4d707df0abea85"
PREREGISTRATION_BYTES = 15730
MANIFEST = SCRIPTS / "v534_v533_actual_oof_execution_manifest.json"
MANIFEST_SHA = "4cd77fa066f65b1776e89d2a0d335294688b8a5d4fbf2514043fce5a1da2e0f4"
MANIFEST_BYTES = 1296108
...
FORENSIC_BYTES = 6124
REJECTED_SCRIPT = SCRIPTS / "v533_rejected_actual_oof_environment_repair_transport_script.sh"
REJECTED_SCRIPT_SHA = "ff46bb2738cc85270c0b17b4c957fd1ce3432053508a239cf96dee56902f32ca"
REJECTED_SCRIPT_BYTES = 9484
REJECTED_RECORD = SCRIPTS / "v533_rejected_actual_oof_environment_repair_transport_record.json"
REJECTED_RECORD_SHA = "ed3e4dd19279bbbf335310288680ad53aa887969e7635fe435b1962382e961c0"
REJECTED_RECORD_BYTES = 923
REQUIRED_ENVIRONMENT_EXACT = {"CUBLAS_WORKSPACE_CONFIG": ":4096:8", "PYTHONHASHSEED": "0"}
AUTHORITY_ROOT = J / "v534_v533_actual_oof_environment_repair_execution_authority_seed1667_20260827"
AUTHORITY_PREP = AUTHORITY_ROOT.with_name(AUTHORITY_ROOT.name + ".authority-prep")
EVIDENCE_ROOT = J / "v534_v533_actual_oof_environment_repair_execution_authority_materialization_evidence_seed1667_20260827"
...
AT_FDCWD = -100
RENAME_NOREPLACE = 1
FROZEN_FINAL10_SHA256 = (CONTRACT_SHA, MATERIALIZER_SHA, PREREGISTRATION_SHA, MANIFEST_SHA,
                        EXECUTOR_SHA, AUDITOR_SHA, LAUNCHER_SHA, FORENSIC_SHA,
                        REJECTED_SCRIPT_SHA, REJECTED_RECORD_SHA)
...
def deployment_plan() -> dict:
    rows = [
        ...
        {"role": "actual_oof_execution_preregistration", "path": str(PREREGISTRATION), "sha256": PREREGISTRATION_SHA, "logical_bytes": PREREGISTRATION_BYTES},
        {"role": "actual_oof_execution_manifest", "path": str(MANIFEST), "sha256": MANIFEST_SHA, "logical_bytes": MANIFEST_BYTES},
        {"role": "actual_oof_executor", "path": str(EXECUTOR), "sha256": EXECUTOR_SHA, "logical_bytes": EXECUTOR_BYTES},
        {"role": "actual_oof_auditor", "path": str(AUDITOR), "sha256": AUDITOR_SHA, "logical_bytes": AUDITOR_BYTES},
        {"role": "actual_oof_launcher", "path": str(LAUNCHER), "sha256": LAUNCHER_SHA, "logical_bytes": LAUNCHER_BYTES},
        {"role": "v532_environment_failure_forensic", "path": str(FORENSIC), "sha256": FORENSIC_SHA, "logical_bytes": FORENSIC_BYTES},
        {"role": "v533_rejected_transport_script", "path": str(REJECTED_SCRIPT), "sha256": REJECTED_SCRIPT_SHA,
         "logical_bytes": REJECTED_SCRIPT_BYTES, "classification": "rejected_nonexecuted_ancestry_snapshot",
         "current_only": True, "executable": False, "required_mode": "0600"},
        {"role": "v533_rejected_transport_record", "path": str(REJECTED_RECORD), "sha256": REJECTED_RECORD_SHA,
         "logical_bytes": REJECTED_RECORD_BYTES, "classification": "rejected_nonexecuted_ancestry_snapshot",
         "current_only": True, "executable": False, "required_mode": "0600"},
    ]
    return {
        "fresh_noreplace_targets_exact10": rows,
        "volatile_sources_exact10": [{**row, "source_path": "/dev/shm/" + Path(row["path"]).name} for row in rows],
        "reused_current_only": [],
...
def rejected_ancestry_current() -> dict:
    rows = {}
    for role, path, digest, logical_bytes in (
            ("v533_rejected_transport_script", REJECTED_SCRIPT, REJECTED_SCRIPT_SHA, REJECTED_SCRIPT_BYTES),
            ("v533_rejected_transport_record", REJECTED_RECORD, REJECTED_RECORD_SHA, REJECTED_RECORD_BYTES)):
        row = regular(path, (digest, logical_bytes))
        mode = stat.S_IMODE(os.lstat(path).st_mode)
        if mode != 0o600 or mode & 0o111:
            raise RuntimeError(f"rejected ancestry mode: {role}")
        rows[role] = {**row, "classification": "rejected_nonexecuted_ancestry_snapshot",
                      "current_only": True, "executable": False, "mode": "0600",
                      "invocations": 0}
    return rows
...
def expected_transport_deployment(helper_record: dict, script_record: dict) -> dict:
    return {
        "format": "strict-track2-v534-stage-a-transport-pair-deployment-record-v1",
...
def forbidden_entrypoints() -> set[str]:
    return {
        str(LAUNCHER), str(EXECUTOR), str(AUDITOR),
        str(REJECTED_SCRIPT), str(REJECTED_RECORD),
...
def immutable_snapshot(contract: dict, helper_record: dict, script_record: dict) -> dict:
    ...
    return {
        ...
        "historical_rejected_sources": rejected_ancestry_current(),
...
def validate_materializer_contract(module, contract: dict, materializer_record: dict) -> None:
    ...
    active = {
        ...
        "actual_oof_execution_preregistration": {"path": str(PREREGISTRATION), "sha256": PREREGISTRATION_SHA, "logical_bytes": PREREGISTRATION_BYTES},
        "actual_oof_execution_manifest": {"path": str(MANIFEST), "sha256": MANIFEST_SHA, "logical_bytes": MANIFEST_BYTES},
        "actual_oof_executor": {"path": str(EXECUTOR), "sha256": EXECUTOR_SHA, "logical_bytes": EXECUTOR_BYTES},
        "actual_oof_auditor": {"path": str(AUDITOR), "sha256": AUDITOR_SHA, "logical_bytes": AUDITOR_BYTES},
        "actual_oof_launcher": {"path": str(LAUNCHER), "sha256": LAUNCHER_SHA, "logical_bytes": LAUNCHER_BYTES},
    }
    schema_checks = {
        ...
        "seed": type(seed) is int and seed == 1667,
        ...
        "authority_top_count": len(contract.get("authority_receipt_contract", {}).get("top_keys", [])) == 67,
        "check_count": len(module.CHECK_KEYS) == 36,
        "source_counts": len(module.SOURCE_ROLES) == 33 and len(module.AUTHORITY_SOURCE_ROLES) == 34,
        ...
        "forbidden_ancestry_exact_current_only": (
            contract.get("source_closure", {}).get("v532_environment_failure_forensic")
            == {"path": str(FORENSIC), "sha256": FORENSIC_SHA, "logical_bytes": FORENSIC_BYTES}
            and contract.get("source_closure", {}).get("v533_rejected_transport_script")
            == {"path": str(REJECTED_SCRIPT), "sha256": REJECTED_SCRIPT_SHA, "logical_bytes": REJECTED_SCRIPT_BYTES}
            and contract.get("source_closure", {}).get("v533_rejected_transport_record")
            == {"path": str(REJECTED_RECORD), "sha256": REJECTED_RECORD_SHA, "logical_bytes": REJECTED_RECORD_BYTES}
            and not ({"v532_environment_failure_forensic", "v533_rejected_transport_script",
                      "v533_rejected_transport_record"} & set(module.ACTIVE_SOURCE_ROLE_ORDER))),
        "lineage_exact": cbytes(contract.get("lineage")) == cbytes(module.LINEAGE),
        "required_environment_exact": (
            cbytes(contract.get("required_environment_exact")) == cbytes(REQUIRED_ENVIRONMENT_EXACT)
            and cbytes(contract.get("authority_receipt_contract", {}).get("required_environment_exact"))
            == cbytes(REQUIRED_ENVIRONMENT_EXACT)
            and cbytes(json.loads(PREREGISTRATION.read_text(encoding="utf-8")).get("required_environment"))
            == cbytes(REQUIRED_ENVIRONMENT_EXACT)
            and cbytes(json.loads(MANIFEST.read_text(encoding="utf-8")).get("execution_contract", {}).get("required_environment"))
            == cbytes(REQUIRED_ENVIRONMENT_EXACT)),
    }
    ...
    if (cbytes(contract.get("authorization")) != cbytes(module.AUTHORIZATION)
        ...
        raise RuntimeError("v534 environment-repaired actual-OOF authority boundary")
    for role, row in contract["source_closure"].items():
...
def load_materializer(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    spec = importlib.util.spec_from_file_location("v534_materializer", materializer_source)
    ...
    try:
        ...
        lines = [
            ...
            {"line_origin": "transport_helper", "passed": True,
              ...
              "actual_oof_launcher_invocations": 0, "actual_oof_executor_invocations": 0,
              "actual_oof_auditor_invocations": 0, "oof_readonly_gate_validator_invocations": 0,
              "v533_rejected_transport_script_invocations": 0,
              "v533_rejected_transport_record_invocations": 0,
              "historical_rejected_sources_current": True,
...
def validate_success(module, contract: dict, returncode: int, cleanup: dict,
    ...
    if (os.path.lexists(AUTHORITY_PREP)
            ...
            or len(receipt.get("source_closure", {})) != 34
            ...
            or cbytes(receipt.get("required_environment_exact")) != cbytes(REQUIRED_ENVIRONMENT_EXACT)
            or cbytes(receipt_schema.get("required_environment_exact")) != cbytes(REQUIRED_ENVIRONMENT_EXACT)
            ...
            or any(cbytes(receipt.get(key)) != cbytes(contract[key]) for key in module.ANCHOR_KEYS)
    ...
    rejected_current = rejected_ancestry_current()
    if receipt.get("authorization", {}).get("actual_oof_execution_boundary_invocations_consumed") != 0:
        raise RuntimeError("actual-OOF authority must remain unconsumed")
    ...
    for path in (module.ATTEMPT_ROOT, module.ATTEMPT_PREP, module.OOF_ROOT, module.OOF_PREP):
        if os.path.lexists(path):
            raise RuntimeError("actual-OOF downstream current")
    ...
    return {"authority_receipt": receipt_record, "authority_registration_tree": tree,
            ...
            "historical_rejected_sources_current": True,
            "historical_rejected_sources": rejected_current}


def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v534-v533-actual-oof-environment-repair-execution-authority-materializer-process-receipt-v1",
        ...
        "v533_rejected_transport_script_invocations": 0,
        "v533_rejected_transport_record_invocations": 0,
        "historical_rejected_sources_current": passed,
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v534-v533-actual-oof-environment-repair-execution-authority-materializer-intent-v1",
        ...
        receipt = {
            **base_receipt("passed_exact_once_actual_oof_authority_materialized_no_oof_execution", True, 1, cleanup),
...
def production_fixture(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    checks = {
        ...
        "final10_hardbound": all(value in source for value in FROZEN_FINAL10_SHA256),
        "fresh10_noreplace_plan": (
            len(plan["fresh_noreplace_targets_exact10"]) == 10
            and [row["role"] for row in plan["fresh_noreplace_targets_exact10"]]
            == ["authority_design_contract", "authority_materializer", "actual_oof_execution_preregistration",
                ...
                "v532_environment_failure_forensic", "v533_rejected_transport_script",
                "v533_rejected_transport_record"]
        ...
        "rejected2_data_only_current_nonexecutable": (
            all(row.get("classification") == "rejected_nonexecuted_ancestry_snapshot"
                    and row.get("current_only") is True and row.get("executable") is False
                    and row.get("required_mode") == "0600"
                    for row in plan["fresh_noreplace_targets_exact10"][-2:])
            and classify_forbidden_argv([str(RLPY), str(REJECTED_SCRIPT)]) == [str(REJECTED_SCRIPT)]
            and classify_forbidden_argv([str(RLPY), str(REJECTED_RECORD)]) == [str(REJECTED_RECORD)]),
        "rejected2_never_active_argv_or_popen": (
            not ({"v533_rejected_transport_script", "v533_rejected_transport_record"}
                 & set(module.ACTIVE_SOURCE_ROLE_ORDER))
            and str(REJECTED_SCRIPT) not in command() and str(REJECTED_RECORD) not in command()
            and all(getattr(node, "func", None) is None
        ...
        "authority_schema_67_36_34": (
            len(contract["authority_receipt_contract"]["top_keys"]) == 67
            and len(module.CHECK_KEYS) == 36 and len(module.AUTHORITY_SOURCE_ROLES) == 34),
        "required_environment_exact2": (
            cbytes(contract["required_environment_exact"]) == cbytes(REQUIRED_ENVIRONMENT_EXACT)
            and cbytes(contract["authority_receipt_contract"]["required_environment_exact"])
            == cbytes(REQUIRED_ENVIRONMENT_EXACT)),
        "downstream_not_in_child_argv": (
            all(str(path) not in command() for path in (EXECUTOR, AUDITOR, LAUNCHER,
                                                        REJECTED_SCRIPT, REJECTED_RECORD))
            and not any("launch_" in item or "execute_" in item or "audit_" in item for item in command())),
        "native_empty_to_transport_exact2_source": (
            "materializer native stdout must be empty" in source and "transport stdout exact2 readback" in source),
        "choice_a_external_evidence_exact6": EXACT6 == sorted(EXACT5 + ["process_receipt.json"]),
    }
    with tempfile.TemporaryDirectory(prefix="v534-stage-a-helper-", dir="/dev/shm") as folder:
        temp = Path(folder)
        ...
        result = orchestrate(
            ...
            argv_payload={"format": "synthetic-v534-stage-a", "pre_snapshot": pre,
                          "transport_deployment_record": deployment_record,
        ...
        checks["mapped_normal_exact6"] = (
            ...
            and result["process_receipt"]["actual_oof_execution_boundary_invocations"] == 0)
    ...
    return {"passed": True, "checks": checks, "checks_sha256": csha(checks),
            "materializer_invocations": 0, "actual_oof_execution_boundary_invocations": 0,
            "executor_invocations": 0, "auditor_invocations": 0, "launcher_invocations": 0,
...
def main() -> int:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v534-v533-actual-oof-environment-repair-execution-authority-materializer-argv-v1",
