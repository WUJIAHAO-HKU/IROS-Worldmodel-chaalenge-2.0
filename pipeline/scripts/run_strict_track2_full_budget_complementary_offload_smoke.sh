...
PREREG="$ROOT/pipeline/config/strict_track2_v157_complementary_offload_preregistration.json"
INTERIM="$ROOT/pipeline/config/strict_track2_v157_actor_offload_interim_result.json"
RUN_ROOT="$AUDIT_ROOT/runs/full_budget_complementary_offload_v157_seed1238"
...
"$PYTHON_BIN" - <<'PY' "$PARENT_SCREEN" "$PREREG" "$INTERIM"
import json, sys
assert json.load(open(sys.argv[1]))["passed"] is True
registration = json.load(open(sys.argv[2]))
overrides = registration["diagnostic_overrides"]
assert overrides["actor.enable_offload"] is True
assert overrides["rollout.enable_offload"] is True
assert overrides["bridge.audit_max_items"] == 1
assert registration["acceptance"]["formal_result"] is False
interim = json.load(open(sys.argv[3]))
assert interim["completed_world_model_chunks"] == 13
assert interim["cuda_oom"] is False
PY
