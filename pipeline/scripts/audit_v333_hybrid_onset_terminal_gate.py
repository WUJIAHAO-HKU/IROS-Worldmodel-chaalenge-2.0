#!/usr/bin/env python3
"""Run the frozen v326 causal audit against v333 hybrid targets."""
...
def main() -> int:
    ...
    report["format"] = "strict-track2-v333-hybrid-onset-terminal-gate-v1"
    ...
    report["target_semantics"] = (
        "terminal-ineligible matched episode uses its same-episode frozen onset; "
        "eligible matched episode uses the frozen v326 terminal selector"
    )
    report["passed"] = all(checks.values())
    report["authorization"]["waivers_or_excluded_failed_checks"] = False
    output.write_text(json.dumps(report, indent=2) + "\n")
    print(json.dumps({"v333_passed": report["passed"], "checks": checks}, indent=2))
