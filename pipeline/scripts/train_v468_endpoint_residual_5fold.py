#!/usr/bin/env python3
"""V468 5-fold canonical-no-transport appearance + paired-dynamics S0."""
...
from wam_pipeline.v468_canonical_notransport_paired_delta_runtime import CHECKPOINT_FORMAT,FEATURE_SCHEMA,EndpointResidualUNet128FiLM,action_features,appearance_features,canonical_no_transport,instruction_tokens,no_transport_mask
...
def atomic_json(path,payload):
 ...
 if tmp.exists():raise RuntimeError(f"stale v468 tmp: {tmp}")
...
def release_inventory(release,joint_root):
 ...
 def visit(logical,relative):
  target=logical.resolve();dir_key=(target.stat().st_dev,target.stat().st_ino)
  if not inside(target,joint_root) or not target.is_dir() or dir_key in active or dir_key in seen_dirs:raise RuntimeError("v468 release inventory cycle/duplicate/escape")
  ...
  for child in sorted(logical.iterdir(),key=lambda x:x.name):
   ...
   if not inside(resolved,joint_root):raise RuntimeError("v468 release target escape")
   ...
   if resolved.is_dir():visit(child,rel)
    ...
    target_key=(resolved.stat().st_dev,resolved.stat().st_ino)
    if target_key in seen_files:raise RuntimeError("v468 duplicate release file target")
   ...
   else:raise RuntimeError("v468 unsupported release target")
...
def verify_closure(pre,release,library):
 ...
 for records in (pre["v169"]["release_files"],pre["v169"]["library_files"]):
  for row in records:
   ...
   if key in seen or str(lexical)!=row["lexical_path"] or str(resolved)!=row["resolved_path"] or not inside(resolved,library if row["base_kind"]=="v169_library" else joint):raise RuntimeError("v468 closure drift")
   seen.add(key);link=os.readlink(lexical) if lexical.is_symlink() else None
   if link!=row["link_target"]:raise RuntimeError("v468 link drift")
   digest=directory_target_sha(resolved) if row["record_type"]=="directory_symlink" else sha(resolved)
   if digest!=row["target_sha"]:raise RuntimeError("v468 closure SHA drift")
 ...
 if declared!=release_inventory(release,joint):raise RuntimeError("v468 exact release inventory drift")
def validate_and_load(args,pre):
 if sha(Path(__file__))!=pre["source"]["trainer_sha256"] or sha(Path(pre["source"]["runtime_path"]))!=pre["source"]["runtime_sha256"]:raise RuntimeError("v468 source drift")
 if sha(args.contract)!=pre["source"]["contract_sha256"]:raise RuntimeError("v468 contract drift")
 ...
 if sha(receipt_path)!=pre["v461"]["final_receipt"]["sha256"] or receipt.get("passed") is not True or receipt.get("mode")!="final":raise RuntimeError("v468 blocked: v461 final")
 for key,arg in (("selection",args.v456_selection),("generation_report",args.v456_generation_report)):
  if sha(arg)!=pre["v461"][key]["sha256"]:raise RuntimeError("v468 v461 closure")
 for row in pre["v461"]["files"]:
  path=args.v456_dataset/row["relative"]
  if sha(path)!=row["sha256"]:raise RuntimeError("v468 dataset drift")
 ...
 for batch in range(10):
  for rowdir in sorted((args.v456_dataset/f"batch_{batch:03d}"/"rows").iterdir()):
   ...
   with np.load(rowdir/"endpoint.npz",allow_pickle=False) as z:
    ...
    if variants!=list(BRANCHES) or arrsha(history)!=spec["history_action_sha256"] or [arrsha(x) for x in futures]!=[spec["branch_action_sha256"][x] for x in BRANCHES] or receipt.get("selection_action_sha256")!=spec["branch_action_sha256"]:raise RuntimeError("v468 row action drift")
 ...
 if len(contexts)!=200 or len(samples)!=1000:raise RuntimeError("v468 exact data contract")
...
def common_tensors(contexts,samples,ids,device):
 rows=[samples[int(i)] for i in ids];base=torch.as_tensor(np.stack([contexts[x["context_id"]]["b0"] for x in rows]),dtype=torch.float32,device=device).permute(0,3,1,2)/255.;last=torch.as_tensor(np.stack([contexts[x["context_id"]]["context"] for x in rows]),dtype=torch.float32,device=device).permute(0,3,1,2)/255.;tokens=torch.as_tensor(instruction_tokens([contexts[x["context_id"]]["prompt"] for x in rows]),device=device);return rows,base,last,tokens
...
def main():
 ...
 if pre.get("format")!="strict-track2-v468-canonical-notransport-paired-delta-preregistration-v1" or final.exists() or work.exists():raise RuntimeError("v468 prereg/output contract")
 ...
 for fold in range(5):
  ...
  if schedule_sha(asched)!=pre["appearance_schedules"][fold]["sha256"] or schedule_sha(dsched)!=pre["dynamics_schedules"][fold]["sha256"]:raise RuntimeError("v468 schedule drift")
 ...
 if not early and len(folds)==5:
  ...
  if passed:
   ...
   if schedule_sha(asched)!=pre["all200_appearance_schedule_sha256"] or schedule_sha(dsched)!=pre["all200_dynamics_schedule_sha256"]:raise RuntimeError("v468 all200 schedule drift")
 ...
 s0={"format":"strict-track2-v468-canonical-notransport-paired-delta-s0-v1","created_at":datetime.now(timezone.utc).isoformat(),"passed":passed,"early_stop_mathematically_unreachable":early,"completed_folds":len(folds),"failed_folds":failed,"folds":folds,"aggregate":aggregate,"all200_training_started":bool(passed),"guards":{"reward_loaded":False,"policy_updates":0,"rl_authorized":False}}
 atomic_json(work/"s0_report.json",s0);atomic_json(work/"training_report.json",{**s0,"format":"strict-track2-v468-canonical-notransport-paired-delta-training-report-v1","all200_training_performed":bool(passed),"appearance_checkpoint_sha256":sha(work/"all200_appearance_step50.pt") if passed else None,"dynamics_checkpoint_sha256":sha(work/"all200_dynamics_action_step50.pt") if passed else None})
