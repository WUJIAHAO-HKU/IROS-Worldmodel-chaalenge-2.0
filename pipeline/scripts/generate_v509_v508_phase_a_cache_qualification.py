#!/usr/bin/env python3
"""Fresh v509 Phase-A driver with an exact wam_pipeline import environment.

Each worker receives a newly constructed environment whose sole entry is
``PYTHONPATH=$ROOT/pipeline``.  Auditors and service control never receive that
override.  This one-shot driver qualifies outputs and never trains.
"""
...
EXPECTED_WORKER_ENV={"PYTHONPATH":str(PIPELINE_ROOT)}
DRIVER_ENVIRONMENT_CONTRACT={"worker_environment_exact":dict(EXPECTED_WORKER_ENV),"worker_popen_env_exact":True,"worker_popen_env_uses":2,"service_popen_env_uses":0,"service_environment_overrides_authorized":False,"inherited_environment_authorized":False,"appended_pythonpath_authorized":False,"extra_environment_keys_authorized":False}
...
def validate_package_tree_and_import(manifest_path,manifest_sha,manifest_bytes):
    ...
    if imported.returncode!=0 or imported.stderr!="" or set(EXPECTED_WORKER_ENV)!={"PYTHONPATH"} or module_path.resolve()!=PACKAGE_ROOT/"__init__.py" or not module_path.resolve().is_relative_to(PACKAGE_ROOT):raise RuntimeError("wam_pipeline exact-env import")
    after=package_tree()
...
def run_worker(command,timeout,root,role,intent,worker_env,log_opener=None):
    if worker_env!=EXPECTED_WORKER_ENV or set(worker_env)!={"PYTHONPATH"}:raise RuntimeError("worker environment must be exact")
...
def main():
    ...
    for name in ("preregistration","contract","authority-receipt","scope-source","worker-source","auditor-source","materializer-source","static-auditor-source","launcher-source","driver-source","package-manifest"):p.add_argument(f"--{name}",type=Path,required=True)
    for name in ("preregistration-sha","authority-receipt-sha","scope-sha","worker-sha","auditor-sha","materializer-sha","static-auditor-sha","launcher-sha","driver-sha","package-manifest-sha"):p.add_argument(f"--{name}",required=True)
    p.add_argument("--driver-bytes",type=int,required=True);p.add_argument("--package-manifest-bytes",type=int,required=True)
    args=p.parse_args()
    hashes=[args.preregistration_sha,args.authority_receipt_sha,args.scope_sha,args.worker_sha,args.auditor_sha,args.materializer_sha,args.static_auditor_sha,args.launcher_sha,args.driver_sha,args.package_manifest_sha]
    ...
    if authority.get("fresh_phase_a_driver_source")!=driver_record or authority.get("wam_pipeline_package_tree_manifest_source")!=package_manifest_record or authority.get("wam_pipeline_package_tree")!=package_manifest or authority.get("wam_pipeline_exact_worker_environment")!=EXPECTED_WORKER_ENV or authority.get("driver_environment_contract")!=DRIVER_ENVIRONMENT_CONTRACT:raise RuntimeError("v509 package/driver authority")
