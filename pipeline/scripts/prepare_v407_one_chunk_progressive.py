#!/usr/bin/env python3
"""Preregister the single principled horizon correction to v406."""
...
NAME = "v407_one_chunk_progressive_seed1565_20260823"
RUN = J / NAME
REG = O / "run_registry" / NAME
PARENT_RUN = J / "v406_progressive_then_terminal_seed1564_20260823"
...
def main() -> None:
    ...
    sources = {
        "runtime": ROOT / "pipeline/wam_pipeline/v407_one_chunk_progressive_runtime.py",
        "v406_manifest": PARENT / "progressive_then_terminal_manifest.json",
        "v406_audit": PARENT_RUN / "audit/recursive_reward_causal.json",
        "audit": ROOT / "pipeline/scripts/audit_v407_recursive_reward_causal.py",
    }
    missing = [str(path) for path in sources.values() if not path.exists()]
    if missing:
        raise FileNotFoundError(missing)
    prior = json.loads(sources["v406_audit"].read_text())
    if prior.get("passed") is not False:
        raise RuntimeError("v406 failure evidence drift")
    ...
    manifest = {
        ...
        "model_version": "track2-v407-v406-one-action-chunk-progress",
        "v406_manifest_sha256": sha256(sources["v406_manifest"]),
        "progress_offset_frames": 8,
        "horizon_rationale": "exact official future-action/prediction chunk; v406's 24-frame jump created early high-reward false positives",
    ...
    (RUN / "release/one_chunk_progressive_manifest.json").write_text(
        json.dumps(manifest, indent=2) + "\n"
    )
    prereg = {
        "format": "strict-track2-v407-one-chunk-progressive-preregistration-v1",
