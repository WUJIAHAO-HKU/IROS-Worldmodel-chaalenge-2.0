#!/usr/bin/env python3
"""One-shot v523 exact21-contract-repaired RNG-isolated Phase-A launcher (design-only).

The launcher consumes the fresh v523 authority and exact-eight preregistration,
...
SEED = 1659
AUTHORITY_CONTRACT = SCRIPTS / "v523_v522_phase_a_per_call_rng_contract_repair_execution_authority_contract.json"
AUTHORITY_MATERIALIZER = SCRIPTS / "materialize_v523_v522_phase_a_per_call_rng_contract_repair_execution_authority.py"
AUTHORITY_MATERIALIZER_SHA = "2b131db2f0fb95e24abc646930bcf88cae791ec5b9d696f287e55daf00f7c88a"
AUTHORITY_MATERIALIZER_BYTES = 33316
SELF = SCRIPTS / "launch_v523_v522_phase_a_cache_qualification.py"
DRIVER = SCRIPTS / "generate_v523_v522_phase_a_cache_qualification.py"
DRIVER_SHA = "ee7a2a327cc7fc8f282eef9b97701860c310901cdf383d8bc923b7f276c4f3ab"
DRIVER_BYTES = 47276
WORKER = SCRIPTS / "generate_v523_v522_phase_a_cache_qualification_worker.py"
WORKER_SHA = "89a552416c054fa2590d7f15b5fb829b8eb2cf8a02479e2115b825bf6e49d627"
WORKER_BYTES = 38990
AUDITOR = SCRIPTS / "audit_v523_v522_phase_a_rng_isolated_qualification.py"
...
FAILURE_FORENSIC = SCRIPTS / "v523_v522_phase_a_per_call_rng_contract_failure_forensic.json"
...
AUTHORITY_FORMAT = "strict-track2-v523-v522-phase-a-per-call-rng-contract-repair-execution-authority-v1"
AUTHORITY_STATUS = "authorized_exact_one_external_v523_phase_a_cache_qualification_attempt"
...
INTENT_FORMAT = "strict-track2-v523-v522-phase-a-cache-qualification-attempt-intent-v1"
TERMINAL_FORMAT = "strict-track2-v523-v522-phase-a-cache-qualification-attempt-terminal-v1"
SOURCE_ORDER = [
    ...
    "v523_v522_phase_a_failure_forensic",
    "v522_authority_design_contract",
    "v522_authority_materializer",
    "v522_authority_receipt",
    "v522_phase_a_launcher",
    "v522_phase_a_driver",
    "v522_phase_a_worker",
    "v522_phase_a_independent_auditor",
    "v522_attempt_intent",
    "v522_attempt_stdout",
    "v522_attempt_stderr",
    "v522_attempt_terminal",
...
def load_v523_materializer():
    ...
    if (AUTHORITY_MATERIALIZER.is_symlink() or not AUTHORITY_MATERIALIZER.is_file()
        ...
        raise RuntimeError("v523 materializer source")
    spec = importlib.util.spec_from_file_location("v523_authority_materializer_consumer", AUTHORITY_MATERIALIZER)
...
def validate_authority(args: argparse.Namespace) -> dict:
    ...
    if args.authority_contract != AUTHORITY_CONTRACT or args.authority_receipt != AUTHORITY_RECEIPT or args.launcher_source != SELF:
        raise RuntimeError("canonical active paths")
    materializer = load_v523_materializer()
    ...
    if (set(contract) != CONTRACT_TOP_KEYS or len(contract) != 35
        ...
        raise RuntimeError("v523 schema/terminal")
    ...
    if (set(receipt_schema) != receipt_schema_keys
        ...
        raise RuntimeError("v523 authority boundary")
    ...
    if (any(type(authority["authorization"].get(key)) is not int for key in authorization_count_keys)
        ...
        raise RuntimeError("v523 authorization count types")
    closure = authority.get("source_closure")
    if (authority.get("source_role_order") != SOURCE_ORDER or set(closure or {}) != set(SOURCE_ORDER)
        ...
        raise RuntimeError("v523 authority source closure")
    contract_closure = contract.get("source_closure")
    if (contract.get("source_role_order") != SOURCE_ORDER[1:] or set(contract_closure or {}) != set(SOURCE_ORDER[1:])
        ...
        raise RuntimeError("v523 contract source closure")
    for role in SOURCE_ORDER:
        record = closure[role]
        if authority.get(materializer.SOURCE_ALIASES[role]) != record:
            raise RuntimeError("v523 source alias " + role)
        if regular(Path(record["path"]), record["sha256"], record["logical_bytes"]) != record:
            raise RuntimeError("v523 source current " + role)
    expected_active = {
    ...
    if any(closure[role] != record for role, record in expected_active.items()):
        raise RuntimeError("v523 active source binding")
    expected_lineage = {
        "name": "v523_v522_phase_a_per_call_rng_contract_repair",
        "fresh_authority_root": str(AUTHORITY_ROOT),
        "fresh_phase_a_attempt_root": str(ATTEMPT_ROOT),
        "fresh_qualification_output_root": str(QUALIFICATION_ROOT),
    ...
    if (not json_exact(contract.get("lineage"), expected_lineage)
        ...
        raise RuntimeError("v523 contract active lineage")
    ...
    for key, expected in exact_anchors.items():
        if not json_exact(contract.get(key), expected) or not json_exact(authority.get(key), expected):
            raise RuntimeError("v523 old-state anchor " + key)
    ...
    if any(type(authority["v522_phase_a_execution_partition"].get(key)) is not int for key in partition_count_keys):
        raise RuntimeError("v523 old partition count types")
    ...
    for path in (DRIVER, WORKER, AUDITOR):
        if not json_exact(materializer.literal_assignment(path, "PER_CALL_RNG_CONTRACT"), PER_CALL_RNG_CONTRACT):
            raise RuntimeError("v523 per-call exact21 " + str(path))
    ...
    if contract.get("historical_absences") != contract_historical:
        raise RuntimeError("v523 historical absence record")
    if contract.get("current_absences_after_authority") != contract_current:
        raise RuntimeError("v523 current absence record")
    if authority.get("historical_absences") != receipt_historical:
        raise RuntimeError("v523 historical absence ancestry")
    if authority.get("required_absences") != receipt_current:
        raise RuntimeError("v523 required absences")
    ...
    if authority.get("input_snapshots_exactly_equal") is not True or authority.get("input_pre_snapshot") != authority.get("input_post_snapshot"):
        raise RuntimeError("v523 input snapshots")
    validate_rlpy()
    return {"contract": contract, "authority": authority, "closure": closure, "preregistration": prereg,
            "contract_record": contract_record, "authority_record": authority_record, "self_record": self_record,
            "failure_forensics": {"v522_per_call_contract_failure": closure["v523_v522_phase_a_failure_forensic"]}}
...
def stable_snapshot(context: dict) -> dict:
    ...
    records = [context["contract_record"], context["authority_record"], context["self_record"]]
    records.extend(context["closure"][role] for role in SOURCE_ORDER)
    tree_keys = ("v522_authority_registration_tree", "v522_attempt_tree")
    trees = {key: exact_tree(Path(authority[key]["root"])) for key in tree_keys}
    if any(trees[key] != authority[key] for key in tree_keys):
        raise RuntimeError("v523 ancestry tree drift")
    return {"records": records, "records_sha256": csha(records), "ancestry_trees": trees,
            "v522_authority_semantics": authority["v522_authority_semantics"],
            "v522_phase_a_failure_forensic": authority["v522_phase_a_failure_forensic"],
            "v522_per_call_contract_failure_root_cause": authority["v522_per_call_contract_failure_root_cause"]}
...
def synthetic_self_test() -> int:
    ...
    checks["v522_failure_ancestry"] = all(token in source for token in ("3017c094b9b5067bf134143361fe664e04ece28306855db8c0c671cfb452033b","3fd62a21c9b9de6ea652f8fd42cf448bf39948e55f4f61a3d9b48c0a78e8d433","94e06c8d9dcd31e3026e069dafe8b5bb6adbda85e51fd5e1e448f8dd4e6fab0d"))
    checks["rng_proxy_binding"] = RNG_PROXY_SHA in source and WORKER_SHA in source and "rng_isolation_event_count" in source
    checks["v523_exact8"] = "materializer.validate_prereg(closure)" in source
    ...
    checks["materializer_binding"] = AUTHORITY_MATERIALIZER_SHA in source and AUTHORITY_MATERIALIZER_BYTES == 33316
    checks["failure_repair_full_consumer"] = all(token in source for token in (
        "materializer.validate_old_state()", "v522_authority_registration_tree",
        "v522_attempt_tree", "v522_per_call_contract_failure_root_cause"))
