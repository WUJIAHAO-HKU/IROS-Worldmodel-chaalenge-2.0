...
FROZEN="$OFF/frozen_candidates/v277_v274r1_fullbudget_step5_seed1471"
...
OUTPUT_ROOT="$EVAL/frozen_v277_v274r1_fullbudget_step5_seed1471_final128"
...
WRAPPER="$BASE/pipeline/scripts/run_frozen_v277_v274r1_final128_once.sh"
...
assert freeze["variant"] == "v275_v274r1_fullbudget_step5_seed1471"
...
print("V277_FROZEN_FINAL128_INPUTS_VERIFIED")
PY
...
from pathlib import Path
freeze,prereg,seeds,output=map(Path,sys.argv[1:]); digest=lambda p: hashlib.sha256(p.read_bytes()).hexdigest()
payload={"format":"strict-track2-v277-frozen-final128-execution-v1","started_utc":datetime.now(timezone.utc).isoformat(),"contest_submission":False,"variant":"v275_v274r1_fullbudget_step5_seed1471","freeze_manifest_sha256":digest(freeze),"preregistration_sha256":digest(prereg),"seed_bundle_sha256":digest(seeds),"resume_policy":"only incomplete batches of this exact frozen candidate"}
...
import hashlib,json,sys
from pathlib import Path
invocation_path,freeze,prereg,seeds=map(Path,sys.argv[1:]);x=json.loads(invocation_path.read_text());digest=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
assert x["contest_submission"] is False and x["variant"] == "v275_v274r1_fullbudget_step5_seed1471"
...
print("V277_FINAL128_EXECUTION_IDENTITY_VERIFIED")
PY
