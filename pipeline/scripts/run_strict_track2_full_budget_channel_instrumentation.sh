...
INSTRUMENTED_RUNTIME="$AUDIT_ROOT/full_budget_channel_instrumented_runtime"
PREREG="$ROOT/pipeline/config/strict_track2_full_budget_channel_instrumentation_preregistration.json"
RUN_ROOT="$AUDIT_ROOT/runs/full_budget_channel_instrumentation_v2_v157_seed1238"
PYTHON_BIN=/root/autodl-tmp/conda_envs/rlinf_track2/bin/python
RUNTIME_PYTHONPATH="$INSTRUMENTED_RUNTIME:$DIFFSYNTH_ROOT:$OPENPI_ROOT/packages/openpi-client/src:$OPENPI_ROOT/src:$RLINF_ROOT"
...
assert "stage-boundary log messages only" in registration["instrumentation"]["scope"]
PY
