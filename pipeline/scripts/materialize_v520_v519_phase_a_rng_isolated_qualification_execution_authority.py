#!/usr/bin/env python3
"""Design-only v520 Phase-A authority materializer.

The only runtime authority granted by this receipt is one fresh Phase-A cache
qualification launcher.  The fresh worker must route every runtime delegate
through the frozen RNG-isolation proxy and persist four-stage RNG evidence for
each call; the successful v519 sample is ancestry/evidence, not a substitute
for the per-call production contract.
"""
...
SELF_PATH = S / 'materialize_v520_v519_phase_a_rng_isolated_qualification_execution_authority.py'
CONTRACT_PATH = S / 'v520_v519_phase_a_rng_isolated_qualification_execution_authority_contract.json'
LAUNCHER_PATH = S / 'launch_v520_v519_phase_a_cache_qualification.py'
DRIVER_PATH = S / 'generate_v520_v519_phase_a_cache_qualification.py'
WORKER_PATH = S / 'generate_v520_v519_phase_a_cache_qualification_worker.py'
PROXY_PATH = S / 'v520_v519_rng_isolated_v169_runtime_proxy.py'
AUDITOR_PATH = S / 'audit_v520_v519_phase_a_rng_isolated_qualification.py'
PREREG_ROOT = J / 'v520_v519_phase_a_cache_qualification_prereg_seed1656_20260826'
...
CONTRACT_SOURCE_ORDER = ACTIVE_ROLES + FIXED_ROLES + V519_SOURCE_ROLES + list(FAILURE_SOURCES)
...
OUTPUT_FORMAT = 'strict-track2-v520-v519-phase-a-rng-isolated-qualification-execution-authority-v1'
OUTPUT_STATUS = 'authorized_exact_one_external_v520_phase_a_cache_qualification_attempt'
...
def main():
    ...
    if os.sys.argv[1:] == ['--synthetic-self-test']:
        checks = {
            'source_roles': len(CONTRACT_SOURCE_ORDER) == 22,
    ...
    if set(contract) != CONTRACT_TOP_KEYS or contract.get('format') != CONTRACT_FORMAT or contract.get('status') != CONTRACT_STATUS or contract.get('seed') != 1656:
    ...
    if contract.get('lineage') != expected_lineage or contract.get('authorization') != AUTHORIZATION or contract.get('runtime_observation') != RUNTIME or contract.get('execution_boundary') != EXECUTION_BOUNDARY:
    ...
    if any(contract.get(key) != value for key, value in exact_contract_values.items()):
    ...
    trees = {'v519_evidence': str(V519_EVIDENCE), 'v517_evidence': str(V517_EVIDENCE), 'v517_candidate': str(V517_CANDIDATE_ROOT), 'v517_f813_current': v517_current['f813_tree']['root']}
    ...
    if receipt_schema.get('top_keys') != sorted(receipt) or receipt_schema.get('check_keys') != CHECK_KEYS or receipt_schema.get('authorization_exact') != AUTHORIZATION or receipt_schema.get('per_call_rng_evidence_contract_exact') != PER_CALL_RNG_CONTRACT:
