...
REG="$J/v450_paired_intervention_pilot_seed1603_20260823"
...
test "$(sha256sum "$ROOT/pipeline/scripts/prepare_v450_paired_intervention_pilot.py" | awk '{print $1}')" = 'a8028bfde723c30f2d1f6989d79f725de9eebcaa365e4a9b4ac9cadfa07f1047'
test "$(sha256sum "$ROOT/pipeline/scripts/generate_v450_paired_intervention_pilot.py" | awk '{print $1}')" = '25fe2721756a35d92abd0c8c4cd27497f75bf530dc3671bef5a2e3cffddca31c'
test "$(sha256sum "$ROOT/pipeline/scripts/audit_v450_paired_intervention_pilot.py" | awk '{print $1}')" = '455472813bb5f1f3f6d90f356cc3d2c84e4bc0bb11a73d708942305f7cdb6830'
mkdir -p "$REG"
cd "$ROOT"
PYTHONPATH="$ROOT/pipeline:$ROOT/pipeline/scripts" taskset -c 0-5 "$RLPY" pipeline/scripts/prepare_v450_paired_intervention_pilot.py \
 --split "$SPLIT" --dataset "$DATASET" --windows "$WINDOWS" --seed-file "$DATASET/seed.txt" \
 --collector "$ROOT/pipeline/scripts/generate_v450_paired_intervention_pilot.py" --auditor "$ROOT/pipeline/scripts/audit_v450_paired_intervention_pilot.py" \
...
cd "$SUPPORT"
PYTHONPATH="$SUPPORT:$ROOT/pipeline:$ROOT/pipeline/scripts" timeout --signal=TERM --kill-after=30s 20m nice -n 10 taskset -c 0-7 "$RLPY" "$ROOT/pipeline/scripts/generate_v450_paired_intervention_pilot.py" \
...
cd "$ROOT"
PYTHONPATH="$ROOT/pipeline:$ROOT/pipeline/scripts" taskset -c 0-5 "$RLPY" pipeline/scripts/audit_v450_paired_intervention_pilot.py \
 --preregistration "$REG/preregistration.json" --dataset-dir "$REG/dataset" --generator "$ROOT/pipeline/scripts/generate_v450_paired_intervention_pilot.py" --output "$REG/audit.json" >"$REG/audit.log" 2>&1
...
trap - EXIT
echo V450_TECHNICAL_PILOT_COMPLETE
