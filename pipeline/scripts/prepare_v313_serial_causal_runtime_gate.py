#!/usr/bin/env python3
"""Preregister the exact-batch v313 repair before any held-out scoring."""
...
N = "v313_v311_serial_causal_runtime_gate_seed1486_20260822"
RUN = J / N
...
def main() -> None:
    ...
    paths = {
        "v313_runtime": B / "pipeline/wam_pipeline/v313_serial_causal_terminal_mirror_runtime.py",
        "v312_runtime": B / "pipeline/wam_pipeline/v312_causal_terminal_mirror_runtime.py",
        "backends": B / "pipeline/wam_pipeline/backends.py",
        "contract_test": B / "pipeline/scripts/test_v313_serial_causal_terminal_mirror.py",
        "base_contract_test": B / "pipeline/scripts/test_v312_causal_terminal_mirror.py",
        "audit": B / "pipeline/scripts/audit_v313_causal_runtime_gate.py",
        "base_audit": B / "pipeline/scripts/audit_v312_causal_runtime_gate.py",
    ...
    payload = {
        "format": "strict-track2-v313-serial-causal-runtime-gate-preregistration-v1",
        "registered_at": datetime.now(timezone.utc).isoformat(),
        "repair_from_v312": {
            "observed_failure": "native batch differed from serial by up to three uint8 levels",
            "repair": "batch method invokes the exact frozen single-request path per sample",
            "single_request_model_or_threshold_changed": False,
            "heldout_reward_gate_run_before_registration": False,
        },
        "candidate": {
            "model_version": "track2-v313-serial-public-action-gated-full-terminal-mirror-v271",
