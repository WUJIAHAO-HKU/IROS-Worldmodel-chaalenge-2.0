#!/usr/bin/env python3
"""Atomically register the v500 diagnostic-qualified authority; run no execution source."""
...
CONTRACT_PATH = ROOT / "pipeline/scripts/v500_v499_v498_v497_v496_v495_failure_tree_diagnostic_execution_authority_contract.json"
CONTRACT_FORMAT = "strict-track2-v500-v499-v498-v497-v496-v495-failure-tree-diagnostic-execution-authority-design-contract-v1"
CONTRACT_STATUS = "design_only_frozen_sources_pending_independent_review_no_authority"
OUTPUT_FORMAT = "strict-track2-v500-v499-v498-v497-v496-v495-failure-tree-diagnostic-execution-authority-v1"
OUTPUT_STATUS = "authorized_exact_one_external_v500_failure_tree_diagnostic_outer_attempt"
...
INNER_ATTEMPT_ROOT = J / "v500_v499_v498_v497_v496_v495_failure_tree_diagnostic_reconciliation_inner_attempt_seed1642_20260825"
...
SOURCE_ROLE_ORDER = [
    ...
    "v499_adapter_script", "phase_a_design_contract", "reconciler_r2", "corrected_inner_wrapper",
...
def synthetic_self_test() -> int:
    checks = {
        "authority_top_count": len(AUTH_TOP_KEYS) == 89,
        "check_count": len(AUTH_CHECK_KEYS) == 81,
        "source_roles": len(SOURCE_ROLES) == 31,
        "source_order_aliases": (len(SOURCE_ROLE_ORDER) == 31
                                  and set(SOURCE_ROLE_ORDER) == SOURCE_ROLES
                                  and set(SOURCE_ALIASES) == SOURCE_ROLES
                                  and len(set(SOURCE_ALIASES.values())) == 31),
        ...
        "fresh_roots": all("v500_" in path.name for path in
...
def main() -> int:
    ...
    if (set(contract) != CONTRACT_TOP_KEYS or contract.get("format") != CONTRACT_FORMAT
            or contract.get("status") != CONTRACT_STATUS or contract.get("seed") != 1642
            or pending(contract)):
