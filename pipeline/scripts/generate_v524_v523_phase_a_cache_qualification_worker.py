#!/usr/bin/env python3
"""Fresh-process v524 A/B worker enforcing exact ordered source-role closure."""
...
def validate_exact_source_closure(formal):
    ...
    if exact_roles!=EXACT_SOURCE_ROLES or not isinstance(sources,dict) or set(sources)!=set(EXACT_SOURCE_ROLES) or not isinstance(records,list) or len(records)!=8:raise RuntimeError("exact ordered eight source roles")
    ...
    if formal.get("execution_sources_digest_sha256")!=canonical_sha(records):raise RuntimeError("source records digest")
...
def validate_v523_authorization(authority):
    ...
    if set(authorization)!=AUTHORIZATION_KEYS:raise RuntimeError("v523 authorization exact keys")
    ...
    if authorization!=exact:raise RuntimeError("v523 authorization values")
...
def main():
    ...
    formal=json.loads(args.preregistration.read_text());contract=json.loads(args.contract.read_text());validate_exact_source_closure(formal)
