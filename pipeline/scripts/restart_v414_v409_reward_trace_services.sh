...
VERSION='track2-v414-v409-output-equivalent-reward-trace'
...
service() {
  ...
  exec taskset -c "$CPUSET" env PYTHONPATH="$ROOT/pipeline" \
    ...
    WAM_BACKEND='v414-v409-reward-trace' \
...
case "${1:-start}" in
  ...
  bridge) bridge ;;
  stop)
    screen -S wm_v414_bridge -X quit >/dev/null 2>&1 || true
    screen -S wm_v414_gpu -X quit >/dev/null 2>&1 || true
    ;;
  start)
    mkdir -p "$REG" "$TRACE_DIR"
    test -z "$(find "$TRACE_DIR" -mindepth 1 -maxdepth 1 -print -quit)"
    for name in wm_v414_bridge wm_v414_gpu wm_v218_bridge wm_v218_gpu; do
