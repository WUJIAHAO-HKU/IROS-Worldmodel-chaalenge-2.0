...
"$PY" "$ROOT/pipeline/scripts/prepare_v240_alignment_gated_transport.py" \
  >"$OFF/run_registry/$NAME.prepare.log" 2>&1
mv "$OFF/run_registry/$NAME.prepare.log" "$REG/prepare.log"
cd /tmp
PYTHONPATH="$ROOT/pipeline:$ROOT/pipeline/scripts:$RLINF" "$PY" \
  "$ROOT/pipeline/scripts/analyze_v240_alignment_gated_transport.py" \
...
echo V240_ALIGNMENT_GATED_TRANSPORT_COMPLETE
