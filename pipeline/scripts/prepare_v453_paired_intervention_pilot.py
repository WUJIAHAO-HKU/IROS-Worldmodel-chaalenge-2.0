...
SEED=1606; VARIANTS=("factual","open","static_transport","reverse_transport","hold_all","factual_duplicate")
...
def sha(path:Path)->str:
 ...
 return h.hexdigest()
def function_sha(path:Path,name:str)->str:
 text=path.read_text(); tree=ast.parse(text); node=next(x for x in tree.body if isinstance(x,(ast.FunctionDef,ast.AsyncFunctionDef)) and x.name==name); source=ast.get_source_segment(text,node)
 return hashlib.sha256(source.encode()).hexdigest()
def main()->int:
 p=argparse.ArgumentParser()
 for n in ("split","dataset","windows","seed-file","collector","auditor","gate-runtime","resize-source","output"): p.add_argument(f"--{n}",type=Path,required=True)
 ...
 if len(seeds)!=50 or not set(chosen)<=set(right) or set(chosen)&validation: raise RuntimeError("v453 public-right train seed contract failed")
 contexts=[]
