...
ATTEMPT_TAG="${TRACK2_DIAGNOSTIC_ATTEMPT_TAG:-}"
BASE="$AUDIT/full_budget_gpu_cache_release_runtime"
RUNTIME="$AUDIT/probability_consistent_one_update_runtime${ATTEMPT_TAG}"
RUN="$AUDIT/runs/probability_consistent_one_update_v157_seed1243${ATTEMPT_TAG}"
PATCH="$ROOT/pipeline/patches/rlinf_openpi_frozen_vlm_mode_consistency.patch"
PREREG="$ROOT/pipeline/config/strict_track2_probability_consistent_one_update_preregistration.json"
...
SERVICE_DIR="$AUDIT/probability_consistent_service${ATTEMPT_TAG}"
BRIDGE_DIR="$AUDIT/probability_consistent_bridge${ATTEMPT_TAG}"
...
"$PY" "$RUNTIME/examples/embodiment/train_embodied_agent.py" \
  ...
  runner.logger.log_path="$RUN" runner.max_steps=1 runner.max_epochs=1 runner.save_interval=1000 \
...
printf 'PROBABILITY_CONSISTENT_ONE_UPDATE_COMPLETE_NONFORMAL\n'
