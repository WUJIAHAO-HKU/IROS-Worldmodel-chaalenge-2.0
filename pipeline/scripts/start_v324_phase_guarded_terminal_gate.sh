...
RUN="$J/v324_v317_phase_guarded_terminal_gate_seed1494_20260822"
...
taskset -c 0-7 "$PY" pipeline/scripts/test_v324_phase_guarded_terminal.py \
  "${COMMON[@]}" --output "$RUN/audit/contract_report.json" \
  >>"$RUN/audit/contract.log" 2>&1

taskset -c 0-7 "$PY" pipeline/scripts/audit_v324_phase_guarded_terminal_gate.py \
  "${COMMON[@]}" \
...
printf 'V324_PHASE_GUARDED_TERMINAL_GATE_PASSED\n'
