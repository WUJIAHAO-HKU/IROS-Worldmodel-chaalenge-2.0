#!/usr/bin/env python3
"""Materialize exactly one authority for the v531 actual-OOF execution boundary."""
...
SELF = S / 'materialize_v531_v530_actual_oof_execution_authority.py'
CONTRACT = S / 'v531_v530_actual_oof_execution_authority_contract.json'
PREREG = S / 'v531_v530_actual_oof_execution_preregistration.json'
MANIFEST = S / 'v531_v530_actual_oof_execution_manifest.json'
EXECUTOR = S / 'execute_v531_v530_actual_oof.py'
AUDITOR = S / 'audit_v531_v530_actual_oof.py'
LAUNCHER = S / 'launch_v531_v530_actual_oof.py'

PREREG_EXPECTED = ('6fc85dd57bd6693a276c6f3ddfbbda1ff6cf42af7f5a1417dcdfbab460045329', 15071)
MANIFEST_EXPECTED = ('ec0a5c396972475ead9eb70db93320799a239257e88938490453b6236ecd7fc9', 1485362)
EXECUTOR_EXPECTED = ('14537cfe9f54ba4f1d47609102c12768a9c215317cd9c8f66346f0a7bb33587b', 47719)
AUDITOR_EXPECTED = ('8251aedf27fcbd9966a46e9c5ef7ef3ce55b0e986251a7b9dcc31a820f1d3792', 10155)
LAUNCHER_EXPECTED = ('653de64004c2582ef776bd4965d52693f738e2e12f99377c448ac79f00287895', 5515)

AUTH_ROOT = J / 'v531_v530_actual_oof_execution_authority_seed1665_20260827'
AUTH_PREP = AUTH_ROOT.with_name(AUTH_ROOT.name + '.authority-prep')
ATTEMPT_ROOT = J / 'v531_v530_actual_oof_attempt_seed1665_20260827'
ATTEMPT_PREP = ATTEMPT_ROOT.with_name(ATTEMPT_ROOT.name + '.attempt-prep')
OOF_ROOT = Path('/root/v531_v530_actual_oof_seed1665_20260827')
...
CONTRACT_FORMAT = 'strict-track2-v531-v530-actual-oof-execution-authority-design-contract-v1'
CONTRACT_STATUS = 'design_only_frozen_actual_oof_sources_pending_independent_authority_materialization'
OUTPUT_FORMAT = 'strict-track2-v531-v530-actual-oof-execution-authority-v1'
...
SOURCE_ROLES = [
    ...
    'actual_oof_executor', 'actual_oof_auditor', 'actual_oof_launcher', 'v530_external_terminal_process',
    'v527_candidate_receipt', 'v527_authority_receipt', 'v528_deployment_receipt',
    'v524_qualification_terminal', 'v524_qualification_report', 'v524_qualification_audit',
    'v524_cache_manifest', 'v482_preregistration', 'v482_runtime_source', 'v482_trainer_source',
    'v482_model_design_contract', 'v482_s0_auditor', 'v478_selection',
]
...
CONTRACT_TOP_KEYS = {
    'format', 'status', 'seed', 'lineage', 'source_closure', 'source_role_order', 'source_aliases',
...
def load_executor():
    ...
    spec = importlib.util.spec_from_file_location('v531_authority_executor_schema', EXECUTOR)
...
def validate_context(args):
    ...
    if contract['format'] != CONTRACT_FORMAT or contract['status'] != CONTRACT_STATUS or type(contract['seed']) is not int or contract['seed'] != 1665:
        raise RuntimeError('contract header')
    if not json_exact(contract['authorization'], AUTHORIZATION) or not json_exact(contract['runtime_observation'], RUNTIME) or not json_exact(contract['execution_boundary'], EXECUTION_BOUNDARY):
    ...
    for role in ('authority_materializer', 'actual_oof_execution_preregistration', 'actual_oof_execution_manifest', 'actual_oof_executor', 'actual_oof_auditor', 'actual_oof_launcher'):
...
def main():
    if os.sys.argv[1:] == ['--synthetic-self-test']:
        checks = {
            ...
            'source_count': len(SOURCE_ROLES) == 20,
            'authority_source_count': len(AUTHORITY_SOURCE_ROLES) == 21,
