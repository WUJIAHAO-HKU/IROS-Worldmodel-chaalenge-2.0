#!/usr/bin/env python3
"""Exact-once, signal-safe transport for the v520 Phase-A authority materializer."""
...
SELF_PATH = SCRIPTS / "invoke_v520_v519_phase_a_rng_isolated_qualification_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v520_v519_phase_a_rng_isolated_qualification_authority_materialize_once.sh")
...
EVIDENCE_ROOT = J / "v520_v519_phase_a_rng_isolated_qualification_execution_authority_materialization_evidence_seed1656_20260826"
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v520-v519-phase-a-rng-isolated-qualification-authority-materializer-process-receipt-v1",
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v520-v519-phase-a-rng-isolated-qualification-authority-materializer-intent-v1",
...
def production_fixture(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    checks["final8_hardbound"] = all(value in source for value in (
...
def main() -> int:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v520-v519-phase-a-rng-isolated-qualification-authority-materializer-argv-v1",
