...
def main() -> None:
    ...
    variants = (BASELINE_VARIANT, args.candidate_variant)
    ...
    baseline = instrumented[BASELINE_VARIANT]
    candidate = instrumented[args.candidate_variant]
    paired = {
        "all": exact_mcnemar(
            baseline["seed_results"], candidate["seed_results"],
        ),
        "left": exact_mcnemar(
            baseline["seed_results"], candidate["seed_results"],
            arm="left",
    ...
    baseline_rate = primary[BASELINE_VARIANT]["success_rate"]
    trained_rate = primary[args.candidate_variant]["success_rate"]
    ...
    arm_improvement = {
        arm: rate_delta(
            baseline[arm]["success_rate"], candidate[arm]["success_rate"],
        )
        for arm in ("left", "right")
    }
    grasp_improvement = {
        "all": rate_delta(
            baseline["grasp_completion_rate"], candidate["grasp_completion_rate"],
        ),
        **{
            arm: rate_delta(
                baseline[arm]["grasp_completion_rate"], candidate[arm]["grasp_completion_rate"],
            )
            for arm in ("left", "right")
        },
    }
