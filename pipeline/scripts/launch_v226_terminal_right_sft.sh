...
NAME='v226_v223_terminal_rightsft_kl_h8_step64_seed1426_20260818'
RUN="$OFF/runs/$NAME"
...
BASE_CKPT="$OFF/runs/v223_v169_structured_rightsft_kl_h8_step24_seed1423_20260818/wan_robotwin_adjust_bottle_http_full_grpo_openpi_pi05/checkpoints/global_step_24/actor/model_state_dict/full_weights.pt"
SFT_DATA='/root/autodl-tmp/iros_v15_rl_probability_audit_v2/artifacts/lerobot/adjust_bottle_train40_right_terminal_v1'
DATASET_AUDIT="$OFF/run_registry/v225_right_terminal_dataset_20260818/dataset_audit.json"
LOADER_AUDIT="$OFF/run_registry/v225_right_terminal_dataset_20260818/transformed_loader_audit.json"
...
test ! -e "$RUN"
test ! -e "$REG"
test -f "$OFF/run_registry/v225_right_terminal_dataset_20260818/V225_RIGHT_TERMINAL_DATASET_ACCEPTED"
df --output=avail -B1 /root/autodl-tmp | tail -1 | awk '$1 >= 25000000000 {ok=1} END {exit !ok}'
"$PY" "$P/prepare_v226_terminal_right_sft.py" "$REG/preregistration.json" \
  "$BASE_CKPT" "$SFT_DATA" "$DATASET_AUDIT" "$LOADER_AUDIT" \
  "$RLROOT/rlinf/workers/actor/fsdp_actor_worker.py" \
  "$RLROOT/rlinf/models/embodiment/openpi/openpi_action_model.py" \
  "$P/run_strict_track2_conservative_kl.sh" > "/tmp/${NAME}.prepare.log"
...
export TRACK2_ACTOR_SEED=1426 TRACK2_ENV_SEED=0
...
"$PY" "$P/verify_v226_terminal_right_sft.py" "$RUN" "$OUTPUT_CKPT" 64 \
  > "$RUN/audit/terminal_right_sft_training_acceptance.log"
...
echo V226_TERMINAL_RIGHT_SFT_TRAINING_ACCEPTED
