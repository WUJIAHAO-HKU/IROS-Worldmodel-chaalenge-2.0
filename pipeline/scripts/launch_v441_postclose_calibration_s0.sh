#!/usr/bin/env bash
# Public-train-only v441 postclose calibration, S0 audit and packaging.
set -euo pipefail
...
PYTHONPATH="$ROOT/pipeline" taskset -c 0-5 "$GO1" \
  pipeline/scripts/prepare_v441_postclose_trainonly_projection.py \
...
PYTHONPATH="$ROOT/pipeline:$ROOT/pipeline/scripts" taskset -c 0-11 "$GO1" \
  pipeline/scripts/calibrate_v441_postclose_trainonly_rgb_projection.py \
...
PYTHONPATH="$ROOT/pipeline:$ROOT/pipeline/scripts" taskset -c 0-5 "$GO1" \
  pipeline/scripts/audit_v441_static_contract.py \
...
PYTHONPATH="$ROOT/pipeline:$ROOT/pipeline/scripts" taskset -c 0-5 "$GO1" \
  pipeline/scripts/package_v441_postclose_aligned_release.py \
