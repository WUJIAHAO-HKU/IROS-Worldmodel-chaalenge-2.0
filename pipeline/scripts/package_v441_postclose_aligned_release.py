#!/usr/bin/env python3
"""Package a passed v441 S0 core; never start service, trace, or RL."""
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=J / "v441_v169_postclose_aligned_projection_release")
    ...
    if index.get("format") != "strict-track2-v441-postclose-trainonly-rgb-alignment-index-v1":
        raise RuntimeError("wrong v441 alignment index")
    if s0.get("format") != "strict-track2-v441-postclose-s0-static-contract-v1" or s0.get("passed") is not True:
        raise RuntimeError("v441 S0 did not pass")
    if s0.get("sha256", {}).get("alignment_index") != sha256(args.alignment_index):
        raise RuntimeError("v441 S0/index binding mismatch")
    ...
    if prereg.get("evidence_sha256", {}).get("runtime") != sha256(runtime):
        raise RuntimeError("v441 runtime differs from preregistration")
    ...
    manifest = {
        ...
        "runtime_class": "wam_pipeline.v441_v169_postclose_aligned_projection_runtime.Track2V441V169PostcloseAlignedProjection",
    ...
    manifest_path = args.output / "v441_postclose_aligned_manifest.json"
