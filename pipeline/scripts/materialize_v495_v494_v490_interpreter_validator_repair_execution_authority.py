#!/usr/bin/env python3
"""Atomically register the v495 validator repair authority; run no execution source."""
...
CONTRACT_PATH = ROOT / "pipeline/scripts/v495_v494_v490_interpreter_validator_repair_execution_authority_contract.json"
CONTRACT_FORMAT = "strict-track2-v495-v494-v490-interpreter-validator-repair-execution-authority-design-contract-v1"
CONTRACT_STATUS = "design_only_frozen_sources_pending_independent_review_no_authority"
OUTPUT_FORMAT = "strict-track2-v495-v494-v490-interpreter-validator-repair-execution-authority-v1"
OUTPUT_STATUS = "authorized_exact_one_external_v495_interpreter_validator_repaired_outer_attempt"
...
INNER_ATTEMPT_ROOT = J / "v495_v494_v490_interpreter_validator_repaired_reconciliation_inner_attempt_seed1637_20260825"
...
def synthetic_self_test() -> int:
    checks = {
        "authority_top_count": len(AUTH_TOP_KEYS) == 47,
        "check_count": len(AUTH_CHECK_KEYS) == 37,
        "source_roles": len(SOURCE_ROLES) == 14,
        ...
        "fresh_roots": all("v495_" in path.name for path in
...
def main() -> int:
    ...
    if (set(contract) != CONTRACT_TOP_KEYS or contract.get("format") != CONTRACT_FORMAT
            or contract.get("status") != CONTRACT_STATUS or contract.get("seed") != 1637
            or pending(contract)):
    ...
    if (v493_evidence_tree["file_count"] != 6
            ...
            or v493_process.get("materializer_invocations") != 1
            or v493_process.get("outer_wrapper_invocations") != 0
            or v493_process.get("inner_wrapper_invocations") != 0
            or v493_process.get("r2_invocations") != 0
            or v493_process.get("retry_authorized") is not False
            or v493_process.get("pre_post_snapshots_exactly_equal") is not True
            or v493_process.get("cleanup", {}).get("group_empty") is not True):
