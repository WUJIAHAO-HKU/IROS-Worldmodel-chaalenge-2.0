#!/usr/bin/env python3
"""Preregistered v464 three-head 5-fold endpoint kill gate; no reward or RL."""
...
from wam_pipeline.v464_v169_endpoint_residual_runtime import CHECKPOINT_FORMAT,EndpointResidualUNet128FiLM,action_features,instruction_tokens
...
def atomic_json(path,payload):
 ...
 if tmp.exists():raise RuntimeError(f"refusing stale v464 temporary receipt: {tmp}")
...
def stable_seed(episode,start):return int.from_bytes(hashlib.sha256(f"v464/{SEED}/{episode}/{start}".encode()).digest()[:8],"little")%(2**31)
...
def release_inventory(release,joint_root):
 ...
 def visit(logical,relative):
  ...
  if not inside(resolved,joint_root) or not resolved.is_dir():raise RuntimeError("v464 inventory directory escape")
  key=(resolved.stat().st_dev,resolved.stat().st_ino)
  if key in active:raise RuntimeError("v464 inventory cycle")
  if key in seen_dirs:raise RuntimeError("v464 inventory duplicate directory target")
  ...
  for child in sorted(logical.iterdir(),key=lambda x:x.name):
   ...
   if not inside(target,joint_root):raise RuntimeError("v464 inventory target escape")
   ...
   if target.is_dir():visit(child,rel)
    ...
    if target_key in seen_files:raise RuntimeError("v464 inventory duplicate file target")
   ...
   else:raise RuntimeError("v464 inventory unsupported target")
  active.remove(key)
...
def verify_closure(pre,release,library):
 ...
 for records in (pre["v169"]["release_files"],pre["v169"]["library_files"]):
  for row in records:
   if set(row)!={"record_type","base_kind","relative","lexical_path","link_target","resolved_path","target_sha"} or row["base_kind"] not in roots:raise RuntimeError("v464 closure record schema")
   ...
   if key in seen:raise RuntimeError("v464 duplicate closure record")
   seen.add(key)
   allowed_root=library if row["base_kind"]=="v169_library" else joint_root
   if os.path.commonpath((str(base),str(lexical)))!=str(base) or not inside(resolved,allowed_root) or str(lexical)!=row["lexical_path"] or str(resolved)!=row["resolved_path"]:raise RuntimeError("v464 closure lexical/resolved drift")
   is_link=lexical.is_symlink();expected_link=os.readlink(lexical) if is_link else None
   if row["link_target"]!=expected_link:raise RuntimeError("v464 symlink text drift")
   if row["record_type"]=="directory_symlink":
    if not is_link or not resolved.is_dir() or directory_target_sha(resolved)!=row["target_sha"]:raise RuntimeError("v464 directory symlink drift")
   else:
    if not resolved.is_file() or sha(resolved)!=row["target_sha"]:raise RuntimeError("v464 file closure drift")
 ...
 if declared!=release_inventory(release,joint_root):raise RuntimeError("v464 exact release logical inventory drift")
 if sum(row["record_type"]=="directory_symlink" for row in release_records)!=4 or sum(row["record_type"]=="file_symlink" for row in release_records)!=2:raise RuntimeError("v464 exact six release symlink mappings required")
 if len({row["resolved_path"] for row in release_records})!=len(release_records):raise RuntimeError("v464 duplicate release resolved targets")
def validate_and_load(args,pre):
 if sha(Path(__file__))!=pre["source"]["trainer_sha256"]:raise RuntimeError("v464 trainer source SHA drift")
 runtime=Path(pre["source"]["runtime_path"])
 if not runtime.is_file() or sha(runtime)!=pre["source"]["runtime_sha256"]:raise RuntimeError("v464 runtime source SHA drift")
 if str(args.v169_release.resolve())!=pre["v169"]["release"] or str(args.v169_library.resolve())!=pre["v169"]["library"]:raise RuntimeError("v464 v169 path drift")
 ...
 if sha(receipt_path)!=pre["v461"]["final_receipt"]["sha256"] or receipt.get("format")!="strict-track2-v461-endpoint200-batch-audit-v1" or receipt.get("mode")!="final" or receipt.get("passed") is not True or not all(receipt.get("checks",{}).values()):raise RuntimeError("v464 hard blocked: bound v461 final receipt is not PASS")
 for key,arg in (("selection",args.v456_selection),("generation_report",args.v456_generation_report)):
  if str(arg.resolve())!=pre["v461"][key]["path"] or sha(arg)!=pre["v461"][key]["sha256"]:raise RuntimeError(f"v464 {key} closure failed")
 if str(args.v456_dataset.resolve())!=pre["v461"]["dataset"]:raise RuntimeError("v464 dataset path drift")
 for row in pre["v461"]["files"]:
  path=args.v456_dataset/row["relative"]
  if not path.is_file() or sha(path)!=row["sha256"]:raise RuntimeError(f"v464 frozen file drift {path}")
 ...
 if len(contexts)!=200 or len(samples)!=1000:raise RuntimeError("v464 exact 200/1000 contract")
...
def donor_ids(pre,fold,contexts,samples,partition):
 ...
 for i,row in enumerate(samples):
  ...
  if key in mapping:
   ...
   if row["episode"] not in expected or dcid==row["context_id"] or contexts[dcid]["episode"]!=row["episode"] or contexts[dcid]["episode"] not in expected:raise RuntimeError("v464 phase donor leakage/self-map")
...
def main():
 ...
 if pre.get("format")!="strict-track2-v464-endpoint-residual-5fold-preregistration-v2" or a.output_dir.exists():raise RuntimeError("v464 prereg/output contract")
 ...
 if pre.get("training")!=expected_training or pre.get("features")!=expected_features or pre.get("kill_gate")!=expected_gate or pre.get("common_v169_seed")!="sha256('v464/1615/{episode}/{start}')[:8] little-endian mod 2**31; branch-independent":raise RuntimeError("v464 prereg constants drift")
 final_dir=a.output_dir;work_dir=final_dir.with_name(final_dir.name+".partial")
 if final_dir.exists() or work_dir.exists():raise RuntimeError("v464 output/partial already exists")
 ...
 for fold in range(5):
  ...
  if schedule_sha(schedule)!=pre["training_schedules"][fold]["sha256"]:raise RuntimeError("v464 training schedule drift")
 ...
 if passed:
  ...
  if schedule_sha(schedule)!=pre["all200_schedule_sha256"]:raise RuntimeError("v464 all200 schedule drift")
