#!/usr/bin/env python3
"""Independent immutable S1 auditor for the frozen v474 endpoint parent."""
from __future__ import annotations
import argparse, hashlib, json, os
from pathlib import Path
import numpy as np

INPUT_FORMAT="strict-track2-v474-s1-offline-inputs-v1"
REPORT_FORMAT="strict-track2-v474-s1-offline-gate-v1"
...
REQUIRED={"format","independent_v169_frames","same_request_v169_frames","v474_true_frames","v474_action_override_frames","target_frames","left_same_request_v169_frames","left_v474_frames","independent_v169_reward","v474_true_reward","target_reward","episode","start","phase","gate","no_transport","left_episode"}
def sha(p):
 h=hashlib.sha256()
...
def main():
 ...
 if pre.get("format")!=PREREG_FORMAT or sel.get("format")!=SELECTION_FORMAT:raise RuntimeError("v474 S1 input format")
 if gen.get("format")!="strict-track2-v474-s1-generation-report-v1" or gen.get("passed") is not True:raise RuntimeError("generation report")
 ...
 right_names=("independent_v169_frames","same_request_v169_frames","v474_true_frames","v474_action_override_frames","target_frames")
 if any(x[k].shape!=(32,32,256,256,3) or x[k].dtype!=np.uint8 for k in right_names):raise RuntimeError("right RGB contract")
 if any(x[k].shape!=(12,8,256,256,3) or x[k].dtype!=np.uint8 for k in ("left_same_request_v169_frames","left_v474_frames")):raise RuntimeError("left RGB contract")
 if any(x[k].shape!=(32,32) or not np.isfinite(x[k]).all() for k in ("independent_v169_reward","v474_true_reward","target_reward")):raise RuntimeError("reward contract")
 ...
 independent=x["independent_v169_frames"];local=x["same_request_v169_frames"];true=x["v474_true_frames"];override=x["v474_action_override_frames"];target=x["target_frames"]
 ...
 br=x["independent_v169_reward"].astype(np.float64);cr=x["v474_true_reward"].astype(np.float64);tr=x["target_reward"].astype(np.float64)
 ...
 nt_decision_ok=decision_shape and all((not bool(decisions[i][c]["no_transport"])) or decisions[i][c]["action_prototype_sha256"]==[] for i in range(32) for c in range(4))
 hashes_ok=all(gen.get("rgb_array_sha256",{}).get(k)==arrsha(x[k]) for k in ("independent_v169_frames","same_request_v169_frames","v474_true_frames","v474_action_override_frames","target_frames","left_same_request_v169_frames","left_v474_frames"))
 checks={
  "static_release_passed":static.get("execution_valid") is True and all(static.get("checks",{}).values()) and static.get("s1_authorized") is False and static.get("rl_authorized") is False and sha(a.static_audit)=="a8b6f6cf972241d7ea001fe0e15b55469dddfecfb71c4a488c62517b9686f9c2",
  ...
  "left_all8_bitexact":np.array_equal(x["left_v474_frames"],x["left_same_request_v169_frames"]),
  "right_g0_all8_bitexact":g0_exact,
  "right_enabled_frames0_to6_bitexact":enabled_0_6,
  "decision_mask_and_no_transport_exact":decision_shape and np.array_equal(decision_mask,expected_gate) and nt_decision_ok,
