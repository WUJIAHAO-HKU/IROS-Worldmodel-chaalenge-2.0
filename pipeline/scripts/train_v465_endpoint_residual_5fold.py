#!/usr/bin/env python3
"""Preregistered v465 three-head 5-fold endpoint kill gate; no reward or RL."""
...
from wam_pipeline.v465_v169_endpoint_residual_runtime import CHECKPOINT_FORMAT,EndpointResidualUNet128FiLM,action_features,instruction_tokens
SEED=1615;STEPS=50;BATCH=8;BRANCHES=("factual","no_transport","scale_0p4","scale_1p25","reverse_direction_0p4")
def sha(path):
 h=hashlib.sha256()
...
def atomic_json(path,payload):
 ...
 if tmp.exists():raise RuntimeError(f"refusing stale v465 temporary receipt: {tmp}")
 with tmp.open("x") as handle:
  json.dump(payload,handle,indent=2,default=lambda value:value.item() if isinstance(value,np.generic) else value.tolist() if isinstance(value,np.ndarray) else (_ for _ in ()).throw(TypeError(f"not JSON serializable: {type(value)!r}")));handle.write("\n");handle.flush();os.fsync(handle.fileno())
 os.replace(tmp,path);descriptor=os.open(path.parent,os.O_RDONLY);os.fsync(descriptor);os.close(descriptor)
def stable_seed(episode,start):return int.from_bytes(hashlib.sha256(f"v465/{SEED}/{episode}/{start}".encode()).digest()[:8],"little")%(2**31)
...
def release_inventory(release,joint_root):
 ...
 def visit(logical,relative):
  ...
  if not inside(resolved,joint_root) or not resolved.is_dir():raise RuntimeError("v465 inventory directory escape")
  key=(resolved.stat().st_dev,resolved.stat().st_ino)
  if key in active:raise RuntimeError("v465 inventory cycle")
  if key in seen_dirs:raise RuntimeError("v465 inventory duplicate directory target")
  ...
  for child in sorted(logical.iterdir(),key=lambda x:x.name):
   ...
   if not inside(target,joint_root):raise RuntimeError("v465 inventory target escape")
   ...
   if target.is_dir():visit(child,rel)
    ...
    if target_key in seen_files:raise RuntimeError("v465 inventory duplicate file target")
   ...
   else:raise RuntimeError("v465 inventory unsupported target")
  active.remove(key)
 visit(release,Path("."));return sorted(items)
def verify_closure(pre,release,library):
 release=Path(release).absolute();library=Path(library).resolve();joint_root=release.resolve().parent;roots={"v169_release":release,"v169_release_target":release,"v169_release_link":release,"v169_base_release":(release/"v168_release/base_release").resolve(),"v169_library":library};seen=set()
 for records in (pre["v169"]["release_files"],pre["v169"]["library_files"]):
  for row in records:
   if set(row)!={"record_type","base_kind","relative","lexical_path","link_target","resolved_path","target_sha"} or row["base_kind"] not in roots:raise RuntimeError("v465 closure record schema")
   ...
   if key in seen:raise RuntimeError("v465 duplicate closure record")
   seen.add(key)
   allowed_root=library if row["base_kind"]=="v169_library" else joint_root
   if os.path.commonpath((str(base),str(lexical)))!=str(base) or not inside(resolved,allowed_root) or str(lexical)!=row["lexical_path"] or str(resolved)!=row["resolved_path"]:raise RuntimeError("v465 closure lexical/resolved drift")
   is_link=lexical.is_symlink();expected_link=os.readlink(lexical) if is_link else None
   if row["link_target"]!=expected_link:raise RuntimeError("v465 symlink text drift")
   if row["record_type"]=="directory_symlink":
    if not is_link or not resolved.is_dir() or directory_target_sha(resolved)!=row["target_sha"]:raise RuntimeError("v465 directory symlink drift")
   else:
    if not resolved.is_file() or sha(resolved)!=row["target_sha"]:raise RuntimeError("v465 file closure drift")
 release_records=pre["v169"]["release_files"]
 declared=sorted((row["record_type"],row["base_kind"],row["relative"],row["lexical_path"],row["link_target"],row["resolved_path"]) for row in release_records)
 if declared!=release_inventory(release,joint_root):raise RuntimeError("v465 exact release logical inventory drift")
 if sum(row["record_type"]=="directory_symlink" for row in release_records)!=4 or sum(row["record_type"]=="file_symlink" for row in release_records)!=2:raise RuntimeError("v465 exact six release symlink mappings required")
 if len({row["resolved_path"] for row in release_records})!=len(release_records):raise RuntimeError("v465 duplicate release resolved targets")
def validate_and_load(args,pre):
 if sha(Path(__file__))!=pre["source"]["trainer_sha256"]:raise RuntimeError("v465 trainer source SHA drift")
 runtime=Path(pre["source"]["runtime_path"])
 if not runtime.is_file() or sha(runtime)!=pre["source"]["runtime_sha256"]:raise RuntimeError("v465 runtime source SHA drift")
 if str(args.v169_release.resolve())!=pre["v169"]["release"] or str(args.v169_library.resolve())!=pre["v169"]["library"]:raise RuntimeError("v465 v169 path drift")
 ...
 if sha(receipt_path)!=pre["v461"]["final_receipt"]["sha256"] or receipt.get("format")!="strict-track2-v461-endpoint200-batch-audit-v1" or receipt.get("mode")!="final" or receipt.get("passed") is not True or not all(receipt.get("checks",{}).values()):raise RuntimeError("v465 hard blocked: bound v461 final receipt is not PASS")
 for key,arg in (("selection",args.v456_selection),("generation_report",args.v456_generation_report)):
  if str(arg.resolve())!=pre["v461"][key]["path"] or sha(arg)!=pre["v461"][key]["sha256"]:raise RuntimeError(f"v465 {key} closure failed")
 if str(args.v456_dataset.resolve())!=pre["v461"]["dataset"]:raise RuntimeError("v465 dataset path drift")
 for row in pre["v461"]["files"]:
  path=args.v456_dataset/row["relative"]
  if not path.is_file() or sha(path)!=row["sha256"]:raise RuntimeError(f"v465 frozen file drift {path}")
 ...
 if len(contexts)!=200 or len(samples)!=1000:raise RuntimeError("v465 exact 200/1000 contract")
...
def donor_ids(pre,fold,contexts,samples,partition):
 ...
 for i,row in enumerate(samples):
  ...
  if key in mapping:
   ...
   if row["episode"] not in expected or dcid==row["context_id"] or contexts[dcid]["episode"]!=row["episode"] or contexts[dcid]["episode"] not in expected:raise RuntimeError("v465 phase donor leakage/self-map")
...
def main():
 ...
 if pre.get("format")!="strict-track2-v465-endpoint-residual-5fold-preregistration-v2" or a.output_dir.exists():raise RuntimeError("v465 prereg/output contract")
 ...
 if pre.get("training")!=expected_training or pre.get("features")!=expected_features or pre.get("kill_gate")!=expected_gate or pre.get("common_v169_seed")!="sha256('v465/1615/{episode}/{start}')[:8] little-endian mod 2**31; branch-independent":raise RuntimeError("v465 prereg constants drift")
 final_dir=a.output_dir;work_dir=final_dir.with_name(final_dir.name+".partial")
 if final_dir.exists() or work_dir.exists():raise RuntimeError("v465 output/partial already exists")
 ...
 for fold in range(5):
  ...
  if schedule_sha(schedule)!=pre["training_schedules"][fold]["sha256"]:raise RuntimeError("v465 training schedule drift")
 ...
 if passed:
  ...
  if schedule_sha(schedule)!=pre["all200_schedule_sha256"]:raise RuntimeError("v465 all200 schedule drift")
