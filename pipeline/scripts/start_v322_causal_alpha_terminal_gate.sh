...
RUN="$J/v322_v317_causal_alpha_terminal_gate_seed1492_20260822"
COMMON=(
...
taskset -c 0-7 "$PY" pipeline/scripts/test_v322_causal_alpha_terminal.py \
  "${COMMON[@]}" --output "$RUN/audit/contract_report_v2.json" \
  >>"$RUN/audit/contract_v2.log" 2>&1

taskset -c 0-7 "$PY" pipeline/scripts/audit_v322_causal_alpha_terminal_gate.py \
  "${COMMON[@]}" \
  ...
  --contract-report "$RUN/audit/contract_report_v2.json" \
...
taskset -c 0-1 "$PY" pipeline/scripts/authorize_expensive_rl.py \
  ...
  --absolute-gate "contract=$RUN/audit/contract_report_v2.json" \
...
printf 'V322_CAUSAL_ALPHA_TERMINAL_GATE_PASSED\n'
