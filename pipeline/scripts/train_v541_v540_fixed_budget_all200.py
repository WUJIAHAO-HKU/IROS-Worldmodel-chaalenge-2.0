...
import argparse, copy, ctypes, hashlib, importlib.util, io, json, os, random, signal, stat, sys, types
...
SELF=S/'train_v541_v540_fixed_budget_all200.py'; PREREG=S/'v541_v540_fixed_budget_all200_training_preregistration.json'; MANIFEST=S/'v541_v540_fixed_budget_all200_training_manifest.json'; CONTRACT=S/'v541_v540_fixed_budget_all200_training_execution_authority_contract.json'
AUDITOR=S/'audit_v541_v540_fixed_budget_all200.py'; LAUNCHER=S/'launch_v541_v540_fixed_budget_all200.py'
...
AUTH_ROOT=J/'v541_v540_fixed_budget_all200_training_execution_authority_seed1672_20260828'; AUTH=AUTH_ROOT/'authority_receipt.json'; AUTH_PREP=AUTH_ROOT.with_name(AUTH_ROOT.name+'.authority-prep')
...
OUTPUT=Path('/root/v541_v540_fixed_budget_all200_training_seed1624_20260828'); OUTPUT_PREP=OUTPUT.with_name(OUTPUT.name+'.training-prep')
...
CHECKPOINT_FORMAT='strict-track2-v541-v540-fixed-budget-all200-checkpoint-v1'; RENAME_NOREPLACE=1; AT_FDCWD=-100
...
CHECK_KEYS='contract_schema preregistration_schema manifest_schema authorization_exact_strict budget_arithmetic_exact schedule_permutation_exact runtime_boundary_exact execution_boundary_exact source_role_order_exact source_aliases_exact source_closure28_current active_sources6_current contract_active_records6 authority_active_records7 v540_exact8_current v540_authority_immutable_consumed0 v540_durable_boundary1 v540_metrics_not_parsed v534_oof_current_readonly v524_qualification_exact20_readonly v524_cache_current_readonly v482_base_model_sources_current v474_selection_current base_data_sources_readonly fresh_roots6_absent input_prepost_equal services_current gpu_empty execution_pids_empty all_unsafe_disabled publish_noreplace_exact1 lineage_exact two_stage_boundary_only output_exact6_contract'.split()
...
def validate_documents(prereg,manifest,contract,authority,args,*,profile_steps=500,enforce_production_paths=True):
 if (set(prereg)!=PREREG_KEYS or set(manifest)!=MANIFEST_KEYS or set(contract)!=CONTRACT_KEYS or set(authority)!=AUTHORITY_KEYS
     or prereg.get('format')!='strict-track2-v541-v540-fixed-budget-all200-training-preregistration-v1'
     or manifest.get('format')!='strict-track2-v541-v540-fixed-budget-all200-training-manifest-v1'
     or contract.get('format')!='strict-track2-v541-v540-fixed-budget-all200-training-execution-authority-design-contract-v1'
 ...
 if type(prereg.get('seed')) is not int or prereg['seed']!=1672 or any(type(doc.get('seed')) is not int or doc['seed']!=1672 for doc in (manifest,contract,authority)): raise RuntimeError('seed strict int')
 ...
 if (not isinstance(receipt,dict) or set(receipt)!=RECEIPT_CONTRACT_KEYS or not json_exact(receipt.get('active_source_role_order_exact'),ACTIVE_ROLE_ORDER)
     ...
     or receipt.get('authority_design_record_included') is not True or type(receipt.get('source_closure_contract_count')) is not int or receipt['source_closure_contract_count']!=28
...
def production_inputs(torch,args):
 oldpre=load_json(OLD_PREREG); old=import_source('v541_v482_training_components',OLD_TRAINER)
...
def execute(args,*,validation_profile=None,enforce_production_paths=True,event_hook=None):
 ...
 try:
  ...
  input_builder=production_inputs if validation_profile is None else (lambda t,a:fixture_inputs(t,steps))
  components,oldpre,model,optimizer,batch_builder,loss_builder,lower,upper=input_builder(torch,args)
  input_before=None if validation_profile is not None else dataset_snapshot(oldpre)
  ...
  input_after=None if validation_profile is not None else dataset_snapshot(oldpre)
...
def main(argv=None):
 ...
 if args.seed!=1672 or args.output_root!=OUTPUT: raise RuntimeError('literal args')
