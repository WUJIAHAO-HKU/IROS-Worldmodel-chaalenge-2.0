#!/usr/bin/env python3
"""Materialize one fresh v542 runtime-package-repaired fixed-budget authority."""
...
SELF=S/'materialize_v542_v541_fixed_budget_all200_training_runtime_package_repair_execution_authority.py'
CONTRACT=S/'v542_v541_fixed_budget_all200_training_runtime_package_repair_execution_authority_contract.json'
PREREG=S/'v542_v541_fixed_budget_all200_training_runtime_package_repair_preregistration.json'; MANIFEST=S/'v542_v541_fixed_budget_all200_training_runtime_package_repair_manifest.json'
TRAINER=S/'train_v542_v541_fixed_budget_all200_runtime_package_repair.py'; AUDITOR=S/'audit_v542_v541_fixed_budget_all200_runtime_package_repair.py'; LAUNCHER=S/'launch_v542_v541_fixed_budget_all200_runtime_package_repair.py'
PREREG_EXPECTED=('4b2880d743f45ad33dd25421e147641d3a15085a6a5adcaac1cd16962caa833b',5997)
MANIFEST_EXPECTED=('911c5fcccedd88101f2b07541e4b37b49e5f30766b5a43652b5fb5642a096146',6658)
TRAINER_EXPECTED=('c572d6ed9843e128f8b191c1a28c93ad8762aa5f0c87a7c25327b7a4a33c8cee',52003)
AUDITOR_EXPECTED=('926053bbed29f403b625e4940cf61ced05701430302ac4b17f820f217b058c0b',4784)
LAUNCHER_EXPECTED=('e8917e5f4361bda438bac3c9697ca7dea7415e616b26f59c5391dcc2e0f12b2a',2720)
AUTH_ROOT=J/'v542_v541_fixed_budget_all200_training_runtime_package_repair_execution_authority_seed1673_20260828'; AUTH_PREP=AUTH_ROOT.with_name(AUTH_ROOT.name+'.authority-prep')
...
OUTPUT_ROOT=Path('/root/v542_v541_fixed_budget_all200_training_runtime_package_repair_seed1624_20260828'); OUTPUT_PREP=OUTPUT_ROOT.with_name(OUTPUT_ROOT.name+'.training-prep')
...
OUTPUT_FORMAT='strict-track2-v542-v541-fixed-budget-all200-training-runtime-package-repair-execution-authority-v1'
...
def load_trainer():
 regular(TRAINER,TRAINER_EXPECTED); spec=importlib.util.spec_from_file_location('v542_authority_trainer_schema',TRAINER); mod=importlib.util.module_from_spec(spec); spec.loader.exec_module(mod); return mod
def tree(root):
...
def main():
 ...
 if args.synthetic_self_test:
  trainer=load_trainer(); checks={'contract_top':len(trainer.CONTRACT_KEYS)==53,'authority_top':len(trainer.AUTHORITY_KEYS)==63,'checks':len(trainer.CHECK_KEYS)==34,'sources':len(trainer.SOURCE_ROLE_ORDER)==32 and len(trainer.AUTHORITY_SOURCE_ALIASES)==33,'active':len(trainer.ACTIVE_ROLE_ORDER)==7,'authorization':len(json.loads(PREREG.read_text())['authorization'])==23}; print(json.dumps({'passed':all(checks.values()),'checks':checks,'checks_sha256':csha(checks)},sort_keys=True)); return 0 if all(checks.values()) else 1
