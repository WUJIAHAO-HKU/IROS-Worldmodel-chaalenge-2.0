...
SELF_PATH=ROOT/"pipeline/scripts/launch_v492_v491_v490_reconciliation_via_v490_wrapper.py"
AUTH_CONTRACT=ROOT/"pipeline/scripts/v492_v491_v490_reconciliation_execution_authority_contract.json"
AUTH_ROOT=J/"v492_v491_v490_reconciliation_execution_authority_seed1634_20260825";AUTH_PREP=AUTH_ROOT.with_name(AUTH_ROOT.name+".registration-prep");AUTH_RECEIPT=AUTH_ROOT/"authority_receipt.json"
...
OUTER_EVIDENCE=J/"v492_v491_v490_reconciliation_outer_execution_evidence_seed1634_20260825";OUTER_PREP=OUTER_EVIDENCE.with_name(OUTER_EVIDENCE.name+".outer-prep")
FAILED_V491_CONTRACT=ROOT/"pipeline/scripts/v491_v490_reconciliation_execution_authority_contract.json";FAILED_V491_CONTRACT_SHA="f06b3f42d91d339c19fe25c78605bc328c595ae6bba73e5613ff57533e5c38d1";FAILED_V491_CONTRACT_BYTES=19734
FAILED_V491_MATERIALIZER=ROOT/"pipeline/scripts/materialize_v491_v490_reconciliation_execution_authority.py";FAILED_V491_MATERIALIZER_SHA="a8e691ef683bf826fe04be91406705742cb6d03239cd72d4864666d20aa307c3";FAILED_V491_MATERIALIZER_BYTES=33374
FAILED_V491_OUTER=ROOT/"pipeline/scripts/launch_v491_v490_reconciliation_via_v490_wrapper.py";FAILED_V491_OUTER_SHA="2342a8477b62decbabe25dfd01dcdd03a4acc8e2502bc25c990d084b38463101";FAILED_V491_OUTER_BYTES=37367
FAILED_V491_EVIDENCE=J/"v491_v490_reconciliation_execution_authority_materialization_evidence_seed1633_20260825";FAILED_V491_PROCESS=FAILED_V491_EVIDENCE/"process_receipt.json";FAILED_V491_AUTH_ROOT=J/"v491_v490_reconciliation_execution_authority_seed1633_20260825";FAILED_V491_AUTH_PREP=FAILED_V491_AUTH_ROOT.with_name(FAILED_V491_AUTH_ROOT.name+".registration-prep");FAILED_V491_OUTER_ROOT=J/"v491_v490_reconciliation_outer_execution_evidence_seed1633_20260825";FAILED_V491_OUTER_PREP=FAILED_V491_OUTER_ROOT.with_name(FAILED_V491_OUTER_ROOT.name+".outer-prep")
QUALIFICATION=Path("/root/v485_v169_cache_qualification_seed1627_20260824")
AUTH_FORMAT="strict-track2-v492-v491-v490-reconciliation-execution-authority-v1";AUTH_STATUS="authorized_exact_one_external_v490_reconciliation_wrapper_attempt"
CONTRACT_FORMAT="strict-track2-v492-v491-v490-reconciliation-execution-authority-design-contract-v1";CONTRACT_STATUS="design_only_frozen_sources_pending_independent_review_no_authority"
...
def authority_preflight(args):
 ...
 if contract.get("format")!=CONTRACT_FORMAT or contract.get("status")!=CONTRACT_STATUS or contract.get("seed")!=1634:raise RuntimeError("contract identity")
 ...
 if set(schema["top_keys"])!=AUTH_TOP_KEYS or set(receipt)!=AUTH_TOP_KEYS or len(receipt)!=44 or receipt.get("format")!=AUTH_FORMAT or receipt.get("status")!=AUTH_STATUS or receipt.get("passed") is not True:raise RuntimeError("authority schema")
 if not isinstance(schema["check_keys"],list) or len(schema["check_keys"])!=33 or receipt.get("checks")!={k:True for k in schema["check_keys"]} or receipt.get("check_keys")!=schema["check_keys"] or receipt.get("check_key_set_sha256")!=csha(schema["check_keys"]) or receipt.get("checks_sha256")!=csha(receipt["checks"]):raise RuntimeError("authority checks")
 ...
 aliases={"rc2_forensic_source":"rc2_forensic_source","corrected_transport_script":"corrected_transport_script_source","v490_authority_contract":"v490_authority_contract","v490_authority_materializer":"v490_authority_materializer_source","v490_wrapper":"v490_wrapper_source","reconciler_r2":"reconciler_r2_source","outer_execution_wrapper":"outer_execution_wrapper_source","failed_v491_authority_contract":"failed_v491_authority_contract","failed_v491_authority_materializer":"failed_v491_authority_materializer_source","failed_v491_outer_wrapper":"failed_v491_outer_wrapper_source"}
 if any(receipt.get(alias)!=closure[role] for role,alias in aliases.items()):raise RuntimeError("source aliases")
 if receipt.get("failed_v491_authority_contract")!=regular(FAILED_V491_CONTRACT,FAILED_V491_CONTRACT_SHA,FAILED_V491_CONTRACT_BYTES) or receipt.get("failed_v491_authority_materializer_source")!=regular(FAILED_V491_MATERIALIZER,FAILED_V491_MATERIALIZER_SHA,FAILED_V491_MATERIALIZER_BYTES) or receipt.get("failed_v491_outer_wrapper_source")!=regular(FAILED_V491_OUTER,FAILED_V491_OUTER_SHA,FAILED_V491_OUTER_BYTES):raise RuntimeError("failed v491 source identities")
 failed_record=regular(FAILED_V491_PROCESS,"07687260dd13a4c1553bcbc6cb55cb89dcd9f6f9680d62675df394f27701aed9",401);failed_tree=rooted_tree(FAILED_V491_EVIDENCE);failed=json.loads(FAILED_V491_PROCESS.read_text())
 if receipt.get("failed_v491_process_receipt")!=failed_record or receipt.get("failed_v491_materialization_evidence_tree")!=failed_tree or contract.get("failed_v491_process_receipt")!=failed_record or contract.get("failed_v491_materialization_evidence_tree")!=failed_tree:raise RuntimeError("failed v491 evidence aliases")
 if failed_tree["file_count"]!=6 or failed_tree["logical_file_bytes"]!=23587 or failed_tree["sha256sum_lines_digest_sha256"]!="0d561672ad0f5a820ebe6a52c2860f9020cbbc6d188fe6e1b2bde89e9dd5f519" or failed_tree["canonical_json_triples_digest_sha256"]!="18862d7562871e9e25aa11ad04c6466a63b3ac8ec43cb9522802df6e2603b1a7":raise RuntimeError("failed v491 tree exact6")
 if failed.get("status")!="failed_no_retry" or failed.get("passed") is not False or failed.get("materializer_invocations")!=1 or any(failed.get(k)!=0 for k in ("outer_wrapper_invocations","inner_wrapper_invocations","r2_invocations")) or failed.get("retry_authorized") is not False or failed.get("cleanup",{}).get("reaped") is not True or failed.get("cleanup",{}).get("group_empty") is not True:raise RuntimeError("failed v491 no retry partition")
 failed_argv=json.loads((FAILED_V491_EVIDENCE/"argv.json").read_text());failed_stderr=(FAILED_V491_EVIDENCE/"materializer_stderr.log").read_text()
 if failed_argv.get("sources",{}).get(str(FAILED_V491_CONTRACT))!=regular(FAILED_V491_CONTRACT,FAILED_V491_CONTRACT_SHA,FAILED_V491_CONTRACT_BYTES) or failed_argv.get("sources",{}).get(str(FAILED_V491_MATERIALIZER))!=regular(FAILED_V491_MATERIALIZER,FAILED_V491_MATERIALIZER_SHA,FAILED_V491_MATERIALIZER_BYTES) or failed_argv.get("sources",{}).get(str(FAILED_V491_OUTER))!=regular(FAILED_V491_OUTER,FAILED_V491_OUTER_SHA,FAILED_V491_OUTER_BYTES) or "UnboundLocalError" not in failed_stderr or "pending" not in failed_stderr:raise RuntimeError("failed v491 root cause ancestry")
 superseded=decode_absence_receipt(receipt.get("superseded_v491_absences"),4);contract_superseded=decode_absence_receipt(contract.get("superseded_v491_absences"),4)
 expected_superseded={"superseded_v491_authority_root":FAILED_V491_AUTH_ROOT,"superseded_v491_authority_prep":FAILED_V491_AUTH_PREP,"superseded_v491_outer_evidence_root":FAILED_V491_OUTER_ROOT,"superseded_v491_outer_evidence_prep":FAILED_V491_OUTER_PREP}
 if superseded!=contract_superseded or superseded!=expected_superseded or any(os.path.lexists(p) for p in superseded.values()):raise RuntimeError("superseded v491 absence")
 ...
 historical=decode_absence_receipt(receipt.get("historical_absences"),19);required=decode_absence_receipt(receipt.get("required_absences"),18)
 if set(historical)!=HISTORICAL_ABSENCE_KEYS or set(required)!=CURRENT_ABSENCE_KEYS:raise RuntimeError("absence keysets")
 contract_historical=decode_path_map(contract.get("historical_absences"),19);contract_current=decode_path_map(contract.get("current_absences_after_authority"),18)
 ...
 return {"contract":contract_record,"authority":receipt_record,"authority_tree":auth_tree,"forensic":forensic_record,"self":self_record,"old_authority":regular(OLD_AUTH_RECEIPT,OLD_AUTH_RECEIPT_SHA,OLD_AUTH_RECEIPT_BYTES),"inner_wrapper":regular(INNER_WRAPPER,INNER_WRAPPER_SHA,INNER_WRAPPER_BYTES),"r2":regular(R2,R2_SHA,R2_BYTES),"repair":regular(REPAIR,REPAIR_SHA,REPAIR_BYTES),"static":regular(STATIC,STATIC_SHA,STATIC_BYTES),"f813":regular(F813,F813_SHA,F813_BYTES),"failed_v491_process":failed_record,"failed_v491_tree":failed_tree,"source_closure":closure}
...
def immutable_snapshot(context):
 ...
 trees={"authority":rooted_tree(AUTH_ROOT),"repair":rooted_tree(REPAIR.parent),"static":rooted_tree(STATIC.parent),"failed_v491":rooted_tree(FAILED_V491_EVIDENCE)}
...
def synthetic():
 ...
 fixture_path=Path("/dev/shm/v492_authority_contract_candidate.json")
 if fixture_path.is_file():
  fixture=json.loads(fixture_path.read_text());historical=decode_path_map(fixture["historical_absences"],19);current=decode_path_map(fixture["current_absences_after_authority"],18)
  ...
  checks["live_contract_schema"]=(fixture["format"]==CONTRACT_FORMAT and fixture["status"]==CONTRACT_STATUS and fixture["seed"]==1634 and set(fixture["authority_receipt_contract"]["top_keys"])==AUTH_TOP_KEYS and len(fixture["authority_receipt_contract"]["check_keys"])==33 and fixture["authority_receipt_contract"]["authorization_exact"]==AUTHORIZATION and fixture["authority_receipt_contract"]["runtime_observation_exact"]==RUNTIME and fixture["execution_boundary"]==EXECUTION_BOUNDARY and set(fixture["source_closure"])==SOURCE_ROLES)
  checks["live_absence_schema_transform"]=(decode_absence_receipt(receipt_h,19)==historical and decode_absence_receipt(receipt_c,18)==current and current=={k:v for k,v in historical.items() if k!="execution_authority_root"} and historical["outer_evidence_prep"]==OUTER_PREP and historical["qualification_root"]==QUALIFICATION)
 checks["failed_v491_immutable_ancestry"]=(rooted_tree(FAILED_V491_EVIDENCE)["sha256sum_lines_digest_sha256"]=="0d561672ad0f5a820ebe6a52c2860f9020cbbc6d188fe6e1b2bde89e9dd5f519" and regular(FAILED_V491_PROCESS,"07687260dd13a4c1553bcbc6cb55cb89dcd9f6f9680d62675df394f27701aed9",401)["logical_bytes"]==401 and all(not os.path.lexists(p) for p in (FAILED_V491_AUTH_ROOT,FAILED_V491_AUTH_PREP,FAILED_V491_OUTER_ROOT,FAILED_V491_OUTER_PREP)))
