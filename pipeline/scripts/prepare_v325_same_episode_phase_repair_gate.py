#!/usr/bin/env python3
"""Preregister v325 before contract or reward results exist."""
...
N = "v325_v317_same_episode_phase_repair_gate_seed1495_20260822"
RUN = J / N
...
def main() -> None:
    ...
    paths = {
        "runtime": B / "pipeline/wam_pipeline/v325_same_episode_phase_repair_runtime.py",
        ...
        "contract_wrapper": B / "pipeline/scripts/test_v325_same_episode_phase_repair.py",
        "inherited_contract": B / "pipeline/scripts/test_v324_phase_guarded_terminal.py",
        "audit_wrapper": B / "pipeline/scripts/audit_v325_same_episode_phase_repair_gate.py",
    ...
    if json.loads(paths["rejected_v324"].read_text())["passed"] is not False:
        raise RuntimeError("v324 was not rejected")
    payload = {
        "format": "strict-track2-v325-same-episode-phase-repair-preregistration-v1",
        "registered_at": datetime.now(timezone.utc).isoformat(),
        "candidate": {
            "model_version": "track2-v325-same-episode-phase-repair-v317",
            "parent": "frozen authorized v317",
            "change": "only for a matched public episode with a sustained successful phase but a terminal reward below 0.90, replace zero-alpha v317 output with that same episode's frozen phase-onset row when action probability is >=0.99",
            "deployment_batch_size": 8,
            "action_probability_min": 0.99,
            "baseline_alpha_zero_max": 1e-8,
            "cross_episode_terminal_retrieval": False,
        ...
        "measurement_rule": {
            ...
            "reason": "the same frozen reward model can differ by approximately 1e-8 solely from batch position",
