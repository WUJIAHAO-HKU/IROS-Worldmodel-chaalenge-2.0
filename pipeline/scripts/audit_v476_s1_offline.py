#!/usr/bin/env python3
"""Independent immutable S1 auditor for v476, the reward-import-only v475 migration."""
...
REPORT_FORMAT="strict-track2-v476-s1-offline-gate-v1"
...
REQUIRED={"format","independent_v169_frames","same_request_v169_frames","v476_true_frames","v476_action_override_frames","target_frames","left_same_request_scalar_v169_frames","left_v476_frames","independent_v169_reward","v476_true_reward","target_reward","episode","start","phase","gate","no_transport","left_episode"}
def sha(p):
 h=hashlib.sha256()
...
def main():
 ...
 if pre.get("format")!=PREREG_FORMAT or sel.get("format")!=SELECTION_FORMAT:raise RuntimeError("v476 S1 input format")
 if gen.get("format")!="strict-track2-v476-s1-generation-report-v1" or gen.get("passed") is not True:raise RuntimeError("generation report")
 ...
 right_names=("independent_v169_frames","same_request_v169_frames","v476_true_frames","v476_action_override_frames","target_frames")
 if any(x[k].shape!=(32,32,256,256,3) or x[k].dtype!=np.uint8 for k in right_names):raise RuntimeError("right RGB contract")
 if any(x[k].shape!=(12,8,256,256,3) or x[k].dtype!=np.uint8 for k in ("left_same_request_scalar_v169_frames","left_v476_frames")):raise RuntimeError("left RGB contract")
 if any(x[k].shape!=(32,32) or not np.isfinite(x[k]).all() for k in ("independent_v169_reward","v476_true_reward","target_reward")):raise RuntimeError("reward contract")
 ...
 independent=x["independent_v169_frames"];local=x["same_request_v169_frames"];true=x["v476_true_frames"];override=x["v476_action_override_frames"];target=x["target_frames"]
 ...
 br=x["independent_v169_reward"].astype(np.float64);cr=x["v476_true_reward"].astype(np.float64);tr=x["target_reward"].astype(np.float64)
 ...
 hashes_ok=all(gen.get("rgb_array_sha256",{}).get(k)==arrsha(x[k]) for k in ("independent_v169_frames","same_request_v169_frames","v476_true_frames","v476_action_override_frames","target_frames","left_same_request_scalar_v169_frames","left_v476_frames"))
 checks={
  ...
  "left_all8_bitexact":np.array_equal(x["left_v476_frames"],x["left_same_request_scalar_v169_frames"]),
