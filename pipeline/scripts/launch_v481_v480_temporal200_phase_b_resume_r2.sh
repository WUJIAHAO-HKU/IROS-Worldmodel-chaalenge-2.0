#!/usr/bin/env bash
# v481 resume-r2 of Phase-B temporal200 after immutable preflight-only failure.
set -euo pipefail
ROOT='/root/autodl-tmp/IROS_WAM_2.0 challenge'
J="$ROOT/artifacts/strict_track2_joint_augmentation_20260810"
S="$ROOT/pipeline/scripts"
REG="$J/v481_v480_phase_b_resume_r2_seed1622_20260824"
...
exec 9>/var/lock/v481_v480_temporal200_phase_b_resume_r2.lock
flock -n 9 || exit 73
START_EPOCH=$(date +%s)
...
root=Path(sys.argv[1]).resolve(); launcher,pre,gen,aud,col=map(Path,sys.argv[2:7]);total=3238;successes=[];terminal_failures=[]
def sha(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
...
test "$(sha256sum "$PRE"|awk '{print $1}')" = '31f5e15a62b3103e23513bd0f1499773ea21717e1a4e8896c5c0dee08708c959'
...
assert sha(c['v481_parent_resume_preregistration']['path'])==c['v481_parent_resume_preregistration']['sha256']=='a1614185001ab98981bd394b76fac6b7220bd9e6878fff1098a712750c9a8f79'
assert sha(c['v481_failed_resume_launcher']['path'])==c['v481_failed_resume_launcher']['sha256']=='9775ad899b1a2009e574175a807a31298d11d8024eab7450ce8d207c65737d60'
assert sha(c['v481_failed_resume_attempt_intent']['path'])==c['v481_failed_resume_attempt_intent']['sha256']=='b4df508bf1899bd027755739c5b522c0bdfc11a8e98ffa293bc88845d4be0d22'
assert sha(c['v481_failed_resume_receipt']['path'])==c['v481_failed_resume_receipt']['sha256']=='7b580c67ea7ed4f5d6bdbc9c97eb6a5f6aecf0fbda1505fed9f0c8ea05fe120b'
assert sha(c['v481_failed_resume_restore_log']['path'])==c['v481_failed_resume_restore_log']['sha256']=='bf36c8bfe0d2f0af1690b20840a80e622e67e3029daab4b4985a88fc2989acb1'
assert sha(c['v481_failed_resume_outer_log']['path'])==c['v481_failed_resume_outer_log']['sha256']=='bf53059c107ba95f4d2c9716876f1f50941290d5a0b05def202eadfdbc55269d'
assert sha(c['v481_resume_r2_materializer']['path'])==c['v481_resume_r2_materializer']['sha256']=='06de40e8b645a5a60953cf854eb7495e82728f02262ed7b9b03ad90bcef53f68'
assert sha(c['registered_batch0_audit']['path'])==c['registered_batch0_audit']['sha256']=='713c6976a42e817d1f66414b30b35c6e72f505a2a4f68e9a4b778adc8bca04b9'
assert pre['dataset_root']=='/root/v478_temporal8_dataset_seed1622_20260824' and len(pre['contexts'])==200
r=pre['resume_registration'];w=pre['wall_accounting'];a=pre['resume_r2_attempt_accounting'];g=pre['guards']
...
close=lambda x,y: math.isclose(float(x),float(y),rel_tol=0.0,abs_tol=1e-12)
assert set(w)=={'accounting_difference_seconds','dataset_and_launcher_accounting_are_distinct','launcher_watchdog_prior_wall_seconds_ceil','prior_dataset_cumulative_attempt_wall_seconds','prior_launcher_cumulative_wall_seconds','remaining_dataset_attempt_wall_seconds','remaining_launcher_wall_seconds','v480_reconciliation_excluded_from_collection_wall','whole_wall_seconds_max'}
assert w['dataset_and_launcher_accounting_are_distinct'] is True and w['launcher_watchdog_prior_wall_seconds_ceil']==3232 and w['v480_reconciliation_excluded_from_collection_wall'] is True
assert close(w['whole_wall_seconds_max'],43200.0) and close(w['prior_dataset_cumulative_attempt_wall_seconds'],3214.017167359125) and close(w['prior_launcher_cumulative_wall_seconds'],3231.3513736724854)
assert close(w['remaining_dataset_attempt_wall_seconds'],w['whole_wall_seconds_max']-w['prior_dataset_cumulative_attempt_wall_seconds']) and close(w['remaining_launcher_wall_seconds'],w['whole_wall_seconds_max']-w['prior_launcher_cumulative_wall_seconds'])
assert close(w['accounting_difference_seconds'],w['prior_launcher_cumulative_wall_seconds']-w['prior_dataset_cumulative_attempt_wall_seconds'])
assert set(a)=={'original_launcher_raw_seconds','original_inherited_watchdog_seconds_ceil','failed_resume_attempt_seconds','failed_resume_receipt_cumulative_raw_seconds','effective_launcher_watchdog_prior_wall_seconds_ceil','remaining_launcher_raw_seconds','remaining_launcher_watchdog_integer_seconds','dataset_prior_cumulative_attempt_wall_seconds','effective_launcher_minus_dataset_raw_seconds','effective_watchdog_minus_dataset_seconds','derived_float_validation','v480_reconciliation_excluded_from_collection_wall'}
assert a['original_inherited_watchdog_seconds_ceil']==3232 and a['effective_launcher_watchdog_prior_wall_seconds_ceil']==3238 and a['remaining_launcher_watchdog_integer_seconds']==39962 and a['derived_float_validation']=={'comparison':'math.isclose','relative_tolerance':0.0,'absolute_tolerance':1e-12} and a['v480_reconciliation_excluded_from_collection_wall'] is True
assert close(a['original_launcher_raw_seconds'],w['prior_launcher_cumulative_wall_seconds']) and close(a['failed_resume_attempt_seconds'],5.613072872161865) and close(a['failed_resume_receipt_cumulative_raw_seconds'],a['original_inherited_watchdog_seconds_ceil']+a['failed_resume_attempt_seconds'])
assert a['effective_launcher_watchdog_prior_wall_seconds_ceil']==math.ceil(a['failed_resume_receipt_cumulative_raw_seconds']) and close(a['remaining_launcher_raw_seconds'],w['whole_wall_seconds_max']-a['failed_resume_receipt_cumulative_raw_seconds']) and a['remaining_launcher_watchdog_integer_seconds']==int(w['whole_wall_seconds_max'])-a['effective_launcher_watchdog_prior_wall_seconds_ceil']
assert close(a['dataset_prior_cumulative_attempt_wall_seconds'],w['prior_dataset_cumulative_attempt_wall_seconds']) and close(a['effective_launcher_minus_dataset_raw_seconds'],a['failed_resume_receipt_cumulative_raw_seconds']-a['dataset_prior_cumulative_attempt_wall_seconds']) and close(a['effective_watchdog_minus_dataset_seconds'],a['effective_launcher_watchdog_prior_wall_seconds_ceil']-a['dataset_prior_cumulative_attempt_wall_seconds'])
assert g['effect_used_for_retry_filter_batch_gate'] is False and g['v481_failed_resume_preflight_bound'] is True and g['next_batch_id']==1 and g['batch0_reuse_required'] is True and g['batch0_rerun_authorized'] is False and g['batch0_generator_invocations_allowed']==0 and g['effect_or_outcome_conditioned_retry'] is False and g['training_authorized'] is False and g['s1_authorized'] is False and g['zero_update_authorized'] is False and g['policy_updates']==0 and g['rl_authorized'] is False
PY
within_whole
