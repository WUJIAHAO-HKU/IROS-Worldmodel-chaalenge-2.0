#!/usr/bin/env python3
"""One-shot v508 Phase-A cache qualification launcher (design-only source).

The launcher consumes the fresh v508 authority, preserves the frozen v485
exact-seven execution closure expected by the Phase-A driver, owns the driver
...
SEED = 1649
AUTHORITY_CONTRACT = SCRIPTS / "v508_v507_phase_a_authority_prep_snapshot_repair_execution_authority_contract.json"
AUTHORITY_MATERIALIZER = SCRIPTS / "materialize_v508_v507_phase_a_authority_prep_snapshot_repair_execution_authority.py"
AUTHORITY_MATERIALIZER_SHA = "94ffba60ca9c75bded629ab3948d658bbf8360600889d8522611896dea6450c1"
AUTHORITY_MATERIALIZER_BYTES = 25746
SELF = SCRIPTS / "launch_v508_v507_phase_a_cache_qualification.py"
AUTHORITY_ROOT = J / "v508_v507_phase_a_authority_prep_snapshot_repair_execution_authority_seed1649_20260825"
...
PREREG = J / "v485_v169_cache_qualification_prereg_seed1627_20260824/preregistration.json"
PREREG_SHA = "b73fad8071d8df0b1bed3dc53212bf8a86b2350911620feac81d96a1505492e0"
PREREG_BYTES = 55761
...
AUTHORITY_FORMAT = "strict-track2-v508-v507-phase-a-authority-prep-snapshot-repair-execution-authority-v1"
AUTHORITY_STATUS = "authorized_exact_one_external_v508_phase_a_cache_qualification_attempt"
...
INTENT_FORMAT = "strict-track2-v508-v507-phase-a-cache-qualification-attempt-intent-v1"
TERMINAL_FORMAT = "strict-track2-v508-v507-phase-a-cache-qualification-attempt-terminal-v1"
SOURCE_ORDER = [
...
CHECK_KEYS = [
    "authority_contract_current", "authority_materializer_current", "authorization_boundary", "current_absences",
    "execution_boundary", "f813_exact3", "gpu_empty", "historical_absences", "input_snapshots_equal",
    "no_live_phase_a_process", "no_pending_values", "old_exact7_sources_exact", "old_roots_absent",
    "phase_a_preregistration_exact", "phase_a_static_pass", "publish_stable_excludes_authority_prep",
    "qualification_root_absent", "source_aliases_exact", "source_closure_current", "source_order_exact",
    "transparent_pass_check47", "v506_authority_exact1", "v506_materialization_evidence_exact6",
    "v506_reconciliation_attempt_exact4", "v506_terminal_pass", "v507_failure_evidence_exact6",
    "v507_failure_forensic_exact", "v507_failure_partition", "v507_failure_process_exact",
    "v507_failure_rootcause_exact", "v507_semantic_contract_exact",
]
CONTRACT_TOP_KEYS = {
    "authority_materializer_source", "authority_receipt_contract", "authorization", "current_absences_after_authority",
    "execution_boundary", "f813_registration_tree", "format", "historical_absences", "lineage",
    "phase_a_design_contract_record", "phase_a_launcher_source", "phase_a_preregistration_record",
    "phase_a_static_audit_record", "qualification_output_root", "runtime_observation", "seed", "source_aliases",
    "source_closure", "source_closure_sha256", "source_role_order", "status", "transparent_static_receipt",
    "v506_authority_receipt", "v506_authority_registration_tree", "v506_materialization_evidence_tree",
    "v506_materialization_process_receipt", "v506_reconciliation_attempt_tree", "v506_reconciliation_terminal_receipt",
    "v507_failure_evidence_tree", "v507_failure_forensic", "v507_failure_partition",
    "v507_failure_process_receipt", "v507_failure_stderr", "v507_failure_stdout",
}
AUTH_TOP_KEYS = {
    "authority_design_contract", "authority_materializer_source", "authorization", "cache_scope_helper_source",
    "check_key_set_sha256", "check_keys", "checks", "checks_sha256", "contract_sha256", "execution_boundary",
    "f813_registration_tree", "format", "fresh_phase_a_launcher_source", "frozen_v485_launcher_source",
    "historical_absences", "input_post_snapshot", "input_pre_snapshot", "input_snapshots_exactly_equal", "passed",
    "phase_a_attempt_root", "phase_a_cache_qualification_authorized", "phase_a_design_contract_source",
    "phase_a_driver_source", "phase_a_independent_auditor_source", "phase_a_output_materializer_source",
    "phase_a_preregistration_source", "phase_a_static_audit_source", "phase_a_static_auditor_source",
    "phase_a_worker_source", "preregistration_sha256", "qualification_output_root", "required_absences",
    "restart_v218_source_source", "runtime_observation", "source_aliases", "source_closure", "source_closure_sha256",
    "source_role_order", "status", "training_authorized", "transparent_static_receipt", "v506_authority_receipt",
    "v506_authority_registration_tree", "v506_materialization_evidence_tree",
    "v506_materialization_process_receipt", "v506_reconciliation_attempt_tree", "v506_reconciliation_terminal_receipt",
    "v507_authority_contract_source", "v507_authority_materializer_source", "v507_failure_evidence_tree",
    "v507_failure_forensic", "v507_failure_forensic_source", "v507_failure_partition",
    "v507_failure_process_receipt", "v507_failure_stderr", "v507_failure_stdout",
    "v507_phase_a_launcher_source", "v507_transport_helper_source", "v507_transport_script_source",
}
...
def validate_v507_failure(contract: dict, authority: dict, closure: dict) -> dict:
    forensic_record = regular(V507_FAILURE_FORENSIC, V507_FAILURE_FORENSIC_SHA, V507_FAILURE_FORENSIC_BYTES)
    if closure.get("v507_failure_forensic") != forensic_record:
        raise RuntimeError("v507 forensic source binding")
    forensic = json.loads(V507_FAILURE_FORENSIC.read_text())
    expected_forensic_keys = {
        "absence_transition", "authorization", "date", "format", "passed", "post_observation", "root_cause",
        "runtime_roots", "status", "stderr", "stdout", "v507_materialization_evidence_tree",
        "v507_materialization_process_receipt", "v507_sources",
    }
    if set(forensic) != expected_forensic_keys or csha(sorted(forensic)) != "84731af591f846a81fdfc1c386430e01766bc5345b7b3519aefc2c88eeab1e41":
        raise RuntimeError("v507 forensic schema")
    if forensic.get("format") != "strict-track2-v508-v507-phase-a-authority-prep-snapshot-failure-forensic-v1" or forensic.get("status") != "v507_authority_materialization_failed_no_retry_before_authority_promotion" or forensic.get("passed") is not True:
        raise RuntimeError("v507 forensic terminal")
    root_cause = forensic.get("root_cause", {})
    if root_cause.get("predicate_name") != "current_absences_after_authority.authority_prep.absent" or root_cause.get("sole_false_predicate_inside_publisher") is not True:
        raise RuntimeError("v507 forensic root cause")
    before = {"authority_prep": True, "phase_a_attempt_prep": True, "phase_a_attempt_root": True, "qualification_output_root": True}
    inside = {**before, "authority_prep": False}
    transition = forensic.get("absence_transition", {})
    if transition.get("before_publish_call") != before or transition.get("inside_publisher_after_prep_mkdir") != inside or transition.get("after_failed_cleanup_current") != before or transition.get("unique_false_inside_publisher") != ["authority_prep"]:
        raise RuntimeError("v507 forensic transition")
    evidence_tree = exact_tree(V507_FAILURE_EVIDENCE)
    if any(evidence_tree.get(key) != value for key, value in V507_FAILURE_TREE.items()):
        raise RuntimeError("v507 failure evidence tree")
    process_record = regular(V507_FAILURE_PROCESS, V507_FAILURE_PROCESS_SHA, V507_FAILURE_PROCESS_BYTES)
    stderr_record = regular(V507_FAILURE_STDERR, V507_FAILURE_STDERR_SHA, V507_FAILURE_STDERR_BYTES)
    stdout_record = regular(V507_FAILURE_STDOUT, hashlib.sha256(b"").hexdigest(), 0)
    process = json.loads(V507_FAILURE_PROCESS.read_text())
    partition = {
        "status": "failed_no_retry", "passed": False, "transport_helper_invocations": 1,
        "authority_materializer_invocations": 1, "phase_a_launcher_invocations": 0,
        "phase_a_driver_invocations": 0, "retry_authorized": False, "authority_root_absent": True,
        "root_cause": "publisher stable_absences incorrectly included its owned authority_prep after mkdir",
    }
    if any(process.get(key) != value for key, value in partition.items() if key in process):
        raise RuntimeError("v507 failure process partition")
    expected = {
        "v507_failure_evidence_tree": evidence_tree,
        "v507_failure_process_receipt": process_record,
        "v507_failure_stderr": stderr_record,
        "v507_failure_stdout": stdout_record,
        "v507_failure_partition": partition,
        "v507_failure_forensic": forensic,
    }
    if any(authority.get(key) != value or contract.get(key) != value for key, value in expected.items()):
        raise RuntimeError("v507 failure contract/authority ancestry")
    if any(os.path.lexists(path) for path in (V507_AUTHORITY_ROOT, V507_AUTHORITY_PREP, V507_ATTEMPT_ROOT, V507_ATTEMPT_PREP, QUALIFICATION_ROOT, QUALIFICATION_PREP)):
        raise RuntimeError("v507 failed lineage roots present")
    return expected
...
def validate_authority(args: argparse.Namespace) -> dict:
    ...
    if set(contract) != CONTRACT_TOP_KEYS or set(authority) != AUTH_TOP_KEYS or authority.get("check_keys") != CHECK_KEYS or set(authority.get("checks", {})) != set(CHECK_KEYS):
        raise RuntimeError("v508 schema counts")
    if set(authority["checks"].values()) != {True}:
        raise RuntimeError("v508 authority checks")
    if authority.get("check_key_set_sha256") != csha(CHECK_KEYS) or authority.get("checks_sha256") != csha(authority["checks"]):
        raise RuntimeError("v508 check digests")
    ...
    required = {
        ...
        "phase_a_preregistration": regular(PREREG, PREREG_SHA, PREREG_BYTES),
    ...
    if contract.get("lineage") != lineage or contract.get("authority_materializer_source") != closure["authority_materializer"] or contract.get("phase_a_launcher_source") != self_record:
        raise RuntimeError("v508 contract active lineage")
    if contract.get("authorization") != AUTHORIZATION or contract.get("runtime_observation") != RUNTIME_PRE or contract.get("execution_boundary") != authority.get("execution_boundary"):
        raise RuntimeError("v508 contract boundary")
    if authority.get("authority_design_contract") != contract_record:
        raise RuntimeError("contract alias")
    # Fresh lineage anchors are bound exactly and rehashed by the authority.
    anchors = {
        "v506_authority_receipt": regular(V506_AUTHORITY_RECEIPT, V506_AUTHORITY_SHA, V506_AUTHORITY_BYTES),
        "v506_reconciliation_terminal_receipt": regular(V506_TERMINAL, V506_TERMINAL_SHA, V506_TERMINAL_BYTES),
        "v506_materialization_process_receipt": regular(V506_MATERIALIZATION_PROCESS, V506_MATERIALIZATION_PROCESS_SHA, V506_MATERIALIZATION_PROCESS_BYTES),
        "transparent_static_receipt": regular(TRANSPARENT, TRANSPARENT_SHA, TRANSPARENT_BYTES),
    }
    for key, record in anchors.items():
        if authority.get(key) != record:
            raise RuntimeError(f"lineage anchor {key}")
    v507_failure = validate_v507_failure(contract, authority, closure)
    if authority.get("v506_authority_registration_tree") != exact_tree(V506_AUTHORITY_ROOT):
        raise RuntimeError("v506 authority tree")
    if authority.get("v506_materialization_evidence_tree") != exact_tree(V506_MATERIALIZATION_EVIDENCE):
        raise RuntimeError("v506 materialization tree")
    if authority.get("v506_reconciliation_attempt_tree") != exact_tree(V506_ATTEMPT):
        raise RuntimeError("v506 attempt tree")
    tree = exact_tree(F813.parent)
    for key, value in F813_TREE.items():
        if tree[key] != value:
            raise RuntimeError(f"F813 tree {key}")
    if authority.get("f813_registration_tree") != tree:
        raise RuntimeError("F813 authority tree")
    prereg = json.loads(PREREG.read_text())
    if prereg.get("execution_sources_digest_sha256") != "9adc5bdbfaa0022b745c44abac2ac2bd02f801f751ebce35d0116c0a8e8e53e0":
        raise RuntimeError("v485 exact7 digest")
    records = prereg.get("execution_source_records")
    if not isinstance(records, list) or len(records) != 7 or csha(records) != prereg["execution_sources_digest_sha256"]:
        raise RuntimeError("v485 exact7 records")
    for record in records:
        if regular(Path(record["path"]), record["sha256"], record["logical_bytes"]) != {k: record[k] for k in ("path", "sha256", "logical_bytes")}:
            raise RuntimeError("v485 exact7 current")
    validate_rlpy()
    return {"contract": contract, "authority": authority, "closure": closure, "preregistration": prereg,
            ...
            "f813_tree": tree, "v507_failure": v507_failure}
...
def stable_snapshot(context: dict) -> dict:
    ...
    records.extend([
        regular(V506_AUTHORITY_RECEIPT, V506_AUTHORITY_SHA, V506_AUTHORITY_BYTES),
        regular(V506_TERMINAL, V506_TERMINAL_SHA, V506_TERMINAL_BYTES),
        regular(V507_FAILURE_PROCESS, V507_FAILURE_PROCESS_SHA, V507_FAILURE_PROCESS_BYTES),
        regular(V507_FAILURE_STDERR, V507_FAILURE_STDERR_SHA, V507_FAILURE_STDERR_BYTES),
        regular(V507_FAILURE_STDOUT, hashlib.sha256(b"").hexdigest(), 0),
        regular(TRANSPARENT, TRANSPARENT_SHA, TRANSPARENT_BYTES),
        regular(F813, F813_SHA, F813_BYTES), regular(F813_LOG, F813_LOG_SHA, F813_LOG_BYTES),
    ])
    return {"records": records, "records_sha256": csha(records), "f813_tree": exact_tree(F813.parent),
            "v507_failure_tree": exact_tree(V507_FAILURE_EVIDENCE)}
...
def derive_driver_command(context: dict) -> list[str]:
    ...
    return [str(RLPY), closure["phase_a_driver"]["path"],
            "--preregistration", str(PREREG), "--preregistration-sha", PREREG_SHA,
            ...
            "--worker-source", closure["phase_a_worker"]["path"], "--worker-sha", closure["phase_a_worker"]["sha256"],
            "--auditor-source", closure["phase_a_independent_auditor"]["path"], "--auditor-sha", closure["phase_a_independent_auditor"]["sha256"],
...
def execute(args: argparse.Namespace, hooks=None) -> int:
    ...
    terminal = {"format": TERMINAL_FORMAT, "status": "passed_exact_one_nested_phase_a_driver" if success else "failed_no_retry",
                ...
                "runtime_observation": {"execution_authority_materialized": True, "phase_a_launcher_executed": True,
                    "phase_a_driver_executed": state["started"], "qualification_output_created": QUALIFICATION_ROOT.is_dir(),
                    "cache_reused": False, "training_launched": False, "folds": 0, "policy_updates": 0,
...
def synthetic_self_test() -> int:
    ...
    checks["source_order"] = len(SOURCE_ORDER) == 20 and len(set(SOURCE_ORDER)) == 20
    checks["driver_uses_frozen_launcher"] = "--launcher-source" in derive_driver_command({"closure": {
        "phase_a_driver": {"path": "driver"}, "cache_scope_helper": {"path": "scope", "sha256": "a"*64},
        "phase_a_worker": {"path": "worker", "sha256": "b"*64}, "phase_a_independent_auditor": {"path": "auditor", "sha256": "c"*64},
    ...
    checks["v506_anchors"] = all(token in source for token in (V506_AUTHORITY_SHA, V506_TERMINAL_SHA, TRANSPARENT_SHA, F813_TREE["sha256sum_lines_digest_sha256"]))
    checks["v507_failure_anchors"] = all(token in source for token in (V507_FAILURE_PROCESS_SHA, V507_FAILURE_STDERR_SHA, V507_FAILURE_FORENSIC_SHA, V507_FAILURE_TREE["sha256sum_lines_digest_sha256"], V507_FAILURE_TREE["canonical_json_triples_digest_sha256"]))
    checks["v507_authority_prep_only"] = 'transition.get("unique_false_inside_publisher") != ["authority_prep"]' in source
    checks["v485_exact7"] = "9adc5bdbfaa0022b745c44abac2ac2bd02f801f751ebce35d0116c0a8e8e53e0" in source
    ...
    checks["materializer_binding"] = AUTHORITY_MATERIALIZER_SHA in source and AUTHORITY_MATERIALIZER_BYTES == 25746
