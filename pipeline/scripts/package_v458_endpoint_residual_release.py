#!/usr/bin/env python3
"""Atomically package the single preregistered v458 all-200 endpoint parent."""
...
def main() -> int:
    ...
    if prereg.get("format") != "strict-track2-v458-endpoint-residual-5fold-preregistration-v2":
        raise RuntimeError("bad v458 preregistration")
    if s0.get("format") != "strict-track2-v458-step50-three-head-5fold-s0-receipt-v1" or s0.get("passed") is not True:
        raise RuntimeError("v458 S0 did not pass")
    if report.get("format") != "strict-track2-v458-step50-three-head-5fold-killgate-v2" or report.get("passed") is not True or report.get("all200_training_performed") is not True:
        raise RuntimeError("v458 final training report did not pass")
    if report.get("s0_report_sha256") != sha(s0_path) or report.get("all200_checkpoint_sha256") != sha(checkpoint_path):
        raise RuntimeError("v458 report artifact hash drift")
    if sha(args.runtime) != prereg["source"]["runtime_sha256"] or sha(args.trainer) != prereg["source"]["trainer_sha256"]:
        raise RuntimeError("v458 source closure drift")
    if str(args.v169_release.resolve()) != prereg["v169"]["release"] or str(args.v169_library.resolve()) != prereg["v169"]["library"]:
        raise RuntimeError("v458 v169 canonical path drift")
    ...
    if state.get("format") != CHECKPOINT_FORMAT or state.get("training_scope") != "all200" or state.get("step") != 50 or state.get("precision") != "bf16" or state.get("feature_schema") != expected_schema:
        raise RuntimeError("bad v458 all200 checkpoint contract")
    if state.get("closure_digest") != closure_digest or state.get("runtime_sha256") != sha(args.runtime) or state.get("trainer_sha256") != sha(args.trainer) or state.get("preregistration_sha256") != sha(args.preregistration) or state.get("schedule_sha256") != prereg["all200_schedule_sha256"]:
        raise RuntimeError("v458 all200 checkpoint closure drift")
    output = args.output.resolve()
    partial = output.with_name(output.name + ".partial")
    if output.exists() or partial.exists():
        raise RuntimeError("refusing to overwrite v458 release/partial")
    partial.mkdir(parents=True)
    copy_fsync(checkpoint_path, partial / "all200_action_step50.pt")
    copy_fsync(args.runtime, partial / "v458_v169_endpoint_residual_runtime.py")
    ...
    manifest = {
        ...
        "runtime_source": "v458_v169_endpoint_residual_runtime.py",
        ...
        "sha256": {
            ...
            "runtime_source": sha(partial / "v458_v169_endpoint_residual_runtime.py"),
    ...
    manifest_path = partial / "v458_endpoint_residual_manifest.json"
    ...
    os.fsync(descriptor)
    os.close(descriptor)
    print(json.dumps({"release": str(output), "manifest_sha256": sha(output / "v458_endpoint_residual_manifest.json")}, indent=2))
    return 0
