#!/usr/bin/env python3
"""Train a time-resolved action-conditioned Track 2 predictor on held-out episodes."""
...
from wam_pipeline.temporal_unet import TemporalActionUNet
...
def loss_for(prediction: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    # Full-resolution error, low-frequency consistency, and boundaries.
    ...
    return pixel + 0.2 * coarse + 0.15 * (edge_x + edge_y) / 2.0
...
def evaluate(loader, model, device, mean, std) -> dict[str, float]:
    ...
    with torch.no_grad(), torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=device.type == "cuda"):
        for context, actions, target in loader:
            ...
            prediction = model(context, ((actions.to(device, non_blocking=True) - mean) / std).float()).clamp(0, 1)
            values.append((
                float(functional.l1_loss(prediction, target).float().cpu()),
                float(functional.mse_loss(prediction, target).float().cpu()),
...
def save_checkpoint(output: Path, model, mean, std, metadata: dict) -> None:
    ...
    torch.save({"format": "track2-temporal-unet-v1", "state_dict": model.state_dict()}, output / "model.pt")
    np.savez(output / "action_normalization.npz", mean=mean.cpu().numpy(), std=std.cpu().numpy())
    np.savez(output / "track2_temporal_unet_config.npz", context_frames=np.asarray(5), action_dim=np.asarray(14), prediction_frames=np.asarray(8), working_resolution=np.asarray(256), serving_resolution=np.asarray(256))
...
def main() -> None:
    ...
    model = TemporalActionUNet().to(device)
