#!/usr/bin/env bash
# Preregistered public-train v466 WM parent gate only. No reward, policy update, RL, dev/final, or submission.
set -euo pipefail
ROOT='/root/autodl-tmp/IROS_WAM_2.0 challenge'
J="$ROOT/artifacts/strict_track2_joint_augmentation_20260810"
V461="$J/v461_endpoint200_seed1612_20260823"
REG="$J/v466_endpoint_residual_seed1616_20260823"
TRAINING="$REG/training"
RELEASE="$J/v466_v169_endpoint_residual_parent_release"
RLPY='/root/autodl-tmp/conda_envs/rlinf_track2/bin/python'
PREP="$ROOT/pipeline/scripts/prepare_v466_endpoint_residual_5fold.py"
TRAINER="$ROOT/pipeline/scripts/train_v466_endpoint_residual_5fold.py"
PACKAGER="$ROOT/pipeline/scripts/package_v466_endpoint_residual_release.py"
RUNTIME="$ROOT/pipeline/wam_pipeline/v466_canonical_notransport_paired_delta_runtime.py"
CONTRACT="$ROOT/pipeline/scripts/v466_canonical_notransport_paired_delta_contract.json"
...
LOCK_FILE='/var/lock/v466_endpoint_residual_seed1616.lock'
...
test "$(sha256sum "$PREP" | awk '{print $1}')" = 'd24896ec154bb8dd4e8d48a9638d9fce4e3fbd6cdf975419d06fe63b2f0a2c01'
test "$(sha256sum "$TRAINER" | awk '{print $1}')" = '68fdd71e6e619f46b5df98ee57c56b6173b258af46019db873b609e106e953cb'
test "$(sha256sum "$PACKAGER" | awk '{print $1}')" = '2fe0a6a1dcc0ea3ddc3735aca30fcf04df9e8bfb5f7035f6c7e8ba41484784de'
test "$(sha256sum "$RUNTIME" | awk '{print $1}')" = '78f6df0f4ee6e1ecac81e810bce21d25830c805ce325261cd5710568f37bcefd'
test "$(sha256sum "$CONTRACT" | awk '{print $1}')" = 'c191f988814eff7c9affe7eeea1ac77b04495004457b9a629f4de473f94a92f1'
...
payload={'format':'strict-track2-v466-launcher-failure-receipt-v1','passed':False,'reason':reason,'exit_code':int(code),'wall_seconds':int(elapsed),'gpu_peak_mib':int(peak),'training_process_group':int(pid) if pid else None,'reward_loaded':False,'policy_updates':0,'rl_authorized':False}
tmp=path+'.tmp'
...
WATCHDOG_PID=''
trap - EXIT TERM INT
echo V466_ENDPOINT_RESIDUAL_S0_AND_ALL200_COMPLETE
