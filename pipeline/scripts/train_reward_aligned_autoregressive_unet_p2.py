    ...
    def __init__(
        ...
        for path in sorted(root.glob("episode*_*.npz")):
            ...
            with np.load(path, allow_pickle=False) as values:
                ...
                target = values["target_frames"].astype(np.float32) / 255.0
                context_last = values["context_frames"][-1:].astype(np.float32) / 255.0
                previous = np.concatenate((context_last, target[:-1]), axis=0)
                motion_score = float(np.mean(np.abs(target - previous)))
        ...
        self.motion_score = np.asarray(motion_scores, dtype=np.float32)
...
def main() -> None:
    ...
    parser.add_argument("--high-motion-weight", type=float, default=1.0)
    parser.add_argument("--high-motion-threshold", type=float, default=0.03)
    parser.add_argument("--parent-anchor-weight", type=float, default=0.0)
    ...
    if min(args.learning_rate, args.reward_loss_weight, args.reward_delta_weight, args.reward_terminal_weight, args.right_weight, args.success_weight, args.late_weight, args.reward_probability_scale_floor, args.official_weight, args.high_motion_weight) <= 0:
        raise SystemExit("weights and learning rate must be positive")
    if args.parent_anchor_weight < 0 or args.high_motion_threshold < 0:
        raise SystemExit("anchor weight and high-motion threshold must be nonnegative")
    ...
    weights[dataset.motion_score >= args.high_motion_threshold] *= args.high_motion_weight
    ...
    anchor_model = None
    if args.parent_anchor_weight > 0:
        anchor_model = OneStepActionUNet().to(device)
        anchor_model.load_state_dict(state["state_dict"], strict=True)
    ...
    for step in range(1, args.steps + 1):
        ...
        with torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=device.type == "cuda"):
            ...
            if anchor_model is not None:
                with torch.no_grad():
                    anchor_prediction = rollout(anchor_model, context, history_actions, future)
                anchor_loss = F.smooth_l1_loss(prediction, anchor_prediction, beta=0.02)
            else:
                anchor_loss = prediction.new_zeros(())
        ...
        loss = (
            image_loss.float()
            + args.reward_loss_weight * task_loss
            + args.parent_anchor_weight * anchor_loss.float()
        )
        loss.backward()
        ...
        if step == 1 or step % 10 == 0:
            row = {
                ...
                "parent_anchor_loss": float(anchor_loss.detach().cpu()),
        ...
        if step % args.checkpoint_interval == 0 or step == args.steps:
            metadata = {
                ...
                "sampling": {
                    ...
                    "expected_high_motion_fraction": float(
                        weights[dataset.motion_score >= args.high_motion_threshold].sum() / weights.sum()
                    ),
                },
                "history": history,
            }
