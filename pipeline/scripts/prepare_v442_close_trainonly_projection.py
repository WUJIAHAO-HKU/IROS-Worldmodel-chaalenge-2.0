#!/usr/bin/env python3
"""Prospectively freeze v442 train-only calibration and S0/S1 gates."""
...
def main() -> int:
    ...
    files = {
        "calibrator": ROOT / "pipeline/scripts/calibrate_v442_close_trainonly_rgb_projection.py",
        "runtime": ROOT / "pipeline/wam_pipeline/v442_v169_close_aligned_projection_runtime.py",
        "s0_auditor": ROOT / "pipeline/scripts/audit_v442_static_contract.py",
        "packager": ROOT / "pipeline/scripts/package_v442_close_aligned_release.py",
        "split": args.split,
    ...
    payload = {
        "format": "strict-track2-v442-close-trainonly-preregistration-v1",
        ...
        "construction": {
            ...
            "phases": ["close"],
            "gate": "explicit right; right7D path > left7D path; history-last open; future first close then held; no mu",
            ...
            "calibration": "leave one right-train episode out; scalar beta per frame/RGB channel",
            "folds": 15,
        ...
        "s1_gate": {
            ...
            "right_intervention_fraction_exact": 0.0625,
            "candidate_minus_v169_pixel_abs_max": 8,
            "left_and_g0_bitexact": True,
            "all_required": True,
