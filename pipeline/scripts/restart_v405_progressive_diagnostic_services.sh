...
VERSION='track2-v405-v400-output-equivalent-progressive-diagnostic'
...
service() {
  ...
  exec taskset -c "$CPUSET" env PYTHONPATH="$ROOT/pipeline" \
    ...
    WAM_BACKEND='v405-v400-progressive-diagnostic' \
...
case "${1:-start}" in
  ...
  bridge) bridge ;;
  stop)
    screen -S wm_v405_bridge -X quit >/dev/null 2>&1 || true
    screen -S wm_v405_gpu -X quit >/dev/null 2>&1 || true
    ;;
  start)
    mkdir -p "$REG"
    test ! -e "$REG/route_trace.jsonl"
    for name in wm_v405_bridge wm_v405_gpu wm_v218_bridge wm_v218_gpu; do
