#!/usr/bin/env python3
"""Prospectively freeze v443 spatial train-only calibration and gates."""
...
def main() -> int:
    ...
    files = {
        "calibrator": ROOT / "pipeline/scripts/calibrate_v443_close_trainonly_spatial_projection.py",
        "runtime": ROOT / "pipeline/wam_pipeline/v443_v169_close_spatial_projection_runtime.py",
        "s0_auditor": ROOT / "pipeline/scripts/audit_v443_static_contract.py",
        "packager": ROOT / "pipeline/scripts/package_v443_close_spatial_release.py",
        "split": args.split,
    ...
    payload = {
        "format": "strict-track2-v443-close-trainonly-preregistration-v1",
        ...
        "construction": {
            ...
            "gate": "v442 close-only: explicit right; right7D path>left7D; history-last open; future first close then held; no mu",
            "samples_per_right_train_episode": 1,
            "selection": "among exact eight close windows choose first-close index nearest future index4, then earliest start",
            "calibration": "15-fold leave-one-right-train-episode-out zero-intercept beta per time/pixel/RGB",
            "sign_agreement_min": 0.80,
            "heldout_pixel_sse_improvement_fraction_min": 0.80,
            "coefficient_abs_max": 1.0,
            "protected_frames_exact_zero": [0, 1, 6, 7],
            "active_pixels_min": 64,
            "aggregate_loeo_sse_ratio_max_exclusive": 1.0,
            "morphology_or_smoothing": False,
            "developer_alpha_or_threshold_sweep": False,
        },
        "data": {
            "train_episodes": train, "right_train_episodes": right,
            "validation_episodes_excluded": validation,
            "reward_or_outcome_for_calibration": False,
            "development_or_final_for_calibration": False,
        },
        "s0_gate": {
            "exact_train40_right15": True, "exact_samples": 15,
            "active_pixels_min": 64,
            "every_active_sign_agreement_min": 0.80,
            "every_active_heldout_pixel_improvement_fraction_min": 0.80,
            "aggregate_loeo_sse_ratio_max_exclusive": 1.0,
            "coefficient_abs_max": 1.0,
            "protected_frames_exact_zero": [0, 1, 6, 7],
            "left_and_g0_bitexact": True,
            "candidate_minus_v169_pixel_abs_max": 8,
            "runtime_reward_free": True, "all_required": True,
            "on_fail": "reject before S1",
        },
        "s1_gate": {
            "fixed_right_dev_samples": 32,
            "action_only_expected_decisions": 128,
            "action_only_gate_active_exact": 8,
            "action_only_gate_distinct_samples_exact": 8,
            "action_only_gate_phase": "grasp only",
            "action_only_gate_chunk_pattern": [1, 0, 0, 0],
            "action_only_gate_other_phase_or_chunk_active_exact": 0,
            "action_only_gate_repeat_active_exact": 0,
            ...
            "protected_frames_0_1_6_7_bitexact": True,
            "candidate_minus_v169_pixel_abs_max": 8,
            "left_and_g0_bitexact": True,
            "all_required": True,
            "on_fail": "reject; no trace and no RL",
            "on_pass": "permit only separately preregistered zero-update short trace",
        },
