#!/usr/bin/env python3
"""Extend the frozen v326 contract with visible-onset terminal checks."""
...
def foreground_fraction(frame: np.ndarray) -> float:
    return float(np.mean(np.min(frame, axis=-1) < 220))
...
def main() -> int:
    ...
    candidate_runtime = Track2V331PhaseOnsetTerminal(
    ...
    phase_ready, phase_episode, _, _ = candidate_runtime._phase(base)
    onset_row, onset_episode, same_episode = candidate_runtime._onset_target(
    ...
    candidate_foreground = foreground_fraction(candidate[-1])
    v326_foreground = foreground_fraction(v326[-1])
    added = {
        "transition_phase_ready": phase_ready,
        "transition_onset_row_matches_frozen_phase": (
            int(candidate_runtime.row_start[onset_row]) == int(candidate_runtime.phase_onset[onset_episode])
        ),
        "transition_terminal_changes_v326": bool(np.any(candidate[-1] != v326[-1])),
        "transition_preterminal_bit_exact_v326": bool(np.array_equal(candidate[:-1], v326[:-1])),
        "transition_next_context_rgb_strictly_improves_v326": candidate_rgb < v326_rgb,
        "transition_temporal_delta_strictly_improves_v326": candidate_temporal < v326_temporal,
        "transition_foreground_fraction_ge_v326_plus_0p05": candidate_foreground >= v326_foreground + 0.05,
        "counterfactual_full_sequences_bit_exact_v317": bool(
            np.array_equal(candidate_counterfactuals, v317_counterfactuals)
        ),
    }
    report["format"] = "strict-track2-v331-phase-onset-terminal-contract-v1"
    ...
    report["onset_transition"] = {
        "window": path.name,
        "phase_episode": phase_episode,
        "onset_episode": onset_episode,
        "same_episode": same_episode,
        "onset_start": int(candidate_runtime.row_start[onset_row]),
        ...
        "candidate_terminal_foreground_fraction": candidate_foreground,
        "v326_terminal_foreground_fraction": v326_foreground,
    }
    report["checks"].update(added)
    report["passed"] = all(report["checks"].values())
    output.write_text(json.dumps(report, indent=2) + "\n")
    print(json.dumps({"v331_passed": report["passed"], "added_checks": added,
                      "onset_transition": report["onset_transition"]}, indent=2))
