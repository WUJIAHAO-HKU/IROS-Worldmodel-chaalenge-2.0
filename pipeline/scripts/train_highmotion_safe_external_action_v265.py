...
def main() -> None:
    ...
    parser.add_argument("--high-motion-batch-size", type=int, default=0)
    parser.add_argument("--learning-rate", type=float, default=5e-6)
    parser.add_argument("--external-gradient-weight", type=float, default=0.25)
    parser.add_argument("--high-motion-gradient-weight", type=float, default=0.5)
    ...
    high_indices = np.flatnonzero(scores >= 0.04).tolist()
    high_loader = None
    if args.high_motion_batch_size:
        if not high_indices:
            raise RuntimeError("high-motion protection requested but no high-motion windows exist")
        high_loader = DataLoader(
            Subset(official, high_indices), batch_size=args.high_motion_batch_size,
    ...
    config.update({
        ...
        "official_high_motion_windows": len(high_indices),
        "algorithm": (
            "PCGrad external gradient projected against official gradient; frozen visual backbone"
            if high_loader is None else
            "high-motion anchored PCGrad: general-official and external gradients projected against high-motion gradient"
        ),
    })
    history = [{"step": 0, "validation": baseline}]
    metadata = {
        "backend": "autoregressive_unet", "format": "track2-autoregressive-unet-v1",
        "stage": "v26.5-high-motion-safe-external-action" if high_loader else "v26.4-conflict-safe-external-action",
        "checkpoint_step": 0,
    ...
    high_iterator = iter(high_loader) if high_loader is not None else None
    best = baseline_mae
    best_overall = baseline_mae
    conflict_steps = 0
    for step in range(1, args.steps + 1):
        ...
        if high_loader is None:
            reference = official_grad
            reference_norm = sum((value.float() ** 2).sum() for value in reference).clamp_min(1e-12)
            external_projection = (dot / reference_norm).clamp_max(0.0)
            if float(dot) < 0:
                conflict_steps += 1
            for parameter, anchor, left, right in zip(parameters, anchors, official_grad, external_grad):
                safe_external = right.float() - external_projection * left.float()
                parameter.grad = (left.float() + args.external_gradient_weight * safe_external
                                  + args.pullback_weight * (parameter.detach() - anchor))
            high_loss = None; general_high_dot = dot; external_high_dot = dot
        else:
            high_batch, high_iterator = next_batch(high_loader, high_iterator)
            with torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=device.type == "cuda"):
                high_loss = loss_for(model, high_batch, mean, std, device)
            high_grad = torch.autograd.grad(high_loss, parameters)
            high_norm = sum((value.float() ** 2).sum() for value in high_grad).clamp_min(1e-12)
            general_high_dot = sum((left.float() * ref.float()).sum()
                                   for left, ref in zip(official_grad, high_grad))
            external_high_dot = sum((right.float() * ref.float()).sum()
                                    for right, ref in zip(external_grad, high_grad))
            general_projection = (general_high_dot / high_norm).clamp_max(0.0)
            external_projection = (external_high_dot / high_norm).clamp_max(0.0)
            if float(general_high_dot) < 0 or float(external_high_dot) < 0:
                conflict_steps += 1
            for parameter, anchor, general, external_value, reference_value in zip(
                parameters, anchors, official_grad, external_grad, high_grad
            ):
                reference_value = reference_value.float()
                safe_general = general.float() - general_projection * reference_value
                safe_external = external_value.float() - external_projection * reference_value
                parameter.grad = (
                    args.high_motion_gradient_weight * reference_value + safe_general
                    + args.external_gradient_weight * safe_external
                    + args.pullback_weight * (parameter.detach() - anchor)
                )
        ...
        if step == 1 or step % 25 == 0:
            print(json.dumps({
                ...
                "high_motion_loss": None if high_loss is None else float(high_loss),
                "official_external_dot": float(dot),
                "general_high_motion_dot": float(general_high_dot),
                "external_high_motion_dot": float(external_high_dot),
                "conflict_fraction": conflict_steps / step,
            }), flush=True)
        if step % args.validation_interval == 0 or step == args.steps:
            ...
            if metric < best_overall:
                ...
                overall_metadata = {
                    "backend": "autoregressive_unet", "format": "track2-autoregressive-unet-v1",
                    "stage": "v26.5-high-motion-safe-external-action" if high_loader else "v26.4-conflict-safe-external-action",
                    "checkpoint_step": step, "config": config, "validation": history,
                    "selection": "best overall official dev; requires separate safety routing",
                    "best_official_dev_mae": best_overall,
                }
                save_checkpoint(output / "best_overall", model, mean, std, overall_metadata)
    ...
    report = {
        ...
        "baseline": baseline, "best_official_dev_mae": best,
        "best_overall_official_dev_mae": best_overall, "history": history,
