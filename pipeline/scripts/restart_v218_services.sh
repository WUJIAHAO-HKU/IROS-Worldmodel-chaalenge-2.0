...
V218="$JOINT/v218_v217_route_aware_service_promotion_seed1417"
...
run_service() {
  ...
  exec env PYTHONPATH="$ROOT/pipeline" \
    ...
    "$GO1" pipeline/scripts/serve.py >> "$V218/v218_service_gpu.log" 2>&1
}
...
case "${1:-start}" in
  ...
  bridge) run_bridge ;;
  stop)
    screen -S wm_v218_bridge -X quit >/dev/null 2>&1 || true
    screen -S wm_v218_gpu -X quit >/dev/null 2>&1 || true
    ;;
  start)
    for name in wm_v218_bridge wm_v218_gpu wm_v217_gpu; do
    ...
    if ss -ltn | grep -qE ':(8005|18084) '; then
      echo 'v218 ports did not stop cleanly' >&2
      exit 3
    fi
    screen -dmS wm_v218_gpu bash "$0" service
