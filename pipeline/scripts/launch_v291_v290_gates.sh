...
N='v291_v290_right_closed_mirror_gates_seed1479_20260821'; RUN="$J/$N"; REG="$O/run_registry/$N"; S="$B/pipeline/scripts"
...
V='track2-v290-right-closed-mirror-v271'; URL='http://127.0.0.1:8005'; PUBLIC="$B/artifacts/adjust_bottle_windows_full"; FAIL="$J/onpolicy_windows_full128_stride4"
...
restore(){ bash "$S/restart_v290_services.sh" stop >/dev/null 2>&1 || true; TRACK2_CPUSET=0-23 bash "$S/restart_v271_v274_services.sh" start >"$REG/restart_v271_v274.log" 2>&1 || true; }
trap restore EXIT
"$RLPY" "$S/prepare_v291_v290_gates.py"
...
TRACK2_SERVICE_REG="$REG" TRACK2_CPUSET=0-23 bash "$S/restart_v290_services.sh" start >"$RUN/start_service.log" 2>&1
...
capture_rc=$?
set -e
if (( capture_rc != 0 )); then touch "$RUN/V291_CAPTURE_GATE_REJECTED"; exit "$capture_rc"; fi
touch "$RUN/V291_CAPTURE_GATE_PASSED"
cd "$B"
...
bash "$S/restart_v290_services.sh" stop
...
PY
touch "$RUN/V291_LONG_GATE_PASSED" "$RUN/AUDIT_COMPLETE"
echo V291_V290_GATES_PASSED
