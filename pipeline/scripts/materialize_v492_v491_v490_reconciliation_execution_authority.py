#!/usr/bin/env python3
"""Atomically register a v492 repair authority; never run either wrapper or r2."""
...
CONTRACT_PATH = ROOT / "pipeline/scripts/v492_v491_v490_reconciliation_execution_authority_contract.json"
CONTRACT_FORMAT = "strict-track2-v492-v491-v490-reconciliation-execution-authority-design-contract-v1"
CONTRACT_STATUS = "design_only_frozen_sources_pending_independent_review_no_authority"
OUTPUT_FORMAT = "strict-track2-v492-v491-v490-reconciliation-execution-authority-v1"
OUTPUT_STATUS = "authorized_exact_one_external_v490_reconciliation_wrapper_attempt"
AUTHORITY_ROOT = J / "v492_v491_v490_reconciliation_execution_authority_seed1634_20260825"
OUTER_EVIDENCE_ROOT = J / "v492_v491_v490_reconciliation_outer_execution_evidence_seed1634_20260825"
FAILED_V491_AUTHORITY_ROOT = J / "v491_v490_reconciliation_execution_authority_seed1633_20260825"
FAILED_V491_OUTER_ROOT = J / "v491_v490_reconciliation_outer_execution_evidence_seed1633_20260825"
...
SOURCE_ROLES = {
    ...
    "outer_execution_wrapper", "failed_v491_authority_contract",
    "failed_v491_authority_materializer", "failed_v491_outer_wrapper",
}
CONTRACT_TOP_KEYS = {
    ...
    "failed_v491_materialization_evidence_tree", "failed_v491_process_receipt",
    "superseded_v491_absences",
...
AUTH_TOP_KEYS = {
    ...
    "failed_v491_authority_contract", "failed_v491_authority_materializer_source",
    "failed_v491_outer_wrapper_source", "failed_v491_process_receipt",
    "failed_v491_materialization_evidence_tree", "superseded_v491_absences",
}
AUTH_CHECK_KEYS = sorted({
    ...
    "failed_v491_contract_current", "failed_v491_materializer_current",
    "failed_v491_outer_wrapper_current", "failed_v491_evidence_exact6",
    "failed_v491_no_retry_partition", "superseded_v491_roots_absent",
})
AUTHORIZATION = {
...
def synthetic_self_test() -> int:
    checks = {
        "authority_top_count": len(AUTH_TOP_KEYS) == 44,
        "check_count": len(AUTH_CHECK_KEYS) == 33,
        "source_roles": len(SOURCE_ROLES) == 10,
...
def main() -> int:
    ...
    if (set(contract) != CONTRACT_TOP_KEYS or contract.get("format") != CONTRACT_FORMAT
            or contract.get("status") != CONTRACT_STATUS or contract.get("seed") != 1634
            or pending(contract)):
    ...
    failed_v491_tree = verify_tree(contract["failed_v491_materialization_evidence_tree"])
    failed_v491_record = record(contract["failed_v491_process_receipt"])
    failed_v491 = json.loads(Path(failed_v491_record["path"]).read_text())
    if (failed_v491_tree["file_count"] != 6
            or not any(row == ["process_receipt.json", failed_v491_record["sha256"],
                               failed_v491_record["logical_bytes"]]
                       for row in failed_v491_tree["inventory"])
            or failed_v491.get("status") != "failed_no_retry"
            or failed_v491.get("passed") is not False
            or failed_v491.get("materializer_invocations") != 1
            or failed_v491.get("outer_wrapper_invocations") != 0
            or failed_v491.get("inner_wrapper_invocations") != 0
            or failed_v491.get("r2_invocations") != 0
            or failed_v491.get("retry_authorized") is not False
            or failed_v491.get("cleanup", {}).get("reaped") is not True
            or failed_v491.get("cleanup", {}).get("group_empty") is not True):
        raise RuntimeError("failed v491 process boundary")
    failed_argv_path = Path(failed_v491_tree["root"]) / "argv.json"
    failed_stderr_path = Path(failed_v491_tree["root"]) / "materializer_stderr.log"
    failed_argv = json.loads(failed_argv_path.read_text())
    failure_aliases = {
        "failed_v491_authority_contract": sources["failed_v491_authority_contract"],
        "failed_v491_authority_materializer": sources["failed_v491_authority_materializer"],
        "failed_v491_outer_wrapper": sources["failed_v491_outer_wrapper"],
    }
    if (failed_argv.get("sources", {}).get(failure_aliases["failed_v491_authority_contract"]["path"])
            != failure_aliases["failed_v491_authority_contract"]
            or failed_argv.get("sources", {}).get(failure_aliases["failed_v491_authority_materializer"]["path"])
            != failure_aliases["failed_v491_authority_materializer"]
            or failed_argv.get("sources", {}).get(failure_aliases["failed_v491_outer_wrapper"]["path"])
            != failure_aliases["failed_v491_outer_wrapper"]
            or "UnboundLocalError" not in failed_stderr_path.read_text()
            or "pending" not in failed_stderr_path.read_text()):
        raise RuntimeError("failed v491 source/error ancestry")
    ...
    historical_keys = {
        ...
        "superseded_v491_authority_root", "superseded_v491_authority_prep",
        "superseded_v491_outer_evidence_root", "superseded_v491_outer_evidence_prep",
    }
    ...
    superseded = contract["superseded_v491_absences"]
    expected_superseded = {
        "superseded_v491_authority_root": FAILED_V491_AUTHORITY_ROOT,
        "superseded_v491_authority_prep": FAILED_V491_AUTHORITY_ROOT.with_name(
            FAILED_V491_AUTHORITY_ROOT.name + ".registration-prep"),
        "superseded_v491_outer_evidence_root": FAILED_V491_OUTER_ROOT,
        "superseded_v491_outer_evidence_prep": FAILED_V491_OUTER_ROOT.with_name(
            FAILED_V491_OUTER_ROOT.name + ".outer-prep"),
    }
    if (set(superseded) != set(expected_superseded)
            or any(superseded[name] != {"path": str(path), "absent": True}
                   or historical[name] != path or os.path.lexists(path)
                   for name, path in expected_superseded.items())):
        raise RuntimeError("superseded v491 absences")
    ...
    trees = {
        ...
        "failed_v491_materialization": contract["failed_v491_materialization_evidence_tree"]["root"],
    }
    ...
    receipt = {
        ...
        "failed_v491_authority_contract": sources["failed_v491_authority_contract"],
        "failed_v491_authority_materializer_source": sources["failed_v491_authority_materializer"],
        "failed_v491_outer_wrapper_source": sources["failed_v491_outer_wrapper"],
        "failed_v491_process_receipt": failed_v491_record,
        "failed_v491_materialization_evidence_tree": failed_v491_tree,
        "superseded_v491_absences": superseded,
        ...
        if old_mask is not None:
            if hasattr(signal, "sigpending"):
                ...
                for candidate in (signal.SIGINT, signal.SIGTERM):
                    if candidate in pending_signals:
