...
VERSION='track2-v409-v407-half-contracted-progressive'
...
service() {
  ...
  exec taskset -c "$CPUSET" env PYTHONPATH="$ROOT/pipeline" \
    ...
    WAM_BACKEND='v409-half-contracted-progressive' \
    ...
    WAM_NATIVE_BATCH_MICRO_SIZE=8 WAM_RELEASE_CUDA_CACHE="${TRACK2_WAM_RELEASE_CUDA_CACHE:-1}" \
    "$GO1" pipeline/scripts/serve.py >>"$REG/v409_service.log" 2>&1
}
...
case "${1:-start}" in
  ...
  bridge) bridge ;;
  stop)
    screen -S wm_v409_bridge -X quit >/dev/null 2>&1 || true
    screen -S wm_v409_gpu -X quit >/dev/null 2>&1 || true
    ;;
  start)
    mkdir -p "$REG"
    for name in wm_v409_bridge wm_v409_gpu wm_v218_bridge wm_v218_gpu; do
