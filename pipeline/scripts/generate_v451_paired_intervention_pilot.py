#!/usr/bin/env python3
"""Two-process, branch-serial fixed-horizon simulator collection for v451."""
...
def ensure_open3d_importable() -> bool:
    ...
    if existing is not None and getattr(existing, "__v451_rgb_only_stub__", False):
        return True
...
def runtime_provenance() -> dict:
    ...
    try:
        ...
        stub.__v451_rgb_only_stub__ = True
        stub.__all__ = []
...
def timeout_handler(_signum, _frame) -> None:
    raise TimeoutError("v451 fixed context exceeded 300 seconds")
...
def main() -> int:
    ...
    if (
        prereg.get("format")
        != "strict-track2-v451-paired-intervention-pilot-preregistration-v1"
        or out.exists()
    ):
        raise RuntimeError("v451 prereg/output contract failed")
                ...
                technical.append(
                    {
                        ...
                        "error": f"GPU peak {peak} MiB exceeded 16384 MiB",
                    }
                )
                aborted = True
                break
    ...
    report = {
        "format": "strict-track2-v451-paired-intervention-pilot-generation-report-v1",
        ...
        "guards": {
            ...
            "pointcloud_calls": 0,
            "optional_curobo_traceback_nonfatal": True,
        },
    }
