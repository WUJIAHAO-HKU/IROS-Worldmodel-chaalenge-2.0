...
def reconstruction_loss(
    ...
    """Weight changed pixels so the moving arms are not dominated by background."""
    if motion_weight > 1.0:
        ...
        pixel = (weights * (prediction - target).abs()).mean() + 0.05 * (weights * (prediction - target).square()).mean()
    else:
    ...
    coarse = functional.l1_loss(
        functional.avg_pool2d(prediction, kernel_size=2), functional.avg_pool2d(target, kernel_size=2)
    )
...
def evaluate(loader, model, device, mean, std) -> dict[str, float]:
    ...
    with torch.no_grad(), torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=device.type == "cuda"):
        for context, history, future, target in loader:
            ...
            values.append((
                float(functional.l1_loss(prediction, target).float().cpu()),
                float(functional.l1_loss(prediction[:, 0], target[:, 0]).float().cpu()),
                float(functional.l1_loss(context[:, -1:].expand_as(target), target).float().cpu()),
            ))
    mae, first_mae, copy = np.mean(values, axis=0)
    return {"rollout_mae": float(mae), "first_frame_mae": float(first_mae), "copy_last_mae": float(copy)}
...
def main() -> None:
    ...
    parser.add_argument("--train-rollout-steps", type=int, default=1, choices=range(1, 9))
    ...
    if args.motion_weight < 1.0 or args.motion_threshold < 0.0:
        raise SystemExit("motion weight must be >= 1 and motion threshold must be >= 0")
    ...
    validation_loader = DataLoader(Subset(validation, validation_indices), batch_size=args.batch_size, shuffle=False, num_workers=2, pin_memory=True)
    ...
    output, best_mae, history = Path(args.output), float("inf"), []
    ...
    for step in range(1, args.steps + 1):
        ...
        if step % args.validation_interval == 0 or step == args.steps:
            ...
            metadata = {"backend": "autoregressive_unet", "format": "track2-autoregressive-unet-v1", "windows": str(Path(args.windows).resolve()), "split_manifest": str(Path(args.split_manifest).resolve()), "train_window_count": len(train), "validation_window_count": len(validation), "validation_sample_count": sample_count, "context_frames": 5, "history_actions": 4, "future_actions": 8, "target_frames": 8, "action_dim": 14, "working_resolution": 256, "serving_resolution": 256, "initialization_checkpoint": str(Path(initialization).resolve()) if initialization else None, "train_rollout_steps": args.train_rollout_steps, "motion_weight": args.motion_weight, "motion_threshold": args.motion_threshold, "initialized_parameter_count": len(initialized_keys), "checkpoint_step": step, "validation": history, "best_rollout_mae": min(best_mae, result["rollout_mae"])}
            ...
            print(json.dumps({"step": step, "validation": result, "best_rollout_mae": best_mae}), flush=True)
