...
def main() -> None:
    ...
    for step in range(start_step + 1, args.steps + 1):
        ...
        if step % args.validation_interval == 0 or step == args.steps:
            ...
            right_action_result = evaluate(right_validation_loader, model, device, mean, std, args.high_motion_threshold) if right_validation_indices else None
            model.train()
            ...
            if right_action_result is not None:
                selection_metric = (
                    ...
                    + args.right_action_selection_weight * float(right_action_result["rollout_mae"])
                )
