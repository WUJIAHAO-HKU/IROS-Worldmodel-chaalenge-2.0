#!/usr/bin/env python3
"""Preregister v458 only after a passed immutable v456 final audit receipt."""
...
def tree(root):
 ...
 if root==globals().get("_V169_LIBRARY_ROOT"):
  release=globals()["_V169_RELEASE_ROOT"];manifest=json.loads((release/"v168_release/base_release/release_manifest.json").read_text());relative=set(manifest["sha256"]);relative.add(manifest["retrieval"]["split_manifest"]);windows=root/manifest["retrieval"]["windows_directory"]
  relative.update(str(path.relative_to(root)) for path in windows.rglob("*") if path.is_file());return [{"relative":name,"sha256":sha(root/name)} for name in sorted(relative)]
 return [{"relative":str(path.relative_to(root)),"sha256":sha(path)} for path in sorted(root.rglob("*")) if path.is_file()]
def arrsha(value):
 import numpy as np
...
def request_phase(row,cache):
 ...
 if source not in cache:
  if sha(source)!=row["source_hdf5_sha256"]:raise RuntimeError("v458 HDF5 SHA drift")
 ...
 if arrsha(history)!=row["history_action_sha256"] or {n:arrsha(v) for n,v in branches.items()}!=row["branch_action_sha256"] or endpoint!=row["endpoint_diffs"] or fullpath!=row["path_diffs"] or min(endpoint.values())<.01 or min(fullpath.values())<.01:raise RuntimeError("v458 action closure drift")
...
def shuffle_map(rows,cache):
 ...
 for episode,group in sorted(groups.items()):
  ...
  if n<2:raise RuntimeError(f"v458 phase shuffle requires >=2 contexts episode={episode}")
  offset=max(1,n//2)
...
def main():
 ...
 if len(contexts)!=200 or len(episodes)!=15 or any(len(x["holdout_episodes"])!=3 or len(x["fit_episodes"])!=12 for x in folds):raise RuntimeError("v458 episode-grouped 5fold contract failed")
 files=[]
 ...
 if len(files)!=410:raise RuntimeError(f"v458 expected 410 frozen v456 files, got {len(files)}")
 ...
 globals()["_V169_RELEASE_ROOT"]=a.v169_release.resolve();globals()["_V169_LIBRARY_ROOT"]=a.v169_library.resolve()
 cache={};mappings=[]
 for fold in range(5):
 ...
 schedules=[]
 for fold in range(5):
  fit_contexts=[i for i,x in enumerate(ordered) if int(x["fold"])!=fold];fit_ids=np.asarray([5*i+b for i in fit_contexts for b in range(5)]);rng=np.random.default_rng(1611+fold);schedule=[{"sample_ids":rng.choice(fit_ids,size=8,replace=False).tolist(),"context_id":fit_contexts[step%len(fit_contexts)]} for step in range(50)];encoded=json.dumps(schedule,sort_keys=True,separators=(",",":"));schedules.append({"fold":fold,"sha256":hashlib.sha256(encoded.encode()).hexdigest(),"same_for_heads":["action","context_only","phase_shuffle"],"optimizer_independent":True})
 rng=np.random.default_rng(1711);all200_schedule=[{"sample_ids":rng.choice(np.arange(1000),size=8,replace=False).tolist(),"context_id":step%200} for step in range(50)];all200_schedule_sha=hashlib.sha256(json.dumps(all200_schedule,sort_keys=True,separators=(",",":")).encode()).hexdigest()
 payload={"format":"strict-track2-v458-endpoint-residual-5fold-preregistration-v2","created_at":datetime.now(timezone.utc).isoformat(),"classification":"public-train endpoint world-model parent pilot only; no reward, policy training, RL, dev/final, or submission","seed":1611,"v461":{"final_receipt":{"path":str(a.v456_final_receipt.resolve()),"sha256":sha(a.v456_final_receipt)},"selection":{"path":str(a.v456_selection.resolve()),"sha256":sha(a.v456_selection)},"generation_report":{"path":str(a.v456_generation_report.resolve()),"sha256":sha(a.v456_generation_report)},"dataset":str(a.v456_dataset.resolve()),"files":files,"contexts":200,"endpoint_samples":1000,"branches":["factual","no_transport","scale_0p4","scale_1p25","reverse_direction_0p4"]},"folds":folds,"phase_shuffle":{"formula":"request-only q0/history+future right6; score=.5 if path<=1e-8 else first4_step_sum/total; within episode sort(score,sha256(float32 history||factual future)); offset=max(1,n//2)","mappings":mappings},"v169":{"release":str(a.v169_release.resolve()),"library":str(a.v169_library.resolve()),"runtime_source":{"path":str(a.v169_runtime_source.resolve()),"sha256":sha(a.v169_runtime_source)},"release_manifest_sha256":sha(v169_manifest),"library_manifest":str(library_manifest.resolve()),"library_manifest_sha256":sha(library_manifest),"release_files":tree(a.v169_release),"library_files":tree(a.v169_library)},"source":{"trainer_path":str(a.trainer.resolve()),"trainer_sha256":sha(a.trainer),"runtime_path":str(a.runtime.resolve()),"runtime_sha256":sha(a.runtime)},"training":{"heads_per_fold":["action","context_only","phase_shuffle"],"steps_per_head":50,"batch_size":8,"working_resolution":128,"channels":16,"optimizer":"AdamW","learning_rate":0.0003,"weight_decay":0.0001,"gradient_clip":1.0,"endpoint_l1":1.0,"gradient_l1":0.25,"same_context_action_delta_l1":0.5,"precision":"bf16","all200_action_head_steps_after_oof_pass":50},"features":{"normalized_actions":168,"right6_endpoint_delta":6,"right6_anchor_relative_path":48,"request_only_postclose":1,"arm_tokens":2,"postclose_formula":"shape/finite and history[-1,13]<.5 and all future[:,13]<.5"},"kill_gate":{"continuous_over_v169_max":0.95,"uint8_over_v169_max":1.0,"factual_branch_max":1.0,"each_counterfactual_branch_max":0.98,"action_over_context_only_max":0.95,"action_over_phase_shuffle_max":0.97,"positive_target_delta_cosine_fraction_min":0.60,"distinct_action_prediction_nonzero_fraction_min":0.95,"passing_folds_min":4,"improved_episodes_min":12},"runtime":{"base":"frozen v169","right_postclose_frame8_only":True,"left_g0_frames0_to6_bitexact":True,"four_apis":["predict","predict_with_baseline","predict_batch","predict_batch_with_baseline"],"official_reward_modified":False,"policy_modified":False},"guards":{"v461_final_pass_required":True,"development_or_final_used":False,"reward_loaded":False,"policy_updates":0,"rl_authorized":False,"real_submission":False}}
 payload["common_v169_seed"]="sha256('v458/1611/{episode}/{start}')[:8] little-endian mod 2**31; branch-independent";payload["training_schedules"]=schedules;payload["all200_schedule_sha256"]=all200_schedule_sha;payload["context_only_semantics"]="same branch-specific frozen-v169 visual baseline retained; only explicit normalized/action endpoint/path features zeroed; arm tokens and postclose=1 retained";payload["phase_shuffle_semantics"]="same branch-specific frozen-v169 visual baseline retained; explicit action features replaced by fixed within-episode donor"
 if a.output.exists() or a.output.with_name(a.output.name+".tmp").exists():raise RuntimeError("refusing to overwrite v458 preregistration/tmp")
