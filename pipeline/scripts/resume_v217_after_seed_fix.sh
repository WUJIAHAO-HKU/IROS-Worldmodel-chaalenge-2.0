...
restore_parent() {
  ...
  bash "$P/restart_v209_services.sh" start >"$REG/restart_v209_after_v217_resume.log" 2>&1 || true
}
trap restore_parent EXIT

test -s "$RUN/audit/service_acceptance.json"
test ! -e "$RUN/audit/public_success_candidate.npz"
test ! -e "$RUN/audit/public_failure_candidate.npz"
test -s "$RUN/audit/public_success_export.log"
mv "$RUN/audit/public_success_export.log" \
  "$RUN/audit/public_success_export.pre_amendment.log"
"$PY" "$P/amend_v217_seed_protocol.py" >"$RUN/protocol_seed_amendment.log" 2>&1
...
bash "$P/restart_v217_services.sh" start >"$RUN/v217_service_resume.log" 2>&1
