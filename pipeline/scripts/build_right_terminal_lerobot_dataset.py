#!/usr/bin/env python3
"""Build a public right-arm grasp-to-terminal LeRobot dataset."""
...
dataset = LeRobotDataset.create(
    repo_id="local/adjust_bottle_train40_right_terminal",
    root=output_path,
    fps=10,
    robot_type="aloha",
    features=features,
    ...
    records.append(
        {
            ...
            "selected_frames": len(actions) - start,
            "selected_postclose_fraction": float(
                (len(actions) - first_close) / (len(actions) - start)
            ),
        }
    )
...
result = {
    "format": "track2-public-right-grasp-to-terminal-sft-v1",
    "source_conversion_summary": str(summary_path),
    "source": str(source),
    "output": str(output_path),
    "repo_id": "local/adjust_bottle_train40_right_terminal",
    "selection": "frames from 8 steps before first right gripper close through successful endpoint",
