...
RUN="$J/v376_ood_routed_cartesian_phase_seed1539_20260823"
...
test ! -e "$RUN/audit/recursive_reward_causal.json"
...
taskset -c 0-7 "$PY" "$ROOT/pipeline/scripts/audit_v376_recursive_reward_causal.py" \
  --baseline-checkpoint-dir "$J/v355_v202_v354_parametric_arm_routed_release" \
  ...
  --output "$RUN/audit/recursive_reward_causal.json" \
  --device cuda --inference-batch-size 8 --reward-batch-size 32 \
  >"$RUN/audit/recursive_reward_causal.log" 2>&1
touch "$RUN/RECURSIVE_REWARD_CAUSAL_COMPLETE"
printf 'V376_RECURSIVE_REWARD_CAUSAL_COMPLETE\n'
