...
RUN="$J/v419_soft_reward_monotone_successor_seed1572_20260823"
REG="$O/run_registry/v419_soft_reward_monotone_successor_seed1572_20260823"
...
set +e
taskset -c 0-11 "$PY" "$ROOT/pipeline/scripts/audit_v419_recursive_reward_causal.py" \
...
status=$?
set -e
if test -s "$RUN/audit/recursive_reward_causal.json"; then cp "$RUN/audit/recursive_reward_causal.json" "$REG/recursive_reward_causal.json"; fi
touch "$RUN/RECURSIVE_COMPLETE"
printf '%s\n' "$status" >"$REG/audit_exit_code.txt"
echo "V419_RECURSIVE_AUDIT_EXIT=$status"
exit "$status"
