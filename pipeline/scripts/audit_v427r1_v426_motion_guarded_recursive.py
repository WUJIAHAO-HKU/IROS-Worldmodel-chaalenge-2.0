#!/usr/bin/env python3
"""Paired all-offset recursive gate of v426 versus frozen v209."""
...
from wam_pipeline.v426_motion_guarded_right_runtime import Track2V426MotionGuardedRight

def instantiate(name,args):
    if name=="baseline":
        root=args.baseline_checkpoint_dir
    ...
    root=args.candidate_checkpoint_dir
    manifest=json.loads((root/"motion_guarded_right_manifest.json").read_text())
    return Track2V426MotionGuardedRight(
        root/manifest["left_expert"], root/manifest["learned_right_expert"],
        root/manifest["frozen_right_expert"], args.device,
...
def main():
    ...
    if json.loads(args.preregistration.read_text()).get("format")!="strict-track2-v427r1-v426-motion-guarded-recursive-preregistration-v1": raise RuntimeError("wrong v427r1 preregistration")
    ...
    report={"format":"strict-track2-v427r1-v426-motion-guarded-recursive-gate-v1","created_at":datetime.now(timezone.utc).isoformat(),
            "candidate":"v426 v423-low-motion/v208-high-motion right dynamics","coverage":"512 public right holdout windows in 8 recursive alignment chains",
            "aggregates":aggregates,"v426_over_v209":comparisons,"checks":checks,"passed":passed,
            ...
            "evidence_sha256":{"preregistration":sha256(args.preregistration),
                               ...
                               "candidate_manifest":sha256(args.candidate_checkpoint_dir/"motion_guarded_right_manifest.json")},
    ...
    args.output.write_text(json.dumps(report,indent=2)+"\n"); print(json.dumps({"v426_over_v209":comparisons,"checks":checks,"passed":passed},indent=2),flush=True)
