...
R2_BYTES = 51845
STATIC_PATH = ROOT / "pipeline/scripts/audit_v489_v488_v487_c71_exact7_schema_repair_static.py"
STATIC_ROOT = J / "v489_v488_v487_c71_exact7_schema_repair_static_audit_seed1631_20260825"
...
AUTHORITY_ROOT = J / "v489_v488_v487_c71_exact7_schema_repair_authority_seed1631_20260825"
ATTEMPT_ROOT = J / "v489_v488_v487_c71_exact7_schema_repair_attempt_seed1631_20260825"
...
FORMAT = "strict-track2-v489-v488-v487-c71-exact7-schema-repair-static-audit-v1"
...
def synthetic_self_test() -> int:
    ...
    result={"passed":tamper["passed"] and path_synthetic["passed"] and atomic_fixture["passed"],"tamper":tamper,"strict_path_parser":path_synthetic,"whole_directory_atomic_fixture":atomic_fixture};result["evidence_sha256"]=csha(result)
...
def main() -> int:
    ...
    tamper_synthetic=synthetic_suite(observed_exact7,derived_map,r2_record);path_synthetic=strict_path_parser_synthetic(r2_tree)
    synthetic={"passed":tamper_synthetic["passed"] and path_synthetic["passed"],"tamper":tamper_synthetic,"strict_path_parser":path_synthetic,"whole_directory_atomic_fixture":{"covered_by_self_test":True,"repair_REG_exact1_preserved":True,"static_REG_exact1_promoted":True}};synthetic["evidence_sha256"]=csha(synthetic)
    ...
    for proc in Path("/proc").iterdir():
        ...
        if int(proc.name)!=os.getpid() and (str(R2_PATH) in cmd or str(MATERIALIZER_PATH) in cmd or str(SUPERSEDED_STATIC_SOURCE_PATH) in cmd or str(STATIC_PATH) in cmd or "launch_v488_v487_c71_exact7_schema_repair.py" in cmd or "launch_v489_v488_v487_c71_exact7_schema_repair.py" in cmd):live.append({"pid":int(proc.name),"cmdline":cmd})
    ...
    def immutable_input_snapshot():
        files={"formal":regular(FORMAL_PATH,FORMAL_SHA,FORMAL_BYTES),"contract":regular(CONTRACT_PATH,CONTRACT_SHA,CONTRACT_BYTES),"materializer":regular(MATERIALIZER_PATH,MATERIALIZER_SHA,MATERIALIZER_BYTES),"r2":regular(R2_PATH,R2_SHA,R2_BYTES),"static":regular(STATIC_PATH,args.static_auditor_sha,args.static_auditor_source.stat().st_size),"superseded_f88_static":regular(SUPERSEDED_STATIC_SOURCE_PATH,SUPERSEDED_STATIC_SOURCE_SHA,SUPERSEDED_STATIC_SOURCE_BYTES)}
        sources_now={role:regular(row["path"],row["sha256"],row["logical_bytes"]) for role,row in source_closure.items()}
        trees={"repair":exact_tree(FORMAL_PATH.parent),"evidence":exact_tree(EVIDENCE_ROOT),"superseded_f88_failure":exact_tree(FAILED_STATIC_EVIDENCE_ROOT),"f813":exact_tree(Path(source_closure["f813_reconciliation_preregistration"]["path"]).parent),"v487_authority":exact_tree(Path(source_closure["v487_authority_receipt"]["path"]).parent),"v487_helper":exact_tree(Path(source_closure["v487_helper_process_receipt"]["path"]).parent),"failed_attempt":exact_tree(Path(contract["failed_reconciliation_attempt_tree"]["root"]))}
