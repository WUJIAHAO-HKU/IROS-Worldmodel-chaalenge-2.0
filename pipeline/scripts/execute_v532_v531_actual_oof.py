#!/usr/bin/env python3
"""Single-boundary v532 actual OOF repair computation over the immutable v524 cache."""
...
SEED = 1666
FORMAT = "strict-track2-v532-v531-actual-oof-execution-receipt-v1"
...
SOURCE_PATHS = {
    ...
    "actual_oof_auditor": "/root/autodl-tmp/IROS_WAM_2.0 challenge/pipeline/scripts/audit_v532_v531_actual_oof.py",
    "actual_oof_launcher": "/root/autodl-tmp/IROS_WAM_2.0 challenge/pipeline/scripts/launch_v532_v531_actual_oof.py",
    "v531_failure_forensic": "/root/autodl-tmp/IROS_WAM_2.0 challenge/pipeline/scripts/v532_v531_actual_oof_preregistration_active_source_paths_failure_forensic.json",
    "v531_invalid_authority_design_contract": "/root/autodl-tmp/IROS_WAM_2.0 challenge/pipeline/scripts/v531_v530_actual_oof_execution_authority_contract.json",
    "v531_invalid_authority_materializer": "/root/autodl-tmp/IROS_WAM_2.0 challenge/pipeline/scripts/materialize_v531_v530_actual_oof_execution_authority.py",
    "v531_invalid_actual_oof_execution_preregistration": "/root/autodl-tmp/IROS_WAM_2.0 challenge/pipeline/scripts/v531_v530_actual_oof_execution_preregistration.json",
    "v531_invalid_actual_oof_execution_manifest": "/root/autodl-tmp/IROS_WAM_2.0 challenge/pipeline/scripts/v531_v530_actual_oof_execution_manifest.json",
    "v531_invalid_actual_oof_executor": "/root/autodl-tmp/IROS_WAM_2.0 challenge/pipeline/scripts/execute_v531_v530_actual_oof.py",
    "v531_invalid_actual_oof_auditor": "/root/autodl-tmp/IROS_WAM_2.0 challenge/pipeline/scripts/audit_v531_v530_actual_oof.py",
    "v531_invalid_actual_oof_launcher": "/root/autodl-tmp/IROS_WAM_2.0 challenge/pipeline/scripts/launch_v531_v530_actual_oof.py",
    "v531_unconsumed_authority_receipt": "/root/autodl-tmp/IROS_WAM_2.0 challenge/artifacts/strict_track2_joint_augmentation_20260810/v531_v530_actual_oof_execution_authority_seed1665_20260827/authority_receipt.json",
...
SOURCE_ALIASES = {
    ...
    "v531_failure_forensic": "v531_failure_forensic_source",
    "v531_invalid_authority_design_contract": "v531_invalid_authority_design_contract_source",
    "v531_invalid_authority_materializer": "v531_invalid_authority_materializer_source",
    "v531_invalid_actual_oof_execution_preregistration": "v531_invalid_actual_oof_execution_preregistration_source",
    "v531_invalid_actual_oof_execution_manifest": "v531_invalid_actual_oof_execution_manifest_source",
    "v531_invalid_actual_oof_executor": "v531_invalid_actual_oof_executor_source",
    "v531_invalid_actual_oof_auditor": "v531_invalid_actual_oof_auditor_source",
    "v531_invalid_actual_oof_launcher": "v531_invalid_actual_oof_launcher_source",
    "v531_unconsumed_authority_receipt": "v531_unconsumed_authority_receipt_source",
...
def validate_authority(authority: dict, contract: dict, prereg: dict, manifest: dict, args, *, enforce_production_paths: bool = True) -> None:
    if set(contract) != CONTRACT_KEYS or contract.get("format") != "strict-track2-v532-v531-actual-oof-execution-authority-design-contract-v1" or contract.get("status") != "design_only_frozen_actual_oof_sources_pending_independent_authority_materialization" or type(contract.get("seed")) is not int or contract["seed"] != SEED:
...
def validate_static_documents(prereg: dict, manifest: dict) -> None:
    ...
    if prereg["format"] != "strict-track2-v532-v531-actual-oof-execution-preregistration-v1" or prereg["status"] != "preregistered_actual_oof_execution_pending_external_authority" or type(prereg["seed"]) is not int or prereg["seed"] != SEED:
        raise RuntimeError("preregistration identity")
    if manifest["format"] != "strict-track2-v532-v531-actual-oof-execution-manifest-v1" or manifest["status"] != "preregistered_actual_oof_order_and_output_schema" or type(manifest["seed"]) is not int or manifest["seed"] != SEED:
...
def validate_input_records(prereg: dict) -> None:
    ...
    forensic_record = prereg["input_contract"]["v532_v531_failure_forensic"]
    forensic = json.loads(Path(forensic_record["path"]).read_text())
    if (set(forensic) != {"actual_state", "authorization_split", "classification", "failure", "format",
                          "frozen_sources", "historical_call_partition", "no_retry", "repair_boundary", "status"}
            or forensic["format"] != "strict-track2-v532-v531-actual-oof-preregistration-active-source-paths-failure-forensic-v1"
            or forensic["status"] != "frozen_failed_no_retry_pending_fresh_lineage_repair"
            or forensic["classification"] != "failed_before_attempt_or_output_publication_preregistration_active_source_paths_consumer_mismatch"
            or type(forensic["failure"].get("numeric_returncode")) is not int
            or forensic["failure"]["numeric_returncode"] != 1
            or forensic["failure"].get("captured_combined_output") != '{"error": "\'preregistration\'", "error_type": "KeyError", "passed": false, "status": "failed_no_retry"}\n'
            or forensic["failure"].get("combined_output_sha256") != "3c094f8f8bd28932ad110ab6aec013cd91d1df2511b506c4ba383bda007afebb"
            or forensic["failure"]["root_cause"].get("executor_lookup_order") != ["preregistration", "manifest", "auditor", "launcher"]
            or forensic["failure"]["root_cause"].get("first_missing_key") != "preregistration"
            or set(forensic["failure"]["root_cause"].get("frozen_preregistration_active_source_paths", {}))
               != {"authority_materializer", "actual_oof_executor", "actual_oof_auditor", "actual_oof_launcher"}
            or forensic["repair_boundary"].get("required_role_order") != ACTIVE_SOURCE_ROLE_ORDER
            or forensic["no_retry"].get("retry_authorized") is not False
            or forensic["authorization_split"].get("durable_consumption_receipt_available") is not False
            or forensic["authorization_split"]["immutable_authorization_fields"].get("actual_oof_execution_boundary_invocations_consumed") != 0
            or not all(row.get("absent") is True for row in forensic["actual_state"]["v531_failed_roots"].values())
            or not all(row.get("absent") is True for key, row in forensic["repair_boundary"].items() if key.startswith("fresh_") and isinstance(row, dict))):
        raise RuntimeError("v531 failure forensic contract")
...
def execute(args) -> dict:
    ...
    try:
        ...
        intent = {
            "format": "strict-track2-v532-v531-actual-oof-attempt-intent-v1", "seed": SEED,
        ...
        for fold in range(5):
            ...
            atomic_json(prep / f"fold_{fold}_receipt.json", {"format": "strict-track2-v532-v531-actual-oof-fold-receipt-v1", "status": "computed_pending_independent_audit", "fold": fold, "rows": 200, "event_digest_sha256": canonical_sha(ordered_event_digests[fold * 200:(fold + 1) * 200]), "absolute_error_sum_uint64": sum(x["absolute_error_sum_uint64"] for x in subset), "pixel_count_uint64": sum(x["pixel_count_uint64"] for x in subset)})
        atomic_json(prep / "source_manifest.json", {"format": "strict-track2-v532-v531-actual-oof-source-manifest-v1", "manifest_sha256": args.manifest_sha, "preregistration_sha256": args.preregistration_sha, "contract_sha256": args.contract_sha, "authority_receipt_sha256": args.authority_receipt_sha, "executor_sha256": args.executor_sha, "auditor_sha256": args.auditor_sha, "launcher_sha256": args.launcher_sha})
