#!/usr/bin/env python3
"""Materialize exactly one fresh v534 authority after the v532 environment failure."""
...
SELF = S / 'materialize_v534_v533_actual_oof_environment_repair_execution_authority.py'
CONTRACT = S / 'v534_v533_actual_oof_environment_repair_execution_authority_contract.json'
PREREG = S / 'v534_v533_actual_oof_execution_preregistration.json'
MANIFEST = S / 'v534_v533_actual_oof_execution_manifest.json'
EXECUTOR = S / 'execute_v534_v533_actual_oof.py'
AUDITOR = S / 'audit_v534_v533_actual_oof.py'
LAUNCHER = S / 'launch_v534_v533_actual_oof.py'
FORENSIC = S / 'v533_v532_actual_oof_execution_environment_failure_forensic.json'
V533_REJECTED_SCRIPT = S / 'v533_rejected_actual_oof_environment_repair_transport_script.sh'
V533_REJECTED_RECORD = S / 'v533_rejected_actual_oof_environment_repair_transport_record.json'

PREREG_EXPECTED = ('0370169bdf7850dcf16e954e3833c37db26887be72d7794a8c4d707df0abea85', 15730)
MANIFEST_EXPECTED = ('4cd77fa066f65b1776e89d2a0d335294688b8a5d4fbf2514043fce5a1da2e0f4', 1296108)
EXECUTOR_EXPECTED = ('1451682cc156574748a865cde4d99675599346c310f532dbbc1db4309ad29373', 65276)
AUDITOR_EXPECTED = ('a721637813feea55c6aeeeb721a064bfaccb7a3fa1bc5002f27eb988ccf80a4d', 12047)
LAUNCHER_EXPECTED = ('ce2d9f1110d738fad5b309f1d2595dfd44826a38f8ef34a129011c272ea54593', 16514)
FORENSIC_EXPECTED = ('0592fcc92340e1bb58a8c87b9e868a26055686ae8e100048a9a1b8b7c684228d', 6124)

AUTH_ROOT = J / 'v534_v533_actual_oof_environment_repair_execution_authority_seed1667_20260827'
AUTH_PREP = AUTH_ROOT.with_name(AUTH_ROOT.name + '.authority-prep')
ATTEMPT_ROOT = J / 'v534_v533_actual_oof_attempt_seed1667_20260827'
ATTEMPT_PREP = ATTEMPT_ROOT.with_name(ATTEMPT_ROOT.name + '.attempt-prep')
OOF_ROOT = Path('/root/v534_v533_actual_oof_seed1667_20260827')
...
CONTRACT_FORMAT = 'strict-track2-v534-v533-actual-oof-environment-repair-execution-authority-design-contract-v1'
CONTRACT_STATUS = 'design_only_frozen_actual_oof_sources_pending_independent_authority_materialization'
OUTPUT_FORMAT = 'strict-track2-v534-v533-actual-oof-environment-repair-execution-authority-v1'
...
SOURCE_ROLES = [
    ...
    'v533_rejected_transport_script', 'v533_rejected_transport_record',
    'v530_external_terminal_process', 'v527_candidate_receipt',
...
SOURCE_ALIASES['v482_runtime_source'] = 'v482_runtime_source_source'
SOURCE_ALIASES['v482_trainer_source'] = 'v482_trainer_source_source'
LINEAGE = {
    'canonical_v534_execution_not_performed': True, 'fresh_actual_oof_authority_only': True,
...
CHECK_KEYS = sorted({
    ...
    'v532_environment_failure_forensic_exact', 'v532_old_failure_roots_absent_unconsumed_authority',
    'required_environment_exact',
})
...
CONTRACT_TOP_KEYS = {
    ...
    'authority_receipt_contract', 'authorization', 'required_environment_exact',
...
AUTH_TOP_KEYS = {
    ...
    'execution_boundary', 'required_environment_exact',
}
...
def source_specs():
    return {
        ...
        'v532_environment_failure_forensic': (FORENSIC, FORENSIC_EXPECTED),
        'v532_invalid_authority_design_contract': (S / 'v532_v531_actual_oof_preregistration_active_source_paths_repair_execution_authority_contract.json', ('92766e99940a0ba3d27ff886487808155810c312791e9d77b341f32711bfa9d2', 110854)),
        'v532_invalid_authority_materializer': (S / 'materialize_v532_v531_actual_oof_preregistration_active_source_paths_repair_execution_authority.py', ('1901ce1583ef5389f958c077e880d0c6b68aef06aa9bf872469228a9869cc78b', 39099)),
        'v532_invalid_actual_oof_execution_preregistration': (S / 'v532_v531_actual_oof_execution_preregistration.json', ('7ef48984ac29d86edd22f75d4fab2d82e12237c1bd72615172da59dd8fdf70a5', 14210)),
        'v532_invalid_actual_oof_execution_manifest': (S / 'v532_v531_actual_oof_execution_manifest.json', ('3a3e83eb03415c9c0f573a10112b4cb18a0593bda10fe6eaff1feed843e7996e', 1296036)),
        'v532_invalid_actual_oof_executor': (S / 'execute_v532_v531_actual_oof.py', ('7f5d154a6bd2d4ae94d372091d36fc6bf45d770889e692b6d9e499745f9e9932', 62773)),
        'v532_invalid_actual_oof_auditor': (S / 'audit_v532_v531_actual_oof.py', ('b8f98db595fc0ddbff1cab1e9fce20a0c4bd7d6fbe8372ae57d4a4c092fc2893', 11763)),
        'v532_invalid_actual_oof_launcher': (S / 'launch_v532_v531_actual_oof.py', ('ca1ecc8bec51776935b10df70121ed4c0c12ad94c3689604bec63d214a3dea4d', 15315)),
        'v532_unconsumed_authority_receipt': (J / 'v532_v531_actual_oof_preregistration_active_source_paths_repair_execution_authority_seed1666_20260827/authority_receipt.json', ('420c5f09895f49f0d80e7d149dbf1754cc3b792f8011f79bdf3fec8cf0d03b34', 317060)),
        'v532_failed_transport_script': (Path('/root/v532_v531_actual_oof_execution_once.sh'), ('396917d890839a16cdea6e38e0ac5322cdf807706ddd91ed910847d24d1fa8e1', 7935)),
        'v532_failed_transport_record': (S / 'v532_v531_actual_oof_execution_transport_deployment_record.json', ('8e1905859e3c6a11fb057541d830bf4ed80a2fe870ec1980f653d2c6280cd1b2', 617)),
        'v533_rejected_transport_script': (V533_REJECTED_SCRIPT, ('ff46bb2738cc85270c0b17b4c957fd1ce3432053508a239cf96dee56902f32ca', 9484)),
        'v533_rejected_transport_record': (V533_REJECTED_RECORD, ('ed3e4dd19279bbbf335310288680ad53aa887969e7635fe435b1962382e961c0', 923)),
...
def validate_live_inputs(module, prereg, manifest):
    ...
    if (not json_exact(prereg.get('required_environment'), REQUIRED_ENVIRONMENT_EXACT)
        ...
        raise RuntimeError('required environment exact')
    forensic = json.loads(FORENSIC.read_bytes())
    repair = forensic.get('repair_boundary', {})
    if (forensic.get('status') != 'frozen_failed_no_retry_pending_fresh_environment_transport_repair'
            or repair.get('old_v532_transport_retry_authorized') is not False
            or repair.get('old_v532_authority_reuse_for_retry_authorized') is not False
            or repair.get('fresh_lineage_required') is not True
            or repair.get('fresh_authority_required') is not True
            or not json_exact(forensic.get('root_cause', {}).get('required_environment'), REQUIRED_ENVIRONMENT_EXACT)
            or forensic.get('root_cause', {}).get('observed_environment') != {'CUBLAS_WORKSPACE_CONFIG': None, 'PYTHONHASHSEED': None}):
        raise RuntimeError('v532 environment forensic')
    old_authority = json.loads((J / 'v532_v531_actual_oof_preregistration_active_source_paths_repair_execution_authority_seed1666_20260827/authority_receipt.json').read_bytes())
    if old_authority.get('authorization', {}).get('actual_oof_execution_boundary_invocations_consumed') != 0:
        raise RuntimeError('v532 authority consumed field')
    old_failed = forensic['failed_roots_current']
    if not all(row.get('absent') is True and not os.path.lexists(row['path']) for row in old_failed.values()):
        raise RuntimeError('v532 failed roots current absence')
...
def validate_context(args):
    ...
    if contract['format'] != CONTRACT_FORMAT or contract['status'] != CONTRACT_STATUS or type(contract['seed']) is not int or contract['seed'] != 1667:
        raise RuntimeError('contract header')
    if (not json_exact(contract['lineage'], LINEAGE)
            or not json_exact(contract['authorization'], AUTHORIZATION)
            or not json_exact(contract['required_environment_exact'], REQUIRED_ENVIRONMENT_EXACT)
            or not json_exact(contract['runtime_observation'], RUNTIME)
            or not json_exact(contract['execution_boundary'], EXECUTION_BOUNDARY)):
...
def main():
    if os.sys.argv[1:] == ['--synthetic-self-test']:
        checks = {
            'contract_top_count': len(CONTRACT_TOP_KEYS) == 31,
            'source_count': len(SOURCE_ROLES) == 33,
            'authority_source_count': len(AUTHORITY_SOURCE_ROLES) == 34,
            ...
            'required_environment_exact': REQUIRED_ENVIRONMENT_EXACT == {'CUBLAS_WORKSPACE_CONFIG': ':4096:8', 'PYTHONHASHSEED': '0'},
        }
