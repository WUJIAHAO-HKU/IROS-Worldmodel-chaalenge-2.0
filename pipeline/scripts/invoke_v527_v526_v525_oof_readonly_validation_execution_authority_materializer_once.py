#!/usr/bin/env python3
"""Exact-once Stage-A transport for the v527 OOF readonly-validation authority."""
...
SELF_PATH = SCRIPTS / "invoke_v527_v526_v525_oof_readonly_validation_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v527_v526_v525_oof_readonly_validation_authority_materialize_once.sh")
DEPLOYMENT_RECORD = SCRIPTS / "v527_v526_v525_oof_readonly_validation_authority_materialization_transport_deployment_record.json"
CONTRACT = SCRIPTS / "v527_v526_v525_oof_readonly_validation_execution_authority_contract.json"
CONTRACT_SHA = "575cead683e9dcb188107feac389b1836d21199ab99be87148029286d7a6c03e"
CONTRACT_BYTES = 198005
MATERIALIZER = SCRIPTS / "materialize_v527_v526_v525_oof_readonly_validation_execution_authority.py"
MATERIALIZER_SHA = "8519c2c589e894c40d74bc730d8569069c8b994d9d9477e4899422633514167c"
MATERIALIZER_BYTES = 30234
VALIDATOR = SCRIPTS / "audit_v527_v526_v525_oof_readonly_validation.py"
VALIDATOR_SHA = "3289297e8ca6ff026f3f5fed64860ac00742374bb10ee5c6012cde82ba2f4756"
VALIDATOR_BYTES = 52025
PREREGISTRATION = SCRIPTS / "v527_v526_v525_oof_readonly_gate_preregistration.json"
PREREGISTRATION_SHA = "9fc4e53117309a77aac773a8eccc8550ad82446b8224872ca47afb8d763167ac"
PREREGISTRATION_BYTES = 101898
...
EVIDENCE_ROOT = J / "v527_v526_v525_oof_readonly_validation_execution_authority_materialization_evidence_seed1662_20260827"
...
AT_FDCWD = -100
RENAME_NOREPLACE = 1
FROZEN_FINAL4_SHA256 = (CONTRACT_SHA, MATERIALIZER_SHA, VALIDATOR_SHA, PREREGISTRATION_SHA)


def deployment_plan() -> dict:
    return {
        "fresh_noreplace_targets_exact4": [
            ...
            {"role": "oof_readonly_gate_validator", "path": str(VALIDATOR), "sha256": VALIDATOR_SHA, "logical_bytes": VALIDATOR_BYTES},
            {"role": "oof_readonly_gate_preregistration", "path": str(PREREGISTRATION), "sha256": PREREGISTRATION_SHA, "logical_bytes": PREREGISTRATION_BYTES},
        ],
        "reused_current_only": [],
...
def expected_transport_deployment(helper_record: dict, script_record: dict) -> dict:
    return {
        "format": "strict-track2-v527-stage-a-transport-pair-deployment-record-v1",
...
def load_validator_source():
    regular(VALIDATOR, (VALIDATOR_SHA, VALIDATOR_BYTES))
    spec = importlib.util.spec_from_file_location("v527_oof_readonly_validator", VALIDATOR)
    validator = importlib.util.module_from_spec(spec); spec.loader.exec_module(validator)
    return validator


def validate_materializer_contract(module, contract: dict, materializer_record: dict) -> None:
    validator_module = load_validator_source()
    seed = contract.get("seed")
    active = {
        "authority_materializer": {"path": str(MATERIALIZER), "sha256": MATERIALIZER_SHA, "logical_bytes": MATERIALIZER_BYTES},
        "oof_readonly_gate_validator": {"path": str(VALIDATOR), "sha256": VALIDATOR_SHA, "logical_bytes": VALIDATOR_BYTES},
        "oof_readonly_gate_preregistration": {"path": str(PREREGISTRATION), "sha256": PREREGISTRATION_SHA, "logical_bytes": PREREGISTRATION_BYTES},
    }
    schema_checks = {
        "contract_path": module.CONTRACT_PATH == CONTRACT,
        "authority_root": module.AUTHORITY_ROOT == AUTHORITY_ROOT,
        ...
        "seed": type(seed) is int and seed == 1662,
        ...
        "authority_top_count": len(contract.get("authority_receipt_contract", {}).get("top_keys", [])) == 70,
        "check_count": len(module.CHECK_KEYS) == 33,
        "source_counts": len(module.CONTRACT_SOURCE_ORDER) == 26 and len(module.AUTHORITY_SOURCE_ORDER) == 27,
        "source_order": contract.get("source_role_order") == module.CONTRACT_SOURCE_ORDER,
    ...
    if (cbytes(contract.get("oof_gate_output_contract")) != cbytes({
                "candidate": validator_module.CANDIDATE_OUTPUT_CONTRACT,
                "external_terminal": validator_module.EXTERNAL_TERMINAL_EVIDENCE_CONTRACT})
            or cbytes(contract.get("oof_gate_candidate_contract")) != cbytes(validator_module.CANDIDATE_OUTPUT_CONTRACT)
            or cbytes(contract.get("oof_gate_external_terminal_evidence_contract")) != cbytes(validator_module.EXTERNAL_TERMINAL_EVIDENCE_CONTRACT)
        ...
        raise RuntimeError("v527 candidate/external-terminal contract")
    for role, row in contract["source_closure"].items():
        if regular(Path(row["path"]), (row["sha256"], row["logical_bytes"])) != row:
            raise RuntimeError(f"source closure current: {role}")
    if (Path(module.ATTEMPT_ROOT) != Path(validator_module.CANDIDATE_OUTPUT_CONTRACT["root"])
            or Path(module.ATTEMPT_PREP) != Path(validator_module.CANDIDATE_OUTPUT_CONTRACT["prep_root"])
            or Path(module.EXTERNAL_EVIDENCE_ROOT) != Path(validator_module.EXTERNAL_TERMINAL_EVIDENCE_CONTRACT["root"])
            or Path(module.EXTERNAL_EVIDENCE_PREP) != Path(validator_module.EXTERNAL_TERMINAL_EVIDENCE_CONTRACT["prep_root"])):
        raise RuntimeError("v527 candidate/external paths")
    context_contract = regular(Path(module.CONTRACT_PATH))
    context_materializer = regular(Path(module.SELF_PATH))
    module.validate_context(argparse.Namespace(
        contract=Path(module.CONTRACT_PATH),
        contract_sha=context_contract["sha256"], contract_bytes=context_contract["logical_bytes"],
        materializer_source=Path(module.SELF_PATH),
        materializer_sha=context_materializer["sha256"], materializer_bytes=context_materializer["logical_bytes"],
        authority_root=Path(module.AUTHORITY_ROOT), read_only_preflight=True))
...
def load_materializer(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    spec = importlib.util.spec_from_file_location("v527_materializer", materializer_source)
    ...
    try:
        ...
        lines = [
            ...
            {"line_origin": "transport_helper", "passed": True,
              ...
              "oof_readonly_gate_validator_invocations": 0,
...
def validate_success(module, contract: dict, returncode: int, cleanup: dict,
    ...
    validator = load_validator_source()
    ...
    if (os.path.lexists(AUTHORITY_PREP)
            or sorted(receipt) != receipt_schema["top_keys"] or len(receipt) != 70
            ...
            or cbytes(receipt.get("oof_gate_output_contract")) != cbytes(contract["oof_gate_output_contract"])
            or cbytes(receipt.get("oof_gate_candidate_contract")) != cbytes(validator.CANDIDATE_OUTPUT_CONTRACT)
            or cbytes(receipt.get("oof_gate_external_terminal_evidence_contract")) != cbytes(validator.EXTERNAL_TERMINAL_EVIDENCE_CONTRACT)
    ...
    validator.validate_prereg(preregistration)
    validator.validate_authority(
        argparse.Namespace(contract=CONTRACT,
                           authority_receipt=AUTHORITY_ROOT / "authority_receipt.json",
                           authority_materializer=MATERIALIZER),
        preregistration, regular(VALIDATOR, (VALIDATOR_SHA, VALIDATOR_BYTES)))
    ...
    for path in (module.ATTEMPT_ROOT, module.ATTEMPT_PREP,
                 module.EXTERNAL_EVIDENCE_ROOT, module.EXTERNAL_EVIDENCE_PREP):
        if os.path.lexists(path):
            raise RuntimeError("validator/external downstream current")
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v527-v526-v525-oof-readonly-validation-execution-authority-materializer-process-receipt-v1",
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v527-v526-v525-oof-readonly-validation-execution-authority-materializer-intent-v1",
        ...
        receipt = {
            **base_receipt("passed_exact_once_authority_materialized_no_validator_or_oof_execution", True, 1, cleanup),
...
def production_fixture(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    checks = {
        ...
        "fresh4_noreplace_plan": (
            len(plan["fresh_noreplace_targets_exact4"]) == 4
            and [row["role"] for row in plan["fresh_noreplace_targets_exact4"]]
            == ["authority_design_contract", "authority_materializer", "oof_readonly_gate_validator", "oof_readonly_gate_preregistration"]),
        "sole_signal_owned_materializer_popen": len(popens) == 1 and command()[1] == str(MATERIALIZER),
        "candidate_contract_exact": cbytes(contract["oof_gate_candidate_contract"]) == cbytes(validator.CANDIDATE_OUTPUT_CONTRACT),
        "external_terminal_contract_exact": cbytes(contract["oof_gate_external_terminal_evidence_contract"]) == cbytes(validator.EXTERNAL_TERMINAL_EVIDENCE_CONTRACT),
        "authority_schema_70_33_27": (
            len(contract["authority_receipt_contract"]["top_keys"]) == 70
            and len(module.CHECK_KEYS) == 33 and len(module.AUTHORITY_SOURCE_ORDER) == 27),
        "validator_and_phase_a_not_in_child_argv": (
            str(VALIDATOR) not in command()
            and not any("launch_" in item or "generate_" in item for item in command())),
        ...
        "choice_a_external_evidence_exact6": (
            EXACT6 == sorted(EXACT5 + ["process_receipt.json"])
            and validator.EXTERNAL_TERMINAL_EVIDENCE_CONTRACT["exact_file_count"] == 6),
    }
    with tempfile.TemporaryDirectory(prefix="v527-stage-a-helper-", dir="/dev/shm") as folder:
        temp = Path(folder)
        ...
        result = orchestrate(
            ...
            argv_payload={"format": "synthetic-v527-stage-a", "pre_snapshot": pre,
                          "transport_deployment_record": deployment_record,
        ...
        checks["mapped_normal_exact6"] = (
            ...
            and result["process_receipt"]["oof_readonly_gate_validator_invocations"] == 0)
    ...
    return {"passed": True, "checks": checks, "checks_sha256": csha(checks),
            "materializer_invocations": 0, "oof_readonly_gate_validator_invocations": 0,
...
def main() -> int:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v527-v526-v525-oof-readonly-validation-execution-authority-materializer-argv-v1",
