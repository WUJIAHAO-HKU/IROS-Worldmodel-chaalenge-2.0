#!/usr/bin/env python3
"""Preregistered v462 three-head 5-fold endpoint kill gate; no reward or RL."""
...
from wam_pipeline.v462_v169_endpoint_residual_runtime import CHECKPOINT_FORMAT,EndpointResidualUNet128FiLM,action_features,instruction_tokens
SEED=1613;STEPS=50;BATCH=8;BRANCHES=("factual","no_transport","scale_0p4","scale_1p25","reverse_direction_0p4")
def sha(path):
 h=hashlib.sha256()
...
def atomic_json(path,payload):
 ...
 if tmp.exists():raise RuntimeError(f"refusing stale v462 temporary receipt: {tmp}")
...
def stable_seed(episode,start):return int.from_bytes(hashlib.sha256(f"v462/{SEED}/{episode}/{start}".encode()).digest()[:8],"little")%(2**31)
...
def verify_closure(pre,release,library):
 release=Path(release).resolve();library=Path(library).resolve();roots={"v169_release":release,"v169_base_release":(release/"v168_release/base_release").resolve(),"v169_library":library}
 for records in (pre["v169"]["release_files"],pre["v169"]["library_files"]):
  for row in records:
   if set(row)!={"base_kind","relative","resolved_path","sha256"} or row["base_kind"] not in roots:raise RuntimeError("v462 closure record schema")
   base=roots[row["base_kind"]];path=(base/row["relative"]).resolve()
   if path!=base and base not in path.parents:raise RuntimeError("v462 closure escape")
   if str(path)!=row["resolved_path"] or not path.is_file() or sha(path)!=row["sha256"]:raise RuntimeError(f"v462 closure drift: {path}")
 release_rows={row["relative"] for row in pre["v169"]["release_files"] if row["base_kind"]=="v169_release"}
 actual={str(path.resolve().relative_to(release)) for path in release.rglob("*") if path.is_file()}
 if release_rows!=actual:raise RuntimeError("v462 exact v169 release enumeration drift")
def validate_and_load(args,pre):
 if sha(Path(__file__))!=pre["source"]["trainer_sha256"]:raise RuntimeError("v462 trainer source SHA drift")
 runtime=Path(pre["source"]["runtime_path"])
 if not runtime.is_file() or sha(runtime)!=pre["source"]["runtime_sha256"]:raise RuntimeError("v462 runtime source SHA drift")
 if str(args.v169_release.resolve())!=pre["v169"]["release"] or str(args.v169_library.resolve())!=pre["v169"]["library"]:raise RuntimeError("v462 v169 path drift")
 ...
 if sha(receipt_path)!=pre["v461"]["final_receipt"]["sha256"] or receipt.get("format")!="strict-track2-v461-endpoint200-batch-audit-v1" or receipt.get("mode")!="final" or receipt.get("passed") is not True or not all(receipt.get("checks",{}).values()):raise RuntimeError("v462 hard blocked: bound v461 final receipt is not PASS")
 for key,arg in (("selection",args.v456_selection),("generation_report",args.v456_generation_report)):
  if str(arg.resolve())!=pre["v461"][key]["path"] or sha(arg)!=pre["v461"][key]["sha256"]:raise RuntimeError(f"v462 {key} closure failed")
 if str(args.v456_dataset.resolve())!=pre["v461"]["dataset"]:raise RuntimeError("v462 dataset path drift")
 for row in pre["v461"]["files"]:
  path=args.v456_dataset/row["relative"]
  if not path.is_file() or sha(path)!=row["sha256"]:raise RuntimeError(f"v462 frozen file drift {path}")
 ...
 if len(contexts)!=200 or len(samples)!=1000:raise RuntimeError("v462 exact 200/1000 contract")
...
def donor_ids(pre,fold,contexts,samples,partition):
 ...
 for i,row in enumerate(samples):
  ...
  if key in mapping:
   ...
   if row["episode"] not in expected or dcid==row["context_id"] or contexts[dcid]["episode"]!=row["episode"] or contexts[dcid]["episode"] not in expected:raise RuntimeError("v462 phase donor leakage/self-map")
...
def main():
 ...
 if pre.get("format")!="strict-track2-v462-endpoint-residual-5fold-preregistration-v2" or a.output_dir.exists():raise RuntimeError("v462 prereg/output contract")
 ...
 if pre.get("training")!=expected_training or pre.get("features")!=expected_features or pre.get("kill_gate")!=expected_gate or pre.get("common_v169_seed")!="sha256('v462/1613/{episode}/{start}')[:8] little-endian mod 2**31; branch-independent":raise RuntimeError("v462 prereg constants drift")
 final_dir=a.output_dir;work_dir=final_dir.with_name(final_dir.name+".partial")
 if final_dir.exists() or work_dir.exists():raise RuntimeError("v462 output/partial already exists")
 ...
 for fold in range(5):
  ...
  if schedule_sha(schedule)!=pre["training_schedules"][fold]["sha256"]:raise RuntimeError("v462 training schedule drift")
 ...
 if passed:
  ...
  if schedule_sha(schedule)!=pre["all200_schedule_sha256"]:raise RuntimeError("v462 all200 schedule drift")
