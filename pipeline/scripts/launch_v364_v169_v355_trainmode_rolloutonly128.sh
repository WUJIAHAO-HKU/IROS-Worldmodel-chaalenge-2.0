...
trap cleanup EXIT
TRACK2_SERVICE_REG="$REG" TRACK2_CPUSET=0-5 TRACK2_WAM_RELEASE_CUDA_CACHE=1 bash "$P/restart_v355_services.sh" start >"$REG/restart_v355_before_rollout.log" 2>&1
...
taskset -c 6-21 "$PY" "$P/trainmode_rollout_only_embodied_agent.py" \
  ...
  runner.logger.log_path="$RUN" runner.ckpt_path="$V169" actor.seed=1497 \
  algorithm.rollout_epoch=4 algorithm.group_size=4 \
  env.train.total_num_envs=32 env.train.group_size=4 env.train.seed=0 env.train.max_episode_steps=200 env.train.max_steps_per_rollout_epoch=200 env.train.http.server_url=http://127.0.0.1:18085 \
  ...
  env.train.video_cfg.video_base_dir="$RUN/video/train" env.train.enable_offload=false rollout.enable_offload=false \
  >"$RUN/launcher.log" 2>&1

cd "$ROOT"
"$PY" "$P/audit_v364_trainmode_rolloutonly.py" --raw "$RUN/audit/raw_training_mode_rollout.json" --preregistration "$REG/preregistration.json" --output "$RUN/audit/trainmode_rollout_go_no_go.json" >"$RUN/audit/trainmode_rollout_go_no_go.log" 2>&1
cat "$RUN/audit/trainmode_rollout_go_no_go.log"
