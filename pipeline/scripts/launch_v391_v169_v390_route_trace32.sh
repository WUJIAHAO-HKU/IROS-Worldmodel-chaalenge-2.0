...
test ! -e "$RUN"
mkdir -p "$RUN/audit"
cleanup(){ "$PY" -m ray.scripts.scripts stop --force >/dev/null 2>&1 || true; TRACK2_SERVICE_REG="$REG" bash "$P/restart_v391_trace_services.sh" stop >/dev/null 2>&1 || true; bash "$P/restart_v218_services.sh" start >"$REG/restore_v218.log" 2>&1 || true; }
trap cleanup EXIT
TRACK2_SERVICE_REG="$REG" TRACK2_CPUSET=0-5 bash "$P/restart_v391_trace_services.sh" start >"$REG/restart_trace_before_rollout.log" 2>&1
