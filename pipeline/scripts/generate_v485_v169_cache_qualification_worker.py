#!/usr/bin/env python3
"""Fresh-process A/B worker for v485 v169 cache qualification; never trains."""
...
def bootstrap_scope(argv=None):
    ...
    formal=json.loads(known.preregistration.read_text());frozen=formal.get("execution_sources",{}).get("phase_a_cache_scope_helper",{})
...
def synthetic_bootstrap_self_test():
    with tempfile.TemporaryDirectory(prefix="v485-worker-bootstrap-") as directory:
        ...
        record={"role":"phase_a_cache_scope_helper","path":str(scope_path.resolve()),"sha256":scope_sha,"logical_bytes":scope_path.stat().st_size};records=[record];formal={"format":"strict-track2-v485-v482-v169-cache-determinism-qualification-preregistration-v1","qualification_output_root":str(out.resolve()),"execution_sources":{"phase_a_cache_scope_helper":{key:value for key,value in record.items() if key!="role"}},"execution_source_records":records,"execution_sources_digest_sha256":canonical_sha(records)}
...
def main():
    ...
    for name in ("preregistration","parent-preregistration","parent-failure","contract","authority-receipt","scope-source","driver-source","auditor-source"): p.add_argument(f"--{name}",type=Path,required=True)
    for name in ("preregistration-sha","contract-sha","authority-receipt-sha","scope-sha","driver-sha","worker-sha","auditor-sha"): p.add_argument(f"--{name}",required=True)
    p.add_argument("--role",choices=("A","B"),required=True);args=p.parse_args()
    for value in (args.preregistration_sha,args.contract_sha,args.authority_receipt_sha,args.scope_sha,args.driver_sha,args.worker_sha,args.auditor_sha):
        if len(value)!=64 or value.startswith("PENDING"): raise RuntimeError("unfrozen closure")
    if sha(args.preregistration)!=args.preregistration_sha or sha(args.contract)!=args.contract_sha or sha(args.authority_receipt)!=args.authority_receipt_sha or sha(args.scope_source)!=args.scope_sha or sha(args.driver_source)!=args.driver_sha or sha(args.auditor_source)!=args.auditor_sha or sha(Path(__file__))!=args.worker_sha: raise RuntimeError("source/formal drift")
    ...
    expected_sources={"phase_a_cache_scope_helper":(args.scope_source,args.scope_sha),"phase_a_process_worker":(Path(__file__),args.worker_sha),"phase_a_driver":(args.driver_source,args.driver_sha),"phase_a_independent_auditor":(args.auditor_source,args.auditor_sha)}
    ...
    if intent.get("format")!="strict-track2-v485-v169-cache-qualification-attempt-intent-v1" or intent.get("preregistration_sha256")!=args.preregistration_sha or intent.get("contract_sha256")!=CONTRACT_SHA or intent.get("authority_receipt_sha256")!=args.authority_receipt_sha or intent.get("qualification_output_root")!=str(qualification_root.resolve()) or intent.get("roles",{}).get(role_name)!=("canonical_0_to_999" if args.role=="A" else "reverse_999_to_0") or intent.get("expected_identity_relative_paths",{}).get(role_name)!=expected_relative or not isinstance(intent.get("attempt_nonce"),str) or len(intent["attempt_nonce"])!=64 or any(character not in "0123456789abcdef" for character in intent["attempt_nonce"]) or intent.get("sys_executable_resolved_path")!=str(resolved) or intent.get("sys_executable_sha256")!=sha(resolved) or intent.get("worker_source_sha256")!=args.worker_sha or intent.get("identity_schema_version")!="strict-track2-v485-v169-cache-worker-identity-v2" or intent.get("execution_source_records")!=formal["execution_source_records"] or intent.get("execution_sources_digest_sha256")!=formal["execution_sources_digest_sha256"]:raise RuntimeError("attempt intent reverse binding")
    ...
    for execution_order,sample_id in enumerate(order):
        ...
        output,event=scope.predict_one(runtime,item["context"],item["history"],item["future"],item["seed"],item["instruction"])
        ...
        row={"ordinal":execution_order,"sample_id":sample_id,"episode":item["episode"],"start":item["start"],"branch":item["branch"],"seed":item["seed"],"request_sha256":item["request_sha256"],"output_sha256":event["output_sha256"],"context_sha256":item["context_sha256"],"history_sha256":item["history_sha256"],"future_sha256":item["future_sha256"],"seed_sha256":item["seed_sha256"],"instruction_sha256":item["instruction_sha256"],"warning_category":event["warning"]["category"],"warning_full_message":event["warning"]["message"],"deterministic_before":event["mode_before"]["enabled"],"warn_only_before":event["mode_before"]["warn_only"],"deterministic_inside":event["mode_during"]["enabled"],"warn_only_inside":event["mode_during"]["warn_only"],"deterministic_after":event["mode_after"]["enabled"],"warn_only_after":event["mode_after"]["warn_only"],"cpu_rng_before_sha256":event["rng_before"]["cpu_sha256"],"cpu_rng_after_sha256":event["rng_after"]["cpu_sha256"],"cuda_rng_before_sha256_by_device":event["rng_before"]["cuda"],"cuda_rng_after_sha256_by_device":event["rng_after"]["cuda"],"rng_unchanged":event["rng_unchanged"]};events.append(row);append_event(events_stream,row)
        ...
        PROGRESS.update({"completed_calls":execution_order+1,"last_warning":event["warning"],"last_rng":{"before":event["rng_before"],"after":event["rng_after"]}})
