#!/usr/bin/env python3
"""Calibrate v442 phase/frame/channel coefficients on public right train15 only."""
...
def stable_seed(episode: int, start: int) -> int:
    payload = f"v442-trainonly/episode{episode}/start{start}/seed1586"
...
def select_samples(dataset: WindowDataset, episodes: list[int], prompts: dict[int, str]) -> list[dict]:
    ...
    for index, path in enumerate(dataset.paths):
        ...
        if decision.get("gate") and decision.get("phase") == "close":
            ...
            grouped[episode].append((abs(int(decision["first_close_index"]) - 4), start, index))
    rows = []
    for episode in episodes:
        candidates = sorted(grouped[episode])
        if len(candidates) != 8:
            raise RuntimeError(f"v442 requires exact eight raw close windows in train episode {episode}, got {len(candidates)}")
        _, start, index = candidates[0]
        arrays = dataset.load_arrays(index)
        rows.append({
            "episode": episode, "phase": "close", "start": start,
            "dataset_index": index, "first_close_index": int(gate_decision(arrays["history_actions"], arrays["future_actions"], prompts[episode])["first_close_index"]),
            "prompt": prompts[episode], **arrays,
        })
    expected = len(episodes) * SAMPLES_PER_EPISODE
    if len(rows) != expected:
        raise RuntimeError(f"v442 expected {expected} calibration rows, got {len(rows)}")
    return rows
...
def main() -> int:
    ...
    if args.inference_batch_size != INFERENCE_BATCH_SIZE:
        raise SystemExit("v442 calibration is preregistered for inference batch size 4")
    ...
    if len(train) != 40 or len(validation) != 10 or len(right_train) != 15 or set(train) & set(validation):
        raise RuntimeError("v442 requires the immutable public train40/right15 boundary")
    if any("right arm" not in prompts[episode].lower() or "left arm" in prompts[episode].lower() for episode in right_train):
        raise RuntimeError("v442 right-train prompt contract failed")
    ...
    if v436.get("format") != "track2-v436-v432-step25-diagnostic-release-v1":
        raise RuntimeError("wrong v442 teacher release")
    ...
    if not np.all(counts == SAMPLES_PER_EPISODE):
        raise RuntimeError(f"v442 calibration count drift: {counts.tolist()}")
    ...
    for phase, p in phase_slot.items():
        ...
        if int(active.sum()) == 0:
            raise RuntimeError(f"v442 LOEO retained no stable {phase} component")
        evidence[phase] = {
    ...
    payload = {
        "format": "strict-track2-v442-close-trainonly-rgb-alignment-index-v1",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "method": "public-right-train15 close-only leave-one-episode-out frame/RGB-channel closed-form alignment",
