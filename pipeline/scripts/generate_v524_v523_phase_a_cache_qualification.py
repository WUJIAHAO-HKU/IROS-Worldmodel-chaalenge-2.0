#!/usr/bin/env python3
"""Fresh v524 Phase-A driver repairing the formal exact source-role order.
...
EXACT_SOURCE_ROLES=["phase_a_preregistration_materializer","phase_a_driver","phase_a_process_worker","phase_a_rng_isolation_proxy","cache_scope_helper","phase_a_independent_auditor","phase_a_static_auditor","frozen_phase_a_launcher"]
...
def validate_v523_authorization(authority):
    ...
    if set(authorization)!=AUTHORIZATION_KEYS or authorization!=expected:raise RuntimeError("v523 authorization")
    if canonical_sha(authority.get("per_call_rng_evidence_contract"))!=canonical_sha(PER_CALL_RNG_CONTRACT):raise RuntimeError("v523 per-call RNG contract")
def validate_durable_marker_rows(markers,role):
    expected=list(range(PER_CALL_RNG_CONTRACT["calls_per_branch"])) if role=="A" else list(reversed(range(PER_CALL_RNG_CONTRACT["calls_per_branch"])))
    if len(markers)!=PER_CALL_RNG_CONTRACT["durable_process_log_markers_per_branch"]:raise RuntimeError(f"worker {role} durable marker count")
    for ordinal,sample_id in enumerate(expected):
        started,completed=markers[2*ordinal:2*ordinal+2]
        if [started.get("event").removeprefix("rng_proxy_call_"),completed.get("event").removeprefix("rng_proxy_call_")]!=PER_CALL_RNG_CONTRACT["process_log_marker_order_per_call"] or started.get("ordinal")!=ordinal or completed.get("ordinal")!=ordinal or started.get("sample_id")!=sample_id or completed.get("sample_id")!=sample_id or started.get("started")!=1 or started.get("completed")!=0 or completed.get("started")!=1 or completed.get("completed")!=1:raise RuntimeError(f"worker {role} durable marker order")
    return {"role":role,"started_markers":len(expected),"completed_markers":len(expected),"marker_pairs":len(expected),"total_markers":len(markers),"alternating_started_completed":True}
def validate_rng_worker_output(root,role,rng_proxy_module,rng_proxy_record):
    ...
    if receipt.get("format")!="strict-track2-v523-v522-rng-isolated-cache-worker-receipt-v1" or receipt.get("passed") is not True or receipt.get("role")!=role or any(receipt.get(key)!=1000 for key in count_keys) or receipt.get("durable_call_markers_in_process_log") is not True or receipt.get("durable_call_started_markers")!=PER_CALL_RNG_CONTRACT["calls_per_branch"] or receipt.get("durable_call_completed_markers")!=PER_CALL_RNG_CONTRACT["calls_per_branch"] or receipt.get("other_warning_count")!=0 or receipt.get("rng_proxy_source")!=rng_proxy_record:raise RuntimeError(f"worker {role} RNG aggregate")
    ...
    return validate_durable_marker_rows(markers,role)
def package_tree():
...
def actual_source_closure(formal,expected_sources):
    ...
    frozen_records=formal.get("execution_source_records")
    if exact_roles!=EXACT_SOURCE_ROLES or list(expected_sources)!=EXACT_SOURCE_ROLES:raise RuntimeError("exact ordered eight source roles")
    if not isinstance(sources,dict) or set(sources)!=set(EXACT_SOURCE_ROLES) or not isinstance(frozen_records,list) or len(frozen_records)!=8:raise RuntimeError("source role bijection")
    for index,role in enumerate(EXACT_SOURCE_ROLES):
        frozen=frozen_records[index]
        if not isinstance(frozen,dict) or set(frozen)!={"role","path","sha256","logical_bytes"} or frozen.get("role")!=role or type(frozen.get("logical_bytes")) is not int or frozen["logical_bytes"]<0 or sources.get(role)!={key:value for key,value in frozen.items() if key!="role"}:raise RuntimeError("source record bijection")
    if formal.get("execution_sources_digest_sha256")!=canonical_sha(frozen_records):raise RuntimeError("frozen source digest")
    records=[]
...
def main():
    ...
    validate_v523_authorization(authority)
    ...
    if authority.get("fresh_phase_a_driver_source")!=driver_record or authority.get("worker_environment_exact")!=EXPECTED_WORKER_ENV or canonical_sha(authority.get("per_call_rng_evidence_contract"))!=canonical_sha(PER_CALL_RNG_CONTRACT):raise RuntimeError("v523 driver authority")
    ...
    marker_a=validate_rng_worker_output(root,"A",rng_proxy_module,rng_proxy_record)
    ...
    marker_b=validate_rng_worker_output(root,"B",rng_proxy_module,rng_proxy_record)
    durable_marker_summary={"branches":{"A":marker_a,"B":marker_b},"branch_count":2,"calls_per_branch":1000,"total_calls":2000,"total_marker_pairs":marker_a["marker_pairs"]+marker_b["marker_pairs"],"total_markers":marker_a["total_markers"]+marker_b["total_markers"],"all_pairs_alternate_started_completed":marker_a["alternating_started_completed"] is marker_b["alternating_started_completed"] is True}
    if durable_marker_summary!={"branches":{"A":marker_a,"B":marker_b},"branch_count":2,"calls_per_branch":1000,"total_calls":2000,"total_marker_pairs":2000,"total_markers":PER_CALL_RNG_CONTRACT["total_durable_process_log_markers"],"all_pairs_alternate_started_completed":True}:raise RuntimeError("v523 durable marker aggregate")
    ...
    report.update({"wam_pipeline_package_tree_manifest":package_manifest_record,"wam_pipeline_package_tree":post_worker_package_tree,"wam_pipeline_import_fixture":package_import_fixture,"worker_environment":{"mode":"replace_not_inherit","exact_env":dict(EXPECTED_WORKER_ENV)},"service_control_env_override":False,"per_call_rng_evidence_contract":PER_CALL_RNG_CONTRACT,"durable_marker_summary":durable_marker_summary})
