...
cleanup() {
  ...
  bash "$P/restart_v326_services.sh" stop >/dev/null 2>&1 || true
}
trap cleanup EXIT

TRACK2_SERVICE_REG="$REG" TRACK2_CPUSET=0-5 TRACK2_WAM_RELEASE_CUDA_CACHE=1 bash "$P/restart_v326_services.sh" start >"$REG/restart_v326_before_control.log" 2>&1
...
taskset -c 6-21 "$PY" "$RLINF/examples/embodiment/eval_embodied_agent.py" \
  ...
  env.eval.total_num_envs=32 env.eval.group_size=1 env.eval.seed=0 env.eval.max_episode_steps=200 env.eval.max_steps_per_rollout_epoch=200 env.eval.http.server_url=http://127.0.0.1:18084 \
