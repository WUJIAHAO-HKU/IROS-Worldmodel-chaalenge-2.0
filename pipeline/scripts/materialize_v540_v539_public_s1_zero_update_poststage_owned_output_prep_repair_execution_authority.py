#!/usr/bin/env python3
"""Materialize one fresh v540 public-S1 zero-update execution authority."""
...
SELF=S/'materialize_v540_v539_public_s1_zero_update_poststage_owned_output_prep_repair_execution_authority.py'
CONTRACT=S/'v540_v539_public_s1_zero_update_poststage_owned_output_prep_repair_execution_authority_contract.json'
PREREG=S/'v540_v539_public_s1_zero_update_poststage_owned_output_prep_repair_preregistration.json'; MANIFEST=S/'v540_v539_public_s1_zero_update_poststage_owned_output_prep_repair_manifest.json'
EVALUATOR=S/'evaluate_v540_v539_public_s1_zero_update_poststage_owned_output_prep_repair.py'; AUDITOR=S/'audit_v540_v539_public_s1_zero_update_poststage_owned_output_prep_repair.py'; LAUNCHER=S/'launch_v540_v539_public_s1_zero_update_poststage_owned_output_prep_repair.py'
PREREG_EXPECTED=('370f6991ad0f85e966467a594efc4cbb7378cf6a82f5982bdd7ff077cfbf8b6c',81186)
MANIFEST_EXPECTED=('df9196600ef9cb728e9300dabacf9fccf10f2a55501c86f9076eda5473b40d29',83407)
EVALUATOR_EXPECTED=('d1c2a65f087cca7175d9638219e6174bb91d5afefbbd71f7290b72387f3cd911',67828)
AUDITOR_EXPECTED=('e5ce8c289f108eb0aaf593a21afb2164fc2838ef915e4a49e909a85078227e46',5528)
LAUNCHER_EXPECTED=('9f6d272350907eb072dfb20da536128cf85efed5e9b50325e1a9c273952d3afb',4480)
AUTH_ROOT=J/'v540_v539_public_s1_zero_update_poststage_owned_output_prep_repair_execution_authority_seed1671_20260828'; AUTH_PREP=AUTH_ROOT.with_name(AUTH_ROOT.name+'.authority-prep')
ATTEMPT_ROOT=J/'v540_v539_public_s1_zero_update_gate_attempt_seed1671_20260828'; ATTEMPT_PREP=ATTEMPT_ROOT.with_name(ATTEMPT_ROOT.name+'.attempt-prep')
OUTPUT_ROOT=Path('/root/v540_v539_public_s1_zero_update_gate_seed1671_20260828'); OUTPUT_PREP=Path(str(OUTPUT_ROOT)+'.output-prep')
...
CONTRACT_FORMAT='strict-track2-v540-v539-public-s1-zero-update-runtime-package-repair-execution-authority-design-contract-v1'
CONTRACT_STATUS='design_only_frozen_public_s1_zero_update_sources_pending_independent_authority_materialization'
OUTPUT_FORMAT='strict-track2-v540-v539-public-s1-zero-update-poststage-owned-output-prep-repair-execution-authority-v1'
...
def load_evaluator():
 regular(EVALUATOR,EVALUATOR_EXPECTED); spec=importlib.util.spec_from_file_location('v539_authority_evaluator_schema',EVALUATOR); mod=importlib.util.module_from_spec(spec); spec.loader.exec_module(mod); return mod
def source_specs():
 return {
...
'v539_failure_forensic':(S/'v540_v539_public_s1_zero_update_poststage_owned_output_prep_absence_failure_forensic.json',('ce412975394a8107853945c486e6a859984fd550ba9277f00ae013fbf0d22b43',7541)),
'v539_failed_authority_receipt':(J/'v539_v535_public_s1_zero_update_runtime_package_repair_execution_authority_seed1670_20260828/authority_receipt.json',('0b43b24c004c6a00975d3b14ece7167b312b9735185b1241454e39f856894be4',128865)),
'v539_failed_transport_script':(Path('/root/v539_v535_public_s1_zero_update_runtime_package_repair_gate_once.sh'),('d1b12ed98773fd5c7ef10c3fbc90b5cd9ba3406e0cda18ba2bb6cc2b5303c599',12301)),
'v539_failed_transport_record':(S/'v539_v535_public_s1_zero_update_runtime_package_repair_gate_transport_deployment_record.json',('86aea2da97fa6e7bcac62f426b08cd71860fa318fb82f1243f7dc13af47aa128',1390)),
...
def current_exact1_tree(root, expected_digest, expected_bytes):
    if root.is_symlink() or not root.is_dir():
        raise RuntimeError('current authority root')
    members = sorted(entry.name for entry in os.scandir(root))
    if members != ['authority_receipt.json']:
        raise RuntimeError('current authority exact1')
    member = root / 'authority_receipt.json'
    record = regular(member, (expected_digest, expected_bytes))
    inventory = [['authority_receipt.json', record['sha256'], record['logical_bytes']]]
    lines = f"{record['sha256']}  authority_receipt.json\n".encode()
    return {
        'root': str(root),
        'file_count': 1,
        'logical_file_bytes': record['logical_bytes'],
        'inventory': inventory,
...
def publish_exact1(receipt, stable_snapshot, module, records):
    ...
    try:
        ...
        for name, operation in [
            ...
            ('current_tree', lambda: current_exact1_tree(AUTH_ROOT, digest, len(payload))),
        ]:
            try:
                result = operation()
