...
RUN="$J/v325_v317_same_episode_phase_repair_gate_seed1495_20260822"
...
taskset -c 0-7 "$PY" pipeline/scripts/test_v325_same_episode_phase_repair.py \
  "${COMMON[@]}" --output "$RUN/audit/contract_report.json" \
  >>"$RUN/audit/contract.log" 2>&1

taskset -c 0-7 "$PY" pipeline/scripts/audit_v325_same_episode_phase_repair_gate.py \
  "${COMMON[@]}" \
...
printf 'V325_SAME_EPISODE_PHASE_REPAIR_GATE_PASSED\n'
