...
SELF_PATH = SCRIPTS / "invoke_v503_v502_compact_reconciliation_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v503_compact_reconciliation_authority_materialize_once.sh")
...
EVIDENCE_ROOT = J / "v503_v502_compact_reconciliation_execution_authority_materialization_evidence_seed1645_20260825"
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v503-compact-reconciliation-authority-materializer-process-receipt-v1",
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v503-compact-reconciliation-authority-materializer-intent-v1",
...
def production_fixture() -> dict:
    checks = {}
    with tempfile.TemporaryDirectory(prefix="v503-helper-fixture-") as folder:
        base = Path(folder)
...
def main() -> int:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v503-compact-reconciliation-authority-materializer-argv-v1",
