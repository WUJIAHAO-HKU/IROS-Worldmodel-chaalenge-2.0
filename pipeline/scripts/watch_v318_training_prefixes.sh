...
RUN="$OFF/runs/$NAME"; REG="$OFF/run_registry/$NAME"
PY='/root/miniconda3/envs/go1/bin/python'; AUDITOR="$BASE/pipeline/scripts/audit_v308_training_prefix.py"
LAST=''
for _ in $(seq 1 5760); do
  ...
  if [[ -n "$event" && -f "$event" ]]; then
    ...
    if [[ "$stamp" != "$LAST" ]]; then
      sleep 10
      mkdir -p "$RUN/audit"
      tmp="$RUN/audit/v318_training_prefix_latest.json.tmp"
      log="$RUN/audit/v318_training_prefix_latest.log.tmp"
      set +e
  ...
  if [[ -f "$RUN/audit/V318_TRAINING_ACCEPTED" ]]; then touch "$REG/V318_PREFIX_WATCH_COMPLETE"; exit 0; fi
  if ! screen -ls 2>/dev/null | grep -q '[.]v318_v317_official_rl'; then echo V318_STOPPED_WITHOUT_ACCEPTANCE >&2; exit 3; fi
  sleep 60
done
exit 4
