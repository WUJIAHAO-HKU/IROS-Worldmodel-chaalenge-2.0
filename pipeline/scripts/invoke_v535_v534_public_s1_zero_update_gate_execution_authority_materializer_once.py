...
SELF_PATH = SCRIPTS / "invoke_v535_v534_public_s1_zero_update_gate_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v535_v534_public_s1_zero_update_gate_execution_authority_materialize_once.sh")
DEPLOYMENT_RECORD = SCRIPTS / "v535_v534_public_s1_zero_update_gate_execution_authority_materialization_transport_deployment_record.json"
...
EVIDENCE_ROOT = J / "v535_v534_public_s1_zero_update_gate_execution_authority_materialization_evidence_seed1668_20260827"
...
def expected_transport_deployment(helper_record: dict, script_record: dict) -> dict:
    return {
        "format": "strict-track2-v535-stage-a-transport-pair-deployment-record-v1",
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v535-v534-public-s1-zero-update-gate-execution-authority-materializer-process-receipt-v1",
        "status": status, "passed": passed, "authority_materializer_invocations": invocations,
        "transport_helper_invocations": 1,
        "public_s1_zero_update_boundary_invocations": 0,
        "public_s1_evaluator_invocations": 0, "zero_update_gate_invocations": 0,
        "public_s1_independent_auditor_invocations": 0,
        "v534_actual_oof_durable_boundary_invocations": 1 if passed else 0,
        "actual_oof_execution_invocations": 0,
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v535-v534-public-s1-zero-update-gate-execution-authority-materializer-intent-v1",
        ...
        receipt = {
            **base_receipt("passed_exact_once_public_s1_zero_update_authority_materialized_no_gate_execution", True, 1, cleanup),
...
def production_fixture(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    with tempfile.TemporaryDirectory(prefix="v535-stage-a-helper-", dir="/dev/shm") as folder:
        temp = Path(folder)
        ...
        result = orchestrate(
            ...
            argv_payload={"format": "synthetic-v535-stage-a", "pre_snapshot": pre,
                          "transport_deployment_record": deployment_record,
...
def main() -> int:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v535-v534-public-s1-zero-update-gate-execution-authority-materializer-argv-v1",
