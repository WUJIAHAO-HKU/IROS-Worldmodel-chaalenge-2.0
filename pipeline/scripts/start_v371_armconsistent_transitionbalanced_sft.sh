...
NAME='v371_v169_armconsistent_transitionbalanced_chunk8_sft1697_b4_lr1e6_seed1535_20260822'
SCREEN_NAME='v371_armconsistent_sft'
...
if [[ "${1:-}" != '--run' ]]; then
  ...
  if (( avail_bytes < 14000000000 )); then
    echo "v371 requires at least 14 GB free before launch; available=$avail_bytes" >&2
    exit 12
  fi
  screen -dmS "$SCREEN_NAME" "$0" --run
  sleep 3
  screen -ls | grep -q "$SCREEN_NAME"
  echo V371_ARMCONSISTENT_TRANSITIONBALANCED_SFT_STARTED
  exit 0
fi

export TRACK2_V259_NAME="$NAME"
export TRACK2_V259_VARIANT='v371_armconsistent_transitionbalanced_epoch1_seed1535'
