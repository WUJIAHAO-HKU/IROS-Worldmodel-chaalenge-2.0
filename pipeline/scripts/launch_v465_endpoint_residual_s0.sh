#!/usr/bin/env bash
# Preregistered public-train v465 WM parent gate only. No reward, policy update, RL, dev/final, or submission.
set -euo pipefail
ROOT='/root/autodl-tmp/IROS_WAM_2.0 challenge'
J="$ROOT/artifacts/strict_track2_joint_augmentation_20260810"
V461="$J/v461_endpoint200_seed1612_20260823"
REG="$J/v465_endpoint_residual_seed1615_20260823"
TRAINING="$REG/training"
RELEASE="$J/v465_v169_endpoint_residual_parent_release"
RLPY='/root/autodl-tmp/conda_envs/rlinf_track2/bin/python'
PREP="$ROOT/pipeline/scripts/prepare_v465_endpoint_residual_5fold.py"
TRAINER="$ROOT/pipeline/scripts/train_v465_endpoint_residual_5fold.py"
PACKAGER="$ROOT/pipeline/scripts/package_v465_endpoint_residual_release.py"
RUNTIME="$ROOT/pipeline/wam_pipeline/v465_v169_endpoint_residual_runtime.py"
...
LOCK_FILE='/var/lock/v465_endpoint_residual_seed1615.lock'
...
test "$(sha256sum "$PREP" | awk '{print $1}')" = '7ea0b8fa7ec6d89cc2397e8773bf057d22b093b646795e58ae57f02c62f54ef0'
test "$(sha256sum "$TRAINER" | awk '{print $1}')" = '52ea8324545fefa5c9a2e65a75e3331f8dead5aa29a1ff3ecfe65ad506eb871b'
test "$(sha256sum "$PACKAGER" | awk '{print $1}')" = '3050bc8a3aec1977a7d65eb0ceb53ba7208a8fd74a77d033857f6dd58de58cf5'
test "$(sha256sum "$RUNTIME" | awk '{print $1}')" = '480702b8843f9ca344b20068ff45f261a4c20e4af54362075dd574400cbfc5d8'
...
payload={'format':'strict-track2-v465-launcher-failure-receipt-v1','passed':False,'reason':reason,'exit_code':int(code),'wall_seconds':int(elapsed),'gpu_peak_mib':int(peak),'training_process_group':int(pid) if pid else None,'reward_loaded':False,'policy_updates':0,'rl_authorized':False}
tmp=path+'.tmp'
...
WATCHDOG_PID=''
trap - EXIT TERM INT
echo V465_ENDPOINT_RESIDUAL_S0_AND_ALL200_COMPLETE
