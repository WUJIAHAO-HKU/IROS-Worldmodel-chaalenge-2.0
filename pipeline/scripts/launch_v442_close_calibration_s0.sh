#!/usr/bin/env bash
# Public-train-only v442 close calibration, S0 audit and packaging.
set -euo pipefail
...
PYTHONPATH="$ROOT/pipeline:$ROOT/pipeline/scripts" taskset -c 0-5 "$GO1" \
  pipeline/scripts/prepare_v442_close_trainonly_projection.py \
...
PYTHONPATH="$ROOT/pipeline:$ROOT/pipeline/scripts" taskset -c 0-11 "$GO1" \
  pipeline/scripts/calibrate_v442_close_trainonly_rgb_projection.py \
  --windows "$WINDOWS" --split "$SPLIT" --v169-release "$V169" \
  --v169-library "$ROOT/artifacts" --v436-release "$V436" \
  --preregistration "$REG/preregistration.json" --output "$REG/alignment_index.json" \
  --device cuda --inference-batch-size 4 >"$REG/calibration.log" 2>&1

PYTHONPATH="$ROOT/pipeline:$ROOT/pipeline/scripts" taskset -c 0-5 "$GO1" \
  pipeline/scripts/audit_v442_static_contract.py \
  --alignment-index "$REG/alignment_index.json" --preregistration "$REG/preregistration.json" \
  --split "$SPLIT" --runtime "$ROOT/pipeline/wam_pipeline/v442_v169_close_aligned_projection_runtime.py" \
  --output "$REG/s0_audit.json" >"$REG/s0_audit.log" 2>&1

PYTHONPATH="$ROOT/pipeline:$ROOT/pipeline/scripts" taskset -c 0-5 "$GO1" \
  pipeline/scripts/package_v442_close_aligned_release.py \
  --preregistration "$REG/preregistration.json" --alignment-index "$REG/alignment_index.json" \
  --s0-audit "$REG/s0_audit.json" --output "$RELEASE" >"$REG/package.log" 2>&1
echo V442_S0_COMPLETE
