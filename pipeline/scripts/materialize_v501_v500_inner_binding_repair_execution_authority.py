#!/usr/bin/env python3
"""Atomically register the v501 inner-binding-repair authority; run no execution source."""
...
CONTRACT_PATH = ROOT / "pipeline/scripts/v501_v500_failure_tree_diagnostic_inner_binding_repair_execution_authority_contract.json"
CONTRACT_FORMAT = "strict-track2-v501-v500-failure-tree-diagnostic-inner-binding-repair-execution-authority-design-contract-v1"
CONTRACT_STATUS = "design_only_frozen_sources_pending_independent_review_no_authority"
OUTPUT_FORMAT = "strict-track2-v501-v500-failure-tree-diagnostic-inner-binding-repair-execution-authority-v1"
OUTPUT_STATUS = "authorized_exact_one_external_v501_inner_binding_repaired_outer_attempt"
...
INNER_ATTEMPT_ROOT = J / "v501_v500_failure_tree_diagnostic_inner_binding_repaired_reconciliation_inner_attempt_seed1643_20260825"
...
SOURCE_ROLE_ORDER = [
    ...
    "v501_preexecution_forensic_source", "corrected_inner_wrapper", "outer_execution_wrapper",
]
SOURCE_ALIASES = {
...
def synthetic_self_test() -> int:
    checks = {
        "authority_top_count": len(AUTH_TOP_KEYS) == 107,
        "check_count": len(AUTH_CHECK_KEYS) == 96,
        "source_roles": len(SOURCE_ROLES) == 39,
        "source_order_aliases": (len(SOURCE_ROLE_ORDER) == 39
                                  and set(SOURCE_ROLE_ORDER) == SOURCE_ROLES
                                  and set(SOURCE_ALIASES) == SOURCE_ROLES
                                  and len(set(SOURCE_ALIASES.values())) == 39),
        ...
        "fresh_roots": all("v501_" in path.name for path in
...
def main() -> int:
    ...
    if (set(contract) != CONTRACT_TOP_KEYS or contract.get("format") != CONTRACT_FORMAT
            or contract.get("status") != CONTRACT_STATUS or contract.get("seed") != 1643
            or pending(contract)):
