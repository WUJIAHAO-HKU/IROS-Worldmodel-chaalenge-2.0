#!/usr/bin/env python3
"""Sole launcher for the v534 fresh-authority environment-repaired actual-OOF boundary."""
...
def run(argv=None):
    ...
    if (set(prereg) != PREREG_KEYS
            ...
            or prereg.get("format") != "strict-track2-v534-v533-actual-oof-environment-repair-execution-preregistration-v1"
            or manifest.get("format") != "strict-track2-v534-v533-actual-oof-environment-repair-execution-manifest-v1"
            ...
            or not json_exact(prereg.get("required_environment"), REQUIRED_ENVIRONMENT_EXACT)
            ...
            or not json_exact(authority.get("required_environment_exact"), REQUIRED_ENVIRONMENT_EXACT)
            or not json_exact(contract.get("authority_receipt_contract", {}).get("required_environment_exact"), REQUIRED_ENVIRONMENT_EXACT)
    ...
    if {key: os.environ.get(key) for key in REQUIRED_ENVIRONMENT_EXACT} != REQUIRED_ENVIRONMENT_EXACT:
        raise RuntimeError("launcher required environment exact")
    if args.output_root != Path(prereg["fresh_oof_root"]):
        raise RuntimeError("fresh output root")
    blocked = {signal.SIGINT, signal.SIGTERM}
    previous = signal.pthread_sigmask(signal.SIG_BLOCK, blocked)
    try:
        spec = importlib.util.spec_from_file_location("v534_actual_oof_executor", args.executor_source)
