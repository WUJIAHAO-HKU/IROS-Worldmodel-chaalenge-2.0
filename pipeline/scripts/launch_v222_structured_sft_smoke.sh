...
NAME='v222_v169_structured_rightsft_smoke_h8_step1_seed1422_20260818'
RUN="$OFF/runs/$NAME"
...
"$PY" "$P/prepare_v222_structured_sft_smoke.py" "$REG/preregistration.json" \
...
export TRACK2_ACTOR_SEED=1422 TRACK2_ENV_SEED=0
...
grep -aqE 'sft_arm=1(\.0)?([,[:space:]]|$)' "$RUN/launcher.log"
grep -aqE 'sft_physical_action_weight_sum=10\.75([,[:space:]]|$)' "$RUN/launcher.log"
grep -aE 'sft_loss|weighted_sft_loss|sft_arm|sft_physical_action_weight_sum|actor/kl_loss|actor/total_loss|actor/grad_norm' \
  "$RUN/launcher.log" > "$RUN/audit/structured_sft_smoke_metrics.txt"
sha256sum "$OUTPUT_CKPT" > "$RUN/audit/output_checkpoint_sha256.txt"
touch "$RUN/audit/V222_STRUCTURED_SFT_SMOKE_ACCEPTED"
echo V222_STRUCTURED_SFT_SMOKE_ACCEPTED
