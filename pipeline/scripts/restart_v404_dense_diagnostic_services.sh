...
BASE="$J/v400_supported_posterior_blend_seed1560_20260823"
REG="${TRACK2_SERVICE_REG:?TRACK2_SERVICE_REG required}"
CPUSET="${TRACK2_CPUSET:-0-5}"
VERSION='track2-v404-v400-output-equivalent-dense-diagnostic'
...
service() {
  ...
  exec taskset -c "$CPUSET" env PYTHONPATH="$ROOT/pipeline" \
    ...
    WAM_BACKEND='v404-v400-dense-diagnostic' \
    WAM_CHECKPOINT_DIR="$BASE/release" \
    WAM_V216_LIBRARY_INDEX="$LIBRARY" WAM_V312_ACTION_GATE="$ACTION_GATE" \
    WAM_V324_PHASE_GATE="$PHASE_GATE" WAM_V404_TRACE_PATH="$REG/route_trace.jsonl" \
    WAM_DEVICE=cuda WAM_PORT=8005 WAM_NATIVE_BATCH_ENABLED=1 \
    WAM_NATIVE_BATCH_MICRO_SIZE=8 WAM_RELEASE_CUDA_CACHE=1 \
    "$GO1" pipeline/scripts/serve.py >>"$REG/v404_service.log" 2>&1
}
...
case "${1:-start}" in
  ...
  bridge) bridge ;;
  stop)
    screen -S wm_v404_bridge -X quit >/dev/null 2>&1 || true
    screen -S wm_v404_gpu -X quit >/dev/null 2>&1 || true
    ;;
  start)
    mkdir -p "$REG"
    test ! -e "$REG/route_trace.jsonl"
    for name in wm_v404_bridge wm_v404_gpu wm_v218_bridge wm_v218_gpu; do
