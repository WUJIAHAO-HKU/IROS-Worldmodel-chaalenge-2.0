#!/usr/bin/env bash
# Preregistered public-train v468 WM parent gate only. No reward, policy update, RL, dev/final, or submission.
set -euo pipefail
ROOT='/root/autodl-tmp/IROS_WAM_2.0 challenge'
J="$ROOT/artifacts/strict_track2_joint_augmentation_20260810"
V461="$J/v461_endpoint200_seed1612_20260823"
REG="$J/v468_endpoint_residual_seed1616_20260823"
TRAINING="$REG/training"
RELEASE="$J/v468_v169_endpoint_residual_parent_release"
RLPY='/root/autodl-tmp/conda_envs/rlinf_track2/bin/python'
PREP="$ROOT/pipeline/scripts/prepare_v468_endpoint_residual_5fold.py"
TRAINER="$ROOT/pipeline/scripts/train_v468_endpoint_residual_5fold.py"
PACKAGER="$ROOT/pipeline/scripts/package_v468_endpoint_residual_release.py"
RUNTIME="$ROOT/pipeline/wam_pipeline/v468_canonical_notransport_paired_delta_runtime.py"
CONTRACT="$ROOT/pipeline/scripts/v468_canonical_notransport_paired_delta_contract.json"
...
LOCK_FILE='/var/lock/v468_endpoint_residual_seed1616.lock'
...
test "$(sha256sum "$PREP" | awk '{print $1}')" = 'c41c38561744aba091acd39c4cfd4e67676af805167162358afc211a3a0b925b'
test "$(sha256sum "$TRAINER" | awk '{print $1}')" = '3b36d3e671ec9e3f40c5a528835feb88ce12bd8d6a0e9be19e0f105184069a57'
test "$(sha256sum "$PACKAGER" | awk '{print $1}')" = '534d6126e57209c243a36bf67707ef0fe0c68a9ae50202e1819d62f3ce16c86d'
test "$(sha256sum "$RUNTIME" | awk '{print $1}')" = '66a40e01df50cfe5da619e4141ff03c125c8ceddd0a5cedd2a982b2bd41041c5'
test "$(sha256sum "$CONTRACT" | awk '{print $1}')" = '99dc3715397e027ad64b78b4afcd7413a1f0f8e3c02c4e4631f12478862ca43b'
...
payload={'format':'strict-track2-v468-launcher-failure-receipt-v1','passed':False,'reason':reason,'exit_code':int(code),'wall_seconds':int(elapsed),'gpu_peak_mib':int(peak),'training_process_group':int(pid) if pid else None,'reward_loaded':False,'policy_updates':0,'rl_authorized':False}
tmp=path+'.tmp'
...
WATCHDOG_PID=''
trap - EXIT TERM INT
echo V468_ENDPOINT_RESIDUAL_S0_AND_ALL200_COMPLETE
