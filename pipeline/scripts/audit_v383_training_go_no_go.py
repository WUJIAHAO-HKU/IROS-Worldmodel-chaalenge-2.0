#!/usr/bin/env python3
"""Freeze the training-only v383 continuation decision."""
...
def main() -> int:
    ...
    if prereg.get("format") != "strict-track2-v383-v169-v382-one-update-preregistration-v1":
        raise RuntimeError("wrong preregistration")
    events = sorted((args.run / "tensorboard").glob("events.out.tfevents.*"))
    tags = (
        "env/episode_len", "env/num_trajectories", "env/return", "env/reward",
        "env/success_once", "train/actor/approx_kl", "train/actor/clip_fraction",
        "train/actor/grad_norm", "train/actor/kl_loss",
    )
    metrics = {tag: latest_scalar(events, tag) for tag in tags}
    trajectories = int(round(metrics["env/num_trajectories"]))
    successes = int(round(metrics["env/success_once"] * trajectories))
    threshold = float(prereg["selection_protocol"]["initial_rollout_success_rate_go_signal"])
    comparator = 14
    valid = (
    ...
    go = valid and metrics["env/success_once"] >= threshold
    report = {
        "format": "strict-track2-v383-training-go-no-go-v1",
        ...
        "v327_v326_comparator_count": comparator,
        "delta_vs_v327_count": successes - comparator,
        "checks": {
