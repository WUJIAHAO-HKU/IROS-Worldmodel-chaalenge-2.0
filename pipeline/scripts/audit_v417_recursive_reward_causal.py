...
def main() -> int:
    ...
    if payload.get("format") != "strict-track2-v417-reward-monotone-successor-preregistration-v1":
        raise RuntimeError("wrong v417 preregistration")
    ...
    sys.argv = argv
    base.Track2V385NativeBatchCleanReanchor = Track2V417RewardMonotoneSuccessor
    try:
        code = base.main()
    finally:
        Path(temporary).unlink(missing_ok=True)
    report = json.loads(output.read_text())
    report["format"] = "strict-track2-v417-recursive-reward-causal-gate-v1"
