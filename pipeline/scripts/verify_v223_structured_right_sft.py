...
def values(name: str) -> list[float]:
    found = re.findall(
        rf"(?<![A-Za-z0-9_/]){re.escape(name)}=([-+0-9.eE]+)", log
    )
...
for name in ("sft_arm", "sft_physical_action_weight_sum", "sft_loss", "weighted_sft_loss", "actor/grad_norm"):
...
assert all(
    abs(value - 10.75) < 1e-9
    for value in metrics["sft_physical_action_weight_sum"][-expected_updates:]
)
...
(audit / "structured_right_sft_training_acceptance.json").write_text(
    json.dumps(report, indent=2) + "\n"
)
(audit / "V223_STRUCTURED_RIGHT_SFT_TRAINING_ACCEPTED").touch()
