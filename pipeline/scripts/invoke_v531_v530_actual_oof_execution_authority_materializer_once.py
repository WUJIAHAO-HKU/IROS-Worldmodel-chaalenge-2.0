#!/usr/bin/env python3
"""Exact-once Stage-A transport for the v531 actual-OOF execution authority."""
...
SELF_PATH = SCRIPTS / "invoke_v531_v530_actual_oof_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v531_v530_actual_oof_execution_authority_materialize_once.sh")
DEPLOYMENT_RECORD = SCRIPTS / "v531_v530_actual_oof_execution_authority_materialization_transport_deployment_record.json"
CONTRACT = SCRIPTS / "v531_v530_actual_oof_execution_authority_contract.json"
CONTRACT_SHA = "065152e018ccec41688bd8d0e3d8716a58b1e4feecb763a123f1175ba696f109"
CONTRACT_BYTES = 99457
MATERIALIZER = SCRIPTS / "materialize_v531_v530_actual_oof_execution_authority.py"
MATERIALIZER_SHA = "845494276b0f2fc946dd5c5a48c46edd97ec36592e98aba5ddd69265cf6a8525"
MATERIALIZER_BYTES = 33005
EXECUTOR = SCRIPTS / "execute_v531_v530_actual_oof.py"
EXECUTOR_SHA = "14537cfe9f54ba4f1d47609102c12768a9c215317cd9c8f66346f0a7bb33587b"
EXECUTOR_BYTES = 47719
AUDITOR = SCRIPTS / "audit_v531_v530_actual_oof.py"
AUDITOR_SHA = "8251aedf27fcbd9966a46e9c5ef7ef3ce55b0e986251a7b9dcc31a820f1d3792"
AUDITOR_BYTES = 10155
LAUNCHER = SCRIPTS / "launch_v531_v530_actual_oof.py"
LAUNCHER_SHA = "653de64004c2582ef776bd4965d52693f738e2e12f99377c448ac79f00287895"
LAUNCHER_BYTES = 5515
PREREGISTRATION = SCRIPTS / "v531_v530_actual_oof_execution_preregistration.json"
PREREGISTRATION_SHA = "6fc85dd57bd6693a276c6f3ddfbbda1ff6cf42af7f5a1417dcdfbab460045329"
PREREGISTRATION_BYTES = 15071
MANIFEST = SCRIPTS / "v531_v530_actual_oof_execution_manifest.json"
MANIFEST_SHA = "ec0a5c396972475ead9eb70db93320799a239257e88938490453b6236ecd7fc9"
MANIFEST_BYTES = 1485362
AUTHORITY_ROOT = J / "v531_v530_actual_oof_execution_authority_seed1665_20260827"
AUTHORITY_PREP = AUTHORITY_ROOT.with_name(AUTHORITY_ROOT.name + ".authority-prep")
EVIDENCE_ROOT = J / "v531_v530_actual_oof_execution_authority_materialization_evidence_seed1665_20260827"
...
FROZEN_FINAL7_SHA256 = (CONTRACT_SHA, MATERIALIZER_SHA, PREREGISTRATION_SHA, MANIFEST_SHA,
                       EXECUTOR_SHA, AUDITOR_SHA, LAUNCHER_SHA)
...
def deployment_plan() -> dict:
    ...
    return {
        "fresh_noreplace_targets_exact7": rows,
        "volatile_sources_exact7": [{**row, "source_path": "/dev/shm/" + Path(row["path"]).name} for row in rows],
        "reused_current_only": [],
...
def expected_transport_deployment(helper_record: dict, script_record: dict) -> dict:
    return {
        "format": "strict-track2-v531-stage-a-transport-pair-deployment-record-v1",
...
def validate_materializer_contract(module, contract: dict, materializer_record: dict) -> None:
    ...
    schema_checks = {
        ...
        "seed": type(seed) is int and seed == 1665,
        ...
        "authority_top_count": len(contract.get("authority_receipt_contract", {}).get("top_keys", [])) == 49,
        "check_count": len(module.CHECK_KEYS) == 29,
        "source_counts": len(module.SOURCE_ROLES) == 20 and len(module.AUTHORITY_SOURCE_ROLES) == 21,
    ...
    if (cbytes(contract.get("authorization")) != cbytes(module.AUTHORIZATION)
        ...
        raise RuntimeError("v531 actual-OOF authority boundary")
    for role, row in contract["source_closure"].items():
...
def load_materializer(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    spec = importlib.util.spec_from_file_location("v531_materializer", materializer_source)
...
def validate_success(module, contract: dict, returncode: int, cleanup: dict,
    ...
    if (os.path.lexists(AUTHORITY_PREP)
            ...
            or len(receipt.get("source_closure", {})) != 21
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v531-v530-actual-oof-execution-authority-materializer-process-receipt-v1",
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v531-v530-actual-oof-execution-authority-materializer-intent-v1",
...
def production_fixture(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    checks = {
        ...
        "fresh7_noreplace_plan": (
            len(plan["fresh_noreplace_targets_exact7"]) == 7
            and [row["role"] for row in plan["fresh_noreplace_targets_exact7"]]
        ...
        "authority_schema_49_29_21": (
            len(contract["authority_receipt_contract"]["top_keys"]) == 49
            and len(module.CHECK_KEYS) == 29 and len(module.AUTHORITY_SOURCE_ROLES) == 21),
    ...
    with tempfile.TemporaryDirectory(prefix="v531-stage-a-helper-", dir="/dev/shm") as folder:
        temp = Path(folder)
        ...
        result = orchestrate(
            ...
            argv_payload={"format": "synthetic-v531-stage-a", "pre_snapshot": pre,
                          "transport_deployment_record": deployment_record,
...
def main() -> int:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v531-v530-actual-oof-execution-authority-materializer-argv-v1",
