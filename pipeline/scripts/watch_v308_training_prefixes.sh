...
for _ in $(seq 1 5760); do
  ...
  if [[ -n "$event" && -f "$event" ]]; then
    ...
    if [[ "$stamp" != "$LAST_STAMP" ]]; then
      ...
      mkdir -p "$RUN/audit"
      tmp="$RUN/audit/v308_training_prefix_latest.json.tmp"
      log_tmp="$RUN/audit/v308_training_prefix_latest.log.tmp"
      set +e
      "$PY" "$AUDITOR" --run "$RUN" --preregistration "$REG/preregistration.json" --output "$tmp" >"$log_tmp" 2>&1
      rc=$?
      set -e
      if [[ -s "$tmp" ]]; then
        ...
        mv "$tmp" "$RUN/audit/v308_training_prefix_step${latest}.json"
        mv "$log_tmp" "$RUN/audit/v308_training_prefix_step${latest}.log"
        cp -f "$RUN/audit/v308_training_prefix_step${latest}.json" "$RUN/audit/v308_training_prefix_latest.json"
        printf 'metric_step=%s audit_rc=%s audited_at=%s\n' "$latest" "$rc" "$(date -u +%FT%TZ)" >>"$REG/v308_prefix_watcher_history.log"
        if (( rc != 0 )); then
          touch "$REG/V308_PREFIX_AUDIT_FAILED_STEP_${latest}"
          exit "$rc"
        fi
      else
        rm -f "$tmp"
        mv "$log_tmp" "$REG/v308_prefix_watcher_parse_failure.log"
        exit 5
      fi
      LAST_STAMP=$stamp
    fi
  ...
  if [[ -f "$RUN/audit/V308_TRAINING_ACCEPTED" ]]; then
    touch "$REG/V308_PREFIX_WATCH_COMPLETE"
    exit 0
  fi
...
echo V308_PREFIX_WATCH_TIMEOUT >&2
exit 4
