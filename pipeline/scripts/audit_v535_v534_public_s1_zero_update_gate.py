#!/usr/bin/env python3
"""Independent data-only audit of staged v535 public-S1/zero-update evidence."""
...
def audit_staged(root: Path, manifest: dict, baseline_model: dict, rng_entry: dict) -> dict:
    ...
    with (root/"public_s1_events.ndjson").open("r",encoding="utf-8") as stream:
        for ordinal,line in enumerate(stream):
            row=json.loads(line)
            expected={"absolute_error_sum_uint64","branch","branch_index","episode","event_canonical_sha256","model_snapshot_sha256","ordinal","pixel_count_uint64","prediction_sha256","rng_snapshot_sha256","start","target_sha256"}
            if set(row)!=expected or type(row["ordinal"]) is not int or row["ordinal"]!=ordinal or canonical_sha({k:v for k,v in row.items() if k!="event_canonical_sha256"})!=row["event_canonical_sha256"]:
    ...
    checks={
        ...
        "zero_trace_schema":trace.get("format")=="strict-track2-v535-v534-zero-update-trace-v1" and type(trace.get("stage_count")) is int and trace["stage_count"]==len(trace.get("stages",[]))==46,
