...
N='v297_v295_terminal_frame_gates_seed1480_20260821'; RUN="$J/$N"; REG="$O/run_registry/$N"; S="$B/pipeline/scripts"
...
PARENT_REPORT="$J/v272_v271_endpoint_calibrated_gates_seed1469_20260819/audit/post_grasp_reward_report.json"; PARENT_DETAILS="$J/v272_v271_endpoint_calibrated_gates_seed1469_20260819/audit/post_grasp_reward_details.npz"
...
trap restore EXIT
"$RLPY" "$S/prepare_v297_v295_gates.py"
...
set +e
cd /tmp
taskset -c 0-47 env PYTHONPATH="$B/pipeline:$RLINF" TOKENIZERS_PARALLELISM=false "$RLPY" "$S/replay_v245_post_grasp_reward.py" --audit-dir "$O/run_registry/v211_v209_train_action_capture_h200_r2_step1_seed1410_20260818/bridge_audit" --library "$J/v214_public_right_knn_action_visual_diagnostic_seed1413/library/public_right_knn.npz" --url "$URL" --token local-dev-token --model-version "$V" --reward-checkpoint "$REWARD" --t5-model "$T5" --alpha-scale 8 --delta-regime --delta-clean-max .01 --delta-clean-alignment-floor .95 --delta-ood-min .8 --delta-ood-alignment-floor .5 --success-like-min 4 --group-std-min .005 --global-min .05 --group-min .40 --output "$RUN/audit/post_grasp_reward_report.json" --details "$RUN/audit/post_grasp_reward_details.npz" >"$RUN/audit/post_grasp_reward.log" 2>&1
capture_rc=$?
set -e
if (( capture_rc != 0 && capture_rc != 3 )); then touch "$RUN/V297_CAPTURE_EXECUTION_FAILED"; exit "$capture_rc"; fi
"$RLPY" "$S/verify_v297_capture_relative.py" --parent-report "$PARENT_REPORT" --parent-details "$PARENT_DETAILS" --candidate-report "$RUN/audit/post_grasp_reward_report.json" --candidate-details "$RUN/audit/post_grasp_reward_details.npz" --output "$RUN/audit/capture_relative_gate.json" >"$RUN/audit/capture_relative_gate.log" 2>&1
touch "$RUN/V297_CAPTURE_GATE_PASSED"
cd "$B"
...
PY
touch "$RUN/V297_LONG_GATE_PASSED" "$RUN/AUDIT_COMPLETE"
echo V297_V295_GATES_PASSED
