#!/usr/bin/env python3
"""Exact-once, signal-safe transport for the v508 Phase-A authority materializer."""
...
SELF_PATH = SCRIPTS / "invoke_v508_v507_phase_a_authority_prep_snapshot_repair_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v508_v507_phase_a_authority_prep_snapshot_repair_authority_materialize_once.sh")
CONTRACT = SCRIPTS / "v508_v507_phase_a_authority_prep_snapshot_repair_execution_authority_contract.json"
CONTRACT_SHA = "e26dd6ece95509fd8453bd485965c8c56b2dc2260234c3beca5ea027fdf8f53f"
CONTRACT_BYTES = 34611
MATERIALIZER = SCRIPTS / "materialize_v508_v507_phase_a_authority_prep_snapshot_repair_execution_authority.py"
MATERIALIZER_SHA = "94ffba60ca9c75bded629ab3948d658bbf8360600889d8522611896dea6450c1"
MATERIALIZER_BYTES = 25746
PHASE_A_LAUNCHER = SCRIPTS / "launch_v508_v507_phase_a_cache_qualification.py"
PHASE_A_LAUNCHER_SHA = "c1c549dd2e0094c9dbb8e027cfd377e290a76e021589ab0305130ff4ac7edbb8"
PHASE_A_LAUNCHER_BYTES = 58802
FAILURE_FORENSIC = SCRIPTS / "v508_v507_phase_a_authority_prep_snapshot_failure_forensic.json"
FAILURE_FORENSIC_SHA = "9ac9e9cd0c9dd8d39ad790194c74cc77d1f1ba94a30a18ae87f5e9e589db1e9b"
...
EVIDENCE_ROOT = J / "v508_v507_phase_a_authority_prep_snapshot_repair_execution_authority_materialization_evidence_seed1649_20260825"
...
def validate_materializer_contract(module, contract: dict, materializer_record: dict) -> None:
    ...
    if (module.CONTRACT_PATH != CONTRACT or module.AUTHORITY_ROOT != AUTHORITY_ROOT
            ...
            or isinstance(seed, bool) or not isinstance(seed, int) or seed != 1649
            ...
            or contract.get("source_closure", {}).get("v507_failure_forensic") != {
                "path": str(FAILURE_FORENSIC), "sha256": FAILURE_FORENSIC_SHA,
                "logical_bytes": FAILURE_FORENSIC_BYTES}
            or len(module.AUTH_TOP_KEYS) != 59 or len(module.CHECK_KEYS) != 31
            or len(module.CONTRACT_SOURCE_ORDER) != 19 or len(module.AUTHORITY_SOURCE_ORDER) != 20
...
def validate_success(module, contract: dict, returncode: int, cleanup: dict,
    ...
    if (os.path.lexists(AUTHORITY_PREP)
            or set(receipt) != module.AUTH_TOP_KEYS or len(receipt) != 59
            ...
            or len(receipt.get("source_closure", {})) != 20
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v508-v507-phase-a-authority-prep-snapshot-repair-authority-materializer-process-receipt-v1",
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v508-v507-phase-a-authority-prep-snapshot-repair-authority-materializer-intent-v1",
...
def production_fixture(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER) -> dict:
    ...
    def rejects(contract_value, module_value=live_module, materializer_value=live_materializer_record):
        try:
            ...
            return True
    tamper_checks = []
    for bad_seed in (1644, 1648, "1649", True, None):
    ...
    bad_forensic["source_closure"]["v507_failure_forensic"]["sha256"] = "0" * 64
...
def main() -> int:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v508-v507-phase-a-authority-prep-snapshot-repair-authority-materializer-argv-v1",
