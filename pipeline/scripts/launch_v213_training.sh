...
restart_services() {
  bash "$P/restart_v209_services.sh" start >"$REG/restart_v209_after_v213.log" 2>&1 || true
}
trap restart_services EXIT

"$PY" "$P/prepare_v213_right_logit_lr5e7.py"
...
"$PY" "$P/train_multichunk_reward_aligned_autoregressive_unet.py" \
  ...
  --steps 100 --checkpoint-interval 25 --batch-size 1 \
