#!/usr/bin/env bash
set -euo pipefail
: "${TRACK2_ROLLOUTONLY_NAME:?required}"
: "${TRACK2_SERVICE_WRAPPER:?required}"
: "${TRACK2_MODEL_VERSION:?required}"
: "${TRACK2_BRIDGE_PORT:?required}"
...
REG="$O/run_registry/$TRACK2_ROLLOUTONLY_NAME"
RUN="$O/runs/$TRACK2_ROLLOUTONLY_NAME"
...
ACTOR_SEED="${TRACK2_ACTOR_SEED:-1497}"
GROUP_SIZE="${TRACK2_EVAL_GROUP_SIZE:-4}"
...
cleanup() {
  ...
  bash "$P/$TRACK2_SERVICE_WRAPPER" stop >/dev/null 2>&1 || true
}
trap cleanup EXIT

TRACK2_SERVICE_REG="$REG" TRACK2_CPUSET=0-5 TRACK2_WAM_RELEASE_CUDA_CACHE=1 bash "$P/$TRACK2_SERVICE_WRAPPER" start >"$REG/restart_worldmodel_before_rollout.log" 2>&1
curl -fsS "http://127.0.0.1:$TRACK2_BRIDGE_PORT/health" | grep -q '"status":"ready"'
curl -fsS http://127.0.0.1:8005/v1/health | grep -q "$TRACK2_MODEL_VERSION"
...
taskset -c 6-21 "$PY" "$RLINF/examples/embodiment/eval_embodied_agent.py" \
  ...
  runner.logger.log_path="$RUN" runner.ckpt_path="$V169" actor.seed="$ACTOR_SEED" \
  algorithm.eval_rollout_epoch=1 algorithm.group_size="$GROUP_SIZE" algorithm.sampling_params.temperature_eval=1.6 \
  env.eval.total_num_envs=32 env.eval.group_size="$GROUP_SIZE" env.eval.seed=0 env.eval.max_episode_steps=200 env.eval.max_steps_per_rollout_epoch=200 env.eval.http.server_url="http://127.0.0.1:$TRACK2_BRIDGE_PORT" \
