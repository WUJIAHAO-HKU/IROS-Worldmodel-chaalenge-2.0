#!/usr/bin/env python3
"""Exact-once Stage-A transport for the v539 public-S1 zero-update package-repair authority."""
...
SELF_PATH = SCRIPTS / "invoke_v539_v535_public_s1_zero_update_runtime_package_repair_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v539_v535_public_s1_zero_update_runtime_package_repair_execution_authority_materialize_once.sh")
DEPLOYMENT_RECORD = SCRIPTS / "v539_v535_public_s1_zero_update_runtime_package_repair_execution_authority_materialization_transport_deployment_record.json"
CONTRACT = SCRIPTS / "v539_v535_public_s1_zero_update_runtime_package_repair_execution_authority_contract.json"
CONTRACT_SHA = "862f24ec07c6e613785cbf3ac84281c1b8cb862c80353420c32ac339746d5bb6"
CONTRACT_BYTES = 147177
MATERIALIZER = SCRIPTS / "materialize_v539_v535_public_s1_zero_update_runtime_package_repair_execution_authority.py"
MATERIALIZER_SHA = "fd59d56120cd9c50c552c890735a07262fa49595ad47852613f06f8865569a97"
MATERIALIZER_BYTES = 29556
EVALUATOR = SCRIPTS / "evaluate_v539_v535_public_s1_zero_update_gate.py"
EVALUATOR_SHA = "8bb00ea33adffa325048c0068345d2464d26a08d818e3fcf888453304b7fd63c"
EVALUATOR_BYTES = 60880
AUDITOR = SCRIPTS / "audit_v539_v535_public_s1_zero_update_gate.py"
AUDITOR_SHA = "16745619eb3aec1e3973dbe371991c273bb487cce60b096d0b1bca98bdc98f08"
AUDITOR_BYTES = 5528
LAUNCHER = SCRIPTS / "launch_v539_v535_public_s1_zero_update_gate.py"
LAUNCHER_SHA = "bdbb9c472d476bf245f279bdebb60d70bf4d8489e110cb2c3aff4025db797164"
LAUNCHER_BYTES = 4306
PREREGISTRATION = SCRIPTS / "v539_v535_public_s1_zero_update_gate_preregistration.json"
PREREGISTRATION_SHA = "a25efe1faae9c6145e736623af04afe2217d7fa1c9d001870598a1408a919822"
PREREGISTRATION_BYTES = 78773
MANIFEST = SCRIPTS / "v539_v535_public_s1_zero_update_gate_manifest.json"
MANIFEST_SHA = "7e808af59cf39a83771e36f8cd49d401d9e50b26945f69a8ac2142e4864209bc"
MANIFEST_BYTES = 81060
FORENSIC = SCRIPTS / "v539_v535_public_s1_zero_update_runtime_package_import_failure_forensic.json"
FORENSIC_SHA = "c67c699c795686134869b0002d46731b21707b82a1aa33db83e610b6983676b9"
FORENSIC_BYTES = 11442
...
AUTHORITY_ROOT = J / "v539_v535_public_s1_zero_update_runtime_package_repair_execution_authority_seed1670_20260828"
AUTHORITY_PREP = AUTHORITY_ROOT.with_name(AUTHORITY_ROOT.name + ".authority-prep")
EVIDENCE_ROOT = J / "v539_v535_public_s1_zero_update_runtime_package_repair_execution_authority_materialization_evidence_seed1670_20260828"
...
EXPECTED_CURRENT_ABSENCE_PATHS = {
    "attempt_prep": "/root/autodl-tmp/IROS_WAM_2.0 challenge/artifacts/strict_track2_joint_augmentation_20260810/v539_v535_public_s1_zero_update_gate_attempt_seed1670_20260828.attempt-prep",
    "attempt_root": "/root/autodl-tmp/IROS_WAM_2.0 challenge/artifacts/strict_track2_joint_augmentation_20260810/v539_v535_public_s1_zero_update_gate_attempt_seed1670_20260828",
    "authority_prep": "/root/autodl-tmp/IROS_WAM_2.0 challenge/artifacts/strict_track2_joint_augmentation_20260810/v539_v535_public_s1_zero_update_runtime_package_repair_execution_authority_seed1670_20260828.authority-prep",
    "output_prep": "/root/v539_v535_public_s1_zero_update_gate_seed1670_20260828.output-prep",
    "output_root": "/root/v539_v535_public_s1_zero_update_gate_seed1670_20260828",
}
EXPECTED_CURRENT_ABSENCE_MAP_SHA256 = "89d16d414c60eadb5bbdeafd0fd5fca853f3304abc5846f1fc9841082534dbb3"
...
def deployment_plan() -> dict:
    final8 = [
        ...
        {"role": "v535_runtime_package_import_failure_forensic", "path": str(FORENSIC), "sha256": FORENSIC_SHA, "logical_bytes": FORENSIC_BYTES},
    ]
    return {
...
def validate_failure_forensic() -> dict:
    ...
    expected_top = ["captured_at_date", "classification", "format", "fresh_authority_repair_boundary",
                    "invocation_partition", "old_execution", "post_failure_readonly_closure",
                    "root_cause", "state_restoration_and_cleanup", "status"]
    boundary = value.get("fresh_authority_repair_boundary", {})
    partition = value.get("invocation_partition", {})
    root_cause = value.get("root_cause", {})
    if (list(value) != expected_top
            or value.get("format") != "strict-track2-v539-v535-public-s1-zero-update-runtime-package-import-failure-forensic-v1"
            or value.get("status") != "frozen_failed_no_retry_pending_fresh_authority_package_qualified_runtime_loader_successor"
            or boundary.get("fresh_authority_required") is not True
            or boundary.get("old_normal_retry_authorized") is not False
            or boundary.get("old_authority_reuse_authorized") is not False
            or boundary.get("runtime_package_loader_gate_required_before_prep_or_intent") is not True
            or type(partition.get("non_durable_evaluator_entry_invocations")) is not int
            or partition.get("non_durable_evaluator_entry_invocations") != 1
            or partition.get("public_s1_invocations") != 0
            or partition.get("zero_update_gate_invocations") != 0
            or root_cause.get("sole_failure_class") != "anonymous_spec_relative_import_without_package_context"):
        raise RuntimeError("v539 failure forensic exact")
...
def expected_transport_deployment(helper_record: dict, script_record: dict) -> dict:
    return {
        "format": "strict-track2-v539-stage-a-transport-pair-deployment-record-v1",
...
def immutable_snapshot(contract: dict, helper_record: dict, script_record: dict) -> dict:
    ...
    return {
        ...
        "v535_runtime_package_import_failure_forensic": validate_failure_forensic(),
...
def validate_materializer_contract(module, contract: dict, materializer_record: dict) -> None:
    ...
    schema_checks = {
        ...
        "seed": type(seed) is int and seed == 1670,
        ...
        "authority_top_count": len(contract.get("authority_receipt_contract", {}).get("top_keys", [])) == 64,
...
def load_materializer(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    spec = importlib.util.spec_from_file_location("v539_materializer", materializer_source)
...
def validate_success(module, contract: dict, returncode: int, cleanup: dict,
    ...
    if (os.path.lexists(AUTHORITY_PREP)
            or sorted(receipt) != receipt_schema["top_keys"] or len(receipt) != 64
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v539-v535-public-s1-zero-update-runtime-package-repair-authority-materializer-process-receipt-v1",
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v539-v535-public-s1-zero-update-runtime-package-repair-authority-materializer-intent-v1",
...
def production_fixture(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    checks = {
        ...
        "authority_schema_64_31_31": (
            len(contract["authority_receipt_contract"]["top_keys"]) == 64
    ...
    with tempfile.TemporaryDirectory(prefix="v539-stage-a-helper-", dir="/dev/shm") as folder:
        temp = Path(folder)
        ...
        result = orchestrate(
            ...
            argv_payload={"format": "synthetic-v539-stage-a", "pre_snapshot": pre,
                          "transport_deployment_record": deployment_record,
...
def main() -> int:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v539-v535-public-s1-zero-update-runtime-package-repair-authority-materializer-argv-v1",
