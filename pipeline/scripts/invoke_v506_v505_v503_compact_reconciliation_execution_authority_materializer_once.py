#!/usr/bin/env python3
"""Exact-once, signal-safe transport for the v506 compact-reconciliation authority materializer."""
...
SELF_PATH = SCRIPTS / "invoke_v506_v505_v503_compact_reconciliation_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v506_v505_v503_compact_reconciliation_authority_materialize_once.sh")
CONTRACT = SCRIPTS / "v506_v505_v503_compact_reconciliation_execution_authority_contract.json"
CONTRACT_SHA = "18f6f8f8d6765f89716873f1a518347e209ec9d2c0544f261ed1c42c4678ec5b"
CONTRACT_BYTES = 18511
MATERIALIZER = SCRIPTS / "materialize_v506_v505_v503_compact_reconciliation_execution_authority.py"
MATERIALIZER_SHA = "5a7711adac18b1d0b1e9d5a4c62f856d781df39691adfe21860360b7d4dff3ea"
MATERIALIZER_BYTES = 26234
DIRECT_WRAPPER = SCRIPTS / "launch_v506_v505_v503_compact_single_reconciler.py"
DIRECT_WRAPPER_SHA = "0e44e9831cfa4cc5565501378fb3dbe33bcd79ae74c5afc468ed300b2d5d0bb6"
DIRECT_WRAPPER_BYTES = 70823
...
EVIDENCE_ROOT = J / "v506_v505_v503_compact_reconciliation_execution_authority_materialization_evidence_seed1647_20260825"
...
def validate_materializer_contract(module, contract: dict, materializer_record: dict) -> None:
    ...
    if (module.CONTRACT_PATH != CONTRACT or module.AUTHORITY_ROOT != AUTHORITY_ROOT
            ...
            or isinstance(seed, bool) or not isinstance(seed, int) or seed != 1647
            ...
            or contract.get("v505_split_state_forensic") != {
                "path": str(SPLIT_FORENSIC), "sha256": SPLIT_FORENSIC_SHA, "logical_bytes": SPLIT_FORENSIC_BYTES}
            or len(module.AUTH_TOP_KEYS) != 40 or len(module.CHECK_KEYS) != 29
            or len(module.CONTRACT_SOURCE_ORDER) != 7 or len(module.AUTHORITY_SOURCE_ORDER) != 8
...
def load_materializer(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
                      split_source: Path = SPLIT_FORENSIC):
    ...
    split_record = regular(split_source, (SPLIT_FORENSIC_SHA, SPLIT_FORENSIC_BYTES))
    if split_source != SPLIT_FORENSIC:
        split_record = {**split_record, "path": str(SPLIT_FORENSIC)}
    contract = json.loads(contract_source.read_text(encoding="utf-8"))
    spec = importlib.util.spec_from_file_location("v506_materializer", materializer_source)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    validate_materializer_contract(module, contract, materializer_record)
    if contract.get("v505_split_state_forensic") != split_record:
        raise RuntimeError("split forensic current")
...
def validate_success(module, contract: dict, returncode: int, cleanup: dict,
    ...
    if (os.path.lexists(AUTHORITY_PREP)
            or set(receipt) != module.AUTH_TOP_KEYS or len(receipt) != 40
            ...
            or len(receipt.get("source_closure", {})) != 8
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v506-v505-v503-compact-reconciliation-authority-materializer-process-receipt-v1",
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v506-v505-v503-compact-reconciliation-authority-materializer-intent-v1",
...
def production_fixture(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    def rejects(contract_value, module_value=live_module, materializer_value=live_materializer_record):
        try:
            ...
            return True
    tamper_checks = []
    for bad_seed in (1644, 1645, "1647", True, None):
    ...
    with tempfile.TemporaryDirectory(prefix="v506-helper-fixture-") as folder:
        base = Path(folder)
...
def main() -> int:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v506-v505-v503-compact-reconciliation-authority-materializer-argv-v1",
