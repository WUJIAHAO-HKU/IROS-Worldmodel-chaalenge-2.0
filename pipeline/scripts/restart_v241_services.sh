...
V241="$JOINT/v241b_v240_alignment_gated_transport_service_seed1441_20260819"
...
run_service() {
  ...
  exec env PYTHONPATH="$ROOT/pipeline" \
    ...
    WAM_BACKEND='v241-alignment-gated-transport' WAM_CHECKPOINT_DIR="$V209" \
    ...
    "$GO1" pipeline/scripts/serve.py >>"$V241/v241_service.log" 2>&1
}
...
case "${1:-start}" in
  ...
  bridge) run_bridge ;;
  stop)
    screen -S wm_v241_bridge -X quit >/dev/null 2>&1 || true
    screen -S wm_v241_gpu -X quit >/dev/null 2>&1 || true
    ;;
  start)
    for name in wm_v241_bridge wm_v241_gpu wm_v236_bridge wm_v236_gpu wm_v218_bridge wm_v218_gpu; do
