...
restore(){ "$PY" -m ray.scripts.scripts stop --force >/dev/null 2>&1||true;bash "$P/restart_v271_v274_services.sh" stop >/dev/null 2>&1||true;};trap restore EXIT
"$PY" "$P/prepare_v274_retry1_v271_fullbudget_rl.py" >"$OFF/run_registry/$NAME.prepare.tmp.log" 2>&1;mv "$OFF/run_registry/$NAME.prepare.tmp.log" "$REG/prepare.log"
...
export TRACK2_ACTOR_OFFLOAD=true
...
touch "$RUN/audit/V274_RETRY1_TRAINING_ACCEPTED"
test -s "$CHECKPOINT"
echo V274_RETRY1_FULLBUDGET_TRAINING_ACCEPTED
