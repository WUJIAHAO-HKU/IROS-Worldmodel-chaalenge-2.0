#!/usr/bin/env python3
"""Exact-once Stage-A transport for the v532 repaired actual-OOF execution authority."""
...
SELF_PATH = SCRIPTS / "invoke_v532_v531_actual_oof_preregistration_active_source_paths_repair_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v532_v531_actual_oof_preregistration_active_source_paths_repair_execution_authority_materialize_once.sh")
DEPLOYMENT_RECORD = SCRIPTS / "v532_v531_actual_oof_preregistration_active_source_paths_repair_execution_authority_materialization_transport_deployment_record.json"
...
MATERIALIZER = SCRIPTS / "materialize_v532_v531_actual_oof_preregistration_active_source_paths_repair_execution_authority.py"
MATERIALIZER_SHA = "1901ce1583ef5389f958c077e880d0c6b68aef06aa9bf872469228a9869cc78b"
MATERIALIZER_BYTES = 39099
EXECUTOR = SCRIPTS / "execute_v532_v531_actual_oof.py"
EXECUTOR_SHA = "7f5d154a6bd2d4ae94d372091d36fc6bf45d770889e692b6d9e499745f9e9932"
EXECUTOR_BYTES = 62773
AUDITOR = SCRIPTS / "audit_v532_v531_actual_oof.py"
...
FORENSIC = SCRIPTS / "v532_v531_actual_oof_preregistration_active_source_paths_failure_forensic.json"
FORENSIC_SHA = "9f8edc098b04f548fdc0638f5796dd1f4b7d5635b2b3b22a6baadab756563506"
FORENSIC_BYTES = 10911
AUTHORITY_ROOT = J / "v532_v531_actual_oof_preregistration_active_source_paths_repair_execution_authority_seed1666_20260827"
AUTHORITY_PREP = AUTHORITY_ROOT.with_name(AUTHORITY_ROOT.name + ".authority-prep")
EVIDENCE_ROOT = J / "v532_v531_actual_oof_preregistration_active_source_paths_repair_execution_authority_materialization_evidence_seed1666_20260827"
...
def deployment_plan() -> dict:
    rows = [
        ...
        {"role": "v531_failure_forensic", "path": str(FORENSIC), "sha256": FORENSIC_SHA, "logical_bytes": FORENSIC_BYTES},
    ]
    return {
        "fresh_noreplace_targets_exact8": rows,
        "volatile_sources_exact8": [{**row, "source_path": "/dev/shm/" + Path(row["path"]).name} for row in rows],
        "reused_current_only": [],
...
def expected_transport_deployment(helper_record: dict, script_record: dict) -> dict:
    return {
        "format": "strict-track2-v532-stage-a-transport-pair-deployment-record-v1",
...
def validate_materializer_contract(module, contract: dict, materializer_record: dict) -> None:
    ...
    schema_checks = {
        ...
        "seed": type(seed) is int and seed == 1666,
        ...
        "check_count": len(module.CHECK_KEYS) == 35,
        "source_counts": len(module.SOURCE_ROLES) == 29 and len(module.AUTHORITY_SOURCE_ROLES) == 30,
    ...
    if (cbytes(contract.get("authorization")) != cbytes(module.AUTHORIZATION)
        ...
        raise RuntimeError("v532 repaired actual-OOF authority boundary")
    for role, row in contract["source_closure"].items():
...
def validate_success(module, contract: dict, returncode: int, cleanup: dict,
    ...
    if (os.path.lexists(AUTHORITY_PREP)
            ...
            or len(receipt.get("source_closure", {})) != 30
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v532-v531-actual-oof-preregistration-active-source-paths-repair-execution-authority-materializer-process-receipt-v1",
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v532-v531-actual-oof-preregistration-active-source-paths-repair-execution-authority-materializer-intent-v1",
...
def production_fixture(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    checks = {
        ...
        "fresh8_noreplace_plan": (
            len(plan["fresh_noreplace_targets_exact8"]) == 8
            and [row["role"] for row in plan["fresh_noreplace_targets_exact8"]]
        ...
        "authority_schema_62_35_30": (
            len(contract["authority_receipt_contract"]["top_keys"]) == 62
            and len(module.CHECK_KEYS) == 35 and len(module.AUTHORITY_SOURCE_ROLES) == 30),
    ...
    with tempfile.TemporaryDirectory(prefix="v532-stage-a-helper-", dir="/dev/shm") as folder:
        temp = Path(folder)
        ...
        result = orchestrate(
            ...
            argv_payload={"format": "synthetic-v532-stage-a", "pre_snapshot": pre,
                          "transport_deployment_record": deployment_record,
...
def main() -> int:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v532-v531-actual-oof-preregistration-active-source-paths-repair-execution-authority-materializer-argv-v1",
