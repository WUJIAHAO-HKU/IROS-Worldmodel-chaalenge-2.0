...
RUNTIME="$AUDIT/probability_consistent_one_update_v169_runtime_step7_seed1244"
RUN="$AUDIT/runs/v169_parent_step7_seed1244_20260813"
...
SERVICE_DIR="$AUDIT/probability_consistent_service_step7_seed1244"
BRIDGE_DIR="$AUDIT/probability_consistent_bridge_step7_seed1244"
...
test ! -e "$RUNTIME"; test ! -e "$RUN"; test ! -e "$SERVICE_DIR"; test ! -e "$BRIDGE_DIR"
...
"$PY" "$RUNTIME/examples/embodiment/train_embodied_agent.py" \
  ...
  runner.logger.log_path="$RUN" runner.max_steps=1 runner.max_epochs=1000 runner.save_interval=1 runner.ckpt_path="$AUDIT/runs/v169_parent_step5_seed1243_20260813/wan_robotwin_adjust_bottle_http_full_grpo_openpi_pi05/checkpoints/global_step_1/actor/model_state_dict/full_weights.pt" \
  actor.seed=1244 actor.enable_offload=true rollout.enable_offload=true \
...
printf 'V169_PROBABILITY_CONSISTENT_ONE_UPDATE_COMPLETE_NONFORMAL\n'
