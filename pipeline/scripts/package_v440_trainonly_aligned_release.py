#!/usr/bin/env python3
"""Package a passed v440 S0 core; never start service, trace, or RL."""
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=J / "v440_v169_trainonly_aligned_projection_release")
    ...
    if index.get("format") != "strict-track2-v440-trainonly-rgb-alignment-index-v1":
        raise RuntimeError("wrong v440 alignment index")
    if s0.get("format") != "strict-track2-v440-s0-static-contract-v1" or s0.get("passed") is not True:
        raise RuntimeError("v440 S0 did not pass")
    if s0.get("sha256", {}).get("alignment_index") != sha256(args.alignment_index):
        raise RuntimeError("v440 S0/index binding mismatch")
    ...
    if prereg.get("evidence_sha256", {}).get("runtime") != sha256(runtime):
        raise RuntimeError("v440 runtime differs from preregistration")
    ...
    manifest = {
        ...
        "runtime_class": "wam_pipeline.v440_v169_trainonly_aligned_projection_runtime.Track2V440V169TrainOnlyAlignedProjection",
    ...
    manifest_path = args.output / "v440_aligned_projection_manifest.json"
