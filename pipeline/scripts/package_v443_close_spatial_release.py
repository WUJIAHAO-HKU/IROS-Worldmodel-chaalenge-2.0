#!/usr/bin/env python3
"""Package a passed v443 S0 core; never start service, trace, or RL."""
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=J / "v443_v169_close_spatial_projection_release")
    args = parser.parse_args()
    if args.output.exists():
        raise FileExistsError(args.output)
    prereg = json.loads(args.preregistration.read_text())
    with np.load(args.alignment_index, allow_pickle=False) as archive:
        index = json.loads(str(archive["metadata_json"].item()))
    ...
    if index.get("format") != "strict-track2-v443-close-trainonly-spatial-alignment-index-v1":
        raise RuntimeError("wrong v443 alignment index")
    if s0.get("format") != "strict-track2-v443-close-s0-static-contract-v1" or s0.get("passed") is not True:
        raise RuntimeError("v443 S0 did not pass")
    if s0.get("sha256", {}).get("alignment_index") != sha256(args.alignment_index):
        raise RuntimeError("v443 S0/index binding mismatch")
    runtime = ROOT / "pipeline/wam_pipeline/v443_v169_close_spatial_projection_runtime.py"
    ...
    if prereg.get("evidence_sha256", {}).get("runtime") != sha256(runtime):
        raise RuntimeError("v443 runtime differs from preregistration")
    ...
    os.symlink(args.alignment_index.resolve(), args.output / "alignment_index.npz")
    ...
    manifest = {
        ...
        "v436_release": "v436_release", "alignment_index": "alignment_index.npz",
        "runtime_class": "wam_pipeline.v443_v169_close_spatial_projection_runtime.Track2V443V169CloseSpatialProjection",
        ...
        "formula": {
            ...
            "beta": "train-only 15-fold LOEO time/pixel/RGB coefficient with abs<=1",
            "output": "round_clip_uint8(B + g * beta_map * D)",
            "protected_frames_exact_zero": [0, 1, 6, 7],
            "morphology_or_smoothing": False,
            "gate": "v442 close-only explicit-right + right7D-path>left7D + history-last-open + future-close-then-held; no mu",
        },
        "s1_action_coverage_contract": {
            "fixed_right_dev_samples": 32, "decisions": 128,
            "active_exact": 8, "distinct_samples_exact": 8,
            "active_phase": "grasp only", "chunk_pattern": [1, 0, 0, 0],
            "other_phase_or_chunk_active_exact": 0, "repeat_active_exact": 0,
        },
        "official_reward_runtime_used": False,
        "sha256": {
            "alignment_index": sha256(args.alignment_index), "v169_manifest": sha256(v169_manifest),
            "v436_manifest": sha256(v436_manifest), "runtime": sha256(runtime),
        ...
        "guards": {
            ...
            "active_pixels_min": 64, "aggregate_train_loeo_sse_ratio_max_exclusive": 1.0,
    ...
    manifest_path = args.output / "v443_close_spatial_manifest.json"
