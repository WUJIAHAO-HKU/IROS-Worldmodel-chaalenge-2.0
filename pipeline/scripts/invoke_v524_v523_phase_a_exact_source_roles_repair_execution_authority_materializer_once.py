#!/usr/bin/env python3
"""Exact-once transport for the v524 ordered-source-roles repair authority."""
...
SELF_PATH = SCRIPTS / "invoke_v524_v523_phase_a_exact_source_roles_repair_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v524_v523_phase_a_exact_source_roles_repair_authority_materialize_once.sh")
CONTRACT = SCRIPTS / "v524_v523_phase_a_exact_source_roles_repair_execution_authority_contract.json"
CONTRACT_SHA = "f3fdd5851231a984aa1ce04685a5433fd5f99e73065271969c30c62597c9b95d"
CONTRACT_BYTES = 39332
MATERIALIZER = SCRIPTS / "materialize_v524_v523_phase_a_exact_source_roles_repair_execution_authority.py"
MATERIALIZER_SHA = "1b33ba41297a998379cceebe3fcb4085a59f017d46dc481fdc2d3859622767dd"
MATERIALIZER_BYTES = 35575
PHASE_A_LAUNCHER = SCRIPTS / "launch_v524_v523_phase_a_cache_qualification.py"
PHASE_A_LAUNCHER_SHA = "ad57a6669f037f84fda54b75c787e4ce91575780ca61c1bb40335f00ada7eb04"
PHASE_A_LAUNCHER_BYTES = 81576
PHASE_A_DRIVER = SCRIPTS / "generate_v524_v523_phase_a_cache_qualification.py"
PHASE_A_DRIVER_SHA = "c9f7c836781bb8311a37024068be6e5d080a5b8e56c44fffc2cef85002447e64"
PHASE_A_DRIVER_BYTES = 48394
PHASE_A_WORKER = SCRIPTS / "generate_v524_v523_phase_a_cache_qualification_worker.py"
PHASE_A_WORKER_SHA = "51653b8cebea6fa5f75ccb5a3f3ffc4d3dd4bfc6436fac3c49a494102556252a"
PHASE_A_WORKER_BYTES = 40272
...
INDEPENDENT_AUDITOR = SCRIPTS / "audit_v524_v523_phase_a_rng_isolated_qualification.py"
INDEPENDENT_AUDITOR_SHA = "fce5b2bfa906183f49c1fe96e4bae88bddf8efb891917a56e2bfe0a7804b5078"
INDEPENDENT_AUDITOR_BYTES = 48260
...
PREREG_BYTES = 57506
SPLIT_FORENSIC = SCRIPTS / "v524_v523_phase_a_exact_source_roles_failure_forensic.json"
SPLIT_FORENSIC_SHA = "bda89f4244031c8b3719868a78477a5251ac864e399df0e8b47a619c8125d9f0"
SPLIT_FORENSIC_BYTES = 5650
...
EVIDENCE_ROOT = J / "v524_v523_phase_a_exact_source_roles_repair_execution_authority_materialization_evidence_seed1660_20260826"
...
FROZEN_FINAL9_SHA256 = (
    "f3fdd5851231a984aa1ce04685a5433fd5f99e73065271969c30c62597c9b95d",
    "1b33ba41297a998379cceebe3fcb4085a59f017d46dc481fdc2d3859622767dd",
    "ad57a6669f037f84fda54b75c787e4ce91575780ca61c1bb40335f00ada7eb04",
    "c9f7c836781bb8311a37024068be6e5d080a5b8e56c44fffc2cef85002447e64",
    "51653b8cebea6fa5f75ccb5a3f3ffc4d3dd4bfc6436fac3c49a494102556252a",
    "2b8ab0e2aec4a8de5d4bf8cffbb3525bbcd69b9b03270012a4a48f69f430b51b",
    "fce5b2bfa906183f49c1fe96e4bae88bddf8efb891917a56e2bfe0a7804b5078",
    "133c13ffc994671f2309d98ca294f47e0b838e291156d1523f211dc9f3c60da7",
    "bda89f4244031c8b3719868a78477a5251ac864e399df0e8b47a619c8125d9f0",
)
EXACT_PREREG_ROLES = [
...
def validate_ordered8(formal: dict) -> None:
    records = formal.get("execution_source_records")
    roles = [row.get("role") for row in records] if isinstance(records, list) else None
    exact_roles = formal.get("phase_a_output_and_receipt_schema", {}).get("source_closure_contract", {}).get("exact_roles")
    sources = formal.get("execution_sources")
    if roles != EXACT_PREREG_ROLES or exact_roles != EXACT_PREREG_ROLES or not isinstance(sources, dict) or set(sources) != set(EXACT_PREREG_ROLES) or formal.get("execution_sources_digest_sha256") != csha(records):
        raise RuntimeError("ordered exact8 roles/digest")
    for index, role in enumerate(EXACT_PREREG_ROLES):
        row = records[index]
        if not isinstance(row, dict) or set(row) != {"role", "path", "sha256", "logical_bytes"} or type(row.get("logical_bytes")) is not int or row["logical_bytes"] < 0 or sources.get(role) != {key: value for key, value in row.items() if key != "role"}:
            raise RuntimeError("ordered exact8 record bijection")
...
def deployment_plan() -> dict:
    return {
        "fresh_noreplace_targets_exact8": [
            ...
            {"role": "v524_v523_phase_a_failure_forensic", "path": str(SPLIT_FORENSIC), "sha256": SPLIT_FORENSIC_SHA, "logical_bytes": SPLIT_FORENSIC_BYTES},
        ],
...
def v524_phase_a_pids() -> list[dict]:
...
def immutable_snapshot(contract: dict, helper_record: dict, script_record: dict) -> dict:
    ...
    return {
        ...
        "v524_phase_a_pids": v524_phase_a_pids(), "deployment_plan": deployment_plan(),
    }
...
def validate_materializer_contract(module, contract: dict, materializer_record: dict) -> None:
    ...
    active = {
        ...
        "v524_v523_phase_a_failure_forensic": {"path": str(SPLIT_FORENSIC), "sha256": SPLIT_FORENSIC_SHA, "logical_bytes": SPLIT_FORENSIC_BYTES},
    }
    if (module.CONTRACT_PATH != CONTRACT or module.AUTHORITY_ROOT != AUTHORITY_ROOT
            ...
            or type(seed) is not int or seed != 1660
    ...
    formal = module.validate_prereg(closure)
    validate_ordered8(formal)
    ...
    if (csha(contract.get("worker_environment_exact")) != csha(module.WORKER_ENV)
        ...
        raise RuntimeError("v524 execution/failure-repair contract")
...
def load_materializer(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    spec = importlib.util.spec_from_file_location("v524_materializer", materializer_source)
...
def validate_success(module, contract: dict, returncode: int, cleanup: dict,
    ...
    validate_ordered8(module.validate_prereg(receipt["source_closure"]))
    split_keys = (
        "v523_authority_registration_tree", "v523_authority_receipt", "v523_authority_semantics",
        "v523_attempt_tree", "v523_attempt_intent", "v523_attempt_stdout", "v523_attempt_stderr",
        "v523_attempt_terminal", "v523_phase_a_failure_forensic", "v523_phase_a_execution_partition",
        "v523_exact_source_roles_failure_root_cause",
    )
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v524-v523-phase-a-exact-source-roles-repair-execution-authority-materializer-process-receipt-v1",
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v524-v523-phase-a-exact-source-roles-repair-execution-authority-materializer-intent-v1",
...
def production_fixture(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    checks["evidence_format_lineage_exact"] = source.count(
        "strict-track2-v524-v523-phase-a-exact-source-roles-repair-execution-authority-materializer-") == 4
    ...
    checks["services_pid_gpu_in_stable_snapshot"] = all(value in source for value in (
        '"service_health": service_health()', '"gpu_compute_pids": gpu_compute_pids()',
        '"v524_phase_a_pids": v524_phase_a_pids()'))
    ...
    checks["old_v523_authority_and_failed_attempt_current"] = (
        exact_tree(Path(live_contract["v523_authority_registration_tree"]["root"]))
            == live_contract["v523_authority_registration_tree"] == old_state["authority_tree"]
        and exact_tree(Path(live_contract["v523_attempt_tree"]["root"]))
            == live_contract["v523_attempt_tree"] == old_state["attempt_tree"])
    def rejects(contract_value, module_value=live_module, materializer_value=live_materializer_record):
        try:
            ...
            return True
    tamper_checks = {}
    for bad_seed in (1659, 1661, "1660", True, None):
    ...
    bad_format = copy.deepcopy(live_contract)
    bad_format["format"] = "strict-track2-v524-v523-phase-a-per-call-rng-contract-repair-execution-authority-design-contract-v1"
    tamper_checks["stale_format"] = rejects(bad_format)
    ...
    bad_partition_bool["v523_phase_a_execution_partition"]["launcher_invocations"] = True
    tamper_checks["partition_bool_int"] = rejects(bad_partition_bool)
    module_values = {name: getattr(live_module, name) for name in (
        "CONTRACT_PATH", "AUTHORITY_ROOT", "CONTRACT_FORMAT", "CONTRACT_STATUS", "CONTRACT_TOP_KEYS", "CHECK_KEYS",
    ...
    formal = live_module.validate_prereg(live_contract["source_closure"])
    def formal_rejects(name, mutate):
        candidate = copy.deepcopy(formal); mutate(candidate)
        try:
            validate_ordered8(candidate)
            tamper_checks[name] = False
        except RuntimeError:
            tamper_checks[name] = True
    formal_rejects("formal_legacy_alias", lambda value: value["phase_a_output_and_receipt_schema"]["source_closure_contract"]["exact_roles"].__setitem__(0, "phase_a_materializer"))
    formal_rejects("formal_missing", lambda value: value["phase_a_output_and_receipt_schema"]["source_closure_contract"]["exact_roles"].pop())
    formal_rejects("formal_extra", lambda value: value["phase_a_output_and_receipt_schema"]["source_closure_contract"]["exact_roles"].append("unexpected"))
    formal_rejects("formal_order", lambda value: value["phase_a_output_and_receipt_schema"]["source_closure_contract"]["exact_roles"].__setitem__(slice(0, 2), list(reversed(value["phase_a_output_and_receipt_schema"]["source_closure_contract"]["exact_roles"][:2]))))
    formal_rejects("formal_same_count", lambda value: value["phase_a_output_and_receipt_schema"]["source_closure_contract"]["exact_roles"].__setitem__(3, "phase_a_rng_proxy"))
    formal_rejects("formal_bool_int", lambda value: value["execution_source_records"][0].__setitem__("logical_bytes", True))
    checks["actual_load_materializer_tamper_suite"] = all(tamper_checks.values()) and len(tamper_checks) == 20
    for name, passed in tamper_checks.items():
        checks[f"contract_tamper_rejected_{name}"] = passed
    with tempfile.TemporaryDirectory(prefix="v524-helper-fixture-") as folder:
        base = Path(folder)
...
def main() -> int:
    ...
    if pre["gpu_compute_pids"] or pre["v524_phase_a_pids"]:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v524-v523-phase-a-exact-source-roles-repair-execution-authority-materializer-argv-v1",
