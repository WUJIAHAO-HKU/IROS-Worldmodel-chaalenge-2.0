...
SELF_PATH = SCRIPTS / "invoke_v525_v524_phase_a_worker_receipt_format_readonly_reconciliation_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v525_v524_phase_a_worker_receipt_format_readonly_reconciliation_authority_materialize_once.sh")
...
EVIDENCE_ROOT = J / "v525_v524_phase_a_worker_receipt_format_readonly_reconciliation_execution_authority_materialization_evidence_seed1661_20260827"
    ...
    try:
        ...
        lines = [
            ...
            {"line_origin": "transport_helper", "passed": True,
              ...
              "readonly_reconciler_invocations": 0,
...
def validate_success(module, contract: dict, returncode: int, cleanup: dict,
    ...
    if (os.path.lexists(AUTHORITY_PREP)
            or sorted(receipt) != receipt_schema["top_keys"] or len(receipt) != 55
            ...
            or len(receipt.get("source_closure", {})) != 17
            ...
            or cbytes(receipt.get("reconciler_candidate_contract")) != cbytes(module.RECONCILER_CANDIDATE_CONTRACT)
            or cbytes(receipt.get("external_terminal_evidence_contract")) != cbytes(module.EXTERNAL_TERMINAL_EVIDENCE_CONTRACT)
    ...
    module.validate_state(contract)
    if any(cbytes(receipt.get(key)) != cbytes(contract.get(key)) for key in (
            "v524_authority_registration_tree", "v524_authority_receipt", "v524_authority_semantics",
            "v524_attempt_tree", "v524_attempt_intent", "v524_attempt_stdout", "v524_attempt_stderr",
            "v524_attempt_terminal", "v524_execution_partition", "v524_qualification_tree",
            "v524_qualification_terminal", "v524_qualification_report",
            "v524_qualification_independent_audit", "v524_process_a_receipt", "v524_process_b_receipt",
            "normalization_contract")):
        raise RuntimeError("authority v524 closure anchors")
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v525-v524-phase-a-worker-receipt-format-readonly-reconciliation-execution-authority-materializer-process-receipt-v1",
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v525-v524-phase-a-worker-receipt-format-readonly-reconciliation-execution-authority-materializer-intent-v1",
...
def production_fixture(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    checks = {
        ...
        "reconciler_and_phase_a_not_in_child_argv": (
            str(RECONCILER) not in command()
    ...
    with tempfile.TemporaryDirectory(prefix="v525-stage-a-helper-", dir="/dev/shm") as folder:
        temp = Path(folder)
        ...
        result = orchestrate(
            ...
            argv_payload={"format": "synthetic-v525-stage-a", "pre_snapshot": pre,
        ...
        checks["mapped_normal_exact6"] = (
            ...
            and result["process_receipt"]["readonly_reconciler_invocations"] == 0)
...
def main() -> int:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v525-v524-phase-a-worker-receipt-format-readonly-reconciliation-execution-authority-materializer-argv-v1",
