...
restart_v206() {
  bash "$P/restart_v206_services.sh" start > "$REG/restart_v206_after_pipeline.log" 2>&1 || true
}
trap restart_v206 EXIT

curl -fsS http://127.0.0.1:18083/health | grep -q '"status":"ready"'
curl -fsS http://127.0.0.1:8004/v1/health | grep -q "$MODEL_VERSION"
...
export TRACK2_ACTOR_SEED=1406 TRACK2_ENV_SEED=0
...
"$PY" "$P/prepare_p3_public_policy_screen.py" \
  ...
  --variant "$VARIANT" \
  --output "$REG/public_policy_screen_preregistration.json" \
...
for batch in 00 01; do
  ...
  TRACK2_DEV_BATCH_FILTER="$batch" \
    bash "$P/run_strict_track2_dev_eval.sh"
done
...
echo V207_PUBLIC_POLICY_STAGE_B_PASSED
