#!/usr/bin/env python3
"""Public-only causal, reward-strength, and non-regression audit for v322."""
...
from wam_pipeline.v322_causal_alpha_terminal_runtime import Track2V322CausalAlphaTerminal
...
def main() -> int:
    ...
    candidate = Track2V322CausalAlphaTerminal(
        args.checkpoint_dir, args.library_index, args.device, args.action_gate
    )
    ...
    checks = {
        ...
        "validation_positive_reward_mean_ge_0p95": validation["positive_reward_mean"] >= 0.95,
        "local_test_positive_reward_mean_ge_0p85": local_test["positive_reward_mean"] >= 0.85,
        "validation_positive_hit_rate_ge_0p90": validation["positive_hit_rate_at_0p9"] >= 0.90,
        "local_test_positive_hit_rate_ge_0p80": local_test["positive_hit_rate_at_0p9"] >= 0.80,
        ...
        "validation_all_window_rgb_ratio_le_1p05": all_summary["validation"]["terminal_rgb"]["mae_ratio"] <= 1.05,
        "local_test_all_window_rgb_ratio_le_1p05": all_summary["local_test"]["terminal_rgb"]["mae_ratio"] <= 1.05,
    ...
    report = {
        "format": "strict-track2-v322-causal-alpha-terminal-gate-v1",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "candidate": "track2-v322-causal-alpha-terminal-v317",
        ...
        "authorization": {
            ...
            "right_terminal_ranking": all(
                checks[name]
                for name in (
                    "validation_positive_reward_mean_ge_0p95",
                    "local_test_positive_reward_mean_ge_0p85",
                    "validation_positive_hit_rate_ge_0p90",
                    "local_test_positive_hit_rate_ge_0p80",
