#!/usr/bin/env bash
# Resumable Phase-B temporal200 collection. Never starts training/S1/RL.
set -euo pipefail
ROOT='/root/autodl-tmp/IROS_WAM_2.0 challenge'
J="$ROOT/artifacts/strict_track2_joint_augmentation_20260810"
S="$ROOT/pipeline/scripts"
REG="$J/v478_temporal200_seed1622_20260824"
PRE="$REG/phase_b_preregistration.json"
SEL="$REG/selection.json"
...
AUD="$S/audit_v478_temporal200_sharded.py"
...
exec 9>/var/lock/v478_temporal200_phase_b.lock
flock -n 9 || exit 73
START_EPOCH=$(date +%s)
...
root=Path(sys.argv[1]).resolve(); launcher,pre,gen,aud,col=map(Path,sys.argv[2:7]);total=0;successes=[];terminal_failures=[]
def sha(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
...
test "$(sha256sum "$PRE"|awk '{print $1}')" = '5d6db6e5aca4502b79347620a7732e4471f63b77780ad7228bcd53c6cb753ad9'
test "$(sha256sum "$GEN"|awk '{print $1}')" = 'a20f5e9d702fa6febe946d9f713f4713ef035705b35119c00ca328046da70a58'
test "$(sha256sum "$AUD"|awk '{print $1}')" = '2e1adbe72e2d77d19a2bf6f4a3d0822940ca458055851c36e06b28fe62c6c748'
...
assert pre['guards']=={'effect_used_for_retry_filter_batch_gate':False,'hidden_final_submission_used':False,'launcher_bound_in_preregistration':False,'policy_updates':0,'rl_authorized':False,'s1_authorized':False,'training_authorized':False,'v479_phase_b_authorization_bound':True,'zero_update_authorized':False}
PY
within_whole
