...
WINDOWS="$JOINT/pilot_batch00_windows_stride4"
OUTPUT="$JOINT/pilot_parent_onpolicy_expert_closedloop8_400"
...
exec "$PYTHON_BIN" pipeline/scripts/train_autoregressive_unet.py \
  ...
  --statistics-cache "$JOINT/pilot_onpolicy_expert_training_stats.npz" \
  --seed 20260810 \
  "${RESUME[@]}"
