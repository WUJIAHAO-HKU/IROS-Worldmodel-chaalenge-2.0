...
NAME="${TRACK2_V259_NAME:-v259_v169step5_rightterminal_chunk8_sft128_h8_step1_lr5e6_seed1461_20260819}"
VARIANT="${TRACK2_V259_VARIANT:-v259_rightterminal_chunk8_sft128_seed1461}"
RUN="$OFF/runs/$NAME"
REG="$OFF/run_registry/$NAME"
BASE_CKPT="${TRACK2_V259_BASE_CKPT:-$OFF/runs/v169_parent_step5_seed1243_20260813/wan_robotwin_adjust_bottle_http_full_grpo_openpi_pi05/checkpoints/global_step_1/actor/model_state_dict/full_weights.pt}"
DATA="${TRACK2_V259_DATA:-/root/autodl-tmp/iros_v15_rl_probability_audit_v2/artifacts/lerobot/adjust_bottle_train40_right_terminal_v1}"
DATA_AUDIT="${TRACK2_V259_DATA_AUDIT:-$OFF/run_registry/v225_right_terminal_dataset_20260818/dataset_audit.json}"
CONVERSION="${TRACK2_V259_CONVERSION:-$DATA/conversion_summary.json}"
TEMPORAL_AUDIT="${TRACK2_V259_EVIDENCE_AUDIT:-$OFF/diagnostics/right_terminal_temporal_loss_audit_20260819.json}"
PREPARE_SCRIPT="${TRACK2_V259_PREPARE_SCRIPT:-prepare_v259_chunk8_right_sft.py}"
ACTOR_SEED="${TRACK2_V259_ACTOR_SEED:-1461}"
EXTRA_UPDATES="${TRACK2_V259_EXTRA_UPDATES:-128}"
ACTOR_LR="${TRACK2_V259_ACTOR_LR:-5e-6}"
KL_BETA="${TRACK2_V259_KL_BETA:-0.1}"
SFT_LOSS_WEIGHT="${TRACK2_V259_SFT_LOSS_WEIGHT:-0.3}"
SFT_EXTRA_LOSS_WEIGHT="${TRACK2_V259_SFT_EXTRA_LOSS_WEIGHT:-1.0}"
SFT_BATCH_SIZE="${TRACK2_V259_SFT_BATCH_SIZE:-2}"
SFT_SINGLE_ACTIVE_ARM="${TRACK2_V259_SFT_SINGLE_ACTIVE_ARM:-1}"
USE_STRUCTURED_SFT_ACTION_LOSS="${TRACK2_V259_USE_STRUCTURED_SFT_ACTION_LOSS:-true}"
USE_MIXED_ARM_STRUCTURED_SFT_ACTION_LOSS="${TRACK2_V259_USE_MIXED_ARM_STRUCTURED_SFT_ACTION_LOSS:-false}"
SFT_PHYSICAL_ACTION_DIM="${TRACK2_V259_SFT_PHYSICAL_ACTION_DIM:-}"
ACTIVE_JOINT_WEIGHT="${TRACK2_V259_ACTIVE_JOINT_WEIGHT:-1.0}"
ACTIVE_GRIPPER_WEIGHT="${TRACK2_V259_ACTIVE_GRIPPER_WEIGHT:-3.0}"
INACTIVE_KEEP_WEIGHT="${TRACK2_V259_INACTIVE_KEEP_WEIGHT:-0.25}"
GRIPPER_ACTIVITY_WEIGHT="${TRACK2_V259_GRIPPER_ACTIVITY_WEIGHT:-0.25}"
...
"$PY" "$P/$PREPARE_SCRIPT" \
  ...
  "$TEMPORAL_AUDIT" "$ACTOR_SOURCE" "$RUNNER" "$NAME" \
...
export TRACK2_ACTOR_SEED="$ACTOR_SEED" TRACK2_ENV_SEED=0
export TRACK2_ACTOR_LR="$ACTOR_LR" TRACK2_KL_BETA="$KL_BETA" TRACK2_KL_PENALTY=low_var_kl
...
export TRACK2_SFT_LOSS_WEIGHT="$SFT_LOSS_WEIGHT" TRACK2_SFT_BATCH_SIZE="$SFT_BATCH_SIZE" TRACK2_SFT_NUM_WORKERS=0
export TRACK2_SFT_SINGLE_ACTIVE_ARM="$SFT_SINGLE_ACTIVE_ARM" TRACK2_USE_STRUCTURED_SFT_ACTION_LOSS="$USE_STRUCTURED_SFT_ACTION_LOSS"
export TRACK2_USE_MIXED_ARM_STRUCTURED_SFT_ACTION_LOSS="$USE_MIXED_ARM_STRUCTURED_SFT_ACTION_LOSS"
export TRACK2_SFT_PHYSICAL_ACTION_DIM="$SFT_PHYSICAL_ACTION_DIM"
export TRACK2_SFT_USE_ACTION_CHUNK_LOSS=true
export TRACK2_SFT_ACTIVE_JOINT_WEIGHT="$ACTIVE_JOINT_WEIGHT" TRACK2_SFT_ACTIVE_GRIPPER_WEIGHT="$ACTIVE_GRIPPER_WEIGHT"
export TRACK2_SFT_INACTIVE_KEEP_WEIGHT="$INACTIVE_KEEP_WEIGHT"
export TRACK2_SFT_GRIPPER_ACTIVITY_WEIGHT="$GRIPPER_ACTIVITY_WEIGHT"
export TRACK2_SFT_EXTRA_UPDATES_PER_GLOBAL_BATCH="$EXTRA_UPDATES" TRACK2_SFT_EXTRA_LOSS_WEIGHT="$SFT_EXTRA_LOSS_WEIGHT"
...
touch "$RUN/audit/V259_EXTRA_SFT_ACCEPTED"
...
echo V259_PUBLIC_RIGHT_R1_PASSED
