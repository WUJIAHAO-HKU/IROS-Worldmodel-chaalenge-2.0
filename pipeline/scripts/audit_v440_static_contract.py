#!/usr/bin/env python3
"""S0 audit for v440 train-only alignment, runtime purity, and RGB bounds."""
...
def main() -> int:
    ...
    close_future = np.zeros((8, 14), dtype=np.float32); close_future[:, 13] = 1.0
    close_future[2:, 13] = 0.25
    close_future[:, 7:13] = np.linspace(0, 1, 8)[:, None] * np.asarray([-0.330, -1.425, -1.563, 1.617, 0.494, 0.779])[None]
    ...
    checks = {
        "index_format": index.get("format") == "strict-track2-v440-trainonly-rgb-alignment-index-v1",
        "preregistration_format": prereg.get("format") == "strict-track2-v440-trainonly-alignment-preregistration-v1",
        ...
        "exact_120_samples": len(index.get("samples", [])) == 120,
        ...
        "close_gate_fixture": close.get("gate") is True and close.get("phase") == "close",
    ...
    report = {
        "format": "strict-track2-v440-s0-static-contract-v1",
        "passed": passed,
        "checks": checks,
        "fixtures": {"close": close, "postclose": postclose, "left": left},
        "sha256": {
