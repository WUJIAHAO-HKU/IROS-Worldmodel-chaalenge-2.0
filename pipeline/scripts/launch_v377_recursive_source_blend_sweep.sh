#!/usr/bin/env bash
set -euo pipefail
ROOT='/root/autodl-tmp/IROS_WAM_2.0 challenge'
J="$ROOT/artifacts/strict_track2_joint_augmentation_20260810"
RUN="$J/v377_recursive_source_blend_seed1540_20260823"
PY=/root/miniconda3/envs/go1/bin/python
test -s "$RUN/preregistration.json"
test ! -e "$RUN/audit/source_blend_sweep.json"
export PYTHONPATH="$ROOT/pipeline"
export OMP_NUM_THREADS=4 MKL_NUM_THREADS=4 TOKENIZERS_PARALLELISM=false
taskset -c 0-7 "$PY" "$ROOT/pipeline/scripts/audit_v377_recursive_source_blend_sweep.py" \
  --baseline-release "$J/v355_v202_v354_parametric_arm_routed_release" \
  --v375-release "$J/v375_bounded_cartesian_phase_pilot_seed1538_20260823/release" \
  --library "$J/v214_public_right_knn_action_visual_diagnostic_seed1413/library/public_right_knn.npz" \
  --gate "$J/v339_high_specificity_recursive_ood_gate_seed1508_20260822/recursive_ood_gate.npz" \
  --windows "$ROOT/artifacts/adjust_bottle_windows_full" \
  --instruction-map "$J/v205_v202_public_right_terminal_multichunk_seed1405/public_demo_split.json" \
  --preregistration "$RUN/preregistration.json" \
  --output "$RUN/audit/source_blend_sweep.json" --device cuda --batch-size 8 \
  >"$RUN/audit/source_blend_sweep.log" 2>&1
touch "$RUN/SOURCE_BLEND_SWEEP_COMPLETE"
printf 'V377_SOURCE_BLEND_SWEEP_COMPLETE\n'
