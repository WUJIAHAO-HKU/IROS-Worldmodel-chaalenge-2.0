#!/usr/bin/env python3
"""Calibrate v440 phase/frame/channel coefficients on public right train15 only."""
...
def stable_seed(episode: int, start: int) -> int:
    payload = f"v440-trainonly/episode{episode}/start{start}/seed1586"
...
def select_samples(dataset: WindowDataset, episodes: list[int], prompts: dict[int, str]) -> list[dict]:
    ...
    for episode in episodes:
        for phase in PHASES:
            ...
            if len(candidates) < SAMPLES_PER_EPISODE_PHASE:
                raise RuntimeError(f"v440 requires four gated {phase} samples in train episode {episode}, got {len(candidates)}")
            ...
            if len({index for _, index in chosen}) != SAMPLES_PER_EPISODE_PHASE:
                raise RuntimeError("v440 deterministic calibration selection duplicated a row")
    ...
    if len(rows) != expected:
        raise RuntimeError(f"v440 expected {expected} calibration rows, got {len(rows)}")
    return rows
...
def main() -> int:
    ...
    if args.inference_batch_size != INFERENCE_BATCH_SIZE:
        raise SystemExit("v440 calibration is preregistered for inference batch size 4")
    prereg = json.loads(args.preregistration.read_text())
    if prereg.get("format") != "strict-track2-v440-trainonly-alignment-preregistration-v1":
        raise RuntimeError("wrong v440 preregistration")
    ...
    if len(train) != 40 or len(validation) != 10 or len(right_train) != 15 or set(train) & set(validation):
        raise RuntimeError("v440 requires the immutable public train40/right15 boundary")
    if any("right arm" not in prompts[episode].lower() or "left arm" in prompts[episode].lower() for episode in right_train):
        raise RuntimeError("v440 right-train prompt contract failed")
    ...
    if v436.get("format") != "track2-v436-v432-step25-diagnostic-release-v1":
        raise RuntimeError("wrong v440 teacher release")
    ...
    if not np.all(counts == SAMPLES_PER_EPISODE_PHASE):
        raise RuntimeError(f"v440 calibration count drift: {counts.tolist()}")
    ...
    for phase, p in phase_slot.items():
        ...
        if int(active.sum()) == 0:
            raise RuntimeError(f"v440 LOEO retained no stable {phase} component")
        evidence[phase] = {
    ...
    payload = {
        "format": "strict-track2-v440-trainonly-rgb-alignment-index-v1",
