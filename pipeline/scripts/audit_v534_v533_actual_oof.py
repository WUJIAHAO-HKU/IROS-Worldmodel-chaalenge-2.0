#!/usr/bin/env python3
"""Independent in-boundary auditor for v534 environment-repaired actual-OOF output."""
...
FORMAT = "strict-track2-v534-v533-actual-oof-independent-audit-v1"
...
def audit_staged(staged_root: Path, manifest: dict, cache_path: Path) -> dict:
    ...
    if (manifest.get("active_source_role_order") != ACTIVE_SOURCE_ROLE_ORDER
            ...
            or json.dumps(manifest.get("execution_contract", {}).get("required_environment"), sort_keys=True, separators=(",", ":"))
               != json.dumps(REQUIRED_ENVIRONMENT_EXACT, sort_keys=True, separators=(",", ":"))):
