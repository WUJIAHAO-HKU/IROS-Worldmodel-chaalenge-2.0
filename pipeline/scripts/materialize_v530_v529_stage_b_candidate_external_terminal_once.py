...
SELF=S/'materialize_v530_v529_stage_b_candidate_external_terminal_once.py'
SCRIPT=Path('/root/v530_v529_stage_b_candidate_external_terminal_once.sh')
DEPLOY=S/'v530_v529_stage_b_candidate_external_terminal_transport_deployment_record.json'
DESIGN=S/'v529_v527_stage_b_candidate_external_terminal_design_contract.json'
MAPPED_FORENSIC=S/'v529_v527_stage_b_mapped_fixture_default_argument_canonical_publication_forensic.json'
FAILURE_FORENSIC=S/'v530_v529_stage_b_post_selftest_wrong_path_gate_failure_forensic.json'
...
FAILURE_FORENSIC_EXPECTED=('1665b75c4612e3f2c43fcc9791f775e44ed17f1d7454e83251445578fd0f9266',11009)
...
OUT=J/'v530_v529_stage_b_candidate_external_terminal_evidence_seed1664_20260827';PREP=OUT.with_name(OUT.name+'.evidence-prep')
...
def strict_int(v,expected,label):
 if type(v) is not int or v!=expected:raise RuntimeError(label)
def validate_failure_value(v):
 exact_keys(v,{'format','status','classification','seed','retry_authorized','failure_cause','execution_sequence','transport_selftest_tool_record','post_selftest_readonly_gate_tool_record','final5_current','candidate_current','fresh_external_output_current','current_closure','audit'},'failure top')
 required={'format':'strict-track2-v530-v529-stage-b-post-selftest-wrong-path-gate-failure-forensic-v1','status':'transport_selftest_passed_then_readonly_wrong_path_gate_failed_no_normal_no_retry','classification':'immutable_post_selftest_outer_collection_failure_forensic','seed':1664,'retry_authorized':False,'failure_cause':'post_selftest_readonly_candidate_digest_gate_used_wrong_job_runs_root_instead_of_canonical_artifacts_root'}
 if any(type(v.get(k)) is not type(x) or v.get(k)!=x for k,x in required.items()):raise RuntimeError('failure header')
 seq=v['execution_sequence'];exact_keys(seq,{'atomic_noreplace_final5_deployment_completed','transport_selftest_issued_invocations','transport_selftest_native_returncode','post_selftest_readonly_gate_issued_invocations','post_selftest_readonly_gate_native_returncode','stage_b_normal_invocations','external_terminal_helper_invocations','current_oof_readonly_gate_validator_invocations','historical_oof_readonly_gate_validator_invocations','oof_model_execution_invocations','phase_a_invocations','training_invocations','cache_reuse_invocations','reward_read_invocations','dev_hidden_final_outcome_read_invocations','submission_invocations'},'failure partition')
 if seq['atomic_noreplace_final5_deployment_completed'] is not True:raise RuntimeError('failure deploy partition')
 for k,x in {'transport_selftest_issued_invocations':1,'transport_selftest_native_returncode':0,'post_selftest_readonly_gate_issued_invocations':1,'post_selftest_readonly_gate_native_returncode':1,'stage_b_normal_invocations':0,'external_terminal_helper_invocations':0,'current_oof_readonly_gate_validator_invocations':0,'historical_oof_readonly_gate_validator_invocations':1,'oof_model_execution_invocations':0,'phase_a_invocations':0,'training_invocations':0,'cache_reuse_invocations':0,'reward_read_invocations':0,'dev_hidden_final_outcome_read_invocations':0,'submission_invocations':0}.items():strict_int(seq.get(k),x,'failure partition '+k)
 st=v['transport_selftest_tool_record'];exact_keys(st,{'issued_command_exact','native_returncode','tool_wall_time_seconds','combined_output_exact','separate_stdout_stderr','persistent_stdout_log_created','persistent_stderr_log_created','persistent_selftest_record_created'},'selftest tool')
 strict_int(st.get('native_returncode'),0,'selftest rc')
 if st['issued_command_exact']!='ssh 5090-autodl-new "/bin/bash /root/v529_v527_stage_b_candidate_external_terminal_once.sh --transport-self-test"' or st['separate_stdout_stderr']!='unavailable_tool_merged_streams' or any(st[k] is not False for k in ('persistent_stdout_log_created','persistent_stderr_log_created','persistent_selftest_record_created')):raise RuntimeError('selftest fact')
 pg=v['post_selftest_readonly_gate_tool_record'];exact_keys(pg,{'issued_command_exact','native_returncode','combined_output_exact','separate_stdout_stderr','per_command_wall_time_seconds','enclosing_sequential_tool_call_wall_time_seconds','stopped_on_this_first_command','wrong_path_current_absent','wrong_path','correct_path'},'postgate tool')
 strict_int(pg.get('native_returncode'),1,'postgate rc')
 wrong='/root/autodl-tmp/IROS_WAM_2.0 challenge/job_runs/v527_v526_v525_oof_readonly_validation_attempt_seed1662_20260827/oof_readonly_gate_validation_receipt.json'
 if pg['wrong_path']!=wrong or pg['correct_path']!=str(CANDIDATE_RECEIPT) or '/job_runs/' not in pg['issued_command_exact'] or pg['stopped_on_this_first_command'] is not True or pg['wrong_path_current_absent'] is not True:raise RuntimeError('postgate path fact')
 if os.path.lexists(Path(wrong)):raise RuntimeError('wrong job_runs path became current')
 old5=[
  {'role':'mapped_fixture_failure_forensic','path':'/root/autodl-tmp/IROS_WAM_2.0 challenge/pipeline/scripts/v529_v527_stage_b_mapped_fixture_default_argument_canonical_publication_forensic.json','sha256':FORENSIC_EXPECTED[0],'logical_bytes':FORENSIC_EXPECTED[1]},
  {'role':'external_terminal_design_contract','path':'/root/autodl-tmp/IROS_WAM_2.0 challenge/pipeline/scripts/v529_v527_stage_b_candidate_external_terminal_design_contract.json','sha256':DESIGN_EXPECTED[0],'logical_bytes':DESIGN_EXPECTED[1]},
  {'role':'external_terminal_helper','path':'/root/autodl-tmp/IROS_WAM_2.0 challenge/pipeline/scripts/materialize_v529_v527_stage_b_candidate_external_terminal_once.py','sha256':'a7c9c3ea215b32d840c08fe0d696be146af32cf6a3ade953552022cac1888181','logical_bytes':23561},
  {'role':'transport_script','path':'/root/v529_v527_stage_b_candidate_external_terminal_once.sh','sha256':'7c61e88246168e6633c38e1bda03bc695e91ebd63d4f6e6fbec823bd4f8b0966','logical_bytes':917},
  {'role':'noncyclic_transport_deployment_record','path':'/root/autodl-tmp/IROS_WAM_2.0 challenge/pipeline/scripts/v529_v527_stage_b_candidate_external_terminal_transport_deployment_record.json','sha256':'fdd816ce4ee1d820c4d09f01bad2201392817eaa2bfcbae1971c95fe13735804','logical_bytes':828}]
 if v['final5_current']!=old5:raise RuntimeError('old final5')
 for row in v['final5_current']:
  exact_keys(row,{'role','path','sha256','logical_bytes'},'old final5 row');strict_int(row['logical_bytes'],row['logical_bytes'],'old final5 bytes')
 if v['candidate_current']['receipt']!={'path':str(CANDIDATE_RECEIPT),'sha256':'5b443f2275c9caced1974a0fa0f10e8da414faa15f672eae83fd4a8630861439','logical_bytes':152048}:raise RuntimeError('failure candidate')
 fresh=v['fresh_external_output_current']
 if fresh['root']!='/root/autodl-tmp/IROS_WAM_2.0 challenge/artifacts/strict_track2_joint_augmentation_20260810/v529_v527_stage_b_candidate_external_terminal_evidence_seed1663_20260827' or fresh['root_absent'] is not True or fresh['prep_absent'] is not True or fresh['terminal_temp_absent'] is not True or fresh['failed_terminal_temp_absent'] is not True:raise RuntimeError('old output fact')
 audit=v['audit'];
 if any(audit.get(k) is not True for k in ('final5_exact_current','candidate_unchanged','correct_candidate_current','wrong_expanded_candidate_absent','fresh_external_root_prep_and_temps_absent','services_current','relevant_execution_pids_empty','gpu_compute_pids_empty','no_normal_or_external_terminal_helper_execution')):raise RuntimeError('failure audit')
 return v
def expected_deploy(h,s):
 return {'format':'strict-track2-v530-v529-stage-b-transport-pair-deployment-record-v1','status':'preregistered_atomic_noreplace_transport_pair_pending_external_deployment','classification':'design_only_non_authority_non_execution_transport_pair_manifest','deployment_executed':False,'pair_exact_count':2,'targets':[{'role':'external_terminal_helper',**h},{'role':'transport_script',**s}],'overwrite_authorized':False,'canonical_readback_required':True}
def deployment(h,s):
...
def snapshot(design,allow_owned_prep=False,allow_output=False):
 forensic=regular(MAPPED_FORENSIC,FORENSIC_EXPECTED);fv=json.loads(MAPPED_FORENSIC.read_text())
 ...
 failure_forensic=regular(FAILURE_FORENSIC,FAILURE_FORENSIC_EXPECTED);failure_value=validate_failure_value(json.loads(FAILURE_FORENSIC.read_bytes()))
 old_final5=[regular(Path(row['path']),(row['sha256'],row['logical_bytes'])) for row in failure_value['final5_current']]
 old_output=failure_value['fresh_external_output_current']
 for k in ('root','prep_root','terminal_temp','failed_terminal_temp'):
  if os.path.lexists(Path(old_output[k])):raise RuntimeError('historical no-retry output became present:'+k)
 ...
 return {'design':regular(DESIGN,DESIGN_EXPECTED),'mapped_fixture_forensic':forensic,'post_selftest_failure_forensic':failure_forensic,'old_final5_current':old_final5,'historical_execution_partition':failure_value['execution_sequence'],'candidate_tree':ct,'candidate_receipt':cr,'candidate_receipt_canonical_sha256':csha(candidate),'authority_tree':at,'authority_receipt':ar,'stage_a_evidence_tree':st,'stage_a_process_receipt':sp,'qualification_tree':qt,'v525_external_tree':v525,'v528_deployment_tree':v528,'v528_deployment_receipt':v528_receipt,'v528_final7_current':final7,'normalized_view_sha256':csha(view),'services':services,'gpu_compute_pids':gp,'relevant_pids':pids}
...
def held_member_records(root,members):
 out=[]
 if set(members)!=set(EXACT5):raise RuntimeError('held member keyset')
 for name in sorted(members):
  row=members[name];fd=row.get('fd')
  if type(fd) is not int or fd<0:raise RuntimeError('held member fd:'+name)
  fm=os.fstat(fd);payload=os.pread(fd,fm.st_size+1,0)
  if (fm.st_dev,fm.st_ino)!=row['identity'] or len(payload)!=fm.st_size or payload!=os.pread(fd,fm.st_size,0) or os.pread(fd,1,fm.st_size)!=b'':raise RuntimeError('held member current record:'+name)
  actual_sha=hashlib.sha256(payload).hexdigest()
  p=root/name;path_identity_matches=False;path_kind='absent'
  if os.path.lexists(p):
   lm=os.lstat(p);path_kind='symlink' if p.is_symlink() else ('regular' if stat.S_ISREG(lm.st_mode) else 'special');path_identity_matches=(lm.st_dev,lm.st_ino)==row['identity']
  out.append({'name':name,'expected_sha256':row['sha256'],'expected_logical_bytes':row['logical_bytes'],'actual_sha256':actual_sha,'actual_logical_bytes':fm.st_size,'payload_matches_expected':actual_sha==row['sha256'] and fm.st_size==row['logical_bytes'],'held_dev':fm.st_dev,'held_inode':fm.st_ino,'pread_exact':True,'pread_eof':True,'path_kind':path_kind,'path_identity_matches_held':path_identity_matches})
 return out
def build_prep(helper_record,script_record,deploy_record,design_record,member_hook=lambda _name:None):
 ...
 try:
  ...
  member_hook('argv.json');members['argv.json']=held_member(PREP/'argv.json',cbytes({'format':'strict-track2-v530-v529-stage-b-external-terminal-argv-v1','transport_helper':helper_record,'transport_script':script_record,'transport_deployment_record':deploy_record,'design_contract':design_record,'post_selftest_failure_forensic':regular(FAILURE_FORENSIC,FAILURE_FORENSIC_EXPECTED)}))
  ...
  member_hook('intent.json');members['intent.json']=held_member(PREP/'intent.json',cbytes({'format':'strict-track2-v530-v529-stage-b-external-terminal-intent-v1','status':'committed_after_historical_selftest_and_postgate_failure_before_validator_zero_external_terminal','retry_authorized':False,'historical_transport_selftest_invocations':1,'historical_post_selftest_gate_failures':1,'historical_stage_b_normal_invocations':0,'current_external_terminal_helper_invocations_authorized':1,'current_oof_readonly_gate_validator_invocations_authorized':0}))
...
def run(helper_record,script_record,deploy_record,design,design_record,hook=lambda _p:None,terminal_options=None,failed_terminal_hook=lambda _stage,_members,_records:None,stdout_writer=write_all):
 ...
 try:
  ...
  if not committed:raise RuntimeError('terminal not committed')
  postcommit=None;postcommit_errors=[];t=None
  try:
   t=tree(OUT);expected_names=sorted(EXACT6+(['.process_receipt.json.noreplace-tmp'] if foreign_terminal_preserved else []))
   if t['file_count']!=len(expected_names) or [x[0] for x in t['inventory']]!=expected_names:raise RuntimeError('terminal exact6/exact7 diagnostic')
  except BaseException as e:postcommit_errors.append({'stage':'terminal_tree_diagnostic','error_type':type(e).__name__,'error':str(e),'success_priority_after_commit':True})
  for row in members.values():
   if row.get('fd') is not None:
    try:os.fsync(row['fd'])
    except BaseException as e:postcommit_errors.append({'stage':'member_fsync_diagnostic','member':row['name'],'error_type':type(e).__name__,'error':str(e),'success_priority_after_commit':True})
    try:os.close(row['fd'])
    except BaseException as e:postcommit_errors.append({'stage':'member_close_diagnostic','member':row['name'],'error_type':type(e).__name__,'error':str(e),'success_priority_after_commit':True})
    row['fd']=None
  try:
   postcommit=snapshot(design,allow_output=True)
   if postcommit!=first:raise RuntimeError('postcommit current closure drift')
  except BaseException as e:postcommit_errors.append({'stage':'postcommit_current_diagnostic','error_type':type(e).__name__,'error':str(e),'success_priority_after_commit':True})
  for sig in (signal.SIGINT,signal.SIGTERM):
   try:signal.signal(sig,signal.SIG_IGN)
   except BaseException as e:postcommit_errors.append({'stage':'postcommit_signal_diagnostic','signal':sig,'error_type':type(e).__name__,'error':str(e),'success_priority_after_commit':True})
  return {'passed':receipt['passed'],'receipt':receipt,'tree':t,'postcommit_snapshot_sha256':None if postcommit is None else csha(postcommit),'postcommit_diagnostic':None if not postcommit_errors else postcommit_errors,'foreign_terminal_temp_preserved':foreign_terminal_preserved,'terminal_visibility_committed':True}
 except BaseException as run_error:
  if identity is not None and promoted and OUT.exists():
   foreign_process_target_present=os.path.lexists(OUT/'process_receipt.json')
   terminal_name='failed_process_receipt.json' if foreign_process_target_present else 'process_receipt.json'
   terminal_temp_name='.failed_process_receipt.json.noreplace-tmp' if foreign_process_target_present else '.process_receipt.json.failed-noreplace-tmp'
   if os.path.lexists(OUT/terminal_name):raise RuntimeError('authoritative failed terminal target already present')
   held_before=held_member_records(OUT,members);failed_terminal_hook('prepublish',members,held_before)
   failure={'format':'strict-track2-v530-v529-stage-b-candidate-external-terminal-process-receipt-v1','status':'failed_no_retry','passed':False,'candidate_consumable':False,'standalone_candidate_consumable':False,'external_terminal_helper_invocations':1,'historical_transport_selftest_invocations':1,'historical_post_selftest_gate_failures':1,'historical_stage_b_normal_invocations':0,'historical_oof_readonly_gate_validator_invocations':1,'current_oof_readonly_gate_validator_invocations':0,'oof_model_execution_invocations':0,'phase_a_invocations':0,'training_invocations':0,'retry_authorized':False,'failure_stage':'postpromote_preterminal','failure_error':{'error_type':type(run_error).__name__,'error':str(run_error)},'candidate_receipt':None if 'first' not in locals() else first['candidate_receipt'],'candidate_consumable_contract':'authoritative_failed_terminal_overrides_any_foreign_or_special_nonterminal_members','authoritative_terminal_member_name':terminal_name,'foreign_process_receipt_target_preserved':foreign_process_target_present,'terminal_tree_contract':'exact7_foreign_process_receipt_plus_owned_failed_process_receipt_nonconsumable' if foreign_process_target_present else 'five_members_may_include_preserved_foreign_or_special_plus_one_authoritative_failed_terminal','held_member_count_at_failed_terminal_publish':len(held_before),'held_member_records_at_failed_terminal_publish':held_before,'terminal_publication_contract':{'noreplace':True,'held_odrdw_exclusive_nofollow_temp':True,'commit_semantics':'failed_terminal_current_visibility_after_owned_temp_unlink','no_tree_scan_required_before_publication':True,'foreign_members_preserved':True,'foreign_process_receipt_name_never_overwritten_or_deleted':True,'alternate_failed_terminal_when_process_receipt_foreign':True,'five_member_fds_held_through_failed_terminal_commit':True,'retry_authorized':False}}
   state={'visible':False,'committed':False};atomic_terminal(OUT/terminal_name,failure,state,temp_name=terminal_temp_name);committed=state['committed']
   held_after=held_member_records(OUT,members);failed_terminal_hook('postcommit',members,held_after)
   for row in members.values():
    if row.get('fd') is not None:
     try:os.fsync(row['fd'])
     except BaseException:pass
   ...
   return {'passed':False,'receipt':failure,'tree':None,'postcommit_snapshot_sha256':None,'postcommit_diagnostic':{'stage':'postpromote_failed_terminal','error_type':type(run_error).__name__,'error':str(run_error)},'foreign_terminal_temp_preserved':False,'terminal_visibility_committed':committed}
  for row in members.values():
   if row.get('fd') is not None:
    try:os.close(row['fd'])
    except OSError:pass
