#!/usr/bin/env python3
"""Two-process, branch-serial fixed-horizon simulator collection for v448."""
...
def timeout_handler(_signum, _frame) -> None:
    raise TimeoutError("v448 fixed context exceeded 300 seconds")
...
def main() -> int:
    ...
    if (
        prereg.get("format")
        != "strict-track2-v448-paired-intervention-pilot-preregistration-v1"
        or out.exists()
    ):
        raise RuntimeError("v448 prereg/output contract failed")
    ...
    report = {
        "format": "strict-track2-v448-paired-intervention-pilot-generation-report-v1",
