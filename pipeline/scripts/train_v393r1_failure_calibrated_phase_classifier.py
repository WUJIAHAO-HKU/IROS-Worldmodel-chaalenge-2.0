...
def teacher_examples(
    ...
    for episode in episodes:
        for path in sorted(windows.glob(f"episode{episode}_*.npz")):
            ...
            tasks.append((episode, start, path))

    def load(item):
        episode, start, path = item
        return episode, start, *load_window(path)

    with ThreadPoolExecutor(max_workers=IO_WORKERS) as executor:
        loaded = executor.map(load, tasks)
        for episode, start, context, history, future in loaded:
            append_example(
                *output,
                context,
                history,
...
def failure_examples(
    ...
    for row in episode_rows:
        ...
        for path in paths:
            tasks.append((episode, path))

    def load(item):
        episode, path = item
        return episode, *load_window(path)

    with ThreadPoolExecutor(max_workers=IO_WORKERS) as executor:
        loaded = executor.map(load, tasks)
        for episode, context, history, future in loaded:
...
def rates(
    ...
    result = {
        ...
        "onpolicy_failure_all": group_metrics(kinds == "onpolicy_failure_train"),
    }
...
def eligible(row: dict) -> bool:
    ...
    return bool(
        ...
        and row["onpolicy_failure_all"]["rows"] == 2256
        and row["onpolicy_failure_all"]["negative_specificity"] >= 0.99
        and all(
...
def main() -> int:
    ...
    if prereg.get("format") != "strict-track2-v393r1-failure-calibrated-phase-preregistration-v1":
        raise RuntimeError("wrong v393r1 preregistration")
    ...
    if passed:
        ...
        checks["validation_failure_route_rows_exact3"] = int(validation_route.sum()) == 3
        checks["validation_failure_route_zero_false_positives"] = bool(
            validation_route.any()
            and (~validation_predicted[validation_route]).mean() == 1.0
        )
        checks["validation_failure_all_specificity_ge0p99"] = bool(
            (~validation_predicted).mean() >= 0.99
        )
        passed = all(checks.values())
        if passed:
            np.savez_compressed(
    ...
    report = {
        "format": "strict-track2-v393r1-failure-calibrated-phase-training-v1",
        ...
        "guards": {
            ...
            "aborted_v393_changed_only_feasibility_counts_and_parallel_io": True,
