#!/usr/bin/env python3
"""Freeze the sole v318/v317 candidate after both public gates pass."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--world-model-authorization", required=True, type=Path)
...
def main() -> None:
    ...
    inputs = (
        ...
        args.parent_runtime,
        args.world_model_authorization,
        args.public_runner,
        args.final_runner,
    ...
    authorization = json.loads(args.world_model_authorization.read_text())
    ...
    if parent.get("candidate", {}).get("model_version") != training_prereg[
        "world_model"
    ]["model_version"]:
        raise RuntimeError("training and release world-model versions differ")
    runtime_evidence = parent.get("evidence", {}).get("runtime", {})
    if not (
        runtime_evidence.get("path") == str(args.parent_runtime)
        and runtime_evidence.get("sha256") == sha256(args.parent_runtime)
    ):
        raise RuntimeError("v317 runtime is not bound by the world release")
    if not (
        authorization.get("passed") is True
        and authorization.get("launch_permission") is True
        and all(authorization.get("checks", {}).values())
        and authorization["checks"].get("no_hidden_or_final_data") is True
        and authorization["checks"].get("no_real_submission") is True
    ):
        raise RuntimeError("v317 world-model authorization did not pass cleanly")
    if not (
    ...
    evidence_paths = {
        ...
        "v317_world_model_authorization": args.world_model_authorization,
    ...
    payload = {
        "format": "strict-track2-v320-v318-unique-final-candidate-freeze-v1",
        "frozen_utc": datetime.now(timezone.utc).isoformat(),
        "variant": args.variant,
        "checkpoint": str(args.checkpoint),
        "checkpoint_sha256": checkpoint_hash,
        "world_model_version": parent["candidate"]["model_version"],
        "evidence": {
