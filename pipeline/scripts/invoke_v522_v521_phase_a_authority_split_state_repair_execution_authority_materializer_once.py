#!/usr/bin/env python3
"""Exact-once, signal-safe transport for the v522 split-state authority materializer."""
...
SELF_PATH = SCRIPTS / "invoke_v522_v521_phase_a_authority_split_state_repair_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v522_v521_phase_a_authority_split_state_repair_authority_materialize_once.sh")
CONTRACT = SCRIPTS / "v522_v521_phase_a_authority_split_state_repair_execution_authority_contract.json"
CONTRACT_SHA = "0f766d4c6f95e81c0e2278cc19ba8124b6f4df4eb0e3ddf0376e8b89280c7610"
CONTRACT_BYTES = 111259
MATERIALIZER = SCRIPTS / "materialize_v522_v521_phase_a_authority_split_state_repair_execution_authority.py"
MATERIALIZER_SHA = "42f2375eb3b468798d2131edddbefab7e579bdc6662e3743dbb0fb3b799ddca5"
MATERIALIZER_BYTES = 53649
PHASE_A_LAUNCHER = SCRIPTS / "launch_v522_v521_phase_a_cache_qualification.py"
PHASE_A_LAUNCHER_SHA = "289204d6b7b7593de390f8600c5cffb6edb496b357d45ec5216a446f586740f4"
PHASE_A_LAUNCHER_BYTES = 77590
PHASE_A_DRIVER = SCRIPTS / "generate_v522_v521_phase_a_cache_qualification.py"
PHASE_A_DRIVER_SHA = "8f4e31da630272586d1fb08a067072734c19c97901c1b8fa25ae2bfd78820320"
PHASE_A_DRIVER_BYTES = 45542
PHASE_A_WORKER = SCRIPTS / "generate_v522_v521_phase_a_cache_qualification_worker.py"
PHASE_A_WORKER_SHA = "3f4376a1aa093b578cf4d6f7ba02b7a85460c7ea4c86f162c96b2d222eaac231"
...
INDEPENDENT_AUDITOR = SCRIPTS / "audit_v522_v521_phase_a_rng_isolated_qualification.py"
INDEPENDENT_AUDITOR_SHA = "3b60f0134fb474422bf792666474bff16b6456273385cfcd24b15e4c685e1b2e"
...
PREREG_BYTES = 57451
SPLIT_FORENSIC = SCRIPTS / "v522_v521_v520_phase_a_authority_split_state_forensic.json"
...
EVIDENCE_ROOT = J / "v522_v521_phase_a_authority_split_state_repair_execution_authority_materialization_evidence_seed1658_20260826"
...
FROZEN_FINAL9_SHA256 = (
    "0f766d4c6f95e81c0e2278cc19ba8124b6f4df4eb0e3ddf0376e8b89280c7610",
    "42f2375eb3b468798d2131edddbefab7e579bdc6662e3743dbb0fb3b799ddca5",
    "289204d6b7b7593de390f8600c5cffb6edb496b357d45ec5216a446f586740f4",
    "8f4e31da630272586d1fb08a067072734c19c97901c1b8fa25ae2bfd78820320",
    "3f4376a1aa093b578cf4d6f7ba02b7a85460c7ea4c86f162c96b2d222eaac231",
    "2b8ab0e2aec4a8de5d4bf8cffbb3525bbcd69b9b03270012a4a48f69f430b51b",
    "3b60f0134fb474422bf792666474bff16b6456273385cfcd24b15e4c685e1b2e",
    "27980589965c0209324e43f908acdfc86ee97df4be1186e8e236f4cda0879f7c",
    "bed75dcfb24a8bb3d25caa3f497d26717d83692aa1b8cf251bd651cc45645267",
)
...
def deployment_plan() -> dict:
    return {
        "fresh_noreplace_targets_exact8": [
            ...
            {"role": "v522_v521_authority_split_state_forensic", "path": str(SPLIT_FORENSIC), "sha256": SPLIT_FORENSIC_SHA, "logical_bytes": SPLIT_FORENSIC_BYTES},
        ],
...
def v522_phase_a_pids() -> list[dict]:
    markers = ("launch_v522_v521_phase_a_cache_qualification.py",
               "generate_v522_v521_phase_a_cache_qualification.py",
               "generate_v522_v521_phase_a_cache_qualification_worker.py")
    rows = []
...
def immutable_snapshot(contract: dict, helper_record: dict, script_record: dict) -> dict:
    ...
    return {
        ...
        "v522_phase_a_pids": v522_phase_a_pids(), "deployment_plan": deployment_plan(),
    }
...
def validate_materializer_contract(module, contract: dict, materializer_record: dict) -> None:
    ...
    active = {
        ...
        "v522_v521_authority_split_state_forensic": {"path": str(SPLIT_FORENSIC), "sha256": SPLIT_FORENSIC_SHA, "logical_bytes": SPLIT_FORENSIC_BYTES},
    }
    if (module.CONTRACT_PATH != CONTRACT or module.AUTHORITY_ROOT != AUTHORITY_ROOT
            or set(contract) != module.CONTRACT_TOP_KEYS
            or isinstance(seed, bool) or not isinstance(seed, int) or seed != 1658
            ...
            or len(contract.get("authority_receipt_contract", {}).get("top_keys", [])) != 88
            or len(module.CHECK_KEYS) != 41
            or len(module.CONTRACT_SOURCE_ORDER) != 32 or len(module.AUTHORITY_SOURCE_ORDER) != 33
    ...
    split = module.validate_split_state()
    split_keys = {
        "v520_authority_registration_tree", "v520_authority_receipt", "v520_authority_semantics",
    ...
    if (contract.get("worker_environment_exact") != module.WORKER_ENV
            or contract.get("per_call_rng_evidence_contract") != module.PER_CALL_RNG_CONTRACT
            ...
            or not split_keys <= set(split)
            or any(csha(contract.get(key)) != csha(split[key]) for key in split_keys)
            or split["old_authority_expected_current"].get("present") is not True
            or split["old_authority_expected_current"].get(
                "historical_absence_validator_reused_post_authority") is not False):
        raise RuntimeError("v522 execution/split-state contract")
...
def load_materializer(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    spec = importlib.util.spec_from_file_location("v522_materializer", materializer_source)
...
def validate_success(module, contract: dict, returncode: int, cleanup: dict,
    ...
    if (os.path.lexists(AUTHORITY_PREP)
            ...
            or receipt.get("old_authority_expected_current") != contract["old_authority_expected_current"]
            or receipt.get("old_authority_expected_current", {}).get(
                "historical_absence_validator_reused_post_authority") is not False
    ...
    if any(receipt.get(key) != contract.get(key) for key in split_keys):
        raise RuntimeError("authority split anchors")
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v522-v521-phase-a-authority-split-state-repair-execution-authority-materializer-process-receipt-v1",
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v522-v521-phase-a-authority-split-state-repair-execution-authority-materializer-intent-v1",
...
def production_fixture(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    checks["services_pid_gpu_in_stable_snapshot"] = all(value in source for value in (
        '"service_health": service_health()', '"gpu_compute_pids": gpu_compute_pids()',
        '"v522_phase_a_pids": v522_phase_a_pids()'))
    ...
    checks["old_authority_current_and_historical_absence_not_reused"] = (
        live_contract["old_authority_expected_current"]["present"] is True
        and live_contract["old_authority_expected_current"][
            "historical_absence_validator_reused_post_authority"] is False
        and exact_tree(Path(live_contract["old_authority_expected_current"]["root"]))
            == live_contract["old_authority_expected_current"]["registration_tree"])
    def rejects(contract_value, module_value=live_module, materializer_value=live_materializer_record):
        try:
            ...
            return True
    tamper_checks = {}
    for bad_seed in (1657, 1659, "1658", True, None):
    ...
    bad_partition_bool["v521_execution_partition"]["v521_transport_helper_invocations"] = True
    ...
    module_values = {name: getattr(live_module, name) for name in (
        ...
        "load_base", "validate_prereg", "validate_v519", "validate_v517", "validate_failures",
    ...
    with tempfile.TemporaryDirectory(prefix="v522-helper-fixture-") as folder:
        base = Path(folder)
...
def main() -> int:
    ...
    if pre["gpu_compute_pids"] or pre["v522_phase_a_pids"]:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v522-v521-phase-a-authority-split-state-repair-execution-authority-materializer-argv-v1",
