...
PARENT_MANIFEST="$JOINT/v317_v315_native_batch_causal_gate_seed1490_20260822/release_registration.json"
PARENT_RUNTIME="$BASE/pipeline/wam_pipeline/v317_batched_sparse_failure_terminal_runtime.py"
...
AUTH="$JOINT/v317_v315_native_batch_causal_gate_seed1490_20260822/audit/expensive_rl_authorization.json"
FREEZE_DIR="$OFF/frozen_candidates/v320_v318_v317_rtx5090_step10_seed1471"
FREEZE="$FREEZE_DIR/freeze_manifest.json"
FINAL_OUTPUT="$OFF/real_robotwin_eval/frozen_v320_v318_v317_rtx5090_step10_seed1471_final128"
...
FINAL_WRAPPER="$P/run_frozen_v320_v318_final128_once.sh"
CPUSET='0-21'

test -f "$RUN/audit/V318_TRAINING_ACCEPTED"
for path in "$CKPT" "$RUN/audit/p3_training_acceptance.json" "$REG/preregistration.json" "$POST_PREREG" "$PARENT_MANIFEST" "$PUBLIC_PREFLIGHT" "$FINAL_PREFLIGHT" "$AUTH" "$DEV/manifest.json" "$DEV/batch_00.json" "$SEEDS"; do test -s "$path"; done
test ! -e "$DEV_OUT/$VARIANT"
test ! -e "$FREEZE_DIR"
test ! -e "$FINAL_OUTPUT"

"$PY" "$P/verify_v304_posttraining_tools.py" --preregistration "$POST_PREREG" \
  --tool run_v318_posttraining_pipeline.sh \
  --tool run_frozen_v320_v318_final128_once.sh \
  --tool prepare_v318_batch00_gate.py \
  --tool audit_v228_public_right_gate.py \
  --tool audit_v276_v274_public112_gate.py \
  --tool freeze_v320_v318_candidate.py \
...
"$PY" "$P/prepare_v318_batch00_gate.py" \
  ...
  --prepare-script "$P/prepare_v318_batch00_gate.py" \
  --launcher "$P/run_v318_posttraining_pipeline.sh" --variant "$VARIANT" >"$REG/public_batch00_prepare.log"
...
"$PY" "$P/freeze_v320_v318_candidate.py" \
  ...
  --parent-manifest "$PARENT_MANIFEST" --parent-runtime "$PARENT_RUNTIME" --world-model-authorization "$AUTH" \
  ...
  --final-wrapper "$FINAL_WRAPPER" --public112-launcher "$P/run_v318_posttraining_pipeline.sh" \
