#!/usr/bin/env python3
"""Exact-once, signal-safe transport for the v509 Phase-A authority materializer."""
...
SELF_PATH = SCRIPTS / "invoke_v509_v508_phase_a_wam_pipeline_import_repair_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v509_v508_phase_a_wam_pipeline_import_repair_authority_materialize_once.sh")
CONTRACT = SCRIPTS / "v509_v508_phase_a_wam_pipeline_import_repair_execution_authority_contract.json"
CONTRACT_SHA = "acee1f332e66fa403a34e7bf3a62955136b92e19364e84a14db3308d2d339697"
CONTRACT_BYTES = 114187
MATERIALIZER = SCRIPTS / "materialize_v509_v508_phase_a_wam_pipeline_import_repair_execution_authority.py"
MATERIALIZER_SHA = "f993b1a464995d84ecaac994bfc7f5630763feabdfcf40e791a0cb49b6a300ff"
MATERIALIZER_BYTES = 28224
PHASE_A_LAUNCHER = SCRIPTS / "launch_v509_v508_phase_a_cache_qualification.py"
PHASE_A_LAUNCHER_SHA = "3427f8972bfe929042ed600d7742070bb873f5dc99c6e3eaae4543600457f33e"
PHASE_A_LAUNCHER_BYTES = 58848
...
EVIDENCE_ROOT = J / "v509_v508_phase_a_wam_pipeline_import_repair_execution_authority_materialization_evidence_seed1650_20260825"
...
def validate_materializer_contract(module, contract: dict, materializer_record: dict) -> None:
    ...
    if (module.CONTRACT_PATH != CONTRACT or module.AUTHORITY_ROOT != AUTHORITY_ROOT
            ...
            or isinstance(seed, bool) or not isinstance(seed, int) or seed != 1650
            ...
            or contract.get("source_closure", {}).get("v509_failure_forensic") != {
                "path": str(FAILURE_FORENSIC), "sha256": FAILURE_FORENSIC_SHA,
                "logical_bytes": FAILURE_FORENSIC_BYTES}
            or len(module.AUTH_TOP_KEYS) != 61 or len(module.CHECK_KEYS) != 33
            or len(module.CONTRACT_SOURCE_ORDER) != 21 or len(module.AUTHORITY_SOURCE_ORDER) != 22
    ...
    base = module.load_old().load_old().load_base()
    module.configure(base)
    closure = contract["source_closure"]
    module.validate_prereg(closure["fresh_phase_a_preregistration"], closure["fresh_phase_a_driver"])
    manifest_record = closure["wam_pipeline_package_tree_manifest"]
    manifest_path = Path(manifest_record["path"])
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    package_tree = base.tree(module.PACKAGE)
    package_fixture = module.package_fixture(base)
    if (regular(manifest_path, (manifest_record["sha256"], manifest_record["logical_bytes"])) != manifest_record
            or manifest != package_tree or contract.get("wam_pipeline_package_tree") != package_tree
            or package_fixture is None or contract.get("wam_pipeline_import_fixture") != package_fixture
            or contract.get("wam_pipeline_exact_worker_environment") != {"PYTHONPATH": str(module.PIPELINE)}
            or contract.get("driver_environment_contract") != module.DRIVER_ENV):
        raise RuntimeError("wam_pipeline actual preflight")
...
def load_materializer(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER):
    ...
    spec = importlib.util.spec_from_file_location("v508_materializer", materializer_source)
...
def validate_success(module, contract: dict, returncode: int, cleanup: dict,
    ...
    if (os.path.lexists(AUTHORITY_PREP)
            or set(receipt) != module.AUTH_TOP_KEYS or len(receipt) != 61
            ...
            or len(receipt.get("source_closure", {})) != 22
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v509-v508-phase-a-wam-pipeline-import-repair-authority-materializer-process-receipt-v1",
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v509-v508-phase-a-wam-pipeline-import-repair-authority-materializer-intent-v1",
...
def production_fixture(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER) -> dict:
    ...
    def rejects(contract_value, module_value=live_module, materializer_value=live_materializer_record):
        try:
            ...
            return True
    tamper_checks = []
    for bad_seed in (1644, 1649, "1650", True, None):
    ...
    bad_forensic = copy.deepcopy(live_contract)
    bad_forensic["source_closure"]["v509_failure_forensic"]["sha256"] = "0" * 64
    tamper_checks.append(rejects(bad_forensic))
    ...
    with tempfile.TemporaryDirectory(prefix="v508-helper-fixture-") as folder:
        base = Path(folder)
...
def main() -> int:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v509-v508-phase-a-wam-pipeline-import-repair-authority-materializer-argv-v1",
