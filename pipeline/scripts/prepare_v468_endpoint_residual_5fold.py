#!/usr/bin/env python3
"""Preregister v468 only after a passed immutable v456 final audit receipt."""
...
def file_record(base,base_kind,path,artifact_root,relative=None,declared_sha256=None):
 ...
 if os.path.commonpath((str(base),str(lexical)))!=str(base) or not inside(resolved,artifact_root) or not resolved.is_file():raise RuntimeError(f"v468 closure path escape/missing: {lexical} -> {resolved}")
 digest=sha(resolved)
 if declared_sha256 is not None and digest!=declared_sha256:raise RuntimeError(f"v468 manifest-declared SHA drift: {lexical}")
...
def release_closure(release,artifact_root):
 ...
 def visit(logical,relative):
  ...
  if not inside(resolved,artifact_root) or not resolved.is_dir():raise RuntimeError(f"v468 release directory escape/missing: {logical}")
  key=(resolved.stat().st_dev,resolved.stat().st_ino)
  if key in active:raise RuntimeError(f"v468 release symlink cycle: {logical}")
  ...
  for child in sorted(logical.iterdir(),key=lambda x:x.name):
   ...
   if not inside(target,artifact_root):raise RuntimeError(f"v468 release target escapes joint artifact root: {child} -> {target}")
   if child.is_symlink():
   ...
   if target.is_dir():
    ...
    if target_key in active:raise RuntimeError(f"v468 release symlink cycle: {child}")
    if target_key in seen_dirs:raise RuntimeError(f"v468 duplicate release directory target: {child} -> {target}")
    visit(child,rel)
   elif target.is_file():
    target_key=(target.stat().st_dev,target.stat().st_ino)
    if target_key in seen_files:raise RuntimeError(f"v468 duplicate release file target: {child} -> {target}")
 ...
 if len(keys)!=len(set(keys)):raise RuntimeError("v468 duplicate release closure records")
...
def library_closure(release,library):
 ...
 if windows!=library and library not in windows.parents:raise RuntimeError("v468 retrieval windows escape v169 library")
 ...
 if len(keys)!=len(set(keys)):raise RuntimeError("v468 duplicate closure records")
 return records
def arrsha(value):
 import numpy as np
...
def request_phase(row,cache):
 ...
 if source not in cache:
  if sha(source)!=row["source_hdf5_sha256"]:raise RuntimeError("v468 HDF5 SHA drift")
 ...
 if arrsha(history)!=row["history_action_sha256"] or {n:arrsha(v) for n,v in branches.items()}!=row["branch_action_sha256"] or endpoint!=row["endpoint_diffs"] or fullpath!=row["path_diffs"] or min(endpoint.values())<.01 or min(fullpath.values())<.01:raise RuntimeError("v468 action closure drift")
...
def shuffle_map(rows,cache):
 ...
 for episode,group in sorted(groups.items()):
  ...
  if n<2:raise RuntimeError(f"v468 phase shuffle requires >=2 contexts episode={episode}")
  offset=max(1,n//2)
...
def main():
 ...
 if sha(a.contract)!="99dc3715397e027ad64b78b4afcd7413a1f0f8e3c02c4e4631f12478862ca43b" or contract.get("format")!="strict-track2-v468-canonical-notransport-paired-delta-contract-v1" or contract.get("status")!="preregistered_public_train_s0_authorized":raise RuntimeError("v468 frozen contract/status drift")
 if receipt.get("format")!="strict-track2-v461-endpoint200-batch-audit-v1" or receipt.get("mode")!="final" or receipt.get("passed") is not True or not all(receipt.get("checks",{}).values()):raise RuntimeError("v468 blocked: v461 final")
 selection=json.loads(a.v456_selection.read_text());generation=json.loads(a.v456_generation_report.read_text())
 if selection.get("format")!="strict-track2-v461-endpoint200-preregistration-v1" or generation.get("passed") is not True or generation.get("exact_contexts")!=200:raise RuntimeError("v468 v461 contract")
 ...
 if len(contexts)!=200 or len(episodes)!=15 or any(len(x["holdout_episodes"])!=3 for x in folds):raise RuntimeError("v468 folds")
 files=[]
 ...
 if len(files)!=410:raise RuntimeError("v468 exact dataset tree")
 ...
 payload={
  "format":"strict-track2-v468-canonical-notransport-paired-delta-preregistration-v1","created_at":datetime.now(timezone.utc).isoformat(),"seed":1616,"contract":{"path":str(a.contract.resolve()),"sha256":sha(a.contract)},
 ...
 if a.output.exists() or a.output.with_name(a.output.name+".tmp").exists():raise RuntimeError("refusing overwrite v468 prereg")
