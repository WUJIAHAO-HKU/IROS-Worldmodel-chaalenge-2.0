#!/usr/bin/env python3
"""Independent v524 auditor enforcing ordered exact8 source closure."""
...
def validate_exact_source_closure(formal):
    exact_roles=formal.get("phase_a_output_and_receipt_schema",{}).get("source_closure_contract",{}).get("exact_roles")
    sources=formal.get("execution_sources");records=formal.get("execution_source_records")
    if exact_roles!=EXACT_ROLES or not isinstance(sources,dict) or set(sources)!=set(EXACT_ROLES) or not isinstance(records,list) or len(records)!=8:raise RuntimeError("final exact eight roles")
    for index,role in enumerate(EXACT_ROLES):
        frozen=records[index]
        if not isinstance(frozen,dict) or set(frozen)!={"role","path","sha256","logical_bytes"} or frozen.get("role")!=role or type(frozen.get("logical_bytes")) is not int or frozen["logical_bytes"]<0 or sources.get(role)!={key:value for key,value in frozen.items() if key!="role"}:raise RuntimeError("final source bijection")
    if formal.get("execution_sources_digest_sha256")!=canonical_sha(records):raise RuntimeError("final source digest")
    return exact_roles
...
def final_source_closure(worker_receipt):
    ...
    exact_roles=validate_exact_source_closure(formal);records=[]
...
def events(path,role):
    ...
    expected_keys={"format","ordinal","sample_id","episode","start","branch","seed","request_sha256","output_sha256","output_schema","context_sha256","history_sha256","future_sha256","seed_sha256","instruction_sha256","warning_category","warning_full_message","raw_warning","raw_warnings_unchanged","sample_call_started","sample_call_completed","rng_isolation","rng_isolation_sha256","event_canonical_sha256","deterministic_before","warn_only_before","deterministic_inside","warn_only_inside","deterministic_after","warn_only_after","cpu_rng_before_sha256","cpu_rng_after_sha256","cuda_rng_before_sha256_by_device","cuda_rng_after_sha256_by_device","rng_unchanged"}
    for row in rows:
        if set(row)!=expected_keys:raise RuntimeError("event exact schema")
        unsigned={key:value for key,value in row.items() if key!="event_canonical_sha256"};rng=row["rng_isolation"]
        if row["format"]!="strict-track2-v520-per-call-rng-isolation-evidence-v1" or row["event_canonical_sha256"]!=canonical_sha(unsigned) or row["rng_isolation_sha256"]!=canonical_sha(rng) or row["output_schema"]!={"shape":[8,256,256,3],"dtype":"uint8","contiguous":True} or row["sample_call_started"]!=1 or row["sample_call_completed"]!=1 or row["raw_warning"]!={"category":ALLOWED_CATEGORY,"message":ALLOWED_MESSAGE} or row["raw_warnings_unchanged"] is not True:raise RuntimeError("event v520 evidence")
        if rng.get("exact_seed")!=row["seed"] or rng.get("delegate_invocations_started")!=1 or rng.get("delegate_invocations_completed")!=1 or rng.get("cuda_device_indices")!=list(range(rng.get("cuda_device_count"))) or any(rng.get(key) is not True for key in ("fork_rng_devices_exact_all_cuda_indices","python_all_four_stages_equal","numpy_all_four_stages_equal","python_exit_restored","numpy_exit_restored","torch_cpu_exit_restored","torch_cuda_exit_restored")):raise RuntimeError("event RNG isolation")
...
def validate_durable_markers(path, role):
    markers=[]
    for line in Path(path).read_text(errors="strict").splitlines():
        try: row=json.loads(line)
        except Exception: continue
        if row.get("event") in ("rng_proxy_call_started","rng_proxy_call_completed"): markers.append(row)
    expected=list(range(1000)) if role=="A" else list(reversed(range(1000)))
    if len(markers)!=PER_CALL_RNG_CONTRACT["durable_process_log_markers_per_branch"]:raise RuntimeError("durable marker count")
    for ordinal,sample_id in enumerate(expected):
        pair=markers[2*ordinal:2*ordinal+2]
        if ([row.get("event").removeprefix("rng_proxy_call_") for row in pair]
                !=PER_CALL_RNG_CONTRACT["process_log_marker_order_per_call"]
                or any(row.get("ordinal")!=ordinal or row.get("sample_id")!=sample_id for row in pair)):
            raise RuntimeError("durable marker order")
    return {"role":role,"marker_pairs":len(expected),"total_markers":len(markers),"alternating_started_completed":True}
...
def validate_precleanup(root):
    ...
    if ar.get("format")!="strict-track2-v523-v522-rng-isolated-cache-worker-receipt-v1" or br.get("format")!="strict-track2-v523-v522-rng-isolated-cache-worker-receipt-v1" or ar.get("passed") is not True or br.get("passed") is not True or ar.get("role")!="A" or br.get("role")!="B":raise RuntimeError("fresh process receipts")
    for identity in (ai,bi):
    ...
    marker_a=validate_durable_markers(a/"process.log","A");marker_b=validate_durable_markers(b/"process.log","B")
    if marker_a["total_markers"]+marker_b["total_markers"]!=PER_CALL_RNG_CONTRACT["total_durable_process_log_markers"]:raise RuntimeError("durable marker total")
    ...
    rng_count_keys=("raw_warning_count","rng_isolation_event_count","rng_isolation_delegate_started_count","rng_isolation_delegate_completed_count","rng_isolation_python_all_four_stages_equal_count","rng_isolation_numpy_all_four_stages_equal_count","rng_isolation_torch_cpu_exit_restored_count","rng_isolation_torch_cuda_exit_restored_count","durable_call_started_markers","durable_call_completed_markers")
    if any(receipt.get(key)!=1000 for receipt in (ar,br) for key in rng_count_keys):raise RuntimeError("v520 RNG receipt counters")
    ...
    if authority.get("passed") is not True or authority.get("phase_a_cache_qualification_authorized") is not True or authority.get("training_authorized") is not False or authority.get("preregistration_sha256")!=ancestry["formal"]["sha256"] or authority.get("contract_sha256")!=CONTRACT_SHA or canonical_sha(authority.get("per_call_rng_evidence_contract"))!=canonical_sha(PER_CALL_RNG_CONTRACT):raise RuntimeError("authority boundary")
    ...
    expected_sources={"scope_source":"cache_scope_helper","rng_proxy_source":"phase_a_rng_isolation_proxy","driver_source":"phase_a_driver","worker_source":"phase_a_process_worker","auditor_source":"phase_a_independent_auditor"}
    ...
    if exact_roles!=EXACT_ROLES:raise RuntimeError("v520 exact8 role order")
