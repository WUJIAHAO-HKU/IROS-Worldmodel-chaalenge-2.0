...
SELF_PATH=ROOT/"pipeline/scripts/launch_v495_v494_v490_interpreter_validator_repaired_reconciliation_outer.py"
AUTH_CONTRACT=ROOT/"pipeline/scripts/v495_v494_v490_interpreter_validator_repair_execution_authority_contract.json"
AUTH_ROOT=J/"v495_v494_v490_interpreter_validator_repair_execution_authority_seed1637_20260825";AUTH_PREP=AUTH_ROOT.with_name(AUTH_ROOT.name+".registration-prep");AUTH_RECEIPT=AUTH_ROOT/"authority_receipt.json"
...
INNER_WRAPPER=ROOT/"pipeline/scripts/launch_v495_v494_v490_interpreter_validator_repaired_reconciliation_inner.py";INNER_WRAPPER_SHA="b52e7eb29870141bda85eead81cbc1c3a47522ad86e6a7b6829f7052766a8f76";INNER_WRAPPER_BYTES=90261
...
INNER_ATTEMPT=J/"v495_v494_v490_interpreter_validator_repaired_reconciliation_inner_attempt_seed1637_20260825";INNER_PREP=INNER_ATTEMPT.with_name(INNER_ATTEMPT.name+".attempt-prep")
TRANSPARENT=F813.parent/"transparent_static_audit.json";TRANSPARENT_TMP=TRANSPARENT.with_name(TRANSPARENT.name+".tmp")
OUTER_EVIDENCE=J/"v495_v494_v490_interpreter_validator_repaired_reconciliation_outer_execution_evidence_seed1637_20260825";OUTER_PREP=OUTER_EVIDENCE.with_name(OUTER_EVIDENCE.name+".outer-prep")
QUALIFICATION=Path("/root/v485_v169_cache_qualification_seed1627_20260824")
AUTH_FORMAT="strict-track2-v495-v494-v490-interpreter-validator-repair-execution-authority-v1";AUTH_STATUS="authorized_exact_one_external_v495_interpreter_validator_repaired_outer_attempt"
CONTRACT_FORMAT="strict-track2-v495-v494-v490-interpreter-validator-repair-execution-authority-design-contract-v1";CONTRACT_STATUS="design_only_frozen_sources_pending_independent_review_no_authority"
OUTER_INTENT_FORMAT="strict-track2-v495-v494-v490-interpreter-validator-repaired-reconciliation-outer-attempt-intent-v1";OUTER_TERMINAL_FORMAT="strict-track2-v495-v494-v490-interpreter-validator-repaired-reconciliation-outer-attempt-terminal-v1"
...
def authority_preflight(args):
 ...
 if set(contract)!=CONTRACT_TOP_KEYS or contract.get("format")!=CONTRACT_FORMAT or contract.get("status")!=CONTRACT_STATUS or contract.get("seed")!=1637 or contract.get("execution_boundary")!=EXECUTION_BOUNDARY:raise RuntimeError("contract identity")
 ...
 historical=decode_absence_receipt(receipt.get("historical_absences"),33);required=decode_absence_receipt(receipt.get("required_absences"),32);contract_historical=decode_path_map(contract.get("historical_absences"),33);contract_current=decode_path_map(contract.get("current_absences_after_authority"),32)
...
def immutable_snapshot(context):
 ...
 trees={"authority":rooted_tree(AUTH_ROOT),"repair":rooted_tree(REPAIR.parent),"static":rooted_tree(STATIC.parent),"v493_authority":rooted_tree(V493_AUTH_RECEIPT.parent),"v493_authority_materialization":rooted_tree(V493_AUTH_MATERIALIZATION_EVIDENCE_ROOT),"failed_v493_outer":rooted_tree(V493_OUTER_FAILURE_ROOT)}
...
def synthetic():
 ...
 fixture_path=Path("/dev/shm/v495_authority_contract_candidate.json")
 if fixture_path.is_file():
  fixture=json.loads(fixture_path.read_text());historical=decode_path_map(fixture["historical_absences"],33);current=decode_path_map(fixture["current_absences_after_authority"],32)
