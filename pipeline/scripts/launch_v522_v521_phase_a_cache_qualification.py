#!/usr/bin/env python3
"""One-shot v522 split-state-repaired RNG-isolated Phase-A launcher (design-only).

The launcher consumes the fresh v522 authority and exact-eight preregistration,
...
SEED = 1658
AUTHORITY_CONTRACT = SCRIPTS / "v522_v521_phase_a_authority_split_state_repair_execution_authority_contract.json"
AUTHORITY_MATERIALIZER = SCRIPTS / "materialize_v522_v521_phase_a_authority_split_state_repair_execution_authority.py"
AUTHORITY_MATERIALIZER_SHA = "42f2375eb3b468798d2131edddbefab7e579bdc6662e3743dbb0fb3b799ddca5"
AUTHORITY_MATERIALIZER_BYTES = 53649
SELF = SCRIPTS / "launch_v522_v521_phase_a_cache_qualification.py"
DRIVER = SCRIPTS / "generate_v522_v521_phase_a_cache_qualification.py"
DRIVER_SHA = "8f4e31da630272586d1fb08a067072734c19c97901c1b8fa25ae2bfd78820320"
DRIVER_BYTES = 45542
WORKER = SCRIPTS / "generate_v522_v521_phase_a_cache_qualification_worker.py"
WORKER_SHA = "3f4376a1aa093b578cf4d6f7ba02b7a85460c7ea4c86f162c96b2d222eaac231"
WORKER_BYTES = 38774
AUDITOR = SCRIPTS / "audit_v522_v521_phase_a_rng_isolated_qualification.py"
AUDITOR_SHA = "3b60f0134fb474422bf792666474bff16b6456273385cfcd24b15e4c685e1b2e"
AUDITOR_BYTES = 45074
...
PREREG = J / "v522_v521_phase_a_cache_qualification_prereg_seed1658_20260826/preregistration.json"
PREREG_SHA = "27980589965c0209324e43f908acdfc86ee97df4be1186e8e236f4cda0879f7c"
PREREG_BYTES = 57451
...
AUTHORITY_FORMAT = "strict-track2-v522-v521-phase-a-authority-split-state-repair-execution-authority-v1"
AUTHORITY_STATUS = "authorized_exact_one_external_v522_phase_a_cache_qualification_attempt"
...
INTENT_FORMAT = "strict-track2-v522-v521-phase-a-cache-qualification-attempt-intent-v1"
TERMINAL_FORMAT = "strict-track2-v522-v521-phase-a-cache-qualification-attempt-terminal-v1"
SOURCE_ORDER = [
    ...
    "v520_authority_design_contract",
    "v520_authority_materializer",
    "v520_authority_receipt",
    "v521_materialization_process_receipt",
    "v521_materialization_stdout",
    "v521_materialization_stderr",
    "v521_transport_failure_forensic",
    "v521_transport_helper",
...
CHECK_KEYS = sorted({"authorization_boundary","contract_current","current_absences","execution_boundary","fresh_prereg_exact8","fresh_roots_absent","gpu_empty","historical_absence_validator_not_reused_post_authority","historical_absences","input_snapshots_equal","materializer_current","no_live_phase_a_process","no_pending_values","old_phase_a_failures_no_retry","per_call_rng_contract_source_enforced","publish_stable_excludes_authority_prep","service_env_unchanged","source_aliases_exact","source_closure_current","source_order_exact","split_forensic_exact","split_transition_sole_authority_absence_leaf","v517_candidate_exact1","v517_dual_bind_current","v517_evidence_exact6","v517_normalization_diff_exact2","v517_process_passed","v519_evidence_exact6","v519_inputs_prepost_equal","v519_output_exact","v519_process_receipt_exact","v519_raw_warning_exact","v519_rng_four_stage_full","v519_sample_call_partition","v519_stdout_exact1","v520_authority_expected_current_exact1","v520_authority_top68_check33_source23","v521_failed_transport_evidence_exact6","v521_stdout_exact2_native_empty","v521_transport_partition_exact","worker_env_exact2"})
CONTRACT_TOP_KEYS = {"authority_materializer_source","authority_receipt_contract","authority_split_state_forensic","authorization","current_absences_after_authority","execution_boundary","format","historical_absences","lineage","old_authority_expected_current","old_phase_a_failure_forensics","per_call_rng_evidence_contract","phase_a_driver_source","phase_a_independent_auditor_source","phase_a_launcher_source","phase_a_preregistration_source","phase_a_worker_source","qualification_output_root","rng_proxy_source","runtime_observation","seed","source_aliases","source_closure","source_closure_sha256","source_role_order","status","v517_candidate_receipt","v517_candidate_tree","v517_evidence_tree","v517_normalized_current_view","v517_process_receipt","v519_child_stdout","v519_evidence_tree","v519_output_record","v519_process_receipt","v519_rng_isolation_evidence","v519_source_records","v520_authority_receipt","v520_authority_registration_tree","v520_authority_semantics","v521_execution_partition","v521_materialization_evidence_tree","v521_materialization_process_receipt","v521_materialization_stderr","v521_materialization_stdout","worker_environment_exact"}
...
def load_v522_materializer():
    ...
    if (AUTHORITY_MATERIALIZER.is_symlink() or not AUTHORITY_MATERIALIZER.is_file()
        ...
        raise RuntimeError("v522 materializer source")
    spec = importlib.util.spec_from_file_location("v522_authority_materializer_consumer", AUTHORITY_MATERIALIZER)
...
def validate_authority(args: argparse.Namespace) -> dict:
    ...
    if args.authority_contract != AUTHORITY_CONTRACT or args.authority_receipt != AUTHORITY_RECEIPT or args.launcher_source != SELF:
        raise RuntimeError("canonical active paths")
    materializer = load_v522_materializer()
    if (materializer.CONTRACT_PATH != AUTHORITY_CONTRACT or materializer.AUTHORITY_ROOT != AUTHORITY_ROOT
            ...
            or materializer.PER_CALL_RNG_CONTRACT != PER_CALL_RNG_CONTRACT):
    ...
    receipt_schema = contract.get("authority_receipt_contract", {})
    if (set(contract) != CONTRACT_TOP_KEYS or len(contract) != 46
            or sorted(authority) != receipt_schema.get("top_keys") or len(authority) != 88
        ...
        raise RuntimeError("v522 schema/terminal")
    if (authority.get("authorization") != AUTHORIZATION or authority.get("runtime_observation") != RUNTIME_PRE
            or authority.get("execution_boundary") != EXECUTION_BOUNDARY
            or receipt_schema.get("authorization_exact") != AUTHORIZATION
            or receipt_schema.get("runtime_observation_exact") != RUNTIME_PRE
            or receipt_schema.get("format") != AUTHORITY_FORMAT or receipt_schema.get("status") != AUTHORITY_STATUS
            or receipt_schema.get("check_keys") != CHECK_KEYS
            or receipt_schema.get("check_key_set_sha256") != csha(CHECK_KEYS)
            or receipt_schema.get("checks_sha256") != csha(authority["checks"])):
        raise RuntimeError("v522 authority boundary")
    ...
    if (any(type(authority["authorization"].get(key)) is not int for key in authorization_count_keys)
        ...
        raise RuntimeError("v522 authorization count types")
    closure = authority.get("source_closure")
    if (authority.get("source_role_order") != SOURCE_ORDER or set(closure or {}) != set(SOURCE_ORDER)
        ...
        raise RuntimeError("v522 authority source closure")
    contract_closure = contract.get("source_closure")
    if (contract.get("source_role_order") != SOURCE_ORDER[1:] or set(contract_closure or {}) != set(SOURCE_ORDER[1:])
        ...
        raise RuntimeError("v522 contract source closure")
    for role in SOURCE_ORDER:
        record = closure[role]
        if authority.get(materializer.SOURCE_ALIASES[role]) != record:
            raise RuntimeError("v522 source alias " + role)
        if regular(Path(record["path"]), record["sha256"], record["logical_bytes"]) != record:
            raise RuntimeError("v522 source current " + role)
    expected_active = {
    ...
    if any(closure[role] != record for role, record in expected_active.items()):
        raise RuntimeError("v522 active source binding")
    expected_lineage = {
    ...
    if (contract.get("lineage") != expected_lineage or contract.get("authorization") != AUTHORIZATION
            or contract.get("runtime_observation") != RUNTIME_PRE or contract.get("execution_boundary") != EXECUTION_BOUNDARY
            or contract.get("phase_a_launcher_source") != self_record
            or contract.get("phase_a_driver_source") != closure["fresh_phase_a_driver"]
            or contract.get("phase_a_worker_source") != closure["fresh_phase_a_worker"]
            or contract.get("rng_proxy_source") != closure["fresh_rng_proxy"]
            ...
            or contract.get("qualification_output_root") != str(QUALIFICATION_ROOT)
            or contract.get("worker_environment_exact") != AUTHORIZATION["worker_environment_exact"]
            or contract.get("per_call_rng_evidence_contract") != PER_CALL_RNG_CONTRACT):
        raise RuntimeError("v522 contract active lineage")
    split = materializer.validate_split_state()
    for key in ("v520_authority_registration_tree", "v520_authority_receipt", "v520_authority_semantics",
                "v521_materialization_evidence_tree", "v521_materialization_process_receipt",
                "v521_materialization_stdout", "v521_materialization_stderr", "v521_execution_partition",
                "authority_split_state_forensic", "old_authority_expected_current"):
        if contract.get(key) != split[key] or authority.get(key) != split[key]:
            raise RuntimeError("v522 split anchor " + key)
    split_count_keys = (
        "v521_transport_helper_invocations", "v520_authority_materializer_invocations",
        "fresh_v522_authority_materializer_invocations", "phase_a_launcher_invocations",
        "phase_a_driver_invocations", "phase_a_worker_invocations", "rng_proxy_delegate_invocations",
        "training_invocations")
    if any(type(authority["v521_execution_partition"].get(key)) is not int for key in split_count_keys):
        raise RuntimeError("v522 split partition count types")
    old_module, base = materializer.load_base()
    materializer.validate_v519(base)
    materializer.validate_v517(base)
    materializer.validate_failures()
    ...
    return {"contract": contract, "authority": authority, "closure": closure, "preregistration": prereg,
            "contract_record": contract_record, "authority_record": authority_record, "self_record": self_record,
            "failure_forensics": authority["old_phase_a_failure_forensics"]}
...
def stable_snapshot(context: dict) -> dict:
    ...
    records = [context["contract_record"], context["authority_record"], context["self_record"]]
    records.extend(context["closure"][role] for role in SOURCE_ORDER)
    tree_keys = ("v519_evidence_tree", "v517_candidate_tree", "v517_evidence_tree",
                 "v520_authority_registration_tree", "v521_materialization_evidence_tree")
    trees = {key: exact_tree(Path(authority[key]["root"])) for key in tree_keys}
    if any(trees[key] != authority[key] for key in tree_keys):
        raise RuntimeError("v522 ancestry tree drift")
    return {"records": records, "records_sha256": csha(records), "ancestry_trees": trees,
            "v519_rng_isolation_evidence": authority["v519_rng_isolation_evidence"],
            "v517_normalized_current_view": authority["v517_normalized_current_view"],
            "authority_split_state_forensic": authority["authority_split_state_forensic"],
            "old_authority_expected_current": authority["old_authority_expected_current"]}
...
def synthetic_self_test() -> int:
    ...
    checks["source_order"] = len(SOURCE_ORDER) == 33 and len(set(SOURCE_ORDER)) == 33
    ...
    checks["v522_exact8"] = "materializer.validate_prereg(closure)" in source
    ...
    checks["materializer_binding"] = AUTHORITY_MATERIALIZER_SHA in source and AUTHORITY_MATERIALIZER_BYTES == 53649
    checks["split_state_full_consumer"] = all(token in source for token in (
        "materializer.validate_split_state()", "v520_authority_registration_tree",
        "v521_materialization_evidence_tree", "old_authority_expected_current"))
    checks["authority_schema_counts"] = len(CONTRACT_TOP_KEYS) == 46 and len(CHECK_KEYS) == 41 and len(SOURCE_ORDER) == 33
