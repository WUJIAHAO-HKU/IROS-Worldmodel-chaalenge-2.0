#!/usr/bin/env python3
"""Two-process, branch-serial fixed-horizon simulator collection for v453."""
...
def ensure_open3d_importable() -> bool:
    ...
    if existing is not None and getattr(existing, "__v453_rgb_only_stub__", False):
        return True
    try:
        __import__("open3d")
        return False
        ...
        stub.__v453_rgb_only_stub__ = True
        stub.__all__ = []
...
def runtime_provenance() -> dict:
    ...
    for name in ("open3d", "toppra", "mplib", "sapien"):
        ...
        modules[name] = {
            "file": str(Path(module_file).resolve()) if module_file else "built-in",
            "version": str(version),
        }
    return {
...
def image(obs: dict) -> np.ndarray:
    ...
    if raw.dtype != np.uint8 or raw.shape != (240, 320, 3):
        raise RuntimeError(f"bad simulator RGB {raw.shape} {raw.dtype}")
    value = resize_rgb(raw, size=256)
    if value.shape != (256, 256, 3) or value.dtype != np.uint8:
        raise RuntimeError(f"bad official-domain RGB {value.shape} {value.dtype}")
    return value
...
def timeout_handler(_signum, _frame) -> None:
    raise TimeoutError("v453 fixed context exceeded 300 seconds")
...
def main() -> int:
    ...
    if (
        prereg.get("format")
        != "strict-track2-v453-paired-intervention-pilot-preregistration-v1"
        or out.exists()
    ):
        raise RuntimeError("v453 prereg/output contract failed")
                ...
                technical.append(
                    {
                        ...
                        "error": f"GPU peak {peak} MiB exceeded 24576 MiB",
                    }
                )
                aborted = True
                break
    ...
    passed = (
        ...
        and all(row.get("open3d_import_mode") == "native" for row in rows)
        and all(
            row["context_passed"]
    ...
    report = {
        "format": "strict-track2-v453-paired-intervention-pilot-generation-report-v1",
        ...
        "official_rgb_preprocess": {
            "function": "wam_pipeline.data.resize_rgb",
            "size": 256,
            "mode": "RGB",
            "interpolation": "PIL.Image.Resampling.BILINEAR",
            "source_sha256": prereg["preprocessing"]["source_sha256"],
            "function_source_sha256": prereg["preprocessing"]["function_source_sha256"],
        },
        "guards": {
            "context_workers": 2,
            ...
            "policy_updates": 0,
            "open3d_import_required_mode": "native",
            "open3d_stub_rgb_only": False,
            "pointcloud_calls": 0,
            "official_rgb_preprocess_applied_before_hash_mae_save": True,
        },
    }
