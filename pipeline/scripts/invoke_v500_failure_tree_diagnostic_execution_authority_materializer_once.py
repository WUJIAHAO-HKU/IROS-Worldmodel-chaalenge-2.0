#!/usr/bin/env python3
"""Exact-once, signal-safe transport for the v500 authority materializer."""
...
SELF_PATH = SCRIPTS / "invoke_v500_failure_tree_diagnostic_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v500_failure_tree_diagnostic_authority_materialize_once.sh")
CONTRACT = SCRIPTS / "v500_v499_v498_v497_v496_v495_failure_tree_diagnostic_execution_authority_contract.json"
CONTRACT_SHA = "55bc38e1e101e97761ac8b8f4e5200b0ccd5a94676f27fc7a3123ce179df2e1e"
CONTRACT_BYTES = 111329
MATERIALIZER = SCRIPTS / "materialize_v500_v499_v498_v497_v496_v495_failure_tree_diagnostic_execution_authority.py"
MATERIALIZER_SHA = "e717c1360e5691865dd299249630b3b4cef1965c68e8a457008b50415fe9afb5"
MATERIALIZER_BYTES = 109256
...
EVIDENCE_ROOT = J / "v500_v499_v498_v497_v496_v495_failure_tree_diagnostic_execution_authority_materialization_evidence_seed1642_20260825"
...
def load_materializer():
    ...
    spec = importlib.util.spec_from_file_location("v500_materializer", MATERIALIZER)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    if (module.CONTRACT_PATH != CONTRACT or module.AUTHORITY_ROOT != AUTHORITY_ROOT
            or set(contract) != module.CONTRACT_TOP_KEYS or contract.get("seed") != 1642
            or contract.get("authority_materializer_source") != materializer_record
            or len(module.AUTH_TOP_KEYS) != 89 or len(module.AUTH_CHECK_KEYS) != 81
            or len(module.SOURCE_ROLE_ORDER) != 31
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v500-failure-tree-diagnostic-authority-materializer-process-receipt-v1",
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v500-failure-tree-diagnostic-authority-materializer-intent-v1",
...
def production_fixture() -> dict:
    checks = {}
    with tempfile.TemporaryDirectory(prefix="v500-helper-fixture-") as folder:
        base = Path(folder)
...
def main() -> int:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v500-failure-tree-diagnostic-authority-materializer-argv-v1",
