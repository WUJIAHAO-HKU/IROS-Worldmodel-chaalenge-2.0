...
export PYTHONPATH="$ROOT/pipeline:$ROOT/pipeline/scripts:$ROOT/third_party/WorldArena-2.0/WorldArena-2.0-main/RL_env_benchmark"
exec taskset -c 0-11 /root/autodl-tmp/conda_envs/rlinf_track2/bin/python "$ROOT/pipeline/scripts/audit_v366_serial_consistent_v355_v326_gate.py" \
  ...
  --split-manifest "$ROOT/artifacts/splits/adjust_bottle_50episodes_full.json" \
  ...
  --contract-report "$RUN/contract_report.json" \
  --training-report "$J/v311_public_action_causal_gate_seed1484_20260822/audit/training_report.json" \
  --phase-training-report "$J/v323_public_terminal_phase_gate_seed1493_20260822/audit/training_report.json" \
  --output "$RUN/causal_gate_report.json" \
