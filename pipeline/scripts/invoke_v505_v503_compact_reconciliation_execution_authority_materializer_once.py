#!/usr/bin/env python3
"""Exact-once, signal-safe transport for the v503 compact-reconciliation authority materializer."""
...
SELF_PATH = SCRIPTS / "invoke_v505_v503_compact_reconciliation_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v505_v503_compact_reconciliation_authority_materialize_once.sh")
CONTRACT = SCRIPTS / "v503_v502_compact_reconciliation_execution_authority_contract.json"
CONTRACT_SHA = "3cb53e7abcbeb54296085ae46ee5073cb842aaaf471e08da1b320dcf1ed9c1c1"
CONTRACT_BYTES = 22139
MATERIALIZER = SCRIPTS / "materialize_v503_v502_compact_reconciliation_execution_authority.py"
MATERIALIZER_SHA = "bfdf33ab525d3b9e98ea116604f309354160cf98021478c94c177caa5060d879"
MATERIALIZER_BYTES = 42234
DIRECT_WRAPPER = SCRIPTS / "launch_v503_v502_compact_single_reconciler.py"
DIRECT_WRAPPER_SHA = "32c9f4c7c32566bb65b5a9d4ec4b53630a2941a9932345e35c3c911ec80aed9f"
DIRECT_WRAPPER_BYTES = 70711
V503_FORENSIC = SCRIPTS / "v503_v502_outer_preintent_v494_ancestry_failure_forensic_reconstructed.json"
V503_FORENSIC_SHA = "418f399030a0f0e13571c38a8e55ac36de45d2012616adef49d7452ab90a4ce9"
V503_FORENSIC_BYTES = 8521
FAILED_TRANSPORT_FORENSIC = SCRIPTS / "v505_v503_compact_authority_transport_seed_literal_failure_forensic_reconstructed.json"
FAILED_TRANSPORT_FORENSIC_SHA = "1dcab7d213ba2cda8c3811bc5bdd3047f874c5758ea857d1f904779c079bca44"
FAILED_TRANSPORT_FORENSIC_BYTES = 6145
OLD_HELPER = SCRIPTS / "invoke_v503_v502_compact_reconciliation_execution_authority_materializer_once.py"
OLD_HELPER_SHA = "777ca1c6c4aee8398f87cf5ae523e58444c8fd6a8a6622fdc8ad84fb6dff7bba"
OLD_HELPER_BYTES = 52208
OLD_SCRIPT = Path("/root/v503_compact_reconciliation_authority_materialize_once.sh")
OLD_SCRIPT_SHA = "f53920d3fed523df09266465f458d265e087a43b1ef3154acf7a4d2b78988b07"
OLD_SCRIPT_BYTES = 2546
...
EVIDENCE_ROOT = J / "v505_v503_compact_reconciliation_execution_authority_materialization_evidence_seed1646_20260825"
EVIDENCE_PREP = EVIDENCE_ROOT.with_name(EVIDENCE_ROOT.name + ".execution-prep")
FAILED_EVIDENCE_ROOT = J / "v503_v502_compact_reconciliation_execution_authority_materialization_evidence_seed1645_20260825"
FAILED_EVIDENCE_PREP = FAILED_EVIDENCE_ROOT.with_name(FAILED_EVIDENCE_ROOT.name + ".execution-prep")
...
def validate_failed_transport_forensic() -> tuple[dict, dict]:
    record = regular(FAILED_TRANSPORT_FORENSIC,
                     (FAILED_TRANSPORT_FORENSIC_SHA, FAILED_TRANSPORT_FORENSIC_BYTES))
    value = json.loads(FAILED_TRANSPORT_FORENSIC.read_text(encoding="utf-8"))
    expected_top = {
        "format", "status", "passed", "date", "failed_command", "root_cause", "tool_capture",
        "failed_transport_sources", "authority_sources", "invocation_partition", "runtime_roots",
        "post_failure_observation", "authorization",
    }
    partition = value.get("invocation_partition", {})
    roots = value.get("runtime_roots", {})
    if (set(value) != expected_top
            or value.get("status") != "failed_no_retry_pre_popen_schema_literal_mismatch"
            or value.get("passed") is not False
            or value.get("failed_command", {}).get("exit_code") != 1
            or value.get("failed_command", {}).get("retry_authorized") is not False
            or value.get("root_cause", {}).get("expected_literal_in_failed_helper") != 1644
            or value.get("root_cause", {}).get("actual_contract_seed") != 1645
            or value.get("root_cause", {}).get("other_schema_predicates_passed") != 12
            or partition.get("transport_helper_invocations") != 1
            or any(partition.get(key) != 0 for key in (
                "materializer_popen_invocations", "authority_materializer_invocations",
                "direct_single_reconciler_invocations", "reconciler_r2_invocations",
                "phase_a_invocations", "training_invocations"))
            or set(roots) != {"authority_root", "authority_prep", "failed_evidence_root",
                              "failed_evidence_prep", "wrapper_attempt_root", "wrapper_attempt_prep"}
            or not all(row.get("absent") is True and not os.path.lexists(row.get("path", ""))
                       for row in roots.values())
            or value.get("failed_transport_sources", {}).get("helper") != {
                "path": str(OLD_HELPER), "sha256": OLD_HELPER_SHA, "logical_bytes": OLD_HELPER_BYTES}
            or value.get("failed_transport_sources", {}).get("script") != {
                "path": str(OLD_SCRIPT), "sha256": OLD_SCRIPT_SHA, "logical_bytes": OLD_SCRIPT_BYTES}
            or value.get("authorization", {}).get("retry_authorized") is not False
            or value.get("post_failure_observation", {}).get("authority_registration_receipt_created") is not False
            or value.get("post_failure_observation", {}).get("evidence_process_receipt_created") is not False):
        raise RuntimeError("failed transport forensic")
    return record, value


def immutable_snapshot(contract: dict, helper_record: dict, script_record: dict) -> dict:
    failed_record, failed_value = validate_failed_transport_forensic()
    ...
    return {
        ...
        "failed_transport_forensic": failed_record,
        "failed_transport_forensic_sha256": csha(failed_value),
        "failed_transport_sources": {
            "helper": regular(OLD_HELPER, (OLD_HELPER_SHA, OLD_HELPER_BYTES)),
            "script": regular(OLD_SCRIPT, (OLD_SCRIPT_SHA, OLD_SCRIPT_BYTES)),
        },
...
def validate_materializer_contract(module, contract: dict, materializer_record: dict) -> None:
    ...
    if (module.CONTRACT_PATH != CONTRACT or module.AUTHORITY_ROOT != AUTHORITY_ROOT
            ...
            or isinstance(seed, bool) or not isinstance(seed, int) or seed != 1645
            or contract.get("authority_materializer_source") != materializer_record
            or contract.get("source_closure", {}).get("direct_reconciler_wrapper") != {
                "path": str(DIRECT_WRAPPER), "sha256": DIRECT_WRAPPER_SHA, "logical_bytes": DIRECT_WRAPPER_BYTES}
            or contract.get("v502_failure_summary") != {
                "path": str(V503_FORENSIC), "sha256": V503_FORENSIC_SHA, "logical_bytes": V503_FORENSIC_BYTES}
            or len(module.AUTH_TOP_KEYS) != 46 or len(module.CHECK_KEYS) != 33
            or len(module.CONTRACT_SOURCE_ORDER) != 6 or len(module.AUTHORITY_SOURCE_ORDER) != 7
...
def load_materializer():
    ...
    spec = importlib.util.spec_from_file_location("v503_materializer", MATERIALIZER)
    ...
    try:
        ...
        lines = [
            ...
            {"line_origin": "transport_helper", "passed": True,
             "authority_materializer_invocations": 1, "direct_single_reconciler_invocations": 0,
             "reconciler_r2_invocations": 0, "committed_success": True},
        ]
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v505-v503-compact-reconciliation-authority-materializer-process-receipt-v1",
        "status": status, "passed": passed, "authority_materializer_invocations": invocations,
        "transport_helper_invocations": 1,
        "direct_single_reconciler_invocations": 0, "reconciler_r2_invocations": 0,
...
def orchestrate(*, root: Path, prep: Path, child_argv: list[str], helper_source: Path,
    ...
    try:
        ...
        write_exclusive(prep / "intent.json", cbytes({
            "format": "strict-track2-v505-v503-compact-reconciliation-authority-materializer-intent-v1",
...
def production_fixture() -> dict:
    ...
    checks["actual_main_load_materializer"] = (
        live_contract_record == regular(CONTRACT, (CONTRACT_SHA, CONTRACT_BYTES))
        and live_materializer_record == regular(MATERIALIZER, (MATERIALIZER_SHA, MATERIALIZER_BYTES))
        and live_contract["seed"] == 1645)
    def rejects(contract_value, module_value=live_module, materializer_value=live_materializer_record):
        try:
            ...
            return True
    tamper_checks = []
    for bad_seed in (1644, "1645", True, None):
    ...
    bad_source["source_closure"]["direct_reconciler_wrapper"]["sha256"] = "0" * 64
    ...
    checks["actual_load_materializer_tamper_suite"] = all(tamper_checks) and len(tamper_checks) == 9
    with tempfile.TemporaryDirectory(prefix="v505-helper-fixture-") as folder:
        base = Path(folder)
...
def main() -> int:
    ...
    if any(os.path.lexists(path) for path in (
            ...
            FAILED_EVIDENCE_ROOT, FAILED_EVIDENCE_PREP)):
        raise RuntimeError("runtime prestate")
    argv = command()
    payload = {
        "format": "strict-track2-v505-v503-compact-reconciliation-authority-materializer-argv-v1",
        ...
        "transport_helper": helper_record,
        "failed_transport_forensic": regular(
            FAILED_TRANSPORT_FORENSIC,
            (FAILED_TRANSPORT_FORENSIC_SHA, FAILED_TRANSPORT_FORENSIC_BYTES)),
    ...
    result = orchestrate(root=EVIDENCE_ROOT, prep=EVIDENCE_PREP, child_argv=argv,
                         ...
                         retain_terminal_priority=True,
                         failure_forensic_record=regular(
                             FAILED_TRANSPORT_FORENSIC,
                             (FAILED_TRANSPORT_FORENSIC_SHA, FAILED_TRANSPORT_FORENSIC_BYTES)))
