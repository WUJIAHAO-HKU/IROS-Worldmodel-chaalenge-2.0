...
RUN="$J/v326_v317_blended_phase_terminal_gate_seed1496_20260822"
...
taskset -c 0-7 "$PY" pipeline/scripts/test_v326_blended_phase_terminal.py \
  "${COMMON[@]}" --output "$RUN/audit/contract_report.json" \
  >>"$RUN/audit/contract.log" 2>&1

taskset -c 0-7 "$PY" pipeline/scripts/audit_v326_blended_phase_terminal_gate.py \
  "${COMMON[@]}" \
...
printf 'V326_BLENDED_PHASE_TERMINAL_GATE_PASSED\n'
