#!/usr/bin/env python3
"""Two-process, branch-serial fixed-horizon simulator collection for v452."""
...
def ensure_open3d_importable() -> bool:
    ...
    if existing is not None and getattr(existing, "__v452_rgb_only_stub__", False):
        return True
    try:
        __import__("open3d")
        return False
        ...
        stub.__v452_rgb_only_stub__ = True
        stub.__all__ = []
...
def timeout_handler(_signum, _frame) -> None:
    raise TimeoutError("v452 fixed context exceeded 300 seconds")
...
def main() -> int:
    ...
    if (
        prereg.get("format")
        != "strict-track2-v452-paired-intervention-pilot-preregistration-v1"
        or out.exists()
    ):
        raise RuntimeError("v452 prereg/output contract failed")
    ...
    report = {
        "format": "strict-track2-v452-paired-intervention-pilot-generation-report-v1",
