#!/usr/bin/env python3
"""Exact-once Stage-A transport for the v541 fixed-budget all200 training authority."""
...
SELF_PATH = SCRIPTS / "invoke_v541_v540_fixed_budget_all200_training_execution_authority_materializer_once.py"
SCRIPT_PATH = Path("/root/v541_v540_fixed_budget_all200_training_execution_authority_materialize_once.sh")
DEPLOYMENT_RECORD = SCRIPTS / "v541_v540_fixed_budget_all200_training_execution_authority_materialization_transport_deployment_record.json"
CONTRACT = SCRIPTS / "v541_v540_fixed_budget_all200_training_execution_authority_contract.json"
CONTRACT_SHA = "f3adaa837658d468a4e4eb0fe5e7c44be6793a7a2733224598b3336659d8a46e"
CONTRACT_BYTES = 43566
MATERIALIZER = SCRIPTS / "materialize_v541_v540_fixed_budget_all200_training_execution_authority.py"
MATERIALIZER_SHA = "e3243165dd9fee10caa49ea99cdba2532c5cbf441a909fb1b92c5fbe5e779015"
MATERIALIZER_BYTES = 16336
TRAINER = SCRIPTS / "train_v541_v540_fixed_budget_all200.py"
TRAINER_SHA = "67896c46f6df51aa3a29076376ff5df0ac7980f8fe97a0f0212907d02f42e316"
TRAINER_BYTES = 46906
AUDITOR = SCRIPTS / "audit_v541_v540_fixed_budget_all200.py"
AUDITOR_SHA = "57e628ba412532cb12d97d9927b8ad574899e14b50c2934e8e88f8adba175a98"
AUDITOR_BYTES = 4784
LAUNCHER = SCRIPTS / "launch_v541_v540_fixed_budget_all200.py"
LAUNCHER_SHA = "8b9a569f19f08a23b448f87e6e7e1d7a95d20e4b048874aece9d9e213ec3f16b"
LAUNCHER_BYTES = 2697
PREREGISTRATION = SCRIPTS / "v541_v540_fixed_budget_all200_training_preregistration.json"
PREREGISTRATION_SHA = "d429b0aa8e8386e28fe6e90db2981da5c7cc57f64d0dcb250a69d66784a0bd34"
PREREGISTRATION_BYTES = 5698
MANIFEST = SCRIPTS / "v541_v540_fixed_budget_all200_training_manifest.json"
MANIFEST_SHA = "c973dcb5a6d2d1418e957c04d3d3cf2655977e3dd35a72bc42a0e911d360ab64"
MANIFEST_BYTES = 6497
...
AUTHORITY_ROOT = J / "v541_v540_fixed_budget_all200_training_execution_authority_seed1672_20260828"
AUTHORITY_PREP = AUTHORITY_ROOT.with_name(AUTHORITY_ROOT.name + ".authority-prep")
EVIDENCE_ROOT = J / "v541_v540_fixed_budget_all200_training_execution_authority_materialization_evidence_seed1672_20260828"
...
AT_FDCWD = -100
RENAME_NOREPLACE = 1
FROZEN_FINAL7_SHA256 = (CONTRACT_SHA, MATERIALIZER_SHA, PREREGISTRATION_SHA, MANIFEST_SHA,
                       TRAINER_SHA, AUDITOR_SHA, LAUNCHER_SHA)
...
EXPECTED_CURRENT_ABSENCE_PATHS = {
    "attempt_prep": "/root/autodl-tmp/IROS_WAM_2.0 challenge/artifacts/strict_track2_joint_augmentation_20260810/v541_v540_fixed_budget_all200_training_attempt_seed1672_20260828.attempt-prep",
    "attempt_root": "/root/autodl-tmp/IROS_WAM_2.0 challenge/artifacts/strict_track2_joint_augmentation_20260810/v541_v540_fixed_budget_all200_training_attempt_seed1672_20260828",
    "authority_prep": "/root/autodl-tmp/IROS_WAM_2.0 challenge/artifacts/strict_track2_joint_augmentation_20260810/v541_v540_fixed_budget_all200_training_execution_authority_seed1672_20260828.authority-prep",
    "output_prep": "/root/v541_v540_fixed_budget_all200_training_seed1624_20260828.training-prep",
    "output_root": "/root/v541_v540_fixed_budget_all200_training_seed1624_20260828",
}
EXPECTED_CURRENT_ABSENCE_MAP_SHA256 = "4f7f5bce87a8b28eef4207084b36be7627821eaa83b8b77801ecc5e40f56f2bf"
...
def deployment_plan() -> dict:
    ...
    return {
        "fresh_final7_noreplace": final7,
        "fresh_final7_volatile_sources": [{**row, "source_path": "/dev/shm/" + Path(row["path"]).name} for row in final7],
...
def expected_transport_deployment(helper_record: dict, script_record: dict) -> dict:
    return {
        "format": "strict-track2-v541-stage-a-transport-pair-deployment-record-v1",
...
def validate_materializer_contract(module, contract: dict, materializer_record: dict) -> None:
    ...
    trainer_spec = importlib.util.spec_from_file_location("v541_stage_a_trainer_schema", trainer_source)
    ...
    schema_checks = {
        ...
        "format": contract.get("format") == "strict-track2-v541-v540-fixed-budget-all200-training-execution-authority-design-contract-v1",
        ...
        "seed": type(seed) is int and seed == 1672,
        ...
        "authority_top_count": len(contract.get("authority_receipt_contract", {}).get("top_keys", [])) == 59,
        "check_count": len(trainer_module.CHECK_KEYS) == 34,
        "source_counts": len(trainer_module.SOURCE_ROLE_ORDER) == 28 and len(trainer_module.AUTHORITY_SOURCE_ALIASES) == 29,
    ...
    if (cbytes(contract.get("authorization")) != cbytes(trainer_module.EXPECTED_AUTHORIZATION)
        ...
        raise RuntimeError("v541 fixed-budget training authority boundary")
    for role, row in contract["source_closure"].items():
...
def validate_success(module, contract: dict, returncode: int, cleanup: dict,
    ...
    if (os.path.lexists(AUTHORITY_PREP)
            or sorted(receipt) != receipt_schema["top_keys"] or len(receipt) != 59
            ...
            or len(receipt.get("source_closure", {})) != 29
...
def base_receipt(status: str, passed: bool, invocations: int, cleanup: dict, error: BaseException | None = None) -> dict:
    row = {
        "format": "strict-track2-v541-v540-fixed-budget-all200-training-authority-materializer-process-receipt-v1",
...
def production_fixture(contract_source: Path = CONTRACT, materializer_source: Path = MATERIALIZER,
    ...
    checks = {
        ...
        "final7_hardbound": all(value in source for value in FROZEN_FINAL7_SHA256),
        "fresh_final7_transport3_deploy10": (
            len(plan["fresh_final7_noreplace"]) == 7
            and len(plan["fresh_final7_volatile_sources"]) == 7
        ...
        "authority_schema_59_34_29": (
            len(contract["authority_receipt_contract"]["top_keys"]) == 59),
        ...
        "downstream_not_in_child_argv": (
            all(str(path) not in command() for path in (TRAINER, AUDITOR, LAUNCHER))
    ...
    with tempfile.TemporaryDirectory(prefix="v541-stage-a-helper-", dir="/dev/shm") as folder:
        temp = Path(folder)
...
def main() -> int:
    ...
    argv = command()
    payload = {
        "format": "strict-track2-v541-v540-fixed-budget-all200-training-authority-materializer-argv-v1",
