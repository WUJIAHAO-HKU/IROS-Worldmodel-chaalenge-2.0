...
RUN="$AUDIT/runs/probability_consistent_four_step_v169_seed1243"
...
SERVICE_DIR="$AUDIT/probability_consistent_service_four_step_v169"
BRIDGE_DIR="$AUDIT/probability_consistent_bridge_four_step_v169"
...
"$PY" "$RUNTIME/examples/embodiment/train_embodied_agent.py" \
  ...
  runner.logger.log_path="$RUN" runner.max_steps=4 runner.max_epochs=1000 runner.save_interval=4 \
...
printf 'V169_PROBABILITY_CONSISTENT_FOUR_STEP_COMPLETE_NONFORMAL\n'
