...
run="artifacts/strict_track2_official_20260810/run_registry/v425_v424_mirror_augmented_recursive_seed1576_20260823"
mkdir -p "$run"
...
if [[ ! -f "$run/preregistration.json" ]]; then
  /root/miniconda3/envs/go1/bin/python pipeline/scripts/prepare_v425_v424_mirror_augmented_recursive.py
fi

exec taskset -c 0-7 /root/miniconda3/envs/go1/bin/python \
  pipeline/scripts/audit_v425_v424_mirror_augmented_recursive.py \
  --baseline-checkpoint-dir artifacts/strict_track2_joint_augmentation_20260810/v209_v202_v208_public_arm_routed_release \
  --candidate-checkpoint-dir artifacts/strict_track2_joint_augmentation_20260810/v424_v202_v423_mirror_augmented_arm_routed_release \
