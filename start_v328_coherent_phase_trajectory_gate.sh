...
RUN="$J/v328_v326_coherent_phase_trajectory_gate_seed1498_20260822"
...
taskset -c 0-7 "$PY" pipeline/scripts/test_v328_coherent_phase_trajectory.py \
  "${COMMON[@]}" --output "$RUN/audit/contract_report.json" \
  >>"$RUN/audit/contract.log" 2>&1

taskset -c 0-7 "$PY" pipeline/scripts/audit_v328_coherent_phase_trajectory_gate.py \
  "${COMMON[@]}" \
...
taskset -c 0-7 "$PY" pipeline/scripts/audit_v328_public_recursive_stability.py \
  "${COMMON[@]}" \
  --instruction-map "$J/v205_v202_public_right_terminal_multichunk_seed1405/public_demo_split.json" \
  --reward-checkpoint "$R/artifacts/official_resources/reward_model/adjust_bottle/full_weights.pt" \
  --t5-model "$R/artifacts/official_resources/reward_model/t5-base" \
  --contract-report "$RUN/audit/contract_report.json" \
  --causal-report "$RUN/audit/causal_gate_report.json" \
  --output "$RUN/audit/recursive_stability_report.json" \
...
taskset -c 0-1 "$PY" pipeline/scripts/authorize_expensive_rl.py \
  ...
  --output "$RUN/audit/expensive_rl_authorization.json" \
  >>"$RUN/audit/authorization.log" 2>&1

printf 'V328_COHERENT_PHASE_TRAJECTORY_GATE_PASSED\n'
