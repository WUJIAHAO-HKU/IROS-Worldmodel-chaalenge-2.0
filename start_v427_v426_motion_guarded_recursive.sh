...
run="artifacts/strict_track2_official_20260810/run_registry/v427_v426_motion_guarded_recursive_seed1577_20260823"
mkdir -p "$run"
...
if [[ ! -f "$run/preregistration.json" ]]; then
  /root/miniconda3/envs/go1/bin/python pipeline/scripts/prepare_v427_v426_motion_guarded_recursive.py
fi
exec taskset -c 0-7 /root/miniconda3/envs/go1/bin/python \
  pipeline/scripts/audit_v427_v426_motion_guarded_recursive.py \
