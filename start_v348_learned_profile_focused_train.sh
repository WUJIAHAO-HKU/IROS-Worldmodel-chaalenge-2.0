...
run="artifacts/strict_track2_joint_augmentation_20260810/v348_learned_profile_focused_train_seed1517_20260822"
...
if [[ ! -f "$run/release_registration.json" ]]; then
  /root/miniconda3/envs/go1/bin/python pipeline/scripts/prepare_v348_learned_profile_focused_train.py
fi
exec taskset -c 0-7 /root/miniconda3/envs/go1/bin/python \
  pipeline/scripts/audit_v348_learned_profile_focused_train.py \
  ...
  --v340-train-report "$v340/train_recursive_gate_report.json" \
  --v347-fit-report "$fit/fit_report.json" \
  --preregistration "$run/release_registration.json" \
  --output "$run/focused_train_gate_report.json" \
