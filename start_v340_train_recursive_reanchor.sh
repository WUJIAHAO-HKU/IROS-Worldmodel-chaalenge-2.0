...
run="artifacts/strict_track2_joint_augmentation_20260810/v340_train_recursive_reanchor_seed1509_20260822"
...
/root/miniconda3/envs/go1/bin/python pipeline/scripts/prepare_v340_train_recursive_reanchor.py
exec taskset -c 0-7 /root/miniconda3/envs/go1/bin/python \
  pipeline/scripts/audit_v340_train_recursive_reanchor.py \
  ...
  --ood-calibration-report "$v339/calibration_report.json" \
  --ood-holdout-report "$v339/holdout_report.json" \
  --preregistration "$run/release_registration.json" \
  --output "$run/train_recursive_gate_report.json" \
  --device cuda \
