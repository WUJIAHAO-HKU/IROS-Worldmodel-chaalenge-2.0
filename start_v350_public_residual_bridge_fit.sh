...
run="artifacts/strict_track2_joint_augmentation_20260810/v350_public_residual_bridge_fit_seed1519_20260822"
...
if [[ ! -f "$run/release_registration.json" ]]; then
  /root/miniconda3/envs/go1/bin/python pipeline/scripts/prepare_v350_public_residual_bridge_fit.py
fi
exec taskset -c 0-7 /root/miniconda3/envs/go1/bin/python \
  pipeline/scripts/fit_v350_public_residual_bridge_profile.py \
  ...
  --output-profile "$run/residual_bridge_profile.npz" \
