...
RUN="$J/v335_v334_all_offset_recursive_gate_seed1505_20260822"
...
taskset -c 0-7 "$PY" pipeline/scripts/test_v335_all_offset_bounded_onset.py \
  "${COMMON[@]}" --output "$RUN/audit/contract_report.json" >>"$RUN/audit/contract.log" 2>&1
taskset -c 0-7 "$PY" pipeline/scripts/audit_v335_bounded_onset_terminal_gate.py \
...
taskset -c 0-7 "$PY" pipeline/scripts/audit_v335_all_offset_recursive_stability.py \
  ...
  --preregistration "$RUN/release_registration.json" --output "$RUN/audit/recursive_stability_report.json" \
  --inference-batch-size 8 --reward-batch-size 32 >>"$RUN/audit/recursive_stability.log" 2>&1
...
printf 'V335_ALL_OFFSET_BOUNDED_ONSET_GATE_PASSED\n'
