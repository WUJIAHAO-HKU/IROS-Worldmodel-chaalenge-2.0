#!/usr/bin/env python3
"""Preregister v333 hybrid semantics and all gates before evaluation."""
...
N = "v333_v326_v325_hybrid_onset_terminal_gate_seed1503_20260822"
RUN = J / N
...
def main() -> None:
    ...
    paths = {
        "runtime": B / "pipeline/wam_pipeline/v333_hybrid_onset_terminal_runtime.py",
        "v326_parent_runtime": B / "pipeline/wam_pipeline/v326_blended_phase_terminal_runtime.py",
        "v325_reference_runtime": B / "pipeline/wam_pipeline/v325_same_episode_phase_repair_runtime.py",
        "backends": B / "pipeline/wam_pipeline/backends.py",
        "contract": B / "pipeline/scripts/test_v333_hybrid_onset_terminal.py",
        "inherited_contract": B / "pipeline/scripts/test_v324_phase_guarded_terminal.py",
        "causal_audit": B / "pipeline/scripts/audit_v333_hybrid_onset_terminal_gate.py",
        "inherited_causal_audit": B / "pipeline/scripts/audit_v324_phase_guarded_terminal_gate.py",
        "recursive_audit": B / "pipeline/scripts/audit_v333_public_recursive_stability.py",
        ...
        "v325_causal": J / "v325_v317_same_episode_phase_repair_gate_seed1495_20260822/audit/causal_gate_report.json",
        ...
        "v332_failed_contract": J / "v332_v326_phase_aligned_successor_gate_seed1502_20260822/audit/contract_report.json",
    ...
    if json.loads(paths["v325_causal"].read_text()).get("passed") is not False:
        raise RuntimeError("v325 evidence must remain rejected")
    if json.loads(paths["v331_failed_causal"].read_text()).get("passed") is not False:
        raise RuntimeError("v331 evidence must remain rejected")
    if json.loads(paths["v332_failed_contract"].read_text()).get("passed") is not False:
        raise RuntimeError("v332 evidence must remain rejected")
    payload = {
        "format": "strict-track2-v333-hybrid-onset-terminal-preregistration-v1",
        "registered_at": datetime.now(timezone.utc).isoformat(),
        "candidate": {
            "model_version": "track2-v333-hybrid-onset-terminal-v326-v325-v317",
            "parent": "frozen v326/v317 action, phase, alpha, splice, and failure semantics",
            "change": (
                "within an already-authorized v326 repair only: terminal-ineligible matched "
                "episodes use their same-episode v323 onset row; eligible matched episodes "
                "retain the exact v326 success-terminal selector"
            ),
        ...
        "frozen_evidence_projection": {
            "episode22_ineligible_windows_99_to_101": "v325 same-episode onset rewards are each 0.939",
            "episode22_eligible_windows_102_to_105": "v326 rewards range 0.978 to 0.998",
            "expected_hit_pattern_not_a_result": "all transition windows except the unchanged pre-onset window98",
        },
        "fixed_thresholds": {
        ...
        "evaluation_sequence": [
            "numeric_hybrid_and_nonregression_contract",
