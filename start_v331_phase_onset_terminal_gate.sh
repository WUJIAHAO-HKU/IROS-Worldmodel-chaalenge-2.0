...
RUN="$J/v331_v326_phase_onset_terminal_gate_seed1501_20260822"
...
taskset -c 0-7 "$PY" pipeline/scripts/test_v331_phase_onset_terminal.py \
  "${COMMON[@]}" --output "$RUN/audit/contract_report.json" \
  >>"$RUN/audit/contract.log" 2>&1

taskset -c 0-7 "$PY" pipeline/scripts/audit_v331_phase_onset_terminal_gate.py \
  "${COMMON[@]}" \
...
taskset -c 0-7 "$PY" pipeline/scripts/audit_v331_public_recursive_stability.py \
  "${COMMON[@]}" \
  --instruction-map "$J/v205_v202_public_right_terminal_multichunk_seed1405/public_demo_split.json" \
  --reward-checkpoint "$R/artifacts/official_resources/reward_model/adjust_bottle/full_weights.pt" \
  --t5-model "$R/artifacts/official_resources/reward_model/t5-base" \
...
printf 'V331_PHASE_ONSET_TERMINAL_GATE_PASSED\n'
